...
bool CursorVisitor::VisitChildren(CXCursor Cursor) {
  ...
  if (Cursor.kind == CXCursor_IBOutletCollectionAttr) {
    ...
    if (const ObjCObjectType *const IBOutletCollectionAttr *A =
        cast<IBOutletCollectionAttr>(cxcursor::getCursorAttr(Cursor)); ObjT = A->getInterface()->getAs<ObjCObjectType>())
  ...
  if (Cursor.kind == CXCursor_MacroDefinition &&
    ...
    if (MacroDefinitionRecord *const MacroInfo *MI =
        getMacroInfo(cxcursor::getCursorMacroDefinition(Cursor), TU); MacroDef =
...
static bool HasTrailingReturnType(FunctionDecl *ND) {
  ...
  if (const FunctionType *const QualType Ty = ND->getType(); AFT = Ty->getAs<FunctionType>()) {
...
bool CursorVisitor::RunVisitorWorkList(VisitorWorkList &WL) {
  while (!WL.empty()) {
    ...
    switch (SetParentRAII SetParent(Parent, StmtParent, LI.getParent()); LI.getKind()) {
    ...
    case VisitorJob::LabelRefVisitKind: {
      ...
      if (LabelStmt *const LabelDecl *LS = cast<LabelRefVisit>(&LI)->get(); stmt = LS->getStmt()) {
    ...
    case VisitorJob::LambdaExprPartsKind: {
      ...
      if (FunctionTypeLoc TypeLoc TL = E->getCallOperator()->getTypeSourceInfo()->getTypeLoc(); Proto = TL.getAs<FunctionProtoTypeLoc>()) {
...
enum CXErrorCode clang_parseTranslationUnit2FullArgv(
  ...
  if (llvm::CrashRecoveryContext CRC; !RunSafely(CRC, ParseTranslationUnitImpl)) {
...
int clang_reparseTranslationUnit(CXTranslationUnit TU,
  ...
  if (llvm::CrashRecoveryContext CRC; !RunSafely(CRC, ReparseTranslationUnitImpl)) {
...
CXString clang_getCursorSpelling(CXCursor C) {
  ...
  if (clang_isStatement(C.kind)) {
    ...
    if (const LabelStmt *const Stmt *S = getCursorStmt(C); Label = dyn_cast_or_null<LabelStmt>(S))
      return cxstring::createRef(Label->getName());
...
CXCursor clang_getCursorReferenced(CXCursor C) {
  ...
  if (clang_isStatement(C.kind)) {
    ...
    if (const GotoStmt *const Stmt *S = getCursorStmt(C); Goto = dyn_cast_or_null<GotoStmt>(S))
...
CXCursor clang_getCursorDefinition(CXCursor C) {
  ...
  case Decl::CXXConversion: {
    ...
    if (const FunctionDecl *Def = nullptr; cast<FunctionDecl>(D)->getBody(Def))
      return MakeCXCursor(Def, TU);
    return clang_getNullCursor();
  }
  ...
  case Decl::FunctionTemplate: {
    ...
    if (const FunctionDecl *Def = nullptr; cast<FunctionTemplateDecl>(D)->getTemplatedDecl()->getBody(Def))
...
const CXXOperatorCallExpr *GetSubscriptOrCallOperator(CXCursor Cursor) {
  ...
  if (const auto *const Expr *E = getCursorExpr(Cursor); OCE = dyn_cast<CXXOperatorCallExpr>(E)) {
...
AnnotateTokensWorker::DetermineChildActions(CXCursor Cursor) const {
  ...
  if (const auto *OCE = GetSubscriptOrCallOperator(Cursor)) {
    ...
    if (const ImplicitCastExpr *const Expr *Callee = OCE->getCallee(); ICE = dyn_cast<ImplicitCastExpr>(Callee)) {
      
      if (const DeclRefExpr *const Expr *SubExpr = ICE->getSubExpr(); DRE = dyn_cast<DeclRefExpr>(SubExpr)) {
...
enum CXChildVisitResult AnnotateTokensWorker::Visit(CXCursor cursor,
  ...
  if (clang_isExpression(cursorK) && MoreTokens()) {
    ...
    if (const Decl *const Expr *E = getCursorExpr(cursor); D = getCursorDecl(cursor)) {
  ...
  if (W.hasContextSensitiveKeywords()) {
    for (unsigned I = 0; I != NumTokens; ++I) {
      ...
      if (Cursors[I].kind == CXCursor_ObjCPropertyDecl) {
        ...
        if (const ObjCPropertyDecl *IdentifierInfo *II = static_cast<IdentifierInfo *>(Tokens[I].ptr_data); Property =
...
CXLinkageKind clang_getCursorLinkage(CXCursor cursor) {
  ...
  if (const NamedDecl *const Decl *D = cxcursor::getCursorDecl(cursor); ND = dyn_cast_or_null<NamedDecl>(D))
...
static void getCursorPlatformAvailabilityForDecl(
  ...
  if (AvailabilityAttrs.empty()) {
    ...
    if (auto *const DeclContext *DC = D->getDeclContext(); IMD = dyn_cast<ObjCImplementationDecl>(D))
...
CXTLSKind clang_getCursorTLSKind(CXCursor cursor) {
  ...
  if (const VarDecl *const Decl *D = cxcursor::getCursorDecl(cursor); VD = dyn_cast<VarDecl>(D)) {
...
unsigned clang_Cursor_getObjCDeclQualifiers(CXCursor C) {
  ...
  if (const ObjCMethodDecl *const Decl *D = getCursorDecl(C); MD = dyn_cast<ObjCMethodDecl>(D))
...
unsigned clang_Cursor_isExternalSymbol(CXCursor C, CXString *language,
  ...
  if (auto *const Decl *D = getCursorDecl(C); attr = D->getExternalSourceSymbolAttr()) {
    if (language)
...
enum CXUnaryOperatorKind clang_getCursorUnaryOperatorKind(CXCursor cursor) {
  if (clang_isExpression(cursor.kind)) {
    ...
    if (const auto *const Expr *expr = getCursorExpr(cursor); op = dyn_cast<UnaryOperator>(expr))
