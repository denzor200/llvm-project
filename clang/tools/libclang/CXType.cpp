...
CXType clang_getTypedefDeclUnderlyingType(CXCursor C) {
  ...
  if (clang_isDeclaration(C.kind)) {
    ...
    if (const TypedefNameDecl *const Decl *D = cxcursor::getCursorDecl(C); TD = dyn_cast_or_null<TypedefNameDecl>(D)) {
...
CXType clang_getEnumDeclIntegerType(CXCursor C) {
  ...
  if (clang_isDeclaration(C.kind)) {
    ...
    if (const EnumDecl *const Decl *D = cxcursor::getCursorDecl(C); TD = dyn_cast_or_null<EnumDecl>(D)) {
...
long long clang_getEnumConstantDeclValue(CXCursor C) {
  ...
  if (clang_isDeclaration(C.kind)) {
    ...
    if (const EnumConstantDecl *const Decl *D = cxcursor::getCursorDecl(C); TD = dyn_cast_or_null<EnumConstantDecl>(D)) {
...
int clang_getFieldDeclBitWidth(CXCursor C) {
  ...
  if (clang_isDeclaration(C.kind)) {
    ...
    if (const FieldDecl *const Decl *D = getCursorDecl(C); FD = dyn_cast_or_null<FieldDecl>(D)) {
...
CXType clang_getCursorResultType(CXCursor C) {
  if (clang_isDeclaration(C.kind)) {
    ...
    if (const ObjCMethodDecl *const Decl *D = cxcursor::getCursorDecl(C); MD = dyn_cast_or_null<ObjCMethodDecl>(D))
...
unsigned clang_Cursor_isAnonymous(CXCursor C){
  ...
  if (const NamespaceDecl *const Decl *D = cxcursor::getCursorDecl(C); ND = dyn_cast_or_null<NamespaceDecl>(D)) {
...
unsigned clang_Cursor_isAnonymousRecordDecl(CXCursor C){
  ...
  if (const RecordDecl *const Decl *D = cxcursor::getCursorDecl(C); FD = dyn_cast_or_null<RecordDecl>(D))
...
CXType clang_Type_getNamedType(CXType CT){
  ...
  if (QualType T = GetQualType(CT); !T.isNull() && !T.isCanonical())
    return MakeCXType(T, GetTU(CT));
...
unsigned clang_Type_isTransparentTagTypedef(CXType TT){
  ...
  if (auto *QualType T = GetQualType(TT); TT = dyn_cast_or_null<TypedefType>(T.getTypePtrOrNull())) {
