...
static void diagnoseBadTypeAttribute(Sema &S, const ParsedAttr &attr,
  ...
  if (IdentifierInfo *II =
      attr.isArgIdent(0) ? attr.getArgAsIdent(0)->getIdentifierInfo() : nullptr; useExpansionLoc && loc.isMacroID() && II) {
    if (II->isStr("strong")) {
...
static DeclaratorChunk *maybeMovePastReturnType(Declarator &declarator,
  ...
  for (; i != 0; --i) {
    ...
    switch (DeclaratorChunk &fnChunk = declarator.getTypeObject(i-1); fnChunk.Kind) {
    ...
    case DeclaratorChunk::Function:
      for (--i; i != 0; --i) {
        ...
        switch (DeclaratorChunk &ptrChunk = declarator.getTypeObject(i-1); ptrChunk.Kind) {
        case DeclaratorChunk::Paren:
        case DeclaratorChunk::Array:
        case DeclaratorChunk::Function:
        case DeclaratorChunk::Reference:
...
static void distributeObjCPointerTypeAttr(TypeProcessingState &state,
  ...
  for (unsigned i = state.getCurrentChunkIndex(); i != 0; --i) {
    ...
    switch (DeclaratorChunk &chunk = declarator.getTypeObject(i-1); chunk.Kind) {
    case DeclaratorChunk::Pointer:
    case DeclaratorChunk::BlockPointer: {
...
static void distributeObjCPointerTypeAttrFromDeclarator(
  ...
  for (unsigned i = 0, e = declarator.getNumTypeObjects(); i != e; ++i) {
    ...
    switch (DeclaratorChunk &chunk = declarator.getTypeObject(i); chunk.Kind) {
    case DeclaratorChunk::Pointer:
    case DeclaratorChunk::BlockPointer:
      innermost = i;
      continue;
...
#include "clang/Basic/HLSLIntangibleTypes.def"
  ...
  if (S.getLangOpts().OpenCL) {
    ...
    if (bool IsOpenCLC30Compatible =
        S.getLangOpts().getOpenCLCompatibleVersion() == 300; (Result->isImageType() || Result->isSamplerT()) &&
  ...
  if (unsigned TypeQuals = DS.getTypeQualifiers()) {
    ...
    if (QualType Qualified = S.BuildQualifiedType(Result, DeclLoc, TypeQuals, &DS); Qualified.isNull())
      declarator.setInvalidType(true);
    else
      Result = Qualified;
  }
...
QualType Sema::BuildArrayType(QualType T, ArraySizeModifier ASM,
  ...
  if (Context.getTargetInfo().getTriple().isWasm() && T->isArrayType()) {
    ...
    if (const auto *ATy = dyn_cast<ArrayType>(T); ATy && ATy->getElementType().isWebAssemblyReferenceType()) {
      ...
      if (unsigned ActiveSizeBits =
          (!T->isDependentType() && !T->isVariablyModifiedType() &&
           !T->isIncompleteType() && !T->isUndeducedType())
              ...
              : ConstVal.getActiveBits(); ActiveSizeBits > ConstantArrayType::getMaxSizeBits(Context)) {
  ...
  if (getLangOpts().OpenCL) {
    ...
    if (const QualType ArrType = Context.getBaseElementType(T); ArrType->isBlockPointerType() || ArrType->isPipeType() ||
...
static bool CheckBitIntElementType(Sema &S, SourceLocation AttrLoc,
  ...
  if (unsigned NumBits = BIT->getNumBits(); !llvm::isPowerOf2_32(NumBits))
...
QualType Sema::BuildExtVectorType(QualType T, Expr *SizeExpr,
  ...
  if (bool IsNoBoolVecLang = getLangOpts().OpenCL || getLangOpts().OpenCLCPlusPlus; (!T->isDependentType() && !T->isIntegerType() &&
...
static void checkExtParameterInfos(Sema &S, ArrayRef<QualType> paramTypes,
  ...
  auto checkCompatible = [&](unsigned paramIndex, RequiredCC required) {
    ...
    if (bool isCompatible =
        (required == RequiredCC::OnlySwift)
            ? (actualCC == CC_Swift)
            : (actualCC == CC_Swift || actualCC == CC_SwiftAsync); isCompatible || emittedError)
      return;
...
QualType Sema::BuildMemberPointerType(QualType T, const CXXScopeSpec &SS,
  ...
  if (bool IsCtorOrDtor =
      (Entity.getNameKind() == DeclarationName::CXXConstructorName) ||
      (Entity.getNameKind() == DeclarationName::CXXDestructorName); T->isFunctionType())
...
static void inferARCWriteback(TypeProcessingState &state,
  ...
  for (unsigned i = 0, e = declarator.getNumTypeObjects(); i != e; ++i) {
    ...
    switch (DeclaratorChunk &chunk = declarator.getTypeObject(chunkIndex); chunk.Kind) {
    case DeclaratorChunk::Paren:
      // Ignore parens.
      break;
...
void Sema::diagnoseIgnoredQualifiers(unsigned DiagID, unsigned Quals,
  ...
  for (auto &E : QualKinds) {
    if (Quals & E.Mask) {
      ...
      if (SourceLocation QualLoc = E.Loc; QualLoc.isValid()) {
...
static void diagnoseRedundantReturnTypeQualifiers(Sema &S, QualType RetTy,
  ...
  if (const DeclaratorChunk::FunctionTypeInfo &FTI =
      D.getTypeObject(FunctionChunkIndex).Fun; FTI.hasTrailingReturnType()) {
  ...
  for (unsigned OuterChunkIndex = FunctionChunkIndex + 1,
    ...
    switch (DeclaratorChunk &OuterChunk = D.getTypeObject(OuterChunkIndex); OuterChunk.Kind) {
    case DeclaratorChunk::Paren:
      continue;
...
static QualType GetDeclSpecTypeForDeclarator(TypeProcessingState &state,
  ...
  if (Deduced) {
    ...
    case DeclaratorContext::Member: {
      ...
      if (bool Cxx = SemaRef.getLangOpts().CPlusPlus; isa<ObjCContainerDecl>(SemaRef.CurContext)) {
...
static void warnAboutAmbiguousFunction(Sema &S, Declarator &D,
  ...
  if (!D.isFirstDeclarator() && D.getIdentifier()) {
    ...
    if (FullSourceLoc Name(D.getIdentifierLoc(), S.SourceMgr); Comma.getFileID() != Name.getFileID() ||
  ...
  if (FTI.NumParams > 0) {
    ...
    // no-op.
    if (const CXXRecordDecl *RD = RT->getAsCXXRecordDecl(); RD && RD->hasDefinition() &&
...
static CallingConv getCCForDeclaratorChunk(
  ...
  for (const ParsedAttr &AL : AttrList) {
    ...
    CALLING_CONV_ATTRS_CASELIST : {
      ...
      if (CallingConv CC; !S.CheckCallingConvAttr(AL, CC, /*FunctionDecl=*/nullptr,
  ...
  if (S.getLangOpts().CUDA) {
    ...
    if (llvm::Triple Triple = S.Context.getTargetInfo().getTriple(); Triple.isSPIRV() && Triple.getVendor() != llvm::Triple::AMD) {
...
static bool hasOuterPointerLikeChunk(const Declarator &D, unsigned endIndex) {
  ...
  while (i != 0) {
    ...
    switch (const DeclaratorChunk &DC = D.getTypeObject(i); DC.Kind) {
    case DeclaratorChunk::Paren:
      break;
    case DeclaratorChunk::Array:
    case DeclaratorChunk::Pointer:
...
static TypeSourceInfo *GetFullTypeForDeclarator(TypeProcessingState &state,
  ...
  if (auto *DT = T->getAs<DeducedType>()) {
    ...
    if (bool IsClassTemplateDeduction = isa<DeducedTemplateSpecializationType>(DT); (AT && AT->isDecltypeAuto()) || IsClassTemplateDeduction) {
      for (unsigned I = 0, E = D.getNumTypeObjects(); I != E; ++I) {
        ...
        case DeclaratorChunk::Function: {
          ...
          if (unsigned FnIndex; D.isFunctionDeclarationContext() &&
  ...
  if (IsTypedefName) {
    ...
    for (unsigned i = 0, n = D.getNumTypeObjects(); i != n; ++i) {
      ...
      switch (DeclaratorChunk &chunk = D.getTypeObject(i); chunk.Kind) {
      case DeclaratorChunk::Array:
      case DeclaratorChunk::Function:
      case DeclaratorChunk::Pipe:
        break;
    ...
    switch (auto bool isFunctionOrMethod = false; context = state.getDeclarator().getContext()) {
    case DeclaratorContext::ObjCParameter:
    case DeclaratorContext::ObjCResult:
    case DeclaratorContext::Prototype:
    case DeclaratorContext::TrailingReturn:
    ...
    case DeclaratorContext::KNRTypeList: {
      ...
      switch (auto wrappingKind = PointerWrappingDeclaratorKind::None; classifyPointerDeclarator(S, T, D, wrappingKind)) {
      ...
      case PointerDeclaratorKind::MaybePointerToCFRef:
        if (isFunctionOrMethod) {
          ...
          if (const auto *auto hasCFReturnsAttr =
          ...
          }; InnermostChunk = D.getInnermostNonParenChunk()) {
  ...
  for (unsigned i = 0, e = D.getNumTypeObjects(); i != e; ++i) {
    ...
    case DeclaratorChunk::Array: {
      ...
      if (chunkIndex != 0 && !ArraySize &&
        ...
        if (DeclaratorChunk &NextDeclType = D.getTypeObject(chunkIndex - 1); NextDeclType.Kind == DeclaratorChunk::Array &&
    ...
    case DeclaratorChunk::Function: {
      ...
      if (!D.isInvalidType()) {
        ...
        if (bool IsFunctionDecl = D.getInnermostNonParenChunk() == &DeclType; First && First->isExplicitObjectParameter() &&
        ...
        if (state.getSema().getLangOpts().OpenCLCPlusPlus && IsClassMember()) {
          ...
          if (FTI.MethodQualifiers)
            for (ParsedAttr &attr : FTI.MethodQualifiers->getAttributes()) {
              ...
              if (LangAS ASIdxNew = attr.asOpenCLLangAS(); DiagnoseMultipleAddrSpaceAttributes(S, ASIdx, ASIdxNew,
                                                      attr.getLoc()))
                D.setInvalidType(true);
              else
                ASIdx = ASIdxNew;
  ...
  if (!LangOpts.requiresStrictPrototypes()) {
    ...
    for (const DeclaratorChunk &DeclType : D.type_objects()) {
      ...
      case DeclaratorChunk::Function: {
        ...
        if (const DeclaratorChunk::FunctionTypeInfo &FTI = DeclType.Fun; !FTI.hasPrototype && FTI.NumParams == 0 && !FTI.isVariadic &&
  ...
  if (LangOpts.CPlusPlus && T->isFunctionType()) {
    ...
    if (D.getName().getKind() == UnqualifiedIdKind::IK_DeductionGuideName)
      ...
      if (DeclContext *DC = S.computeDeclContext(D.getCXXScopeSpec()); !DC || DC->isRecord())
        Kind = Member;
    }

    if (Kind == Member) {
      
      if (unsigned I; D.isFunctionDeclarator(I)) {
        
        if (const DeclaratorChunk &Chunk = D.getTypeObject(I); Chunk.Fun.NumParams) {
          
          if (auto *P = dyn_cast_or_null<ParmVarDecl>(Chunk.Fun.Params->Param); P && P->isExplicitObjectParameter())
...
static void transferARCOwnership(TypeProcessingState &state,
  ...
  if (DeclaratorChunk &chunk = D.getTypeObject(inner); chunk.Kind == DeclaratorChunk::Pointer) {
    if (declSpecTy->isObjCRetainableType())
...
TypeSourceInfo *Sema::GetTypeForDeclaratorCast(Declarator &D, QualType FromTy) {
  ...
  if (getLangOpts().ObjC) {
    ...
    if (Qualifiers::ObjCLifetime ownership = Context.getInnerObjCOwnership(FromTy); ownership != Qualifiers::OCL_None)
...
static bool handleObjCOwnershipTypeAttr(TypeProcessingState &state,
  ...
  if (!type->isDependentType() && !type->isUndeducedType()) {
    if (const PointerType *ptr = type->getAs<PointerType>()) {
      ...
      if (QualType pointee = ptr->getPointeeType(); pointee->isObjCRetainableType() || pointee->isPointerType())
        return false;
    ...
    if (state.isProcessingDeclSpec()) {
      ...
      if (Declarator &D = state.getDeclarator(); maybeMovePastReturnType(D, D.getNumTypeObjects(),
                                  /*onlyBlockPointers=*/true))
        return false;
    }
  }
  ...
  struct FunctionTypeUnwrapper {
    ...
    FunctionTypeUnwrapper(Sema &S, QualType T) : Original(T) {
      while (true) {
        ...
        if (const Type *Ty = T.getTypePtr(); isa<FunctionType>(Ty)) {
  ...
  private:
    ...
    QualType wrap(ASTContext &C, const Type *Old, unsigned I) {
      ...
      case Reference: {
        ...
        if (QualType New = wrap(C, OldRef->getPointeeType(), I); isa<LValueReferenceType>(OldRef))
...
static void HandleSwiftAttr(TypeProcessingState &State, TypeAttrLocation TAL,
  ...
  if (TAL == TAL_DeclChunk) {
    ...
    if (auto chunkIdx = State.getCurrentChunkIndex(); chunkIdx >= 1 &&
  ...
  if (auto ExistingNullability = Desugared->getNullability()) {
    if (Nullability != *ExistingNullability && !Implicit) {
      ...
      if (auto TT = Desugared->getAs<TypedefType>()) {
        ...
        if (auto QualType underlyingType = typedefDecl->getUnderlyingType(); typedefNullability =
...
static bool handleFunctionTypeAttr(TypeProcessingState &state, ParsedAttr &attr,
  ...
  if (!supportsVariadicCall(CC)) {
    ...
    if (const FunctionProtoType *FnP = dyn_cast<FunctionProtoType>(fn); FnP && FnP->isVariadic()) {
...
void Sema::adjustMemberFunctionCC(QualType &T, bool HasThisPointer,
  ...
  // Default adjustment.
    ...
    if (CallingConv DefaultCC =
        Context.getDefaultCallingConvention(IsVariadic, !HasThisPointer); CurCC != DefaultCC)
      return;
...
static void HandleVectorSizeAttr(QualType &CurType, const ParsedAttr &Attr,
  ...
  if (QualType T = S.BuildVectorType(CurType, SizeExpr, Attr.getLoc()); !T.isNull())
    CurType = T;
  else
    Attr.setInvalid();
}
...
static void HandleExtVectorTypeAttr(QualType &CurType, const ParsedAttr &Attr,
  ...
  if (QualType T = S.BuildExtVectorType(CurType, SizeExpr, Attr.getLoc()); !T.isNull())
    CurType = T;
}
...
static void HandleArmMveStrictPolymorphismAttr(TypeProcessingState &State,
  ...
  if (const VectorType *VT = dyn_cast<VectorType>(CurType); !VT || VT->getVectorKind() != VectorKind::Neon) {
...
static void HandleMatrixTypeAttr(QualType &CurType, const ParsedAttr &Attr,
  ...
  if (QualType T = S.BuildMatrixType(CurType, RowsExpr, ColsExpr, Attr.getLoc()); !T.isNull())
    CurType = T;
}
...
static void HandleLifetimeCaptureByAttr(TypeProcessingState &State,
  ...
  if (State.getDeclarator().isDeclarationOfFunction()) {
    ...
    if (auto *Attr = State.getSema().ParseLifetimeCaptureByAttr(PA, "this"); Attr)
  ...
  for (ParsedAttr &attr : AttrsCopy) {
    ...
    if (attr.isStandardAttributeSyntax() || attr.isRegularKeywordAttribute()) {
      ...
      if (attr.isGNUScope()) {
        ...
        if (bool IsTypeAttr = attr.isTypeAttr(); TAL == TAL_DeclChunk) {
          state.getSema().Diag(attr.getLoc(),
                               IsTypeAttr
...
void Sema::completeExprArrayBound(Expr *E) {
  ...
  if (const auto CastE = dyn_cast<ExplicitCastExpr>(E)) {
    ...
    if (const auto *QualType DestType = CastE->getTypeAsWritten(); IAT = Context.getAsIncompleteArrayType(DestType)) {
...
bool Sema::RequireCompleteTypeImpl(SourceLocation Loc, QualType T,
  ...
  if (!Incomplete) {
    ...
    if (NamedDecl *Suggested = nullptr; Def &&
  ...
  if (auto *RD = dyn_cast_or_null<CXXRecordDecl>(Tag)) {
    ...
    if (RD->isDependentContext()) {
      ...
      if (CXXRecordDecl *Pattern = RD->getInstantiatedFromMemberClass(); !RD->isBeingDefined() && Pattern) {
...
QualType Sema::BuildTypeofExprType(Expr *E, TypeOfKind Kind) {
  ...
  if (!E->isTypeDependent()) {
    ...
    if (const TagType *QualType T = E->getType(); TT = T->getAs<TagType>())
  ...
  if (getCurLambda() && isa<ParenExpr>(IDExpr)) {
    if (auto *DRE = dyn_cast<DeclRefExpr>(IDExpr->IgnoreParens())) {
      if (auto *Var = dyn_cast<VarDecl>(DRE->getDecl())) {
        ...
        if (QualType T = getCapturedDeclRefType(Var, DRE->getLocation()); !T.isNull())
...
QualType Sema::BuiltinEnumUnderlyingType(QualType BaseType,
  ...
  if (NamedDecl *FwdDecl = nullptr; BaseType->isIncompleteType(&FwdDecl)) {
...
static QualType ChangeIntegralSignedness(Sema &S, QualType BaseType,
  ...
  if (BaseType->isEnumeralType()) {
    ...
    if (auto *BitInt = dyn_cast<BitIntType>(Underlying)) {
      ...
      if (unsigned int Bits = BitInt->getNumBits(); Bits > 1)
