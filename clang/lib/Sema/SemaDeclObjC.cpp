...
void SemaObjC::CheckObjCMethodOverride(ObjCMethodDecl *NewMethod,
  ...
  if (ASTContext &Context = getASTContext(); Overridden->hasRelatedResultType() &&
    ...
    if (!CurrentClass) {
      ...
      if (ObjCCategoryDecl *DeclContext *DC = NewMethod->getDeclContext(); Cat = dyn_cast<ObjCCategoryDecl>(DC))
...
void SemaObjC::ActOnStartOfObjCMethodDef(Scope *FnBodyScope, Decl *D) {
  ...
  if (QualType ResultType = MDecl->getReturnType(); !ResultType->isDependentType() && !ResultType->isVoidType() &&
      !MDecl->isInvalidDecl() &&
...
void SemaObjC::ActOnSuperClassOfClassInterface(
  ...
  if (!PrevDecl) {
    ...
    if (TypoCorrection ObjCInterfaceValidatorCCC CCC(IDecl); Corrected = SemaRef.CorrectTypo(
...
DeclResult SemaObjC::actOnObjCTypeParam(
  ...
  if (parsedTypeBound) {
    ...
    if (QualType typeBound =
        SemaRef.GetTypeFromParser(parsedTypeBound, &typeBoundInfo); typeBound->isObjCObjectPointerType()) {
      // okay
...
void SemaObjC::ImplMethodsVsClassMethods(Scope *S, ObjCImplDecl *IMPDecl,
  ...
  if (ObjCInterfaceDecl *LazyProtocolNameSet ExplicitImplProtocols; I = dyn_cast<ObjCInterfaceDecl> (CDecl)) {
    for (auto *PI : I->all_referenced_protocols())
...
CheckRelatedResultTypeCompatibility(Sema &S, ObjCMethodDecl *Method,
  ...
  if (const ObjCObjectPointerType *QualType ResultType = Method->getReturnType(); ResultObjectType
...
#include "clang/AST/DeclNodes.inc"
  ...
  void search(const ObjCContainerDecl *container) {
    ...
    if (ObjCMethodDecl *meth = container->getMethod(Method->getSelector(),
                                                Method->isInstanceMethod(),
                                                /*AllowHidden=*/true); meth) {
...
Decl *SemaObjC::ActOnMethodDeclaration(
  ...
  for (unsigned i = 0, e = CNumArgs; i != e; ++i) {
    ...
    if (QualType ArgType = Param->getType(); ArgType.isNull())
      ArgType = Context.getObjCIdType();
    else
...
ObjCInterfaceDecl *SemaObjC::getObjCInterfaceDecl(const IdentifierInfo *&Id,
  ...
  if (!IDecl && DoTypoCorrection) {
    ...
    if (TypoCorrection DeclFilterCCC<ObjCInterfaceDecl> CCC{}; C = SemaRef.CorrectTypo(
