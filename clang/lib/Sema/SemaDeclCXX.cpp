...
bool Sema::MergeCXXFunctionDecl(FunctionDecl *New, FunctionDecl *Old,
  ...
  if (const FunctionDecl *Def; New->getConstexprKind() != Old->getConstexprKind()) {
...
void Sema::CheckCompleteDecompositionDeclaration(DecompositionDecl *DD) {
  ...
  switch (unsigned TupleSize; isTupleLike(*this, DD->getLocation(), DecompType, TupleSize)) {
...
UnsignedOrNone Sema::GetDecompositionElementCount(QualType T,
  ...
  switch (unsigned TupleSize; isTupleLike(*this, Loc, T, TupleSize)) {
  case IsTupleLike::Error:
    return std::nullopt;
  case IsTupleLike::TupleLike:
    return TupleSize;
...
namespace {
  ...
  public:
    ...
    void VisitCXXMemberCallExpr(CXXMemberCallExpr *E) {
      ...
      if (Expr *Callee = E->getCallee(); isa<MemberExpr>(Callee)) {
  ...
  if (TemplateTypeTy) {
    ...
    if (!TyD) {
      ...
      if (SS.isSet() && isDependentScopeSpecifier(SS)) {
        ...
        if (CXXRecordDecl *DeclContext *DC = computeDeclContext(SS, false); Record = dyn_cast_or_null<CXXRecordDecl>(DC))
...
static bool defaultedSpecialMemberIsConstexpr(
  ...
  if (!S.getLangOpts().CPlusPlus23) {
    for (const auto *F : ClassDecl->fields()) {
      ...
      if (const RecordType *QualType BaseType = S.Context.getBaseElementType(F->getType()); RecordTy = BaseType->getAsCanonical<RecordType>()) {
...
void Sema::CheckExplicitlyDefaultedFunction(Scope *S, FunctionDecl *FD) {
  ...
  if (DefKind.isComparison()) {
    ...
    if (const CXXRecordDecl *auto PT = FD->getParamDecl(0)->getType(); RD =
...
bool Sema::CheckExplicitlyDefaultedSpecialMember(CXXMethodDecl *MD,
  ...
  if (CSM == CXXSpecialMemberKind::CopyAssignment ||
    ...
    if (QualType ExpectedReturnType = Context.getLValueReferenceType(DeclType); !Context.hasSameType(ReturnType, ExpectedReturnType)) {
...
protected:
  ...
  Result visitSubobject(QualType Type, Subobject Subobj) {
    ...
    if (auto *const ArrayType *AT = S.Context.getAsArrayType(Type); CAT = dyn_cast_or_null<ConstantArrayType>(AT))
...
private:
  ...
  visitBinaryOperator(OverloadedOperatorKind OO, ArrayRef<Expr *> Args,
    ...
    switch (OverloadCandidateSet::iterator Best; CandidateSet.BestViableFunction(S, FD->getLocation(), Best)) {
    case OR_Success: {
...
bool Sema::CheckExplicitlyDefaultedComparison(Scope *S, FunctionDecl *FD,
  ...
  if (IsMethod) {
    ...
    if (QualType T = MD->getFunctionObjectParameterReferenceType(); !T.getNonReferenceType().isConstQualified() &&
...
public:
  ...
  bool operator()(const CXXBaseSpecifier *Specifier, CXXBasePath &Path) {
    ...
    for (Path.Decls = BaseRecord->lookup(Name).begin();
      ...
      if (CXXMethodDecl *NamedDecl *D = *Path.Decls; MD = dyn_cast<CXXMethodDecl>(D)) {
...
static void findImplicitlyDeclaredEqualityComparisons(
  ...
  if (DeclarationName EqEq = Ctx.DeclarationNames.getCXXOperatorName(OO_EqualEqual); !RD->lookup(EqEq).empty())
...
QualType Sema::CheckDestructorDeclarator(Declarator &D, QualType R,
  ...
  if (const TypedefType *QualType DeclaratorType = GetTypeFromParser(D.getName().DestructorName); TT = DeclaratorType->getAs<TypedefType>())
...
Decl *Sema::ActOnStartNamespaceDef(Scope *NamespcScope,
  ...
  if (II) {
    ...
    if (TranslationUnitDecl *DeclContext *Parent = CurContext->getRedeclContext(); TU = dyn_cast<TranslationUnitDecl>(Parent)) {
...
Decl *Sema::ActOnUsingDeclaration(Scope *S, AccessSpecifier AS,
  ...
  if (DeclarationName TargetName = TargetNameInfo.getName(); !TargetName)
    return nullptr;
...
Decl *Sema::ActOnNamespaceAliasDef(Scope *S, SourceLocation NamespaceLoc,
  ...
  if (PrevR.isSingleResult()) {
    ...
    if (NamespaceAliasDecl *NamedDecl *PrevDecl = PrevR.getRepresentativeDecl(); AD = dyn_cast<NamespaceAliasDecl>(PrevDecl)) {
  ...
  if (auto *ClassDecl = T->getAsCXXRecordDecl()) {
    ...
    if (!S.getLangOpts().CPlusPlus11) {
      ...
      while (F.hasNext()) {
        ...
        if (CXXMethodDecl *NamedDecl *D = F.next(); Method = dyn_cast<CXXMethodDecl>(D))
...
void Sema::DefineImplicitCopyAssignment(SourceLocation CurrentLocation,
  ...
  for (auto *Field : ClassDecl->fields()) {
    ...
    if (QualType BaseType = Context.getBaseElementType(Field->getType()); !BaseType->isRecordType() && BaseType.isConstQualified()) {
      Diag(ClassDecl->getLocation(), diag::err_uninitialized_member_for_assign)
...
static void WriteCharValueForDiagnostic(uint32_t Value, const BuiltinType *BTy,
  ...
  if (K == BuiltinType::Char_S || K == BuiltinType::Char_U ||
    ...
    if (StringRef Escaped = escapeCStyle<EscapeChar::Single>(Value); !Escaped.empty())
...
static bool EvaluateAsStringImpl(Sema &SemaRef, Expr *Message,
  ...
  auto FindMember = [&](StringRef Member) -> std::optional<LookupResult> {
    ...
    if (OverloadCandidateSet Candidates(MemberLookup.getNameLoc(),
                                    OverloadCandidateSet::CSK_Normal); MemberLookup.empty())
      return std::nullopt;
...
DeclResult Sema::ActOnTemplatedFriendTag(
  ...
  if (isAllExplicitSpecializations) {
    ...
    if (QualType T = CheckTypenameType(Keyword, TagLoc, QualifierLoc, *Name,
                                   NameLoc, &TSI, /*DeducedTSTContext=*/true); T.isNull())
      return true;
...
bool Sema::CheckPureMethod(CXXMethodDecl *Method, SourceRange InitRange) {
  ...
  if (SourceLocation EndLoc = InitRange.getEnd(); EndLoc.isValid())
