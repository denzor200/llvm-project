...
bool SemaOpenACC::ForStmtBeginChecker::checkForInit(const Stmt *InitStmt,
  ...
  if (const auto *BO = dyn_cast<BinaryOperator>(InitStmt)) {
    ...
    if (const auto *const Expr *LHS = BO->getLHS()->IgnoreParenImpCasts(); DRE = dyn_cast<DeclRefExpr>(LHS))
      InitVar = DRE->getDecl();
    ...
    if (auto *const Expr *LHS = CE->getArg(0)->IgnoreParenImpCasts(); DRE = dyn_cast<DeclRefExpr>(LHS)) {
      InitVar = DRE->getDecl();
