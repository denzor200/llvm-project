...
ExprResult SemaObjC::BuildObjCStringLiteral(SourceLocation AtLoc,
  ...
  if (!Ty.isNull()) {
    ...
    if (std::string StringClass(getLangOpts().ObjCConstantStringClass); StringClass.empty())
    ...
    if (ObjCInterfaceDecl *NamedDecl *IF = SemaRef.LookupSingleName(SemaRef.TUScope, NSIdent, AtLoc,
                                             Sema::LookupOrdinaryName); StrIF = dyn_cast_or_null<ObjCInterfaceDecl>(IF)) {
      Context.setObjCConstantStringInterface(StrIF);
      Ty = Context.getObjCConstantStringInterface();
      Ty = Context.getObjCObjectPointerType(Ty);
    } else {
...
static bool validateBoxingMethod(Sema &S, SourceLocation Loc,
  ...
  if (QualType ReturnType = Method->getReturnType(); !ReturnType->isObjCObjectPointerType()) {
...
static ExprResult CheckObjCCollectionLiteralElement(Sema &S, Expr *Element,
  ...
  if (S.getLangOpts().CPlusPlus && Element->getType()->isRecordType()) {
    ...
    if (InitializationSequence Seq(S, Entity, Kind, Element); !Seq.Failed())
...
ExprResult SemaObjC::BuildObjCBoxedExpr(SourceRange SR, Expr *ValueExpr) {
  ...
  if (const PointerType *PT = ValueType->getAs<PointerType>()) {
    ...
    if (QualType PointeeType = PT->getPointeeType(); Context.hasSameUnqualifiedType(PointeeType, Context.CharTy)) {
...
CheckObjCDictionaryLiteralDuplicateKeys(Sema &S,
  ...
  auto checkOneKey = [&](auto &Map, const auto &Key, SourceLocation Loc) {
    ...
    if (auto Pair = Map.insert({Key, Loc}); !Pair.second) {
...
ExprResult SemaObjC::BuildObjCDictionaryLiteral(
  ...
  if (SourceLocation Loc = SR.getBegin(); !NSDictionaryDecl) {
...
ExprResult SemaObjC::ParseObjCEncodeExpression(SourceLocation AtLoc,
  ...
  if (QualType EncodedType = SemaRef.GetTypeFromParser(ty, &TInfo); !TInfo)
  ...
  if (!Method) {
    ...
    if (!getLangOpts().DebuggerSupport) {
      ...
      if (const ObjCMethodDecl *OMD = SelectorsForTypoCorrection(Sel, ReceiverType); OMD && !OMD->isInvalidDecl()) {
        ...
        if (SourceRange SelectorRange(SelectorLocs.front(), SelectorLocs.back()); MatchedSel.isUnarySelector())
          Diag(SelLoc, DiagID)
...
ExprResult SemaObjC::HandleExprPropertyRefExpr(
  ...
  if (TypoCorrection Corrected = SemaRef.CorrectTypo(
    ...
    if (DeclarationName TypoResult = Corrected.getCorrection(); TypoResult.isIdentifier() &&
        TypoResult.getAsIdentifierInfo() == Member) {
      // There is no need to try the correction if it is the same.
      
      if (NamedDecl *ChosenDecl =
        Corrected.isKeyword() ? nullptr : Corrected.getFoundDecl(); ChosenDecl && isa<ObjCPropertyDecl>(ChosenDecl))
  ...
  if (ObjCIvarDecl *Ivar =
    ...
    if (const ObjCObjectPointerType * QualType T = Ivar->getType(); OBJPT =
...
SemaObjC::getObjCMessageKind(Scope *S, IdentifierInfo *Name,
  ...
  case LookupResultKind::NotFound:
    ...
    if (ObjCMethodDecl *Method = SemaRef.getCurMethodDecl()) {
      ...
      if (ObjCInterfaceDecl *ClassDeclared; Method->getClassInterface()->lookupInstanceVariable(Name,
                                                              ClassDeclared))
        return ObjCInstanceMessage;
    }
...
DiagnoseCStringFormatDirectiveInObjCAPI(Sema &S,
  ...
  if (ObjCStringFormatFamily SFFamily = Sel.getStringFormatFamily(); SFFamily == ObjCStringFormatFamily::SFF_NSString) {
    Idx = 0;
    Format = true;
  }
  else if (Method) {
...
ExprResult SemaObjC::BuildInstanceMessage(
  ...
  if (Receiver) {
    ...
    if (ReceiverType->isObjCRetainableType()) {
      ...
      if (ExprResult result =
          SemaRef.PerformContextuallyConvertToObjCPointer(Receiver); result.isUsable()) {
        Receiver = result.get();
        ReceiverType = Receiver->getType();
      }
    }
  ...
  if (!Method) {
    ...
    if (bool receiverIsIdLike = ReceiverType->isObjCIdOrObjectKindOfType(Context,
      ...
      if (const ObjCObjectPointerType *ObjCInterfaceDecl *ClassDecl = nullptr; QIdTy
  ...
  if (Method && Method->isDirectMethod()) {
    ...
    if (ReceiverType->isObjCClassType() && !isImplicit &&
        !(Receiver->isObjCSelfExpr() && getLangOpts().ObjCAutoRefCount)) {
      {
        ...
        if (auto Builder = Diag(Receiver->getExprLoc(),
                            diag::err_messaging_class_with_direct_method); Receiver->isObjCSelfExpr()) {
    ...
    if (SuperLoc.isValid()) {
      {
        ...
        if (auto Builder =
            Diag(SuperLoc, diag::err_messaging_super_with_direct_method); ReceiverType->isObjCClassType()) {
          Builder.AddFixItHint(FixItHint::CreateReplacement(
  ...
  if (getLangOpts().ObjCAutoRefCount) {
    ...
    switch (ObjCMethodFamily family =
      (Method ? Method->getMethodFamily() : Sel.getMethodFamily()); family) {
    case OMF_init:
      if (Method)
      ...
      if (Method && NumArgs >= 1) {
        if (const auto *SelExp =
          ...
          if (SelMethod) {
            ...
            switch (ObjCMethodFamily SelFamily = SelMethod->getMethodFamily(); SelFamily) {
              case OMF_alloc:
              case OMF_copy:
              case OMF_mutableCopy:
              case OMF_new:
...
static void addFixitForObjCARCConversion(
  ...
  if (CCK == CheckedConversionKind::CStyleCast) {
    ...
    if (SourceRange range = castedE->getSourceRange(); isa<ParenExpr>(castedE)) {
      DiagB.AddFixItHint(FixItHint::CreateInsertion(range.getBegin(),
                         castCode));
    } else {
      castCode += "(";
...
static inline T *getObjCBridgeAttr(const TypedefType *TD) {
  ...
  if (QualType QT = TDNDecl->getUnderlyingType(); QT->isPointerType()) {
  ...
  while (const auto *TD = T->getAs<TypedefType>()) {
    ...
    if (TB *ObjCBAttr = getObjCBridgeAttr<TB>(TD)) {
      if (IdentifierInfo *Parm = ObjCBAttr->getBridgedType()) {
        ...
        if (LookupResult R(S, DeclarationName(Parm), SourceLocation(),
                       Sema::LookupOrdinaryName); S.LookupName(R, S.TUScope)) {
          
          if (NamedDecl *Target = R.getFoundDecl(); Target && isa<ObjCInterfaceDecl>(Target)) {
            
            if (const ObjCObjectPointerType *ObjCInterfaceDecl *ExprClass = cast<ObjCInterfaceDecl>(Target); InterfacePointerType =
      ...
      if (IdentifierInfo *Parm = ObjCBAttr->getBridgedType()) {
        ...
        if (LookupResult R(S, DeclarationName(Parm), SourceLocation(),
          ...
          if (Target && isa<ObjCInterfaceDecl>(Target)) {
            ...
            if (const ObjCObjectPointerType *ObjCInterfaceDecl *CastClass = cast<ObjCInterfaceDecl>(Target); InterfacePointerType =
...
void SemaObjC::CheckTollFreeBridgeCast(QualType castType, Expr *castExpr) {
  ...
  if (ARCConversionTypeClass castACTC = classifyTypeForARCConversion(castType); castACTC == ACTC_retainable && exprACTC == ACTC_coreFoundation) {
...
Expr *SemaObjC::stripARCUnbridgedCast(Expr *e) {
  ...
  if (ParenExpr *ASTContext &Context = getASTContext(); pe = dyn_cast<ParenExpr>(e)) {
...
DeclResult SemaObjC::LookupIvarInObjCMethod(LookupResult &Lookup, Scope *S,
  ...
  if (LookForIvars) {
    ...
    if (ObjCInterfaceDecl *IFace = CurMethod->getClassInterface()) {
      ...
      if (ObjCIvarDecl *ObjCInterfaceDecl *ClassDeclared; IV = IFace->lookupInstanceVariable(II, ClassDeclared)) {
        if (IV->getAccessControl() != ObjCIvarDecl::Private ||
...
ExprResult SemaObjC::ActOnObjCBoolLiteral(SourceLocation OpLoc,
  ...
  if (!Context.getBOOLDecl()) {
    ...
    if (LookupResult Result(SemaRef, &Context.Idents.get("BOOL"), OpLoc,
                        Sema::LookupOrdinaryName); SemaRef.LookupName(Result, SemaRef.getCurScope()) &&
        Result.isSingleResult()) {
      
      if (TypedefDecl *NamedDecl *ND = Result.getFoundDecl(); TD = dyn_cast<TypedefDecl>(ND))
...
CastKind SemaObjC::PrepareCastToObjCObjectPointer(ExprResult &E) {
  ...
  if (QualType type = E.get()->getType(); type->isObjCObjectPointerType()) {
    return CK_BitCast;
...
SemaObjC::ObjCLiteralKind SemaObjC::CheckLiteralKind(Expr *FromE) {
  ...
  case Stmt::ObjCBoxedExprClass: {
    ...
    case Stmt::ImplicitCastExprClass: {
      ...
      if (CastKind CK = cast<CastExpr>(Inner)->getCastKind(); CK == CK_IntegralToBoolean || CK == CK_IntegralCast)
        return LK_Numeric;
      break;
    }
    default:
