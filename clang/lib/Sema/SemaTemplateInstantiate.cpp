  ...
  if (!isa<ClassTemplateSpecializationDecl>(FTD->getDeclContext())) {
    ...
    for (const Type *Ty = NNS.getKind() == NestedNameSpecifier::Kind::Type
      ...
      if (TSTy->isCurrentInstantiation()) {
        ...
        if (auto *RD = TSTy->getCanonicalTypeInternal()->getAsCXXRecordDecl(); ClassTemplateDecl *CTD = RD->getDescribedClassTemplate())
...
void Sema::PrintInstantiationStack(InstantiationContextDiagFuncRef DiagFunc) {
  ...
  for (SmallVectorImpl<CodeSynthesisContext>::reverse_iterator
    ...
    case CodeSynthesisContext::TemplateInstantiation: {
      ...
      if (Decl *D = Active->Entity; CXXRecordDecl *Record = dyn_cast<CXXRecordDecl>(D)) {
  ...
  public:
    ...
    void transformedLocalDecl(Decl *Old, ArrayRef<Decl *> NewDecls) {
      ...
      if (auto *NewMD = dyn_cast<CXXMethodDecl>(New); NewMD && isLambdaCallOperator(NewMD)) {
        ...
        if (auto *OldMD = dyn_cast<CXXMethodDecl>(Old); auto *NewTD = NewMD->getDescribedFunctionTemplate())
      ...
      for (ParmVarDecl *PVD : MD->parameters()) {
        ...
        if (SourceLocation EqualLoc = UninstExpr->getBeginLoc(); SemaRef.SubstDefaultArgument(EqualLoc, PVD, TemplateArgs)) {
          ...
          if (ExprResult ErrorResult = SemaRef.CreateRecoveryExpr(
              ...
              UninstExpr->getType()); ErrorResult.isUsable())
...
TemplateInstantiator::TransformFirstQualifierInScope(NamedDecl *D,
  ...
  if (TemplateTypeParmDecl *TTPD = dyn_cast_or_null<TemplateTypeParmDecl>(D)) {
    ...
    if (const TemplateTypeParmType *TTP
      = cast<TemplateTypeParmType>(getSema().Context.getTypeDeclType(TTPD)); TTP->getDepth() < TemplateArgs.getNumLevels()) {
...
Sema::InstantiateClassMembers(SourceLocation PointOfInstantiation,
  ...
  for (auto *D : Instantiation->decls()) {
    ...
    if (bool SuppressNew = false; auto *Function = dyn_cast<FunctionDecl>(D)) {
