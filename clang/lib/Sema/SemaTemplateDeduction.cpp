...
checkDeducedTemplateArguments(ASTContext &Context,
  ...
  case TemplateArgument::Type: {
    ...
    if (QualType TX = X.getAsType(), TY = Y.getAsType(); Y.getKind() == TemplateArgument::Type && Context.hasSameType(TX, TY))
...
ConvertDeducedTemplateArgument(Sema &S, NamedDecl *Param,
  ...
  if (Arg.getKind() == TemplateArgument::Pack) {
    ...
    if (SugaredPackedArgsBuilder.empty()) {
      ...
      if (auto *Sema::ArgPackSubstIndexRAII OnlySubstNonPackExpansion(S, std::nullopt); NTTP = dyn_cast<NonTypeTemplateParmDecl>(Param)) {
...
ResolveOverloadForDeduction(Sema &S, TemplateParameterList *TemplateParams,
  ...
  for (UnresolvedSetIterator I = Ovl->decls_begin(),
    ...
    if (FunctionTemplateDecl *FunTmpl = dyn_cast<FunctionTemplateDecl>(D)) {
      ...
      if (TemplateDeductionInfo Info(Ovl->getNameLoc()); S.DeduceTemplateArguments(FunTmpl, &ExplicitTemplateArgs,
                                    Specialization,
