...
HeuristicResolverImpl::resolveExprToDecls(const Expr *E) {
  ...
  if (const auto *CE = dyn_cast<CallExpr>(E)) {
    ...
    if (const auto *QualType T = resolveTypeOfCallExpr(CE); D = resolveTypeToTagDecl(T)) {
      return {D};
    }
    return {};
  }
...
bool HeuristicResolverImpl::findOrdinaryMemberInDependentClasses(
  ...
  if (const auto *TagDecl *TD = resolveTypeToTagDecl(Specifier->getType()); RD = dyn_cast_if_present<CXXRecordDecl>(TD)) {
...
HeuristicResolverImpl::getFunctionProtoTypeLoc(const Expr *Fn) {
  ...
  if (const auto *const Expr *NakedFn = Fn->IgnoreParenCasts(); T = NakedFn->getType().getTypePtr()->getAs<TypedefType>()) {
    Target = T->getDecl()->getTypeSourceInfo()->getTypeLoc();
  } else if (const auto *DR = dyn_cast<DeclRefExpr>(NakedFn)) {
    
    if (const auto *const const auto *D = DR->getDecl(); VD = dyn_cast<VarDecl>(D)) {
      Target = VD->getTypeSourceInfo()->getTypeLoc();
    }
  } else if (const auto *ME = dyn_cast<MemberExpr>(NakedFn)) {
    
    if (const auto *const auto *MD = ME->getMemberDecl(); FD = dyn_cast<FieldDecl>(MD)) {
