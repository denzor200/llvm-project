  ...
  if ((IC = dyn_cast<ObjCImplementationDecl>(ClassImpDecl))) {
    ...
    if (Synthesize && (PIkind & ObjCPropertyAttribute::kind_readonly) &&
      ...
      for (auto *Ext : IDecl->known_extensions()) {
        ...
        if (auto *DeclContext::lookup_result R = Ext->lookup(property->getDeclName()); ExtProp = R.find_first<ObjCPropertyDecl>()) {
  ...
  if (ObjCMethodDecl *setterMethod = property->getSetterMethodDecl()) {
    ...
    if (getLangOpts().CPlusPlus && Synthesize && !CompleteTypeErr &&
      ...
      if (property->getPropertyAttributes() &
        ...
        if (const CXXOperatorCallExpr *Expr *callExpr = Res.getAs<Expr>(); CXXCE =
  ...
  for (const auto *PID : D->property_impls()) {
    ...
    if (const ObjCPropertyDecl *PD = PID->getPropertyDecl(); PD && !PD->hasAttr<NSReturnsNotRetainedAttr>() &&
      ...
      if (family == OMF_alloc || family == OMF_copy ||
        ...
        if (StringRef macroName = PP.getLastMacroWithSpelling(noteLoc, tokens); !macroName.empty())
          spelling = macroName;
...
void SemaObjC::ProcessPropertyDecl(ObjCPropertyDecl *property) {
  ...
  if (!GetterMethod) {
    ...
    if (property->getPropertyAttributes() &
      ...
      if (auto QualType modifiedTy = resultTy; nullability = AttributedType::stripOuterNullability(modifiedTy)) {
  ...
  if (!property->isReadOnly()) {
    ...
    if (!SetterMethod) {
      ...
      if (property->getPropertyAttributes() &
        ...
        if (auto QualType modifiedTy = paramTy; nullability = AttributedType::stripOuterNullability(modifiedTy)){
          if (*nullability == NullabilityKind::Unspecified)
