...
static bool hasCounterHandle(const CXXRecordDecl *RD) {
  ...
  if (const FieldDecl *SecondField = *It; const auto *ResTy =
...
void SemaHLSL::handleVkConstantIdAttr(Decl *D, const ParsedAttr &AL) {
  ...
  if (HLSLVkConstantIdAttr *NewAttr = mergeVkConstantIdAttr(D, AL, Id); NewAttr)
    D->addAttr(NewAttr);
}
...
void SemaHLSL::handlePackOffsetAttr(Decl *D, const ParsedAttr &AL) {
  ...
  if (bool IsAggregateTy = (T->isArrayType() || T->isStructureType()); Component) {
...
void SemaHLSL::handleShaderAttr(Decl *D, const ParsedAttr &AL) {
  ...
  if (HLSLShaderAttr *NewAttr = mergeShaderAttr(D, AL, ShaderType); NewAttr)
    D->addAttr(NewAttr);
}
...
void SemaHLSL::handleResourceBindingAttr(Decl *TheDecl, const ParsedAttr &AL) {
  ...
  if (IdentifierLoc *Loc = AL.getArgAsIdent(0); AL.getNumArgs() == 2) {
...
void DiagnoseHLSLAvailability::HandleFunctionOrMethodRef(FunctionDecl *FD,
  ...
  if (const AvailabilityAttr *AA = FindAvailabilityAttr(FD); AA)
...
static void SetElementTypeAsReturnType(Sema *S, CallExpr *TheCall,
  ...
  if (auto *VecTyA = TheCall->getArg(0)->getType()->getAs<VectorType>(); VecTyA)
    ReturnType =
...
bool SemaHLSL::CheckBuiltinFunctionCall(unsigned BuiltinID, CallExpr *TheCall) {
  ...
  case Builtin::BI__builtin_hlsl_select: {
    ...
    if (auto *VTy = ArgTy->getAs<VectorType>(); VTy && VTy->getElementType()->isBooleanType() &&
...
void SemaHLSL::ActOnVariableDeclarator(VarDecl *VD) {
  if (VD->hasGlobalStorage()) {
    ...
    if (VD->getType()->isHLSLResourceRecordArray()) {
      ...
      if (const CXXRecordDecl *RD = Ty->getAsCXXRecordDecl(); hasCounterHandle(RD)) {
