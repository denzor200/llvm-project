...
public:
  ...
  void handleNeverCalled(const ParmVarDecl *Parameter, const Decl *Function,
    ...
    if (PartialDiagnosticAt Warning(Where->getBeginLoc(), S.PDiag(DiagToReport)
                                                          << Parameter
                                                          << IsCalledDirectly
                                                          << (unsigned)Reason); const auto *Block = dyn_cast<BlockDecl>(Function)) {
 ...
 public:
  ...
  void handleMutexNotHeld(StringRef Kind, const NamedDecl *D,
    ...
    if (unsigned DiagID = 0; PossibleMatch) {
      ...
      if (PartialDiagnosticAt Note(Loc, S.PDiag(diag::note_found_mutex_near_match)
                                        << *PossibleMatch); Verbose && POK == POK_VarAccess) {
      ...
      if (PartialDiagnosticAt Warning(Loc, S.PDiag(DiagID) << Kind
                                                       << D
                                                       << LockName << LK); Verbose && POK == POK_VarAccess) {
