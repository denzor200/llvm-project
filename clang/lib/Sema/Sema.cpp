...
public:
  ...
  void FileChanged(SourceLocation Loc, FileChangeReason Reason,
    ...
    case EnterFile: {
      ...
      if (SourceLocation IncludeLoc = SM.getIncludeLoc(SM.getFileID(Loc)); IncludeLoc.isValid()) {
  ...
  void PragmaDiagnostic(SourceLocation Loc, StringRef Namespace,
    ...
    if (StringRef Group = Str.substr(2); S->PP.getDiagnostics().getDiagnosticIDs()->getDiagnosticsInGroup(
...
void Sema::Initialize() {
  ...
  if (Context.getTargetInfo().hasInt128Type() ||
    ...
    if (DeclarationName Int128 = &Context.Idents.get("__int128_t"); IdResolver.begin(Int128) == IdResolver.end())
  ...
  if (getLangOpts().ObjC) {
    ...
    if (DeclarationName SEL = &Context.Idents.get("SEL"); IdResolver.begin(SEL) == IdResolver.end())
  ...
  if (DeclarationName ConstantString = &Context.Idents.get("__NSConstantString"); IdResolver.begin(ConstantString) == IdResolver.end())
  ...
  if (getLangOpts().OpenCL) {
    ...
    if (auto OCLCompatibleVersion = getLangOpts().getOpenCLCompatibleVersion(); OCLCompatibleVersion >= 200) {
...
NamedDecl *Sema::getCurFunctionOrMethodDecl() const {
  ...
  if (DeclContext *DC = getFunctionLevelDeclContext(); isa<ObjCMethodDecl>(DC) || isa<FunctionDecl>(DC))
...
void Sema::EmitDiagnostic(unsigned DiagID, const DiagnosticBuilder &DB) {
  ...
  if (SFINAETrap *Diagnostic DiagInfo(&Diags, DB); Trap = getSFINAEContext()) {
...
static void checkEscapingByref(VarDecl *VD, Sema &S) {
  ...
  if (auto IE = InitializedEntity::InitializeBlock(Loc, T); S.getLangOpts().CPlusPlus23) {
