...
SmallVector<unsigned> TemplateParamsReferencedInTemplateArgumentList(
  ...
  for (unsigned Index = 0; Index < TemplateParamsList->size(); ++Index) {
    ...
    if (auto *Param = TemplateParamsList->getParam(Index); auto *TTPD = dyn_cast<TemplateTypeParmDecl>(Param))
...
FunctionTemplateDecl *Sema::DeclareAggregateDeductionGuideFromInitList(
  ...
  if (DeclContext *DC = Template->getDeclContext(); DC->isDependentContext())
    return nullptr;
