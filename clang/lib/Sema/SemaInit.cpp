  ...
  if (SemaRef.getLangOpts().CPlusPlus || isa<InitListExpr>(expr)) {
    ...
    if (auto TmpEntity =
        (ElemType->isExtVectorType() && !Entity.getType()->isExtVectorType())
            ...
            : Entity; TmpEntity.getType()->isDependentType()) {
      // C++ [over.match.class.deduct]p1.5:
      //   brace elision is not considered for any aggregate element that has a
...
InitListChecker::CheckDesignatedInitializer(const InitializedEntity &Entity,
  ...
  if (D->isFieldDesignator()) {
    ...
    if (!KnownField) {
      ...
      if (ValueDecl *VD = SemaRef.tryLookupUnambiguousFieldDecl(RD, FieldName); auto *FD = dyn_cast_if_present<FieldDecl>(VD)) {
        KnownField = FD;
      ...
      if (!KnownField) {
        ...
        if (FieldInitializerValidatorCCC CCC(RD); TypoCorrection Corrected = SemaRef.CorrectTypo(
...
static OverloadingResult ResolveConstructorOverload(
  ...
  if (S.getLangOpts().CPlusPlus17 && Args.size() == 1 &&
    ...
    if (auto *SourceRD = Initializer->getType()->getAsCXXRecordDecl(); SourceRD && S.isCompleteType(DeclLoc, Initializer->getType())) {
...
ResolveOverloadedFunctionForReferenceBinding(Sema &S,
  ...
  if (S.Context.getCanonicalType(UnqualifiedSourceType) ==
    ...
    if (bool HadMultipleCandidates = false; FunctionDecl *Fn
...
static void CheckCXX98CompatAccessibleCopy(Sema &S,
  ...
  switch (PartialDiagnostic Diag = S.PDiag(diag::warn_cxx98_compat_temp_copy)
    << OR << (int)Entity.getKind() << CurInitExpr->getType()
    << CurInitExpr->getSourceRange(); OR) {
  case OR_Success:
...
PerformConstructorInitialization(Sema &S,
  ...
  if (isExplicitTemporary(Entity, Kind, NumArgs)) {
    ...
    if (Kind.getKind() == InitializationKind::IK_Value &&
      ...
      if (auto *RD = Step.Type.getCanonicalType()->getAsCXXRecordDecl(); RD && RD->isAggregate() && RD->hasUninitializedExplicitInitFields()) {
        unsigned I = 0;
        for (const FieldDecl *FD : RD->fields()) {
...
ExprResult Sema::PerformQualificationConversion(Expr *E, QualType Ty,
  ...
  if (VK == VK_PRValue) {
    ...
    if (auto ExprPointeeTy = E->getType()->getPointeeType(); !PointeeTy.isNull() &&
...
ExprResult InitializationSequence::Perform(Sema &S,
  ...
  if (getKind() == DependentSequence) {
    ...
    if (ResultType && !Entity.getType()->isDependentType() &&
      ...
      if (QualType DeclType = Entity.getType(); const IncompleteArrayType *ArrayT
  ...
  for (step_iterator Step = step_begin(), StepEnd = step_end();
    ...
    case SK_UserConversion: {
      ...
      if (shouldBindAsTemporary(Entity))
        ...
        if (QualType T = CurInit.get()->getType(); auto *Record = T->castAsCXXRecordDecl()) {
...
bool InitializationSequence::Diagnose(Sema &S,
  ...
  if (Args.size() == 1) {
    ...
    if (OnlyArg->getType() == S.Context.OverloadTy) {
      ...
      if (DeclAccessPair Found; FunctionDecl *FD = S.ResolveAddressOfOverloadedFunction(
...
QualType Sema::DeduceTemplateSpecializationFromInitializer(
  ...
  if (!Template) {
    if (auto *AliasTemplate = dyn_cast_or_null<TypeAliasTemplateDecl>(
      ...
      if (auto UnderlyingType = AliasTemplate->getTemplatedDecl()
                                ->getUnderlyingType()
                                .getCanonicalType(); const auto *TST =
  ...
  if (ListInit) {
    ...
    if (!ListInit->getNumInits()) {
      ...
      if (auto *RD = E->getType()->getAsCXXRecordDecl(); !isa<InitListExpr>(E) && RD &&
