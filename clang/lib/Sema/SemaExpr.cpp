...
static void CheckForNullPointerDereference(Sema &S, Expr *E) {
  ...
  if (const auto *UO = dyn_cast<UnaryOperator>(E->IgnoreParenCasts()); UO && UO->getOpcode() == UO_Deref &&
    ...
    if (const LangAS AS =
        UO->getSubExpr()->getType()->getPointeeType().getAddressSpace(); (!isTargetAddressSpace(AS) ||
...
static bool CheckAlignOfExpr(Sema &S, Expr *E, UnaryExprOrTypeTrait ExprKind) {
  ...
  if (Expr *Inner = E->IgnoreParens(); DeclRefExpr *DRE = dyn_cast<DeclRefExpr>(Inner)) {
    D = DRE->getDecl();
...
ExprResult Sema::BuildCallExpr(Scope *Scope, Expr *Fn, SourceLocation LParenLoc,
  ...
  if (Fn->getType() == Context.OverloadTy) {
    ...
    if (OverloadExpr::FindResult find = OverloadExpr::find(Fn); !find.HasFormOfMemberPointer || find.IsAddressOfOperandWithParen) {
      ...
      if (OverloadExpr *ovl = find.Expression; UnresolvedLookupExpr *ULE = dyn_cast<UnresolvedLookupExpr>(ovl))
...
AssignConvertType Sema::CheckSingleAssignmentConstraints(QualType LHSType,
  ...
  if (getLangOpts().CPlusPlus) {
    ...
    if (DeclAccessPair DAP; FunctionDecl *FD = ResolveAddressOfOverloadedFunction(
  ...
  if ((LHSTypeAfterConversion->isPointerType() ||
    ...
    if (Diagnose || ConvertRHS) {
      ...
      if (Kind != CK_NoOp && !getLangOpts().CPlusPlus &&
        ...
        if (QualType CanLHS = LHSType.getCanonicalType().getUnqualifiedType(); CanRHS->isVoidPointerType() && CanLHS->isPointerType()) {
  ...
  if (int NullValue = PP.isMacroDefined("NULL") ? 0 : 1; !E.get()->getType()->isAnyPointerType() &&
    ...
    if (const auto *CL = dyn_cast<CharacterLiteral>(E.get())) {
      if (CL->getValue() == 0)
        ...
        if (QualType T = Context.getCanonicalType(TI->getType()).getUnqualifiedType(); T == Context.CharTy)
          Diag(E.get()->getExprLoc(), diag::warn_pointer_compare)
              << NullValue
              << FixItHint::CreateReplacement(E.get()->getExprLoc(),
                                              NullValue ? "NULL" : "(void *)0");
...
static void DiagnoseConstAssignment(Sema &S, const Expr *E,
  ...
  while (true) {
    ...
    if (const MemberExpr *ME = dyn_cast<MemberExpr>(E)) {
      ...
      if (const ValueDecl *VD = ME->getMemberDecl(); const FieldDecl *Field = dyn_cast<FieldDecl>(VD)) {
...
static ValueDecl *getPrimaryDecl(Expr *E) {
  ...
  case Stmt::ArraySubscriptExprClass: {
    ...
    if (Expr* Base = cast<ArraySubscriptExpr>(E)->getBase(); ImplicitCastExpr* ICE = dyn_cast<ImplicitCastExpr>(Base)) {
...
static void checkObjCPointerIntrospection(Sema &S, ExprResult &L, ExprResult &R,
  ...
  if (ObjCPointerExpr && isa<IntegerLiteral>(OtherExpr->IgnoreParenCasts())) {
    ...
    if (const Expr *Ex = ObjCPointerExpr->IgnoreParenCasts(); const ObjCMessageExpr *ME = dyn_cast<ObjCMessageExpr>(Ex)) {
...
MarkVarDeclODRUsed(ValueDecl *V, SourceLocation Loc, Sema &SemaRef,
  ...
  if (Var->hasDefinition(SemaRef.Context) == VarDecl::DeclarationOnly &&
    ...
    if (SourceLocation &old = SemaRef.UndefinedButUsed[Var->getCanonicalDecl()]; old.isInvalid())
      old = Loc;
  }
