...
struct DependencyChecker : DynamicRecursiveASTVisitor {
  ...
  DependencyChecker(TemplateParameterList *Params, bool IgnoreNonTypeDependent)
    ...
    if (NamedDecl *ND = Params->getParam(0); TemplateTypeParmDecl *PD = dyn_cast<TemplateTypeParmDecl>(ND)) {
      Depth = PD->getDepth();
...
TemplateParameterList *Sema::MatchTemplateParametersToScopeSpecifier(
  ...
  while (!T.isNull()) {
    ...
    if (const EnumType *EnumT = T->getAsCanonical<EnumType>()) {
      ...
      if (EnumDecl *Enum = EnumT->getDecl(); TypeDecl *Parent = dyn_cast<TypeDecl>(Enum->getParent()))
  ...
  for (unsigned TypeIdx = 0, NumTypes = NestedTypes.size(); TypeIdx != NumTypes;
    ...
    if (CXXRecordDecl *Record = T->getAsCXXRecordDecl()) {
      ...
      if (TemplateName Name = TST->getTemplateName(); TemplateDecl *Template = Name.getAsTemplateDecl()) {
...
static QualType builtinCommonTypeImpl(Sema &S, ElaboratedTypeKeyword Keyword,
  ...
  switch (auto lookUpCommonType = [&](TemplateArgument T1,
  ...
  if (!IsCtorOrDtorName && !IsClassName && SS.isSet()) {
    ...
    if (!LookupCtx && isDependentScopeSpecifier(SS)) {
      ...
      if (AllowImplicitTypename == ImplicitTypenameContext::Yes) {
        ...
        if (auto DB = DiagCompat(SS.getBeginLoc(), diag_compat::implicit_typename)
                  << NNS; !getLangOpts().CPlusPlus20)
...
DeclResult Sema::ActOnClassTemplateSpecialization(
  ...
  if (TemplateParams && TemplateParams->size() > 0) {
    ...
    for (unsigned I = 0, N = TemplateParams->size(); I != N; ++I) {
      ...
      if (Decl *Param = TemplateParams->getParam(I); TemplateTypeParmDecl *TTP = dyn_cast<TemplateTypeParmDecl>(Param)) {
...
bool Sema::CheckFunctionTemplateSpecialization(
  ...
  for (LookupResult::iterator I = Previous.begin(), E = Previous.end();
    ...
    if (NamedDecl *Ovl = (*I)->getUnderlyingDecl(); FunctionTemplateDecl *FunTmpl = dyn_cast<FunctionTemplateDecl>(Ovl)) {
