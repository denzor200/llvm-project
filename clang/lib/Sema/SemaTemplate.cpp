...
NamedDecl *Sema::ActOnTemplateTemplateParameter(
  ...
  if (!Default.isInvalid()) {
    ...
    if (TemplateDecl *Template = Name.getAsTemplateDecl(); Template &&
...
bool Sema::CheckFunctionTemplateSpecialization(
  ...
  if (auto *SpecializationFPT =
      Specialization->getType()->castAs<FunctionProtoType>(); getLangOpts().CPlusPlus17 &&
...
DeclResult Sema::ActOnExplicitInstantiation(
  ...
  if (Def) {
    ...
    if (bool NewlyDLLExported =
        !PreviouslyDLLExported && Specialization->hasAttr<DLLExportAttr>(); Old_TSK == TSK_ImplicitInstantiation && NewlyDLLExported &&
        Context.getTargetInfo().shouldDLLImportComdatSymbols()) {
      // An explicit instantiation definition can add a dll attribute to a
