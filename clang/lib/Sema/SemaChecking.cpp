...
static bool BuiltinAnnotation(Sema &S, CallExpr *TheCall) {
  ...
  if (StringLiteral *Literal = dyn_cast<StringLiteral>(StrArg); !Literal || !Literal->isOrdinary()) {
...
static bool BuiltinOverflow(Sema &S, CallExpr *TheCall, unsigned BuiltinID) {
  ...
  for (unsigned I = 0; I < 2; ++I) {
    ...
    if (bool IsValid = CkdOperation ? ValidCkdIntType(Ty) : Ty->isIntegerType(); !IsValid) {
  ...
  // the other qualifiers aren't possible.
  {
    ...
    if (const auto *PtrTy = Ty->getAs<PointerType>(); !PtrTy ||
...
struct BuiltinDumpStructGenerator {
  ...
  bool dumpUnnamedRecord(const RecordDecl *RD, Expr *E, unsigned Depth) {
    ...
    if (Expr *TypeLit = getTypeString(S.Context.getCanonicalTagType(RD)); IndentLit ? callPrintFunction("%s%s", {IndentLit, TypeLit})
...
public:
  ...
  bool HandleScanfSpecifier(const analyze_scanf::ScanfSpecifier &FS,
    ...
    if (unsigned DestSize = DestSizeAPS->getZExtValue(); DestSize < SourceSize)
...
void Sema::checkFortifiedBuiltinMemoryFunction(FunctionDecl *FD,
  ...
  auto ComputeExplicitObjectSizeArgument =
    ...
    if (Expr *SizeArg = TheCall->getArg(NewIndex); !SizeArg->EvaluateAsInt(Result, getASTContext()))
      return std::nullopt;
...
#include "clang/Basic/Builtins.inc"
  ...
  case Builtin::BI__builtin_elementwise_ldexp: {
    ...
    if (unsigned Arg1Length = Vec1 ? Vec1->getNumElements() : 0; Arg0Length != Arg1Length) {
...
ExprResult Sema::BuiltinShuffleVector(CallExpr *TheCall) {
  ...
  if (!TheCall->getArg(0)->isTypeDependent() &&
    ...
    if (unsigned NumResElements = NumArgs - 2; NumArgs == 2) {
...
CheckPrintfHandler::checkFormatExpr(const analyze_printf::PrintfSpecifier &FS,
  ...
  if (bool Success =
      fixedFS.fixType(IntendedTy, S.getLangOpts(), S.Context, isObjCContext()); Success) {
    ...
    if (const PointerType *DestPtrTy = DestTy->getAs<PointerType>()) {
      ...
      if (SizeOfArg &&
        ...
        if (DestID == SizeOfArgID) {
          ...
          if (SourceManager &SM = getSourceManager(); SM.isMacroArgExpansion(SL)) {
...
void Sema::CheckImplicitConversion(Expr *E, QualType T, SourceLocation CC,
  ...
  if (SourceBT && SourceBT->isSveVLSBuiltinType()) {
    ...
    // specified
    if (const Type *OriginalTarget = Context.getCanonicalType(T).getTypePtr(); ARM().areCompatibleSveTypes(QualType(OriginalTarget, 0),
                                    QualType(Source, 0)) ||
  ...
  if (SourceBT && TargetBT && SourceBT->isIntegerType() &&
    ...
    if (unsigned int TargetPrecision = llvm::APFloatBase::semanticsPrecision(
        Context.getFloatTypeSemantics(QualType(TargetBT, 0))); SourcePrecision > 0 && TargetPrecision > 0 &&
...
getDerivedToBaseAlignmentAndOffset(const CastExpr *CE, QualType DerivedType,
  ...
  for (auto PathI = CE->path_begin(), PathE = CE->path_end(); PathI != PathE;
    ...
    if (const CXXRecordDecl *BaseDecl = Base->getType()->getAsCXXRecordDecl(); Base->isVirtual()) {
