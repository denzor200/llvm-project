...
static bool BuiltinOverflow(Sema &S, CallExpr *TheCall, unsigned BuiltinID) {
  ...
  if (BuiltinID == Builtin::BI__builtin_mul_overflow) {
    for (unsigned I = 0; I < 3; ++I) {
      ...
      if (auto Ty = I < 2 ? Arg->getType() : Arg->getType()->getPointeeType(); Ty->isBitIntType() && Ty->isSignedIntegerType() &&
  ...
  case Builtin::BI__builtin_vsnprintf: {
    ...
    if (SourceSize &&
      ...
      if (const char *FormatBytes = FormatStrRef.data(); !analyze_format_string::ParsePrintfString(
        ...
        if (llvm::APSInt FormatSize =
            llvm::APSInt::getUnsigned(H.getSizeLowerBound())
                .extOrTrunc(SizeTypeWidth); FormatSize > *SourceSize && *SourceSize != 0) {
...
void Sema::checkCall(NamedDecl *FDecl, const FunctionProtoType *Proto,
  ...
  if (FDecl && FDecl->hasAttr<AllocAlignAttr>()) {
    ...
    if (const Expr *Arg = Args[AA->getParamIndex().getASTIndex()]; !Arg->isValueDependent()) {
...
bool Sema::BuiltinCountedByRef(CallExpr *TheCall) {
  ...
  if (const auto *ME = dyn_cast<MemberExpr>(Arg)) {
    ...
    if (auto *CATy =
      ...
      if (const auto *FAMDecl = cast<FieldDecl>(ME->getMemberDecl()); const FieldDecl *CountFD = FAMDecl->findCountedByField()) {
...
tryAgain:
  ...
  case Stmt::DeclRefExprClass: {
    ...
    if (const DeclRefExpr *DR = cast<DeclRefExpr>(E); const VarDecl *VD = dyn_cast<VarDecl>(DR->getDecl())) {
  ...
  case Stmt::CXXMemberCallExprClass: {
    ...
    if (const CallExpr *CE = cast<CallExpr>(E); const NamedDecl *ND = dyn_cast_or_null<NamedDecl>(CE->getCalleeDecl())) {
      bool IsFirst = true;
  ...
  case Stmt::ObjCMessageExprClass: {
    ...
    if (const auto *ME = cast<ObjCMessageExpr>(E); const auto *MD = ME->getMethodDecl()) {
...
static const Expr *maybeConstEvalStringLiteral(ASTContext &Context,
  ...
  if (E->EvaluateAsRValue(Result, Context) && Result.Val.isLValue()) {
    ...
    if (const auto *LVE = Result.Val.getLValueBase().dyn_cast<const Expr *>(); isa_and_nonnull<StringLiteral>(LVE))
      return LVE;
  }
  return nullptr;
}
...
CXXRecordMembersNamed(StringRef Name, Sema &S, QualType Ty) {
  ...
  if (S.LookupQualifiedName(R, RD))
    for (LookupResult::iterator I = R.begin(), E = R.end(); I != E; ++I) {
      ...
      if (NamedDecl *decl = (*I)->getUnderlyingDecl(); MemberKind *FK = dyn_cast<MemberKind>(decl))
        Results.insert(FK);
    }
  return Results;
}
  ...
  if (isObjCContext() &&
    ...
    if (ExprTy->isIntegralOrUnscopedEnumerationType() &&
      ...
      if (const IntegerLiteral *IL = dyn_cast<IntegerLiteral>(E)) {
        ...
        if (const llvm::APInt &V = IL->getValue(); V.getActiveBits() <= S.Context.getTypeSize(IntendedTy))
          return true;
      }
      ...
      if (LookupResult Result(S, &S.Context.Idents.get("unichar"), E->getBeginLoc(),
        ...
        if (NamedDecl *ND = Result.getFoundDecl(); TypedefNameDecl *TD = dyn_cast<TypedefNameDecl>(ND))
...
static const CXXRecordDecl *getContainedDynamicClass(QualType T,
  ...
  for (auto *FD : RD->fields()) {
    ...
    if (bool SubContained; const CXXRecordDecl *ContainedRD =
...
void CheckFreeArgumentsAddressof(Sema &S, const std::string &CalleeName,
  ...
  if (const auto *Lvalue = dyn_cast<DeclRefExpr>(UnaryExpr->getSubExpr())) {
    ...
    if (const Decl *D = Lvalue->getDecl(); const auto *DD = dyn_cast<DeclaratorDecl>(D)) {
  ...
  if (FD) {
    ...
    if (OverloadedOperatorKind Op = FD->getOverloadedOperator(); Op == OO_New || Op == OO_Array_New) {
      ...
      if (const FunctionProtoType *Proto
        = FD->getType()->castAs<FunctionProtoType>(); !Proto->isNothrow(/*ResultIfDependent*/true) &&
          CheckNonNullExpr(*this, RetValExp))
...
void Sema::CheckFloatComparison(SourceLocation Loc, const Expr *LHS,
  ...
  if (getCastAndLiteral(LHS, RHS) || getCastAndLiteral(RHS, LHS)) {
    ...
    if (auto *TargetTy = FPLiteral->getType()->getAs<BuiltinType>(); SourceTy && TargetTy && SourceTy->isFloatingPoint() &&
...
static bool CheckTautologicalComparison(Sema &S, BinaryOperator *E,
  ...
  bool TautologicalTypeCompare = false;
  {
    ...
    if (auto TypeCmp = OtherPromotedTypeRange.compare(Value); auto TypeResult = PromotedRange::constantValue(E->getOpcode(), TypeCmp,
                                                       RhsConstant)) {
...
void Sema::CheckImplicitConversion(Expr *E, QualType T, SourceLocation CC,
  ...
  if (SourceBT && SourceBT->isFloatingPoint()) {
    ...
    if (Target->isBooleanType() && isa<CallExpr>(E)) {
      ...
      if (CallExpr *CEx = cast<CallExpr>(E); unsigned NumArgs = CEx->getNumArgs()) {
...
void Sema::DiagnoseAlwaysNonNullPointer(Expr *E,
  ...
  if (IsAddressOf) {
    ...
    if (PartialDiagnostic PD = PDiag(DiagID) << E->getSourceRange() << Range
                                         << IsEqual; CheckForReference(*this, E, PD)) {
      return;
    }
  }
...
void Sema::DiscardMisalignedMemberAddress(const Type *T, Expr *E) {
  ...
  if (isa<UnaryOperator>(E) &&
    ...
    if (auto *Op = cast<UnaryOperator>(E)->getSubExpr()->IgnoreParens(); isa<MemberExpr>(Op)) {
      ...
      if (auto *MA = llvm::find(MisalignedMembersForExpr, MisalignedMember(Op)); MA != MisalignedMembersForExpr.end() &&
...
static bool checkBuiltinVectorMathArgTypes(Sema &SemaRef,
  ...
  for (Expr *ArgI : Args.drop_front()) {
    ...
    if (const auto *VecI = ArgI->getType()->getAs<VectorType>(); !VecI ||
