...
void ModuleMap::resolveHeader(Module *Mod,
  ...
  if (SmallString<128> RelativePathName; OptionalFileEntryRef File =
          findHeader(Mod, Header, RelativePathName, NeedsFramework)) {
    if (Header.IsUmbrella) {
      
      if (const DirectoryEntry *UmbrellaDir = &File->getDir().getDirEntry(); Module *UmbrellaMod = UmbrellaDirs[UmbrellaDir])
...
Module *ModuleMap::inferFrameworkModule(DirectoryEntryRef FrameworkDir,
  ...
  if (!Parent) {
    ...
    if (llvm::sys::path::has_parent_path(FrameworkDirName)) {
      ...
      if (StringRef Parent = llvm::sys::path::parent_path(FrameworkDirName); auto ParentDir = FileMgr.getOptionalDirectoryRef(Parent)) {
