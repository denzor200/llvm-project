...
static void appendCodePoint(unsigned Codepoint,
  ...
  if (char *ResultPtr = ResultBuf; llvm::ConvertCodePointToUTF8(Codepoint, ResultPtr))
...
static bool ProcessUCNEscape(const char *ThisTokBegin, const char *&ThisTokBuf,
  ...
  if (UcnVal < 0xa0 &&
    ...
    if (Diags) {
      ...
      if (char BasicSCSChar = UcnVal; UcnVal >= 0x20 && UcnVal < 0x7f)
        Diag(Diags, Features, Loc, ThisTokBegin, UcnBegin, ThisTokBuf,
...
CharLiteralParser::CharLiteralParser(const char *begin, const char *end,
  ...
  while (begin != end) {
    ...
    if (begin[1] == 'u' || begin[1] == 'U' || begin[1] == 'N') {
      ...
      if (unsigned short UcnLen = 0; !ProcessUCNEscape(TokBegin, begin, end, *buffer_begin, UcnLen,
...
void StringLiteralParser::init(ArrayRef<Token> StringToks){
  ...
  for (unsigned i = 0, e = StringToks.size(); i != e; ++i) {
    ...
    if (ThisTokEnd[-1] != '"') {
      ...
      if (StringRef UDSuffix(ThisTokEnd, UDSuffixEnd - ThisTokEnd); UDSuffixBuf.empty()) {
        ...
        if (UDSuffixBuf != UDSuffix || UnevaluatedStringHasUDL) {
          if (Diags) {
            ...
            if (SourceLocation TokLoc = StringToks[i].getLocation(); UnevaluatedStringHasUDL) {
  ...
  if (Pascal) {
    ...
    if (unsigned MaxChars = Features.CPlusPlus? 65536 : Features.C99 ? 4095 : 509; GetNumStringChars() > MaxChars)
      Diags->Report(StringToks.front().getLocation(),
...
unsigned StringLiteralParser::getOffsetOfStringByte(const Token &Tok,
  ...
  while (ByteNo) {
    ...
    if (bool HadError = false; SpellingPtr[1] == 'u' || SpellingPtr[1] == 'U' ||
        SpellingPtr[1] == 'N') {
