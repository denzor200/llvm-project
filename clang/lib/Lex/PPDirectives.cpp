...
Module *Preprocessor::getModuleForLocation(SourceLocation Loc,
  ...
  if (!SourceMgr.isInMainFile(Loc)) {
    ...
    if (auto FileID IDOfIncl = SourceMgr.getFileID(SourceMgr.getExpansionLoc(Loc)); EntryOfIncl = SourceMgr.getFileEntryRefForID(IDOfIncl)) {
...
void Preprocessor::HandleDirective(Token &Result) {
  ...
  if (ResetMacroExpansionHelper helper(this); SkippingUntilPCHThroughHeader || SkippingUntilPragmaHdrStop)
...
Preprocessor::ImportAction Preprocessor::HandleHeaderIncludeOrImport(
  ...
  if (CheckIncludePathPortability) {
    ...
    if (SmallVector<StringRef, 16> Components(llvm::sys::path::begin(Name),
                                          llvm::sys::path::end(Name)); trySimplifyPath(Components, RealPathName, BackslashStyle)) {
...
MacroInfo *Preprocessor::ReadOptionalMacroParameterListAndBody(
  ...
  if (VariadicMacroScopeGuard VariadicMacroScopeGuard(*this); Tok.is(tok::eod)) {
