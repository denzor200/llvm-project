...
FrontendAction::CreateWrappedASTConsumer(CompilerInstance &CI,
  ...
  if (!DumpDeserializedDeclarationRangesPath.empty()) {
    ...
    if (auto FileStream = std::make_unique<llvm::raw_fd_ostream>(
        DumpDeserializedDeclarationRangesPath, ErrorCode,
        llvm::sys::fs::OF_TextWithCRLF); !ErrorCode) {
...
bool FrontendAction::BeginSourceFile(CompilerInstance &CI,
  ...
  if (ReplayASTFile) {
    ...
    if (Kind.getFormat() == InputKind::ModuleMap) {
      ...
      if (FileID ID = OldSM.getMainFileID(); auto File = OldSM.getFileEntryRefForID(ID))
  ...
  if (!CI.getPreprocessorOpts().ImplicitPCHInclude.empty()) {
    ...
    if (std::string SpecificModuleCachePath = CI.getSpecificModuleCachePath(); auto PCHDir = FileMgr.getOptionalDirectoryRef(PCHInclude)) {
      std::error_code EC;
      SmallString<128> DirNative;
  ...
  if (CI.getLangOpts().HLSL && CI.hasASTContext()) {
    ...
    if (auto HLSLSema = llvm::makeIntrusiveRefCnt<HLSLExternalSemaSource>(); auto SemaSource = dyn_cast_if_present<ExternalSemaSource>(
