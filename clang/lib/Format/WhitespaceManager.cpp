...
void WhitespaceManager::alignTrailingComments() {
  ...
  for (int I = 0, MaxColumn = INT_MAX, Newlines = 0; I < Size; ++I) {
    ...
    if (C.NewlinesBefore != 0) {
      ...
      if (const bool WasInPP = std::exchange(
          IsInPP, C.Tok->Tok.is(tok::hash) || (IsInPP && C.IsTrailingComment) ||
                      C.ContinuesPPDirective); IsInPP != WasInPP && !Style.AlignTrailingComments.AlignPPAndNotPP) {
...
void WhitespaceManager::alignArrayInitializersLeftJustified(
  ...
  for (auto i = 1U; i < CellDescs.CellCounts[0]; i++, ++CellIter) {
    ...
    if (auto ThisNetWidth =
        getNetWidth(Cells.begin(), CellIter, CellDescs.InitialSpaces); Changes[CellIter->Index].NewlinesBefore == 0) {
