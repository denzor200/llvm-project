...
void CodeGenFunction::EmitParmDecl(const VarDecl &D, ParamValue Arg,
  ...
  if (Arg.isIndirect()) {
    ...
    if (Address OpenMPLocalAddr =
        getLangOpts().OpenMP
            ? CGM.getOpenMPRuntime().getAddressOfLocalVariable(*this, &D)
            : Address::invalid(); getLangOpts().OpenMP && OpenMPLocalAddr.isValid()) {
  ...
  if (IsScalar) {
    ...
    if (Qualifiers::ObjCLifetime Qualifiers qs = Ty.getQualifiers(); lt = qs.getObjCLifetime()) {
  ...
  if (CGDebugInfo *DI = getDebugInfo()) {
    if (CGM.getCodeGenOpts().hasReducedDebugInfo() && !CurFuncIsThunk &&
      ...
      if (const auto *llvm::DILocalVariable *DILocalVar = DI->EmitDeclareOfArgVariable(
          &D, AllocaPtr.getPointer(), ArgNo, Builder, UseIndirectDebugAddress); Var = dyn_cast_or_null<ParmVarDecl>(&D))
