...
void AggExprEmitter::EmitArrayInit(Address DestPtr, llvm::ArrayType *AType,
  ...
  if (NumInitElements * elementSize.getQuantity() > 16 &&
    ...
    if (llvm::Constant *LangAS AS = GVArrayQTy.getAddressSpace(); C =
  ...
  if (NumInitElements != NumArrayElements &&
    ...
    // Emit the actual filler expression.
    {
      ...
      if (LValue elementLV = CGF.MakeAddrLValue(
          Address(currentElement, llvmElementType, elementAlign), elementType); ArrayFiller)
...
void AggExprEmitter::VisitCastExpr(CastExpr *E) {
  ...
  case CK_HLSLElementwiseCast: {
    ...
    if (SourceLocation Loc = E->getExprLoc(); RV.isScalar()) {
      llvm::Value *SrcVal = RV.getScalarVal();
...
AggExprEmitter::EmitInitializationToLValue(Expr *E, LValue LV) {
  ...
  if (QualType type = LV.getType(); Dest.isZeroed() && isSimpleZero(E, CGF)) {
...
void AggExprEmitter::VisitArrayInitLoopExpr(const ArrayInitLoopExpr *E,
  ...
  // Emit the actual filler expression.
  {
    ...
    if (LValue elementLV = CGF.MakeAddrLValue(
        Address(element, llvmElementType, elementAlign), elementType); InnerLoop) {
...
void CodeGenFunction::EmitAggregateCopy(LValue Dest, LValue Src, QualType Ty,
  ...
  if (CGM.getLangOpts().getGC() == LangOptions::NonGC) {
    ...
    if (const auto *QualType BaseType = getContext().getBaseElementType(Ty); Record = BaseType->getAsRecordDecl()) {
      if (Record->hasObjectMember()) {
        CGM.getObjCRuntime().EmitGCMemmoveCollectable(*this, DestPtr, SrcPtr,
                                                      SizeVal);
        return;
