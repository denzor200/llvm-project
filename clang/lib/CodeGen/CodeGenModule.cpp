...
createTargetCodeGenInfo(CodeGenModule &CGM) {
  ...
  switch (const CodeGenOptions &CodeGenOpts = CGM.getCodeGenOpts(); Triple.getArch()) {
  default:
  ...
  case llvm::Triple::thumbeb: {
    ...
    if (StringRef ABIStr = Target.getABI(); ABIStr == "apcs-gnu")
  ...
  case llvm::Triple::x86_64: {
    ...
    switch (X86AVXABILevel AVXLevel = (ABI == "avx512" ? X86AVXABILevel::AVX512
                               : ABI == "avx"  ? X86AVXABILevel::AVX
                                               : X86AVXABILevel::None); Triple.getOS()) {
...
static bool checkAliasedGlobal(
  ...
  if (GV->hasCommonLinkage()) {
    ...
    if (const llvm::Triple &Triple = Context.getTargetInfo().getTriple(); Triple.getObjectFormat() == llvm::Triple::XCOFF) {
  ...
  if (GV->isDeclaration()) {
    ...
    for (const auto &[Decl, Name] : MangledDeclNames) {
      if (const auto *ND = dyn_cast<NamedDecl>(Decl.getDecl())) {
        ...
        if (IdentifierInfo *II = ND->getIdentifier(); II && II->getName() == GV->getName()) {
...
static void checkAliasForTocData(llvm::GlobalVariable *GVar,
  ...
  if (GVar->hasAttribute("toc-data")) {
    ...
    if (auto GVId = GVar->getName(); (llvm::binary_search(CodeGenOpts.TocDataVarsUserSpecified, GVId))) {
...
void CodeGenModule::checkAliases() {
  ...
  for (const GlobalDecl &GD : Aliases) {
    ...
    if (const SectionAttr *SA = D->getAttr<SectionAttr>()) {
      ...
      if (StringRef AliasSection = SA->getName(); AliasSection != AliaseeGV->getSection())
...
void CodeGenModule::Release() {
  ...
  if (T.isRISCV()) {
    ...
    if (auto ParseResult =
        llvm::RISCVISAInfo::parseFeatures(T.isRISCV64() ? 64 : 32, Features); !errorToBool(ParseResult.takeError()))
  ...
  if (CodeGenOpts.CFProtectionBranch &&
    ...
    if (auto Scheme = CodeGenOpts.getCFBranchLabelScheme(); Target.checkCFBranchLabelSchemeSupported(Scheme, getDiags())) {
  ...
  if (getTriple().isPPC() && !MustTailCallUndefinedGlobals.empty()) {
    for (auto &I : MustTailCallUndefinedGlobals) {
      if (!I.first->isDefined())
        ...
        if (llvm::GlobalValue *Entry = GetGlobalValue(MangledName); !Entry || Entry->isWeakForLinker() ||
  ...
  if (TBAA) {
    ...
    if (llvm::MDNode *TBAAAccessInfo TBAAInfo = getTBAAAccessInfo(Context.IntTy); IntegerNode = getTBAAAccessTagInfo(TBAAInfo)) {
...
static std::string getMangledNameImpl(CodeGenModule &CGM, GlobalDecl GD,
  ...
  if (ShouldMangle)
    ...
    if (const auto *FD = dyn_cast<FunctionDecl>(ND); FD &&
...
void CodeGenModule::UpdateMultiVersionNames(GlobalDecl GD,
  ...
  if (GlobalDecl OtherGD; lookupRepresentativeDecl(NonTargetName, OtherGD)) {
    ...
    if (std::string OtherName = getMangledNameImpl(*this, OtherGD, OtherFD); OtherName != NonTargetName) {
...
StringRef CodeGenModule::getMangledName(GlobalDecl GD) {
  ...
  if (!LangOpts.CUDAIsDevice || !getContext().mayExternalize(GD.getDecl())) {
    ...
    if (auto FoundName = MangledDeclNames.find(CanonicalGD); FoundName != MangledDeclNames.end())
...
static void removeImageAccessQualifier(std::string& TyName) {
  ...
  if (std::string::size_type ReadOnlyPos = TyName.find(ReadOnlyQual); ReadOnlyPos != std::string::npos)
    // "+ 1" for the space after access qualifier.
    TyName.erase(ReadOnlyPos, ReadOnlyQual.size() + 1);
  else {
    std::string WriteOnlyQual("__write_only");
    
    if (std::string::size_type WriteOnlyPos = TyName.find(WriteOnlyQual); WriteOnlyPos != std::string::npos)
      TyName.erase(WriteOnlyPos, WriteOnlyQual.size() + 1);
    else {
      std::string ReadWriteQual("__read_write");
      
      if (std::string::size_type ReadWritePos = TyName.find(ReadWriteQual); ReadWritePos != std::string::npos)
...
void CodeGenModule::setNonAliasAttributes(GlobalDecl GD,
  ...
  if (D) {
    ...
    if (auto *F = dyn_cast<llvm::Function>(GO)) {
      ...
      if (llvm::AttrBuilder Attrs(F->getContext()); GetCPUAndFeaturesAttributes(GD, Attrs)) {
...
static void setLinkageForGV(llvm::GlobalValue *GV, const NamedDecl *ND) {
  ...
  if (LinkageInfo LV = ND->getLinkageAndVisibility(); isExternallyVisible(LV.getLinkage()) &&
...
void CodeGenModule::EmitGlobalAnnotations() {
  for (const auto& [MangledName, VD] : DeferredAnnotations) {
    ...
    if (llvm::GlobalValue *GV = GetGlobalValue(MangledName); GV)
...
bool CodeGenModule::isInNoSanitizeList(SanitizerMask Kind,
  ...
  if (!Ty.isNull()) {
    ...
    if (Ty->isRecordType()) {
      ...
      if (std::string TypeStr = Ty.getAsString(getContext().getPrintingPolicy()); NoSanitizeL.containsType(Kind, TypeStr, Category))
        return true;
    }
  }
  return false;
...
CodeGenModule::isFunctionBlockedFromProfileInstr(llvm::Function *Fn,
  ...
  if (auto V = isFunctionBlockedByProfileList(Fn, Loc); V != ProfileList::Allow)
    return V;

  auto NumGroups = getCodeGenOpts().ProfileTotalFunctionGroups;
  if (NumGroups > 1) {
    
    if (auto Group = llvm::crc32(arrayRefFromStringRef(Fn->getName())) % NumGroups; Group != getCodeGenOpts().ProfileSelectedFunctionGroup)
...
bool CodeGenModule::MustBeEmitted(const ValueDecl *Global) {
  ...
  if (const auto *VD = dyn_cast<VarDecl>(Global); VD &&
      ((CodeGenOpts.KeepPersistentStorageVariables &&
        (VD->getStorageDuration() == SD_Static ||
         VD->getStorageDuration() == SD_Thread)) ||
       (CodeGenOpts.KeepStaticConsts && VD->getStorageDuration() == SD_Static &&
...
bool CodeGenModule::MayBeEmittedEagerly(const ValueDecl *Global) {
  ...
  if (LangOpts.OpenMP >= 50 && !LangOpts.OpenMPSimd) {
    ...
    if (std::optional<OMPDeclareTargetDeclAttr *> ActiveAttr =
        OMPDeclareTargetDeclAttr::getActiveAttr(Global); !ActiveAttr || (*ActiveAttr)->getLevel() != (unsigned)-1)
      return false;
  }
...
ConstantAddress CodeGenModule::GetWeakRefReference(const ValueDecl *VD) {
  ...
  if (llvm::GlobalValue *Entry = GetGlobalValue(AA->getAliasee()); Entry)
...
void CodeGenModule::EmitGlobal(GlobalDecl GD) {
  ...
  if (LangOpts.CUDA) {
    ...
    if (const auto *VD = dyn_cast<VarDecl>(Global)) {
      ...
      if (const auto *FD = dyn_cast<FunctionDecl>(Global); (!Global->hasAttr<CUDADeviceAttr>() ||
  ...
  if (const auto *FD = dyn_cast<FunctionDecl>(Global)) {
    ...
    if (FD->hasAttr<AnnotateAttr>()) {
      ...
      if (StringRef MangledName = getMangledName(GD); GetGlobalValue(MangledName))
    ...
    if (VD->isThisDeclarationADefinition() != VarDecl::Definition &&
      ...
      if (LangOpts.OpenMP) {
        ...
        if (std::optional<OMPDeclareTargetDeclAttr::MapTypeTy> Res =
          ...
          if (bool UnifiedMemoryEnabled =
              getOpenMPRuntime().hasRequiresUnifiedSharedMemory(); (*Res == OMPDeclareTargetDeclAttr::MT_To ||
...
namespace {
  ...
  struct DLLImportFunctionVisitor
    ...
    bool VisitDeclRefExpr(DeclRefExpr *E) {
      ...
      if (ValueDecl *VD = E->getDecl(); isa<FunctionDecl>(VD))
    ...
    bool VisitCXXMemberCallExpr(CXXMemberCallExpr *E) {
      ...
      if (CXXMethodDecl *M = E->getMethodDecl(); !M) {
...
getMultiversionLinkage(CodeGenModule &CGM, GlobalDecl GD) {
  ...
  if (const FunctionDecl *FD = cast<FunctionDecl>(GD.getDecl()); FD->getFormalLinkage() == Linkage::Internal)
  ...
  for (GlobalDecl GD : MVFuncsToEmit) {
    ...
    getContext().forEachMultiversionedFunctionVersion(
        FD, [&](const FunctionDecl *CurFD) {
          ...
          if (const auto *bool IsDefined = CurFD->getDefinition() != nullptr; TA = CurFD->getAttr<TargetAttr>()) {
    ...
    if (auto *IFunc = dyn_cast<llvm::GlobalIFunc>(ResolverConstant)) {
      ...
      if (FD->isTargetClonesMultiVersion() &&
        ...
        if (std::string MangledName = getMangledNameImpl(
            *this, GD, FD, /*OmitMultiVersionMangling=*/true); !GetGlobalValue(MangledName + ".ifunc")) {
          const CGFunctionInfo &FI = getTypes().arrangeGlobalDeclaration(GD);
...
void CodeGenModule::emitCPUDispatchDefinition(GlobalDecl GD) {
  ...
  for (const IdentifierInfo *II : DD->cpus()) {
    ...
    if (!Func) {
      ...
      if (GlobalDecl ExistingDecl = Manglings.lookup(MangledName); ExistingDecl.getDecl() &&
  ...
  while (Options.size() > 1 && llvm::all_of(llvm::X86::getCpuSupportsMask(
    ...
    if (StringRef RHSName = (Options.end() - 1)->Function->getName(); LHSName.compare(RHSName) < 0)
...
void CodeGenModule::AddDeferredMultiVersionResolverToEmit(GlobalDecl GD) {
  ...
  if (FD->isTargetVersionMultiVersion() || FD->isTargetClonesMultiVersion()) {
    ...
    if (std::string MangledName =
        getMangledNameImpl(*this, GD, FD, /*OmitMultiVersionMangling=*/true); !DeferredResolversToEmit.insert(MangledName).second)
      return;
  }
...
llvm::Constant *CodeGenModule::GetOrCreateLLVMFunction(
  ...
  if (Entry) {
    if (WeakRefReferences.erase(Entry)) {
      ...
      if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D); FD && !FD->hasAttr<WeakAttr>())
    ...
    if (IsForDefinition && !Entry->isDeclaration()) {
      ...
      if (GlobalDecl OtherGD; lookupRepresentativeDecl(MangledName, OtherGD) &&
  ...
  if (!DontDefer) {
    ...
    if (auto DDI = DeferredDecls.find(MangledName); DDI != DeferredDecls.end()) {
...
static void setWindowsItaniumDLLImport(CodeGenModule &CGM, bool Local,
  ...
  if (!Local && CGM.getTriple().isWindowsItaniumEnvironment() &&
    ...
    if (const FunctionDecl *FD = GetRuntimeFunctionDecl(CGM.getContext(), Name); !FD || FD->hasAttr<DLLImportAttr>()) {
...
CodeGenModule::GetOrCreateLLVMGlobal(StringRef MangledName, llvm::Type *Ty,
  ...
  if (Entry) {
    ...
    if (IsForDefinition && !Entry->isDeclaration()) {
      ...
      // to make sure that we issue an error only once.
      if (const VarDecl *OtherD; D && lookupRepresentativeDecl(MangledName, OtherGD) &&
  ...
  if (D) {
    ...
    if (Context.getLangOpts().CPlusPlus && GV->hasExternalLinkage() &&
      ...
      if (bool HasMutableFields = Record && Record->hasMutableFields(); !HasMutableFields) {
        const VarDecl *InitDecl;
        
        if (const Expr *InitExpr = D->getAnyInitializer(InitDecl); InitExpr) {
          ConstantEmitter emitter(*this);
          
          if (llvm::Constant *Init = emitter.tryEmitForInitializer(*InitDecl); Init) {
            
            if (auto *InitType = Init->getType(); GV->getValueType() != InitType) {
...
void CodeGenModule::EmitExternalDeclaration(const DeclaratorDecl *D) {
  ...
  if (const auto *VD = dyn_cast<VarDecl>(D)) {
    ...
    if (llvm::Function *Fn = cast<llvm::Function>(Addr); !Fn->getSubprogram())
...
LangAS CodeGenModule::GetGlobalVarAddressSpace(const VarDecl *D) {
  ...
  if (LangOpts.OpenMP) {
    ...
    if (LangAS AS; OpenMPRuntime->hasAllocateAttributeForGlobalVar(D, AS))
      return AS;
  }
...
castStringLiteralToDefaultAddressSpace(CodeGenModule &CGM,
  ...
  if (!CGM.getLangOpts().OpenCL) {
    ...
    if (auto AS = CGM.GetGlobalConstantAddressSpace(); AS != LangAS::Default)
...
void CodeGenModule::MaybeHandleStaticInExternC(const SomeDecl *D,
  ...
  if (const SomeDecl *First = D->getFirstDecl(); First->getDeclContext()->isRecord() || !First->isInExternCContext())
    return;
...
void CodeGenModule::EmitGlobalVarDefinition(const VarDecl *D,
  ...
  if (getLangOpts().CUDA &&
    ...
    if (llvm::Constant *Initializer = emitter->tryEmitForInitializer(*InitDecl); !Initializer) {
  ...
  if (const SectionAttr *SA = D->getAttr<SectionAttr>()) {
    ...
    if (const ASTContext::SectionInfo &SI = Context.SectionInfos[SA->getName()]; (SI.SectionFlags & ASTContext::PSF_Write) == 0)
      GV->setConstant(true);
  }
...
void CodeGenModule::HandleCXXStaticMemberVarInstantiation(VarDecl *VD) {
  ...
  if (auto DK = VD->isThisDeclarationADefinition(); (DK == VarDecl::Definition && VD->hasAttr<DLLImportAttr>()) ||
...
void CodeGenModule::EmitGlobalFunctionDefinition(GlobalDecl GD,
  ...
  if (llvm::FunctionType *Ty = getTypes().GetFunctionType(FI); !GV || (GV->getValueType() != Ty))
...
CodeGenModule::GetAddrOfConstantCFString(const StringLiteral *Literal) {
  ...
  if (!CFConstantStringClassRef) {
    ...
    if (Triple.isOSBinFormatELF() || Triple.isOSBinFormatCOFF()) {
      ...
      if (llvm::GlobalValue *GV = nullptr; (GV = dyn_cast<llvm::GlobalValue>(C))) {
...
ConstantAddress CodeGenModule::GetAddrOfGlobalTemporary(
  ...
  if (Linkage == llvm::GlobalVariable::ExternalLinkage) {
    ...
    if (const VarDecl *InitVD; VD->isStaticDataMember() && VD->getAnyInitializer(InitVD) &&
...
void CodeGenModule::EmitObjCPropertyImplementations(const
  ...
  for (const auto *PID : D->property_impls()) {
    ...
    if (PID->getPropertyImplementation() == ObjCPropertyImplDecl::Synthesize) {
      ...
      if (auto *Getter = PID->getGetterMethodDecl(); !Getter || Getter->isSynthesizedAccessorStub())
...
static bool AllTrivialInitializers(CodeGenModule &CGM,
  ...
  for (ObjCImplementationDecl::init_iterator B = D->init_begin(),
    ...
    if (Expr *Init = CtorInitExp->getInit(); !CGF.isTrivialInitializer(Init))
      return false;
  }
  return true;
}
...
void CodeGenModule::EmitTopLevelDecl(Decl *D) {
  ...
  case Decl::ClassTemplateSpecialization: {
    ...
    if (CGDebugInfo *const auto *Spec = cast<ClassTemplateSpecializationDecl>(D); DI = getModuleDebugInfo())
  ...
  case Decl::ObjCProtocol: {
    ...
    if (auto *Proto = cast<ObjCProtocolDecl>(D); Proto->isThisDeclarationADefinition())
  ...
  case Decl::ObjCMethod: {
    ...
    if (auto *OMD = cast<ObjCMethodDecl>(D); OMD->getBody())
  ...
  case Decl::PragmaComment: {
    ...
    switch (const auto *PCD = cast<PragmaCommentDecl>(D); PCD->getCommentKind()) {
    case PCK_Unknown:
...
void CodeGenModule::EmitDeferredUnusedCoverageMappings() {
  ...
  for (const auto &Entry : DeferredEmptyCoverageMappingDecls.takeVector()) {
    ...
    switch (const Decl *D = Entry.first; D->getKind()) {
    case Decl::CXXConversion:
    case Decl::CXXMethod:
    case Decl::Function:
    case Decl::ObjCMethod: {
...
void CodeGenModule::EmitDeclMetadata() {
  ...
  for (auto &I : MangledDeclNames) {
    ...
    if (llvm::GlobalValue *Addr = getModule().getNamedValue(I.second); Addr)
...
void CodeGenFunction::EmitDeclMetadata() {
  ...
  for (auto &I : LocalDeclMap) {
    ...
    if (auto *llvm::Value *Addr = I.second.emitRawPointer(*this); Alloca = dyn_cast<llvm::AllocaInst>(Addr)) {
...
void CodeGenModule::EmitOMPThreadPrivateDecl(const OMPThreadPrivateDecl *D) {
  ...
  for (auto RefExpr : D->varlist()) {
    ...
    if (auto Address Addr(GetAddrOfGlobalVar(VD),
                 getTypes().ConvertTypeForMem(VD->getType()),
                 getContext().getDeclAlign(VD)); InitFunction = getOpenMPRuntime().emitThreadPrivateVarDefinition(
...
CharUnits CodeGenModule::getNaturalTypeAlignment(QualType T,
  ...
  if (const CXXRecordDecl *RD; T.getQualifiers().hasUnaligned()) {
...
bool CodeGenModule::stopAutoInit() {
  ...
  if (unsigned StopAfter = getContext().getLangOpts().TrivialAutoVarInitStopAfter; StopAfter) {
