...
CodeGenModule::CodeGenModule(ASTContext &C,
  ...
  if (!CGO.MSSecureHotPatchFunctionsFile.empty() ||
    ...
    if (!CGO.MSSecureHotPatchFunctionsFile.empty()) {
      ...
      if (auto BufOrErr = FS->getBufferForFile(CGO.MSSecureHotPatchFunctionsFile); BufOrErr) {
...
void CodeGenModule::Release() {
  ...
  if (getCodeGenOpts().CodeModel.size() > 0) {
    ...
    if (unsigned CM = llvm::StringSwitch<unsigned>(getCodeGenOpts().CodeModel)
  ...
  if (TBAA) {
    ...
    if (TBAAAccessInfo TBAAInfo = getTBAAAccessInfo(Context.IntTy); llvm::MDNode *IntegerNode = getTBAAAccessTagInfo(TBAAInfo)) {
...
void CodeGenModule::SetLLVMFunctionAttributesForDefinition(const Decl *D,
  ...
  if (getLangOpts().HLSL && !F->hasFnAttribute(llvm::Attribute::NoInline) &&
    ...
    if (auto *FD = dyn_cast<FunctionDecl>(D)) {
      ...
      if (auto CheckForInline = [](const FunctionDecl *FD) {
  ...
  for (GlobalDecl GD : MVFuncsToEmit) {
    ...
    getContext().forEachMultiversionedFunctionVersion(
        FD, [&](const FunctionDecl *CurFD) {
          ...
          if (bool IsDefined = CurFD->getDefinition() != nullptr; const auto *TA = CurFD->getAttr<TargetAttr>()) {
...
void CodeGenModule::EmitTopLevelDecl(Decl *D) {
  ...
  case Decl::ClassTemplateSpecialization: {
    ...
    if (const auto *Spec = cast<ClassTemplateSpecializationDecl>(D); CGDebugInfo *DI = getModuleDebugInfo())
...
void CodeGenFunction::EmitDeclMetadata() {
  ...
  for (auto &I : LocalDeclMap) {
    ...
    if (llvm::Value *Addr = I.second.emitRawPointer(*this); auto *Alloca = dyn_cast<llvm::AllocaInst>(Addr)) {
...
void CodeGenModule::EmitOMPThreadPrivateDecl(const OMPThreadPrivateDecl *D) {
  ...
  for (auto RefExpr : D->varlist()) {
    ...
    if (Address Addr(GetAddrOfGlobalVar(VD),
                 getTypes().ConvertTypeForMem(VD->getType()),
                 getContext().getDeclAlign(VD)); auto InitFunction = getOpenMPRuntime().emitThreadPrivateVarDefinition(
