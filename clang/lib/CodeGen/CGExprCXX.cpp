...
RValue CodeGenFunction::EmitCXXPseudoDestructorExpr(
  ...
  if (QualType DestroyedType = E->getDestroyedType(); DestroyedType.hasStrongOrWeakObjCLifetime()) {
...
void CodeGenFunction::EmitNewArrayInitializer(
  ...
  if (ILE || CPLIE || SL || OCEE) {
    ...
    if (const ConstantArrayType *QualType AllocType = E->getAllocatedType(); CAT = dyn_cast_or_null<ConstantArrayType>(
...
static void EmitNewInitializer(CodeGenFunction &CGF, const CXXNewExpr *E,
  ...
  if (ApplyDebugLocation DL(CGF, E); E->isArray())
...
llvm::Value *CodeGenFunction::EmitCXXNewExpr(const CXXNewExpr *E) {
  ...
  if (E->isArray() && E->hasInitializer()) {
    ...
    if (const Expr *IgnoreParen = Init->IgnoreParenImpCasts(); (ILE && ILE->isStringLiteralInit()) ||
  ...
  if (const auto *RD = ElementType->getAsCXXRecordDecl()) {
    if (RD->hasDefinition() && !RD->hasTrivialDestructor()) {
      ...
      if (Dtor->isVirtual()) {
        ...
        if (auto *const Expr *Base = DE->getArgument(); DevirtualizedDtor =
