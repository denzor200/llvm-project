...
void CodeGenFunction::EmitInitializationToLValue(
  ...
  switch (QualType Type = LV.getType(); getEvaluationKind(Type)) {
  case TEK_Complex:
...
llvm::MDNode *CodeGenFunction::buildAllocToken(const CallExpr *E) {
  ...
  if (QualType AllocType = infer_alloc::inferPossibleType(E, getContext(), CurCast); !AllocType.isNull())
...
LValue CodeGenFunction::EmitLValueHelper(const Expr *E,
  ...
  switch (ApplyDebugLocation DL(*this, E); E->getStmtClass()) {
  ...
  case Expr::ConstantExprClass: {
    ...
    if (llvm::Value *const ConstantExpr *CE = cast<ConstantExpr>(E); Result = ConstantEmitter(*this).tryEmitConstantExpr(CE)) {
...
void CodeGenFunction::EmitStoreOfScalar(llvm::Value *Value, Address Addr,
  ...
  if (const auto *llvm::Type *SrcTy = Value->getType(); ClangVecTy = Ty->getAs<VectorType>()) {
...
RValue CodeGenFunction::EmitLoadOfAnyValue(LValue LV, AggValueSlot Slot,
  ...
  switch (QualType Ty = LV.getType(); getEvaluationKind(Ty)) {
  case TEK_Scalar:
...
static void setObjCGCLValueClass(const ASTContext &Ctx, const Expr *E,
  ...
  if (isa<ObjCIvarRefExpr>(E)) {
    ...
    if (QualType ExpTy = E->getType(); IsMemberAccess && ExpTy->isPointerType()) {
...
LValue CodeGenFunction::EmitArraySectionExpr(const ArraySectionExpr *E,
  ...
  if (IsLowerBound || E->getColonLocFirst().isInvalid()) {
    ...
    if (llvm::APSInt ConstLength; Length) {
      ...
      if (auto *QualType ArrayTy = BaseTy->isPointerType()
                             ? E->getBase()->IgnoreParenImpCasts()->getType()
                             : BaseTy; VAT = C.getAsVariableArrayType(ArrayTy)) {
...
LValue CodeGenFunction::EmitLValueForLambdaField(const FieldDecl *Field,
  ...
  if (HasExplicitObjectParameter) {
    ...
    if (Address AddrOfExplicitObject = It->getSecond(); D->getType()->isReferenceType())
...
RValue CodeGenFunction::EmitCallExpr(const CallExpr *E,
  ...
  if (llvm::CallBase *CallOrInvokeStorage; !CallOrInvoke) {
...
CGCallee CodeGenFunction::EmitCallee(const Expr *E) {
  ...
  if (auto ICE = dyn_cast<ImplicitCastExpr>(E)) {
    ...
    if (ICE->getCastKind() == CK_LValueToRValue) {
      ...
      if (const auto *const Expr *SubExpr = ICE->getSubExpr(); PtrType = SubExpr->getType()->getAs<PointerType>()) {
...
static LValueOrRValue emitPseudoObjectExpr(CodeGenFunction &CGF,
  ...
  for (PseudoObjectExpr::const_semantics_iterator
    ...
    if (const auto *const Expr *semantic = *i; ov = dyn_cast<OpaqueValueExpr>(semantic)) {
      // Skip unique OVEs.
      if (ov->isUnique()) {
