...
bool ConstantAggregateBuilder::add(llvm::Constant *C, CharUnits Offset,
  ...
  if (Offset >= Size) {
    ...
    if (CharUnits AlignedSize = Size.alignTo(Align); AlignedSize > Offset || Offset.alignTo(Align) != Offset)
      NaturalLayout = false;
...
public:
  ...
  llvm::Constant *ProduceIntToIntCast(const Expr *E, QualType DestType) {
    ...
    if (QualType FromType = E->getType(); FromType->isIntegerType())
  ...
  llvm::Constant *EmitArrayInitialization(const InitListExpr *ILE, QualType T) {
    ...
    for (unsigned i = 0; i < ILE->getNumInits(); ++i) {
      ...
      if (auto *const Expr *Init = ILE->getInit(i); EmbedS = dyn_cast<EmbedExpr>(Init->IgnoreParenImpCasts())) {
...
llvm::Constant *ConstantEmitter::tryEmitPrivateForVarInit(const VarDecl &D) {
  ...
  if (!destType->isReferenceType()) {
    ...
    if (llvm::Constant *QualType nonMemoryDestType = getNonMemoryType(CGM, destType); C = ConstExprEmitter(*this).Visit(E, nonMemoryDestType))
