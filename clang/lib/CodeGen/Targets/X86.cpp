...
ABIArgInfo X86_32ABIInfo::classifyReturnType(QualType RetTy,
  ...
  if (uint64_t NumElts = 0; (State.CC == llvm::CallingConv::X86_VectorCall ||
...
ABIArgInfo X86_32ABIInfo::classifyArgumentType(QualType Ty, CCState &State,
  ...
  if (isAggregateTypeForABI(Ty)) {
    ...
    if (bool InReg; shouldAggregateUseDirect(Ty, State, InReg, NeedsPadding)) {
...
RValue X86_32ABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
  ...
  if (ABIArgInfo AI = classifyArgumentType(Ty, State, /*ArgIndex*/ 0); AI.isIgnore())
...
public:
  ...
  bool isPassedUsingAVXType(QualType type) const {
    ...
    if (ABIArgInfo info = classifyArgumentType(type, 0, neededInt, neededSSE,
                                           /*isNamedArg*/true); info.isDirect()) {
      
      if (llvm::VectorType *llvm::Type *ty = info.getCoerceToType(); vectorTy = dyn_cast_or_null<llvm::VectorType>(ty))
...
void X86_64ABIInfo::classify(QualType Ty, uint64_t OffsetBase, Class &Lo,
  ...
  if (const VectorType *VT = Ty->getAs<VectorType>()) {
    ...
    if (uint64_t Size = getContext().getTypeSize(VT); Size == 1 || Size == 8 || Size == 16 || Size == 32) {
      ...
      if (QualType ElementType = VT->getElementType(); passInt128VectorsInMem() && Size != 128 &&
...
ABIArgInfo WinX86_64ABIInfo::reclassifyHvaArgForVectorCall(
  ...
  if (uint64_t NumElts = 0; !Ty->isBuiltinType() && !Ty->isVectorType() &&
