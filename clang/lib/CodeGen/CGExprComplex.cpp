...
ComplexPairTy ComplexExprEmitter::EmitBinMul(const BinOpInfo &Op) {
  ...
  if (llvm::MDBuilder MDHelper(CGF.getLLVMContext()); Op.LHS.first->getType()->isFloatingPointTy()) {
...
#undef HANDLE_BINOP
  ...
  if (auto result = Visit(const_cast<Expr *>(E)); !PromotionType.isNull())
...
EmitCompoundAssignLValue(const CompoundAssignOperator *E,
  ...
  if (LHSTy->isAnyComplexType()) {
    ...
    if (ComplexPairTy LHSVal = EmitLoadOfLValue(LHS, Loc); !PromotionTypeLHS.isNull())
      OpInfo.LHS =
    ...
    if (LHSTy->isRealFloatingType()) {
      ...
      if (QualType PromotedComplexElementTy; !PromotionTypeLHS.isNull()) {
