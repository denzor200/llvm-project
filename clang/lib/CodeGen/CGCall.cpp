...
void CodeGenTypes::getExpandedTypes(
  ...
  if (auto Exp = getTypeExpansion(Ty, Context); auto CAExp = dyn_cast<ConstantArrayExpansion>(Exp.get())) {
...
void CodeGenFunction::ExpandTypeFromArgs(QualType Ty, LValue LV,
  ...
  if (auto Exp = getTypeExpansion(Ty, getContext()); auto CAExp = dyn_cast<ConstantArrayExpansion>(Exp.get())) {
  ...
  if (TargetDecl) {
    ...
    if (const FunctionDecl *Fn = dyn_cast<FunctionDecl>(TargetDecl)) {
      ...
      if (AttrOnCallSite && Fn->isReplaceableGlobalAllocationFunction()) {
        ...
        if (auto Kind = Fn->getDeclName().getCXXOverloadedOperator(); getCodeGenOpts().AssumeSaneOperatorNew &&
