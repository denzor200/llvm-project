...
static Cl::Kinds ClassifyInternal(ASTContext &Ctx, const Expr *E) {
  ...
  switch (const LangOptions &Lang = Ctx.getLangOpts(); E->getStmtClass()) {
...
#include "clang/AST/StmtNodes.inc"
  ...
  case Expr::UnaryOperatorClass:
    ...
    case UO_Imag: {
      ...
      if (Cl::Kinds K = ClassifyInternal(Ctx, Op); K != Cl::CL_LValue) return K;
  ...
  case Expr::StmtExprClass: {
    ...
    if (const auto *const CompoundStmt *S = cast<StmtExpr>(E)->getSubStmt(); LastExpr = dyn_cast_or_null<Expr>(S->body_back()))
...
static Cl::Kinds ClassifyMemberExpr(ASTContext &Ctx, const MemberExpr *E) {
  ...
  if (isa<FieldDecl>(Member)) {
    ...
    if (Expr *Base = E->getBase()->IgnoreParenImpCasts(); isa<ObjCPropertyRefExpr>(Base))
      return Cl::CL_SubObjCPropertySetting;
