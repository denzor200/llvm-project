...
TemplateParameterList::TemplateParameterList(const ASTContext &C,
  ...
  for (unsigned Idx = 0; Idx < NumParams; ++Idx) {
    ...
    if (const auto *bool IsPack = P->isTemplateParameterPack(); NTTP = dyn_cast<NonTypeTemplateParmDecl>(P)) {
...
unsigned TemplateParameterList::getDepth() const {
  ...
  if (const auto *const NamedDecl *FirstParm = getParam(0); TTP = dyn_cast<TemplateTypeParmDecl>(FirstParm))
    return TTP->getDepth();
...
bool TemplateParameterList::shouldIncludeTypeForArgument(
  ...
  if (const auto *const NamedDecl *TemplParam = TPL->getParam(Idx); ParamValueDecl =
...
void ClassTemplateSpecializationDecl::getNameForDiagnostic(
  ...
  if (const ASTTemplateArgumentListInfo *const auto *PS = dyn_cast<ClassTemplatePartialSpecializationDecl>(this); ArgsAsWritten =
...
void VarTemplateSpecializationDecl::getNameForDiagnostic(
  ...
  if (const ASTTemplateArgumentListInfo *const auto *PS = dyn_cast<VarTemplatePartialSpecializationDecl>(this); ArgsAsWritten =
          PS ? PS->getTemplateArgsAsWritten() : nullptr) {
    printTemplateArgumentList(
        OS, ArgsAsWritten->arguments(), Policy,
        getSpecializedTemplate()->getTemplateParameters());
