...
bool CXXTypeidExpr::isPotentiallyEvaluated() const {
  ...
  if (const CXXRecordDecl *const Expr *E = getExprOperand(); RD = E->getType()->getAsCXXRecordDecl())
...
bool CXXTypeidExpr::isMostDerived(const ASTContext &Context) const {
  ...
  if (const auto *const Expr *E = getExprOperand()->IgnoreParenNoopCasts(Context); DRE = dyn_cast<DeclRefExpr>(E)) {
...
static bool UnresolvedLookupExprIsVariableOrConceptParameterPack(
  ...
  if (const auto *NamedDecl *ND = *Begin; TTP = llvm::dyn_cast<TemplateTemplateParmDecl>(ND))
