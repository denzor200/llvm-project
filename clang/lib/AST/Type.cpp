...
SplitQualType QualType::getSplitDesugaredType(QualType T) {
  ...
  while (true) {
    ...
    switch (const Type *CurTy = Qs.strip(Cur); CurTy->getTypeClass()) {
#define ABSTRACT_TYPE(Class, Parent)
#define TYPE(Class, Parent)                                                    \
  case Type::Class: {                                                          \
...
Type::ScalarTypeKind Type::getScalarTypeKind() const {
  ...
  if (const auto *const Type *T = CanonicalType.getTypePtr(); BT = dyn_cast<BuiltinType>(T)) {
...
QualType Type::getRVVEltType(const ASTContext &Ctx) const {
  ...
  switch (const BuiltinType *BTy = castAs<BuiltinType>(); BTy->getKind()) {
#define RVV_PREDICATE_TYPE(Name, Id, SingletonId, NumEls)                      \
  case BuiltinType::Id:                                                        \
    return Ctx.UnsignedCharTy;
  default:
...
QualType::PrimitiveCopyKind QualType::isNonTrivialToPrimitiveCopy() const {
  ...
  switch (Qualifiers Qs = getQualifiers(); Qs.getObjCLifetime()) {
  case Qualifiers::OCL_Strong:
...
FunctionProtoType::FunctionProtoType(QualType result, ArrayRef<QualType> params,
  ...
  if (!epi.FunctionEffects.empty()) {
    ...
    if (ArrayRef<EffectConditionExpr> SrcConds = epi.FunctionEffects.conditions(); !SrcConds.empty()) {
...
bool Type::canHaveNullability(bool ResultIfUnknown) const {
  ...
  switch (QualType type = getCanonicalTypeInternal(); type->getTypeClass()) {
...
bool FunctionEffect::shouldDiagnoseFunctionCall(
  ...
  case Kind::NonBlocking: {
    ...
    for (FunctionEffect Effect : CalleeFX) {
      ...
      if (const Kind EK = Effect.kind(); EK == CallerKind ||
...
FunctionEffectSet FunctionEffectSet::getIntersection(FunctionEffectsRef LHS,
  ...
  while (IterA != EndA && IterB != EndB) {
    ...
    if (FunctionEffectWithCondition B = *IterB; FEWCLess(A, B))
      ++IterA;
...
HLSLAttributedResourceType::findHandleTypeOnResource(const Type *RT) {
  ...
  if (const RecordDecl *const clang::Type *Ty = RT->getUnqualifiedDesugaredType(); RD = Ty->getAsCXXRecordDecl()) {
    if (!RD->fields().empty()) {
