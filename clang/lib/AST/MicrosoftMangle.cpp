...
MicrosoftMangleContextImpl::MicrosoftMangleContextImpl(ASTContext &Context,
  ...
  if (OptionalFileEntryRef SourceManager &SM = Context.getSourceManager(); FE = SM.getFileEntryRefForID(SM.getMainFileID())) {
...
void MicrosoftCXXNameMangler::mangleTemplateArg(const TemplateDecl *TD,
  ...
  case TemplateArgument::Declaration: {
    ...
    if (const NamedDecl *ND = TA.getAsDecl(); isa<FieldDecl>(ND) || isa<IndirectFieldDecl>(ND)) {
...
void MicrosoftCXXNameMangler::mangleType(QualType T, SourceRange Range,
  ...
  switch (bool IsPointer = T->isAnyPointerType() || T->isMemberPointerType() ||
                   T->isReferenceType() || T->isBlockPointerType(); QMM) {
  case QMM_Drop:
...
void MicrosoftCXXNameMangler::mangleFunctionType(const FunctionType *T,
  ...
  if (IsStructor) {
    ...
    if (QualType ResultType = T->getReturnType(); IsInLambda && isa<CXXConversionDecl>(D)) {
      // The only lambda conversion operators are to function pointers, which
      // can differ by their calling convention and are typically deduced.  So
      // we make sure that this type gets mangled properly.
