...
void TextNodeDumper::Visit(const Stmt *Node) {
  ...
  if (const auto *E = dyn_cast<Expr>(Node)) {
...

    {
      ...
      switch (ColorScope Color(OS, ShowColors, ValueKindColor); E->getValueKind()) {
      case VK_PRValue:
        break;
      case VK_LValue:
        OS << " lvalue";
...

    {
      ...
      switch (ColorScope Color(OS, ShowColors, ObjectKindColor); E->getObjectKind()) {
      case OK_Ordinary:
        break;
      case OK_BitField:
        OS << " bitfield";
...
void TextNodeDumper::Visit(const Type *T) {
  ...
  if (QualType SingleStepDesugar =
      T->getLocallyUnqualifiedSingleStepDesugaredType(); SingleStepDesugar != QualType(T, 0))
    OS << " sugar";
...
void TextNodeDumper::Visit(const concepts::Requirement *R) {
...

  {
    ...
    switch (ColorScope Color(OS, ShowColors, StmtColor); R->getKind()) {
...
void TextNodeDumper::Visit(const APValue &Value, QualType Ty) {
  ...
  case APValue::Union: {
    OS << "Union";
    {
      ...
      if (const FieldDecl *ColorScope Color(OS, ShowColors, ValueColor); FD = Value.getUnionField())
...
void TextNodeDumper::dumpBareDeclRef(const Decl *D) {
  ...
  if (const NamedDecl *ND = dyn_cast<NamedDecl>(D)) {
    ...
    if (DeclarationName ColorScope Color(OS, ShowColors, DeclNameColor); Name = ND->getDeclName())
      OS << " '" << Name << '\'';
    else
      switch (ND->getKind()) {
