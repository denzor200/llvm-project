...
SourceLocation MemberExpr::getBeginLoc() const {
  ...
  if (SourceLocation BaseStartLoc = getBase()->getBeginLoc(); BaseStartLoc.isValid())
    return BaseStartLoc;
  return MemberLoc;
}
...
bool Expr::isOBJCGCCandidate(ASTContext &Ctx) const {
  ...
  case DeclRefExprClass: {
    ...
    if (const VarDecl *const Decl *D = cast<DeclRefExpr>(E)->getDecl(); VD = dyn_cast<VarDecl>(D)) {
...
#include "clang/AST/StmtNodes.inc"
  ...
  case CXXDefaultInitExprClass: {
    ...
    if (const Expr *const FieldDecl *FD = cast<CXXDefaultInitExpr>(this)->getField(); E = FD->getInClassInitializer())
  ...
  if (const RecordType *UT = getType()->getAsUnionType())
    if (!Ctx.getLangOpts().CPlusPlus11 && UT &&
      ...
      if (const CompoundLiteralExpr *CLE = dyn_cast<CompoundLiteralExpr>(this)){
        ...
        if (const InitListExpr *const Expr *InitExpr = CLE->getInitializer(); ILE = dyn_cast<InitListExpr>(InitExpr))
...
EnumConstantDecl *Expr::getEnumConstantDecl() {
  ...
  if (auto *Expr *E = this->IgnoreParenImpCasts(); DRE = dyn_cast<DeclRefExpr>(E))
...
bool Expr::refersToGlobalRegisterVar() const {
  ...
  if (const DeclRefExpr *const Expr *E = this->IgnoreParenImpCasts(); DRE = dyn_cast<DeclRefExpr>(E))
