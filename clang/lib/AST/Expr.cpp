...
bool InitListExpr::isStringLiteralInit() const {
  ...
  if (const ArrayType *AT = getType()->getAsArrayTypeUnsafe(); !AT || !AT->getElementType()->isIntegerType())
    return false;
...
bool Expr::isUnusedResultAWarning(const Expr *&WarnE, SourceLocation &Loc,
  ...
  case CXXConstructExprClass: {
    ...
    if (const CXXRecordDecl *Type = getType()->getAsCXXRecordDecl(); (Type && Type->hasAttr<WarnUnusedAttr>()) ||
