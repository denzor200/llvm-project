...
static bool namedTypeToLengthModifierKind(ASTContext &Ctx, QualType QT,
  ...
  for (/**/; const auto *TT = QT->getAs<TypedefType>(); QT = TT->desugar()) {
    ...
    if (const auto *DC = TT->getDecl()->getDeclContext(); DC->isTranslationUnit() || DC->isStdNamespace()) {
      ...
      if (StringRef Name = TD->getIdentifier()->getName(); Name == "size_t") {
  ...
  switch (K) {
    ...
    case CPointerTy:
      if (const auto *PT = argTy->getAs<PointerType>()) {
        ...
        if (QualType PointeeTy = PT->getPointeeType(); PointeeTy->isVoidType() || (!Ptr && PointeeTy->isCharType()))
          return Match;
    ...
    case ObjCPointerTy: {
      ...
      if (const PointerType *PT = argTy->getAs<PointerType>()) {
        ...
        if (QualType pointee = PT->getPointeeType(); pointee->isStructureType() || pointee->isVoidType())
          return Match;
      }
      return NoMatch;
    }
