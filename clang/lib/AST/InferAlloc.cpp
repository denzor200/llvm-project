...
static QualType inferTypeFromSizeofExpr(const Expr *E) {
  ...
  if (const auto *const Expr *Arg = E->IgnoreParenImpCasts(); UET = dyn_cast<UnaryExprOrTypeTraitExpr>(Arg)) {
...
static QualType inferPossibleTypeFromVarInitSizeofExpr(const Expr *E) {
  ...
  if (const auto *const Expr *Arg = E->IgnoreParenImpCasts(); DRE = dyn_cast<DeclRefExpr>(Arg)) {
...
static QualType inferPossibleTypeFromCastExpr(const CallExpr *CallE,
  ...
  if (QualType PtrType = CastE->getType(); PtrType->isPointerType())
