...
LinkageComputer::getLVForClassMember(const NamedDecl *D,
  ...
  if (const auto *MD = dyn_cast<CXXMethodDecl>(D)) {
    ...
    if (ASTContext &Context = D->getASTContext(); Context.getLangOpts().OpenMP &&
        Context.getLangOpts().OpenMPIsTargetDevice &&
...
bool VarDecl::hasFlexibleArrayInit(const ASTContext &Ctx) const {
  ...
  if (auto *D = getType()->getAsRecordDecl(); !D || !D->hasFlexibleArrayMember())
    return false;
...
bool ParmVarDecl::isDestroyedInCallee() const {
  ...
  if (const auto *RT = getType()->getAsCanonical<RecordType>(); RT && RT->getDecl()->getDefinitionOrSelf()->isParamDestroyedInCallee() &&
