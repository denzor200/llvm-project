...
APValue Pointer::toAPValue(const ASTContext &ASTCtx) const {
  ...
  while (Ptr.isField() || Ptr.isArrayElement()) {
    ...
    if (Ptr.isArrayRoot()) {
      ...
      if (const auto *const Descriptor *Desc = Ptr.getFieldDesc(); BaseOrMember = Desc->asDecl()) {
  ...
  activate = [&activate](Pointer &P) -> void {
    ...
    if (const Record *R = P.getRecord(); R && !R->isUnion()) {
      for (const Record::Field &F : R->fields()) {
        ...
        if (Pointer FieldPtr = P.atField(F.Offset); !FieldPtr.getInlineDesc()->IsActive)
          activate(FieldPtr);
      }
      // FIXME: Bases?
    }
  ...
  while (!B.isRoot() && B.inUnion()) {
    ...
    if (const Record *BR = B.getRecord(); BR && BR->isUnion()) {
      for (const Record::Field &F : BR->fields()) {
        ...
        if (Pointer FieldPtr = B.atField(F.Offset); FieldPtr != Cur)
          deactivate(FieldPtr);
      }
    }
  }
...
std::optional<APValue> Pointer::toRValue(const Context &Ctx,
  ...
  Composite = [&Composite, &Ctx, &ASTCtx](QualType Ty, const Pointer &Ptr,
    ...
    if (const auto *RT = Ty->getAsCanonical<RecordType>()) {
      ...
      if (RT->getDecl()->isUnion()) {
        ...
        for (const auto &F : Record->fields()) {
          ...
          if (QualType FieldTy = F.Decl->getType(); FP.isActive()) {
        ...
        for (unsigned I = 0; I < NF; ++I) {
          ...
          if (OptPrimType APValue &Value = R.getStructField(I); T = Ctx.classify(FieldTy)) {
            TYPE_SWITCH(*T, Value = FP.deref<T>().toAPValue(ASTCtx));
          } else {
            Ok &= Composite(FieldTy, FP, Value);
          }
    ...
    if (const auto *CT = Ty->getAs<ComplexType>()) {
      ...
      if (QualType ElemTy = CT->getElementType(); ElemTy->isIntegerType()) {
        OptPrimType ElemT = Ctx.classify(ElemTy);
        assert(ElemT);
