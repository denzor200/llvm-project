...
bool Compiler<Emitter>::VisitBinaryOperator(const BinaryOperator *BO) {
  ...
  if (!T && BO->getOpcode() == BO_Cmp) {
    ...
    if (!Initializing) {
      ...
      if (UnsignedOrNone ResultIndex = this->allocateLocal(BO); !this->emitGetPtrLocal(*ResultIndex, BO))
        return false;
    }
  ...
  switch (auto Discard = [this, T, BO](bool Result) {
    if (!Result)
      return false;
...
bool Compiler<Emitter>::VisitFixedPointBinOp(const BinaryOperator *E) {
  ...
  switch (auto MaybeCastToBool = [&](bool Result) {
    if (!Result)
      return false;
...
bool Compiler<Emitter>::visitInitList(ArrayRef<const Expr *> Inits,
  ...
  if (QT->isRecordType()) {
    ...
    auto initPrimitiveField = [=](const Record::Field *FieldToInit,
      ...
      if (InitStackScope<Emitter> ISS(this, isa<CXXDefaultInitExpr>(Init)); !this->visit(Init))
        return false;
    ...
    auto initCompositeField = [=](const Record::Field *FieldToInit,
      ...
      if (InitLinkScope<Emitter> ILS(this, InitLink::Field(FieldToInit->Offset)); !this->emitGetPtrField(FieldToInit->Offset, Init))
        return false;
  ...
  if (QT->isArrayType()) {
    ...
    for (const Expr *Init : Inits) {
      if (const auto *EmbedS =
        ...
        if (auto Eval = [&](const IntegerLiteral *IL, unsigned ElemIndex) {
          if (TargetT == PT_Float) {
...
bool Compiler<Emitter>::visitArrayElemInit(unsigned ElemIndex, const Expr *Init,
  ...
  // dimension.
  if (InitLinkScope<Emitter> ILS(this, InitLink::Elem(ElemIndex)); !this->emitConstUint32(ElemIndex, Init))
    return false;
...
bool Compiler<Emitter>::visitCallArgs(ArrayRef<const Expr *> Args,
  ...
  for (const Expr *Arg : Args) {
    ...
    if (!NonNullArgs.empty() && NonNullArgs[ArgIndex]) {
      ...
      if (PrimType ArgT = classify(Arg).value_or(PT_Ptr); ArgT == PT_Ptr) {
...
bool Compiler<Emitter>::VisitUnaryExprOrTypeTraitExpr(
  ...
  if (Kind == UETT_AlignOf || Kind == UETT_PreferredAlignOf) {
    ...
    if (E->isArgumentType()) {
      ...
      if (const auto *const Expr *Arg = E->getArgumentExpr()->IgnoreParens(); DRE = dyn_cast<DeclRefExpr>(Arg))
...
bool Compiler<Emitter>::VisitAbstractConditionalOperator(
  ...
  if (std::optional<bool> llvm::SaveAndRestore LAAA(this->VarScope->LocalsAlwaysEnabled,
                            /*NewValue=*/false); BoolValue = getBoolValue(Condition)) {
    if (*BoolValue)
...
bool Compiler<Emitter>::VisitMaterializeTemporaryExpr(
  ...
  if (UnsignedOrNone LocalIndex =
    ...
    if (InitLinkScope<Emitter> ILS(this, InitLink::Temp(*LocalIndex)); !this->VarScope->LocalsAlwaysEnabled &&
        !this->emitEnableLocal(*LocalIndex, E))
      return false;
...
bool Compiler<Emitter>::VisitCXXStdInitializerListExpr(
  ...
  if (PrimType SecondFieldT = classifyPrim(R->getField(1u)->Decl->getType()); isIntegralType(SecondFieldT)) {
...
bool Compiler<Emitter>::visitExpr(const Expr *E, bool DestroyToplevelScope) {
  ...
  if (UnsignedOrNone LocalOffset = this->allocateLocal(E)) {
    ...
    if (InitLinkScope<Emitter> ILS(this, InitLink::Temp(*LocalOffset)); !this->emitGetPtrLocal(*LocalOffset, E))
      return false;
...
bool Compiler<Emitter>::visitAPValueInitializer(const APValue &Val,
  ...
  if (Val.isStruct()) {
    ...
    for (unsigned I = 0, N = Val.getStructNumFields(); I != N; ++I) {
      ...
      if (OptPrimType QualType FieldType = RF->Decl->getType(); PT = classify(FieldType)) {
  ...
  if (Val.isArray()) {
    ...
    for (unsigned A = 0, AN = Val.getArraySize(); A != AN; ++A) {
      ...
      if (OptPrimType const APValue &Elem = Val.getArrayInitializedElt(A); ElemT = classify(ElemType)) {
...
bool Compiler<Emitter>::VisitCallExpr(const CallExpr *E) {
  ...
  if (const auto *MC = dyn_cast<CXXMemberCallExpr>(E)) {
    if (!FuncDecl && classifyPrim(E->getCallee()) == PT_MemberPtr) {
      ...
      if (const auto *InstancePtr = MC->getImplicitObjectArgument(); isa_and_nonnull<CXXDestructorDecl>(CompilingFunction) ||
        ...
        if (const auto *Stripped = stripCheckedDerivedToBaseCasts(InstancePtr); isa<CXXThisExpr>(Stripped)) {
          FuncDecl =
  ...
  if (IsAssignmentOperatorCall) {
    ...
    if (PrimType Arg2T = classify(Args[1]).value_or(PT_Ptr); !this->emitFlip(Arg2T, Arg1T, E))
      return false;
  }
...
bool Compiler<Emitter>::visitReturnStmt(const ReturnStmt *RS) {
  ...
  if (const Expr *RE = RS->getRetValue()) {
    ...
    if (LocalScope<Emitter> RetScope(this); ReturnType) {
    ...
    if (RE->getType()->isVoidType()) {
      ...
      if (InitLinkScope<Emitter> ILS(this, InitLink::RVO()); !this->emitRVOPtr(RE))
        return false;
...
bool Compiler<Emitter>::visitCXXForRangeStmt(const CXXForRangeStmt *S) {
  ...
  if (LoopScope<Emitter> LS(this, S, EndLabel, IncLabel); Init && !this->visitStmt(Init))
    return false;
...
bool Compiler<Emitter>::emitLambdaStaticInvokerBody(const CXXMethodDecl *MD) {
  ...
  for (const ParmVarDecl *PVD : MD->parameters()) {
    ...
    if (PrimType ParamType = this->classify(PVD->getType()).value_or(PT_Ptr); !this->emitGetParam(ParamType, It->second.Offset, MD))
      return false;
  }
...
bool Compiler<Emitter>::compileConstructor(const CXXConstructorDecl *Ctor) {
  ...
  auto emitFieldInitializer = [&](const Record::Field *F, unsigned FieldOffset,
    ...
    if (InitLinkScope<Emitter> FieldScope(this, InitLink::Field(F->Offset)); !this->emitGetPtrThisField(FieldOffset, InitExpr))
      return false;
  ...
  if (IsUnion && Ctor->isCopyOrMoveConstructor()) {
    ...
    if (LocOverrideScope<Emitter> LOS(this, SourceInfo{}); R->getNumFields() == 0)
  ...
  for (const auto *Init : Ctor->inits()) {
    ...
    if (const FieldDecl *const Expr *InitExpr = Init->getInit(); Member = Init->getMember()) {
      const Record::Field *F = R->getField(Member);

      
      if (LocOverrideScope<Emitter> LOS(this, SourceInfo{},
                                    initNeedsOverridenLoc(Init)); !emitFieldInitializer(F, F->Offset, InitExpr, IsUnion))
        return false;
      ...
      if (InitStackScope<Emitter> ISS(this, isa<CXXDefaultInitExpr>(InitExpr)); !emitFieldInitializer(NestedField, NestedFieldOffset, InitExpr,
                                IsUnion))
        return false;
...
bool Compiler<Emitter>::VisitComplexUnaryOperator(const UnaryOperator *E) {
  ...
  switch (auto getElem = [=](unsigned Offset, unsigned Index) -> bool {
    if (!this->emitGetLocal(PT_Ptr, Offset, E))
      return false;
...
bool Compiler<Emitter>::visitDeclRef(const ValueDecl *D, const Expr *E) {
  ...
  if (auto It = this->LambdaCaptures.find(D);
    ...
    if (auto [Offset, IsPtr] = It->second; IsPtr)
