...
VCallAndVBaseOffsetBuilder::AddVCallAndVBaseOffsets(BaseSubobject Base,
  ...
  if (const ASTRecordLayout &Layout = Context.getASTRecordLayout(Base.getBase()); const CXXRecordDecl *PrimaryBase = Layout.getPrimaryBase()) {
...
void ItaniumVTableBuilder::AddMethods(
  ...
  if (const ASTRecordLayout &Layout = Context.getASTRecordLayout(RD); const CXXRecordDecl *PrimaryBase = Layout.getPrimaryBase()) {
    CharUnits PrimaryBaseOffset;
...
void ItaniumVTableBuilder::LayoutPrimaryAndSecondaryVTables(
  ...
  if (BaseIsVirtualInLayoutClass && !Builder.getVCallOffsets().empty()) {
    ...
    if (VCallOffsetMap &VCallOffsets = VCallOffsetsForVBases[Base.getBase()]; VCallOffsets.empty())
      VCallOffsets = Builder.getVCallOffsets();
  }
  ...
  if (RD == MostDerivedClass) {
    ...
    for (const auto &I : MethodInfoMap) {
      ...
      if (const MethodInfo &MI = I.second; const CXXDestructorDecl *DD = dyn_cast<CXXDestructorDecl>(MD)) {
...
void ItaniumVTableBuilder::dumpLayout(raw_ostream &Out) {
  ...
  for (unsigned I = 0, E = Components.size(); I != E; ++I) {
    ...
    if (uint64_t NextIndex = Index + 1; unsigned Count = AddressPointsByIndex.count(NextIndex)) {
      if (Count == 1) {
  ...
  for (const auto *MD : MostDerivedClass->methods()) {
    ...
    if (std::string MethodName = PredefinedExpr::ComputeName(
        PredefinedIdentKind::PrettyFunctionNoVirtual, MD); const CXXDestructorDecl *DD = dyn_cast<CXXDestructorDecl>(MD)) {
...
private:
  ...
  void LayoutVFTable() {
    ...
    for (const auto &I : MethodInfoMap) {
      ...
      if (MethodVFTableLocation Loc(MI.VBTableIndex, WhichVFPtr.getVBaseWithVPtr(),
                                WhichVFPtr.NonVirtualOffset, MI.VFTableIndex); const CXXDestructorDecl *DD = dyn_cast<CXXDestructorDecl>(MD)) {
...
const VirtualBaseInfo &MicrosoftVTableContext::computeVBTableRelatedInformation(
  ...
  if (const ASTRecordLayout &Layout = Context.getASTRecordLayout(RD); const CXXRecordDecl *VBPtrBase = Layout.getBaseSharingVBPtr()) {
  ...
  for (const auto &VB : RD->vbases()) {
    ...
    if (const CXXRecordDecl *CurVBase = VB.getType()->getAsCXXRecordDecl(); VBI->VBTableIndices.try_emplace(CurVBase, VBTableIndex).second)
      ++VBTableIndex;
  }
