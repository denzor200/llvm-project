...
void Driver::setDriverMode(StringRef Value) {
  ...
  if (static StringRef OptName =
      getOpts().getOption(options::OPT_driver_mode).getPrefixedName(); auto M = llvm::StringSwitch<std::optional<DriverMode>>(Value)
                   .Case("gcc", GCCMode)
                   .Case("g++", GXXMode)
                   .Case("cpp", CPPMode)
                   .Case("cl", CLMode)
...
Compilation *Driver::BuildCompilation(ArrayRef<const char *> ArgList) {
  ...
  if (IsCLMode() && !ContainsError) {
    ...
    if (!CLModePassThroughArgList.empty()) {
      ...
      if (auto CLModePassThroughOptions = std::make_unique<InputArgList>(
...

  {
    ...
    if (auto MultilibMacroDefineList =
        std::make_unique<InputArgList>(ParseArgStrings(
            MLMacroDefinesChar, /*UseDriverMode=*/false, MLContainsError)); !MLContainsError) {
      ...
      for (StringRef Arch : Arg->getValues()) {
        if (Arch == "native" || Arch.empty()) {
          ...
          for (auto ArchStr : *GPUsOrErr) {
            ...
            if (StringRef CanonicalStr = getCanonicalArchString(
                C, Args, Args.MakeArgString(ArchStr), TC.getTriple()); !CanonicalStr.empty())
...
std::string Driver::GetProgramPath(StringRef Name, const ToolChain &TC) const {
  ...
  for (const auto &PrefixDir : PrefixDirs) {
    if (llvm::sys::fs::is_directory(PrefixDir)) {
      ...
      if (SmallString<128> P((PrefixDir + Name).str()); llvm::sys::fs::can_execute(Twine(P)))
        return std::string(P);
    }
  }
...
static void applyOneOverrideOption(raw_ostream &OS,
  ...
  if (Edit[0] == '^') {
    ...
    for (unsigned i = 1, e = Args.size(); i != e; ++i) {
      ...
      if (std::string Repl = llvm::Regex(MatchPattern).sub(ReplPattern, Args[i]); Repl != Args[i]) {
