  ...
  for (const Arg *A : Args) {
    ...
    if (StrictFPModel) {
      ...
      if (HonorINFs && HonorNaNs && !AssociativeMath && !ReciprocalMath &&
        ...
        if (auto RHS = (A->getNumValues() == 0)
                       ? A->getSpelling()
                       : Args.MakeArgString(A->getSpelling() + A->getValue()); A->getSpelling() != "-ffp-contract=") {
...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Triple.isOSWindows() && (Triple.getArch() == llvm::Triple::arm ||
    ...
    if (bool Failure =
        Triple.getArchName().substr(Offset).consumeInteger(10, Version); Failure || Version < 7)
  ...
  if (Arg *A = Args.getLastArg(options::OPT_fgnuc_version_EQ)) {
    ...
    if (unsigned Patch = GNUCVer.getSubminor().value_or(0); Invalid || GNUCVer.getBuild() || Minor >= 100 || Patch >= 100) {
      D.Diag(diag::err_drv_invalid_value)
          << A->getAsString(Args) << A->getValue();
    }
...
void Clang::AddClangCLArgs(const ArgList &Args, types::ID InputType,
 ...
 if (Args.hasArg(options::OPT__SLASH_kernel)) {
  ...
  if (const Arg *A = Args.getLastArg(options::OPT__SLASH_vlen,
    ...
    if (llvm::SmallSet<StringRef, 4> Arch512 = {"AVX512F", "AVX512", "AVX10.1",
                                            "AVX10.2"}; A->getOption().matches(options::OPT__SLASH_vlen_EQ_512)) {
