...
constructHexagonLinkArgs(Compilation &C, const JobAction &JA,
  ...
  auto Find = [&HTC] (const std::string &RootDir, const std::string &SubDir,
    ...
    if (std::string P = HTC.GetFilePath(RelName.c_str()); llvm::sys::fs::exists(P))
      return P;
...
void HexagonToolChain::getHexagonLibraryPaths(const ArgList &Args,
  ...
  if (std::string TargetDir = getHexagonTargetDir(D.Dir, D.PrefixDirs); !llvm::is_contained(RootDirs, TargetDir))
...
HexagonToolChain::HexagonToolChain(const Driver &D, const llvm::Triple &Triple,
  ...
  if (const std::string BinDir(TargetDir + "/bin"); D.getVFS().exists(BinDir))
