...
void tools::addLTOOptions(const ToolChain &ToolChain, const ArgList &Args,
  ...
  if (Args.hasArg(options::OPT_mxcoff_roptr) ||
    ...
    if (StringRef OptStr = HasRoptr ? "-mxcoff-roptr" : "-mno-xcoff-roptr"; !IsOSAIX)
...
SmallString<128> tools::getStatsFileName(const llvm::opt::ArgList &Args,
  ...
  if (A) {
    ...
    if (StringRef SaveStats = A->getValue(); SaveStats == "obj" && Output.isFilename()) {
...
void tools::checkAMDGPUCodeObjectVersion(const Driver &D,
  ...
  if (auto *const unsigned MaxCodeObjVer = 6; CodeObjArg = getAMDGPUCodeObjectArgument(D, Args)) {
