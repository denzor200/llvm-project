...
void Flang::AddLoongArch64TargetArgs(const ArgList &Args,
  ...
  if (const Arg *const Driver &D = getToolChain().getDriver(); A = Args.getLastArg(options::OPT_mabi_EQ)) {
...
void Flang::AddRISCVTargetArgs(const ArgList &Args,
  ...
  if (StringRef ABIName = riscv::getRISCVABI(Args, Triple); ABIName == "lp64" || ABIName == "lp64f" || ABIName == "lp64d")
...
void Flang::addTargetOptions(const ArgList &Args,
  ...
  if (std::string CPU = getCPUName(D, Args, Triple); !CPU.empty()) {
