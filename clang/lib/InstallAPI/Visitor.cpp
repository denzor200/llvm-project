...
void InstallAPIVisitor::recordObjCInstanceVariables(
  ...
  if (const RecordLinkage ContainerLinkage = Record->getLinkage(); ASTCtx.getLangOpts().ObjCRuntime.isFragile())
...
std::string InstallAPIVisitor::getMangledCXXThunk(
  ...
  if (const auto *const auto *Method = cast<CXXMethodDecl>(D.getDecl()); Dtor = dyn_cast<CXXDestructorDecl>(Method))
...
void InstallAPIVisitor::emitVTableSymbols(const CXXRecordDecl *D,
  ...
  if (hasVTable(D)) {
    ...
    if (const CXXLinkage VTableLinkage = getVTableLinkage(D); VTableLinkage == CXXLinkage::ExternalLinkage ||
