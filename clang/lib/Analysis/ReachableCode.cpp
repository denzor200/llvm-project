...
static bool isExpandedFromConfigurationMacro(const Stmt *S,
  ...
  if (L.isMacroID()) {
    ...
    if (SourceManager &SM = PP.getSourceManager(); IgnoreYES_NO) {
      ...
      if (StringRef MacroName = PP.getImmediateMacroName(TopL); MacroName == "YES" || MacroName == "NO")
        return false;
    } else if (!PP.getLangOpts().CPlusPlus) {
      // Do not treat C 'false' and 'true' macros as configuration values.
      SourceLocation TopL = getTopMostMacro(L, SM);
      
      if (StringRef MacroName = PP.getImmediateMacroName(TopL); MacroName == "false" || MacroName == "true")
        return false;
    }
    return true;
  }
