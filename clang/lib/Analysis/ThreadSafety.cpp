...
void VarMapBuilder::VisitDeclStmt(const DeclStmt *S) {
  ...
  for (const auto *D : DGrp) {
    if (const auto *VD = dyn_cast_or_null<VarDecl>(D)) {
      ...
      if (QualType T = VD->getType(); T.isTrivialType(VD->getASTContext())) {
...
void VarMapBuilder::VisitBinaryOperator(const BinaryOperator *BO) {
  ...
  if (const auto *Expr *LHSExp = BO->getLHS()->IgnoreParenCasts(); DRE = dyn_cast<DeclRefExpr>(LHSExp)) {
    ...
    if (ParamType->isReferenceType() &&
      ...
      if (const auto *UO = dyn_cast<UnaryOperator>(Arg)) {
        if (UO->getOpcode() == UO_AddrOf) {
          ...
          if (const auto *const Expr *SubE = UO->getSubExpr()->IgnoreParenCasts(); DRE = dyn_cast<DeclRefExpr>(SubE))
            VDec = DRE->getDecl();
        }
      }
    }
...
private:
  void lock(FactSet &FSet, FactManager &FactMan, const CapabilityExpr &Cp,
    ...
    if (const auto It = FSet.findLockIter(FactMan, Cp); It != FSet.end()) {
      ...
      if (const FactEntry *const auto &Fact = cast<LockableFactEntry>(FactMan[*It]); RFact = Fact.tryReenter(FactMan, kind)) {
        // This capability has been reentrantly acquired.
  ...
  void unlock(FactSet &FSet, FactManager &FactMan, const CapabilityExpr &Cp,
    ...
    if (const auto It = FSet.findLockIter(FactMan, Cp); It != FSet.end()) {
      ...
      if (const FactEntry *const auto &Fact = cast<LockableFactEntry>(FactMan[*It]); RFact = Fact.leaveReentrant(FactMan)) {
        // This capability remains reentrantly acquired.
        FSet.replaceLock(FactMan, It, RFact);
        return;
      }
    ...
    switch (At->getKind()) {
      case attr::AcquiredBefore: {
        ...
        for (const auto *Arg : A->args()) {
          ...
          if (const ValueDecl *CapabilityExpr Cp =
            Analyzer.SxBuilder.translateAttrExpr(Arg, nullptr); Cpvd = Cp.valueDecl()) {
            Info->Vect.push_back(Cpvd);
            
            if (const auto It = BMap.find(Cpvd); It == BMap.end())
      ...
      case attr::AcquiredAfter: {
        ...
        for (const auto *Arg : A->args()) {
          ...
          if (const ValueDecl *CapabilityExpr Cp =
            Analyzer.SxBuilder.translateAttrExpr(Arg, nullptr); ArgVd = Cp.valueDecl()) {
...
const CallExpr* ThreadSafetyAnalyzer::getTrylockCallExpr(const Stmt *Cond,
  ...
  if (const auto *CallExp = dyn_cast<CallExpr>(Cond)) {
    ...
    if (bool TCond, FCond; getStaticBooleanValue(COP->getTrueExpr(), TCond) &&
...
void BuildLockset::examineArguments(const FunctionDecl *FD,
  ...
  for (auto Arg = ArgBegin; Param != Params.end() && Arg != ArgEnd;
    ...
    if (QualType Qt = (*Param)->getType(); Qt->isReferenceType())
...
void BuildLockset::VisitCallExpr(const CallExpr *Exp) {
  ...
  if (const auto *CE = dyn_cast<CXXMemberCallExpr>(Exp)) {
    ...
    switch (OverloadedOperatorKind OEop = OE->getOperator(); OEop) {
      case OO_Equal:
      case OO_PlusEqual:
      case OO_MinusEqual:
      case OO_StarEqual:
