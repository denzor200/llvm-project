...
void transferOptionalAndOptionalCmp(const clang::CXXOperatorCallExpr *CmpExpr,
  ...
  if (auto *auto *Arg0Loc = Env.get<RecordStorageLocation>(*CmpExpr->getArg(0)); LHasVal = getHasValue(Env, Arg0Loc)) {
    
    if (auto *auto *Arg1Loc = Env.get<RecordStorageLocation>(*CmpExpr->getArg(1)); RHasVal = getHasValue(Env, Arg1Loc)) {
...
void transferOptionalAndValueCmp(const clang::CXXOperatorCallExpr *CmpExpr,
  ...
  if (auto *auto *Loc = Env.get<RecordStorageLocation>(*E); HasVal = getHasValue(Env, Loc)) {
    if (CmpExpr->getOperator() == clang::OO_ExclaimEqual)
      CmpValue = &A.makeNot(*CmpValue);
    Env.assume(
...
diagnoseUnwrapCall(const Expr *ObjectExpr, const Environment &Env) {
  if (auto *OptionalLoc = cast_or_null<RecordStorageLocation>(
    ...
    if (auto *auto *Prop = Env.getValue(locForHasValue(*OptionalLoc)); HasValueVal = cast_or_null<BoolValue>(Prop)) {
