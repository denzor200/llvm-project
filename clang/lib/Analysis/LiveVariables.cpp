...
void TransferFunctions::Visit(Stmt *S) {
  ...
  switch (S->getStmtClass()) {
    ...
    case Stmt::CXXMemberCallExprClass: {
      ...
      if (Expr *CXXMemberCallExpr *CE = cast<CXXMemberCallExpr>(S); ImplicitObj = CE->getImplicitObjectArgument()) {
    ...
    case Stmt::DeclStmtClass: {
      ...
      if (const VarDecl *const DeclStmt *DS = cast<DeclStmt>(S); VD = dyn_cast<VarDecl>(DS->getSingleDecl())) {
...
void TransferFunctions::VisitBinaryOperator(BinaryOperator *B) {
  ...
  if (B->isAssignmentOp()) {
    ...
    if (DeclRefExpr *Expr *LHS = B->getLHS()->IgnoreParens(); DR = dyn_cast<DeclRefExpr>(LHS)) {
      const Decl* D = DR->getDecl();
      

      if (const BindingDecl* bool Killed = false; BD = dyn_cast<BindingDecl>(D)) {
...
void TransferFunctions::VisitDeclRefExpr(DeclRefExpr *DR) {
  ...
  if (const auto *bool InAssignment = LV.inAssignment.contains(DR); BD = dyn_cast<BindingDecl>(D)) {
    if (!InAssignment) {
      if (const auto *HV = BD->getHoldingVar())
...
void TransferFunctions::VisitObjCForCollectionStmt(ObjCForCollectionStmt *OS) {
  ...
  if (DeclStmt *Stmt *element = OS->getElement(); DS = dyn_cast<DeclStmt>(element)) {
