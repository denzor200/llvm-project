...
til::SExpr *SExprBuilder::translateVariable(const VarDecl *VD,
  ...
  if (QualType Ty = VD->getType(); !VD->isStaticLocal() && Ty->isPointerType()) {
    ...
    if (LookupLocalVarExpr) {
      ...
      if (const Expr *E = LookupLocalVarExpr(VD)) {
        ...
        // canonical variable.
        if (til::SExpr *Result = translate(E, Ctx); Result && !isa<til::Undefined>(Result))
          return Result;
      }
    }
  }
...
til::SExpr *SExprBuilder::translateCXXOperatorCallExpr(
  ...
  if (CapabilityExprMode) {
    ...
    if (OverloadedOperatorKind k = OCE->getOperator(); k == OO_Star || k == OO_Arrow) {
...
til::SExpr *SExprBuilder::translateCastExpr(const CastExpr *CE,
  ...
  switch (CastKind K = CE->getCastKind(); K) {
...
void SExprBuilder::enterCFG(CFG *Cfg, const NamedDecl *D,
  ...
  for (auto *Pm : Parms) {
    ...
    if (QualType T = Pm->getType(); !T.isTrivialType(Pm->getASTContext()))
      continue;
