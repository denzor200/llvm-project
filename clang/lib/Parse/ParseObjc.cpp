...
void Parser::MaybeSkipAttributes(tok::ObjCKeywordKind Kind) {
  ...
  if (ParsedAttributes attrs(AttrFactory); Tok.is(tok::kw___attribute)) {
...
Decl *Parser::ParseObjCAtInterfaceDeclaration(SourceLocation AtLoc,
  ...
  if (ObjCTypeParamListScope typeParamScope(Actions, getCurScope()); Tok.is(tok::less))
...
static void addContextSensitiveTypeNullability(Parser &P,
  ...
  if (auto getNullabilityAttr = [&](AttributePool &Pool) -> ParsedAttr * {
...
IdentifierInfo *Parser::ParseObjCSelectorPiece(SourceLocation &SelectorLoc) {
  ...
  case tok::caretequal: {
    ...
    if (std::string ThisTok(PP.getSpelling(Tok)); isLetter(ThisTok[0])) {
...
ParsedType Parser::ParseObjCTypeName(ObjCDeclSpec &DS,
  ...
  if (isTypeSpecifierQualifier() || isObjCInstancetype()) {
    ...
    if (!declarator.isInvalidType()) {
      ...
      if (bool addedToDeclSpec = false; DS.getObjCDeclQualifier() & ObjCDeclSpec::DQ_CSNullability)
...
Decl *Parser::ParseObjCMethodDecl(SourceLocation mLoc,
  ...
  while (true) {
    ...
    if (!SelIdent) {
      ...
      if (SourceLocation ColonLoc = Tok.getLocation(); PP.getLocForEndOfToken(ArgInfo.NameLoc) == ColonLoc) {
...
void Parser::parseObjCTypeArgsOrProtocolQualifiers(
  ...
  for (unsigned i = 0, n = identifiers.size(); i != n; ++i) {
    ...
    if (ParsedType typeArg
      = Actions.getTypeName(*identifiers[i], identifierLocs[i], getCurScope()); typeArg) {
      DeclSpec DS(AttrFactory);
      ...
      if (TypeResult fullTypeArg = Actions.ActOnTypeName(D); fullTypeArg.isUsable()) {
...
Parser::ParseObjCAtImplementationDeclaration(SourceLocation AtLoc,
  ...
  if (Tok.is(tok::less)) {
    ...
    if (ObjCTypeParamListScope typeParamScope(Actions, getCurScope()); parseObjCTypeParamListOrProtocolRefs(typeParamScope, lAngleLoc,
...

  {
    ...
    while (!ObjCImplParsing.isFinished() && !isEofOrEom()) {
      ...
      if (DeclGroupPtrTy ParsedAttributes EmptyDeclSpecAttrs(AttrFactory); DGP =
...
Decl *Parser::ParseObjCPropertyDynamic(SourceLocation atLoc) {
  ...
  if (Tok.is(tok::l_paren)) {
    ...
    if (const IdentifierInfo *II = Tok.getIdentifierInfo(); !II) {
      ...
      if (SourceLocation AttrName = ConsumeToken(); II->isStr("class")) {
...
StmtResult Parser::ParseObjCTryStmt(SourceLocation atLoc) {
  ...
  while (Tok.is(tok::at)) {
    ...
    if (Token AfterAt = GetLookAheadToken(1); !AfterAt.isObjCAtKeyword(tok::objc_catch) &&
...
bool Parser::ParseObjCXXMessageReceiver(bool &IsExpr, void *&TypeOrExpr) {
  ...
  if (InMessageExpressionRAIIObject InMessage(*this, true); Tok.isOneOf(tok::identifier, tok::coloncolon, tok::kw_typename,
                  tok::annot_cxxscope))
...
Parser::ParseObjCMessageExpressionBody(SourceLocation LBracLoc,
  ...
  if (InMessageExpressionRAIIObject InMessage(*this, true); Tok.is(tok::code_completion)) {
    cutOffParsing();
    if (SuperLoc.isValid())
