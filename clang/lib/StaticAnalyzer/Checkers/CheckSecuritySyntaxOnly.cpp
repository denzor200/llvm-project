...
void WalkAST::checkCall_strcpy(const CallExpr *CE, const FunctionDecl *FD) {
  ...
  if (const auto *const auto *Target = CE->getArg(0)->IgnoreImpCasts(),
             *Source = CE->getArg(1)->IgnoreImpCasts(); Array = dyn_cast<ConstantArrayType>(Target->getType())) {
    
    if (const auto *uint64_t ArraySize = BR.getContext().getTypeSize(Array) / 8; String = dyn_cast<StringLiteral>(Source)) {
