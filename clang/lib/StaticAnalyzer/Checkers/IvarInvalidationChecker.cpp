...
void IvarInvalidationCheckerImpl::MethodCrawler::markInvalidated(
  ...
  if (IvarSet::iterator I = IVars.find(Iv); I != IVars.end()) {
...
void IvarInvalidationCheckerImpl::MethodCrawler::checkObjCMessageExpr(
  ...
  if (const ObjCMethodDecl *MD = ME->getMethodDecl(); MD) {
    ...
    if (MethToIvarMapTy::const_iterator IvI = PropertyGetterToIvarMap.find(MD); IvI != PropertyGetterToIvarMap.end())
...
void IvarInvalidationCheckerImpl::MethodCrawler::VisitObjCMessageExpr(
  ...
  if (MD && (ME->getNumArgs() == 1) && isZero(ME->getArg(0))) {
    ...
    if (MethToIvarMapTy::const_iterator IvI = PropertySetterToIvarMap.find(MD); IvI != PropertySetterToIvarMap.end()) {
      markInvalidated(IvI->second);
      return;
    }
  }
