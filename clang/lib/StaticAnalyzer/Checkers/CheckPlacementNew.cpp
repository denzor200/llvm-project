...
SVal PlacementNewChecker::getExtentSizeOfNewTarget(const CXXNewExpr *NE,
  ...
  if (NE->isArray()) {
    ...
    if (SVal ElementCount = C.getSVal(SizeExpr); auto ElementCountNL = ElementCount.getAs<NonLoc>()) {
...
void PlacementNewChecker::emitBadAlignReport(const Expr *P, CheckerContext &C,
  ...
  if (ProgramStateRef State = C.getState(); ExplodedNode *N = C.generateErrorNode(State)) {
