...
void RetainCountChecker::processObjCLiterals(CheckerContext &C,
  ...
  for (const Stmt *Child : Ex->children()) {
    ...
    if (SymbolRef SVal V = pred->getSVal(Child); sym = V.getAsSymbol())
...
void RetainCountChecker::checkPostCall(const CallEvent &Call,
  ...
  if (const auto *MC = dyn_cast<ObjCMethodCall>(&Call)) {
    if (MC->isInstanceMessage()) {
      ...
      if (SymbolRef SVal ReceiverV = MC->getReceiverSVal(); Sym = ReceiverV.getAsLocSymbol())
...
void RetainCountChecker::processSummaryOfInlined(const RetainSummary &Summ,
  ...
  for (unsigned idx = 0, e = CallOrMsg.getNumArgs(); idx != e; ++idx) {
    ...
    if (SymbolRef SVal V = CallOrMsg.getArgSVal(idx); Sym = V.getAsLocSymbol()) {
...
void RetainCountChecker::checkSummary(const RetainSummary &Summ,
  ...
  for (unsigned idx = 0, e = CallOrMsg.getNumArgs(); idx != e; ++idx) {
    ...
    if (SymbolRef ArgEffect Effect = Summ.getArg(idx); Sym = V.getAsLocSymbol()) {
      if (const RefVal *T = getRefBinding(state, Sym)) {
