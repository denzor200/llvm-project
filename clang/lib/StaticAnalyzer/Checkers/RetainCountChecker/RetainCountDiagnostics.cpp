...
static std::string getPrettyTypeName(QualType QT) {
  ...
  if (QualType PT = QT->getPointeeType(); !PT.isNull() && !QT->getAs<TypedefType>())
...
static std::optional<std::string> findMetaClassAlloc(const Expr *Callee) {
  if (const auto *ME = dyn_cast<MemberExpr>(Callee)) {
    ...
    if (const auto *const Expr *This = ME->getBase()->IgnoreParenImpCasts(); DRE = dyn_cast<DeclRefExpr>(This)) {
...
RefCountReportVisitor::VisitNode(const ExplodedNode *N, BugReporterContext &BRC,
  ...
  if (auto CallEventManager &CEMgr = BRC.getStateManager().getCallEventManager(); CE = N->getLocationAs<CallExitBegin>())
...
static AllocationInfo GetAllocationSite(ProgramStateManager &StateMgr,
  ...
  while (N) {
    ...
    if (FB) {
      ...
      if (const auto *const MemRegion *R = FB.getRegion(); MR = R->getMemorySpaceAs<StackSpaceRegion>(St))
    ...
    if (!InitMethodContext)
      if (auto CEP = N->getLocation().getAs<CallEnter>()) {
        ...
        if (const auto *const Stmt *CE = CEP->getCallExpr(); ME = dyn_cast_or_null<ObjCMessageExpr>(CE)) {
  ...
  if (InitMethodContext) {
    ...
    if (std::optional<StmtPoint> const ProgramPoint AllocPP = AllocationNode->getLocation(); SP = AllocPP.getAs<StmtPoint>())
