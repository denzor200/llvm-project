...
void StdLibraryFunctionsChecker::RangeConstraint::applyOnOutOfRange(
  ...
  for (size_t I = 1; I != E; ++I) {
    ...
    if (const llvm::APSInt &Max = BVF.getValue(R[I].first - 1ULL, ArgT); Min <= Max) {
      if (!F(Min, Max))
        return;
    }
  }
...
bool StdLibraryFunctionsChecker::Signature::matches(
  ...
  if (!isIrrelevant(RetTy)) {
    ...
    if (QualType FDRetTy = RemoveRestrict(FD->getReturnType().getCanonicalType()); RetTy != FDRetTy)
      return false;
  }
  ...
  for (auto [Idx, ArgTy] : llvm::enumerate(ArgTys)) {
    ...
    if (QualType FDArgTy =
        RemoveRestrict(FD->getParamDecl(Idx)->getType().getCanonicalType()); ArgTy != FDArgTy)
      return false;
  }
