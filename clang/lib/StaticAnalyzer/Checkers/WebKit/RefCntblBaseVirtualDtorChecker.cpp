  ...
  bool VisitCXXDeleteExpr(const CXXDeleteExpr *E) {
    ...
    while (Arg) {
      if (auto *Paren = dyn_cast<ParenExpr>(Arg))
        ...
        if (auto CastType = Cast->getType(); auto *PtrType = dyn_cast<PointerType>(CastType)) {
          
          if (auto PointeeType = PtrType->getPointeeType(); auto *ParmType = dyn_cast<TemplateTypeParmType>(PointeeType)) {
            if (ArgList) {
            ...
            if (auto Type = ST->getReplacementType(); auto *RD = dyn_cast<RecordType>(Type)) {
              if (declaresSameEntity(RD->getDecl(), ClassDecl))
                return true;
            }
          }
