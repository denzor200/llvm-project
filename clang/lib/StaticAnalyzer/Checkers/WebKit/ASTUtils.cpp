    ...
    if (auto *call = dyn_cast<CallExpr>(E)) {
      ...
      if (auto *CalleeDecl = call->getCalleeDecl()) {
        if (auto *FD = dyn_cast<FunctionDecl>(CalleeDecl)) {
          ...
          if (auto *auto RetType = FD->getReturnType(); Subst = dyn_cast<SubstTemplateTypeParmType>(RetType)) {
...
bool isASafeCallArg(const Expr *E) {
  ...
  if (auto *Ref = dyn_cast<DeclRefExpr>(E)) {
    ...
    if (auto *auto *FoundDecl = Ref->getFoundDecl(); D = dyn_cast_or_null<VarDecl>(FoundDecl)) {
