...
void IteratorModeling::checkPostStmt(const BinaryOperator *BO,
  ...
  if (const SVal RVal = State->getSVal(RHS, C.getLocationContext()); isSimpleComparisonOperator(BO->getOpcode())) {
...
IteratorModeling::handleOverloadedOperator(CheckerContext &C,
    ...
    if (isSimpleComparisonOperator(Op)) {
      ...
      if (const auto *InstCall = dyn_cast<CXXInstanceCall>(&Call)) {
        ...
        if (const QualType SecondType = SecondArg->getType(); FirstType->isIntegralOrEnumerationType() ||
