...
static bool isSynthesizedRetainableProperty(const ObjCPropertyImplDecl *I,
  ...
  if (QualType T = (*ID)->getType(); !T->isObjCRetainableType())
    return false;
...
void ObjCDeallocChecker::checkBeginFunction(
  ...
  for (auto *PropImpl : getContainingObjCImpl(LCtx)->property_impls()) {
    ...
    if (ReleaseRequirement Requirement = getDeallocReleaseRequirement(PropImpl); Requirement != ReleaseRequirement::MustRelease)
      continue;
...
ReleaseRequirement ObjCDeallocChecker::getDeallocReleaseRequirement(
  ...
  switch (ObjCPropertyDecl::SetterKind SK = PropDecl->getSetterKind(); SK) {
