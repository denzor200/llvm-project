...
static std::optional<int64_t> getKnownValue(ProgramStateRef State, SVal V) {
  ...
  if (SValBuilder &SVB = State->getStateManager().getSValBuilder(); const llvm::APSInt *Int = SVB.getKnownValue(State, V))
...
StreamChecker::ensureFseekWhenceCorrect(SVal WhenceVal, CheckerContext &C,
  ...
  if (int64_t X = CI->getValue()->getSExtValue(); X == SeekSetVal || X == SeekCurVal || X == SeekEndVal)
    return State;
