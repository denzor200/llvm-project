...
struct FieldConsumer {
  ...
  void consume(const FieldDecl *FD) {
    ...
    if (SVal L = C->getState()->getLValue(FD, loc::MemRegionVal(Reg)); const MemRegion *FR = L.getAsRegion())
      Out->insert(FR);
  }
      ...
      case AF_InnerBuffer: {
        ...
        if (N->getLocation().getKind() == ProgramPoint::PostImplicitCallKind) {
          ...
          if (const Stmt *S = RSCurr->getStmt(); const auto *MemCallE = dyn_cast<CXXMemberCallExpr>(S)) {
            ...
            if (CallEventRef<> Call =
                CEMgr.getSimpleCall(CallE, state, CurrentLC, {nullptr, 0}); const auto *D = dyn_cast_or_null<NamedDecl>(Call->getDecl()))
              OS << D->getDeclName();
            else
              OS << "unknown";
          }
