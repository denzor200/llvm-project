...
static QualType parameterTypeFromSVal(SVal val, CheckerContext &C) {
  ...
  if (std::optional<loc::MemRegionVal> const StackFrameContext * SFC = C.getStackFrame(); X = val.getAs<loc::MemRegionVal>()) {
    
    if (const VarRegion *const MemRegion* R = X->getRegion(); VR = R->getAs<VarRegion>())
