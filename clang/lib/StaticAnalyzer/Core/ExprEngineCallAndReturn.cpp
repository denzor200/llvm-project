...
static SVal adjustReturnValue(SVal V, QualType ExpectedTy, QualType ActualTy,
  ...
  if (const CXXRecordDecl *ActualClass = ActualTy->getPointeeCXXRecordDecl(); ExpectedClass && ActualClass) {
    ...
    if (CXXBasePaths Paths(/*FindAmbiguities=*/true, /*RecordPaths=*/true,
                       /*DetectVirtual=*/false); ActualClass->isDerivedFrom(ExpectedClass, Paths) &&
  ...
  if (CE) {
    if (const ReturnStmt *RS = dyn_cast_or_null<ReturnStmt>(LastSt)) {
      ...
      if (wasDifferentDeclUsedForInlining(Call, calleeCtx)) {
        ...
        if (QualType ReturnedTy =
          CallEvent::getDeclaredResultType(calleeCtx->getDecl()); !ReturnedTy.isNull()) {
...
ProgramStateRef ExprEngine::finishArgumentConstruction(ProgramStateRef State,
  ...
  for (unsigned CallI = 0, CallN = Call.getNumArgs(); CallI != CallN; ++CallI) {
    ...
    if (std::optional<SVal> unsigned I = Call.getASTArgumentIndex(CallI); V = getObjectUnderConstruction(State, {E, I}, LC)) {
      SVal VV = *V;
      (void)VV;
...
ExprEngine::mayInlineCallKind(const CallEvent &Call, const ExplodedNode *Pred,
  ...
  case CE_CXXConstructor: {
    ...
    if (const ConstructionContext *CC = CCE ? CCE->getConstructionContext()
                                        : nullptr; llvm::isa_and_nonnull<NewAllocatedObjectConstructionContext>(CC) &&
...
bool ExprEngine::shouldRepeatCtorCall(ProgramStateRef State,
  ...
  if (const auto *auto Ty = E->getType(); CAT = dyn_cast<ConstantArrayType>(Ty)) {
...
void ExprEngine::VisitReturnStmt(const ReturnStmt *RS, ExplodedNode *Pred,
  ...
  if (StmtNodeBuilder B(dstPreVisit, Dst, *currBldrCtx); RS->getRetValue()) {
