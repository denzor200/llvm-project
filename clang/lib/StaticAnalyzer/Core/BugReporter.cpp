    ...
    switch (piece->getKind()) {
      ...
      case PathDiagnosticPiece::Event: {
        ...
        if (auto *nextEvent =
          ...
          if (auto *auto *event = cast<PathDiagnosticEventPiece>(piece.get()); pieceToKeep =
...
PathDiagnosticLocation PathDiagnosticBuilder::ExecutionContinues(
  ...
  if (Loc.asStmt())
    ...
    if (const Decl *D = C.getCurrLocationContext()->getDecl(); isa<ObjCMethodDecl>(D))
      os << "method";
...
void PathDiagnosticBuilder::generatePathDiagnosticsForNode(
  ...
  if (auto PS = P.getAs<PostStmt>()) {
    ...
    if (const Stmt *const ParentMap &PM = C.getParentMap(); Term = BSrc->getTerminatorStmt()) {
  ...
  for (PathPieces::iterator I = pieces.begin(), E = pieces.end(); I != E; ++I) {
    ...
    while (true) {
      ...
      if (const auto *EV = dyn_cast<PathDiagnosticEventPiece>(NextI->get()); EV) {
        ...
        if (StringRef S = EV->getString(); S == StrEnteringLoop || S == StrLoopBodyZero ||
...
void PathSensitiveBugReport::addVisitor(
  ...
  if (void *InsertPos = nullptr; CallbacksSet.FindNodeOrInsertPos(ID, InsertPos)) {
    return;
  }
