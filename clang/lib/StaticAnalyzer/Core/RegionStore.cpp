  ...
  if (const BlockDataRegion *BR = dyn_cast<BlockDataRegion>(baseR)) {
    for (auto Var : BR->referenced_vars()) {
      ...
      if (const VarDecl *VD = VR->getDecl(); VD->hasAttr<BlocksAttr>() || !VD->hasLocalStorage()) {
        ...
        if (SVal V = RM.getBinding(B, loc::MemRegionVal(VR)); std::optional<Loc> L = V.getAs<Loc>()) {
  ...
  if (const ArrayType *AT = Ctx.getAsArrayType(T)) {
    ...
    if (bool doNotInvalidateSuperRegion = ITraits.hasTrait(
      ...
      for (const auto &[BK, V] : *C) {
        ...
        if (std::optional<uint64_t> ROffset =
            BK.hasSymbolicOffset() ? std::optional<uint64_t>() : BK.getOffset(); !ROffset ||
...
bool RegionStoreManager::includedInBindings(Store store,
  ...
  for (RegionBindingsRef::iterator RI = B.begin(), RE = B.end(); RI != RE; ++RI) {
    ...
    for (ClusterBindings::iterator CI = Cluster.begin(), CE = Cluster.end();
      ...
      if (SVal D = CI.getData(); const MemRegion *R = D.getAsRegion())
