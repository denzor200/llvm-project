...
const ParmVarDecl *ParamVarRegion::getDecl() const {
  ...
  if (const auto *const Decl *D = getStackFrame()->getDecl(); FD = dyn_cast<FunctionDecl>(D)) {
  ...
  while (LC) {
    ...
    if (const auto *BC = dyn_cast<BlockInvocationContext>(LC)) {
      ...
      for (auto Var : BR->referenced_vars()) {
        ...
        if (const auto *const TypedValueRegion *OrigR = Var.getOriginalRegion(); VR = dyn_cast<VarRegion>(OrigR)) {
          if (VR->getDecl() == VD)
...
const VarRegion *MemRegionManager::getVarRegion(const VarDecl *D,
  ...
  if (const auto *PVD = dyn_cast<ParmVarDecl>(D); PVD) {
    ...
    if (const Stmt *CallSite = SFC->getCallSite(); CallSite) {
      ...
      if (const auto *const Decl *D = SFC->getDecl(); FD = dyn_cast<FunctionDecl>(D)) {
