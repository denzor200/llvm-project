...
ProgramStateRef ExprEngine::getInitialState(const LocationContext *InitLoc) {
  ...
  if (const auto *MD = dyn_cast<ObjCMethodDecl>(D)) {
    ...
    if (SVal V = state->getSVal(loc::MemRegionVal(R)); std::optional<Loc> LV = V.getAs<Loc>()) {
  ...
  if (const auto *MD = dyn_cast<CXXMethodDecl>(D)) {
    if (MD->isImplicitObjectMemberFunction()) {
      ...
      if (const StackFrameContext *SFC = InitLoc->getStackFrame(); SFC->getParent() == nullptr) {
        ...
        if (SVal V = state->getSVal(L); std::optional<Loc> LV = V.getAs<Loc>()) {
          state = state->assume(*LV, true);
...
void ExprEngine::VisitGCCAsmStmt(const GCCAsmStmt *A, ExplodedNode *Pred,
  ...
  for (const Expr *I : A->inputs()) {
    ...
    if (SVal X = state->getSVal(I, Pred->getLocationContext()); std::optional<Loc> LV = X.getAs<Loc>())
      state = state->invalidateRegions(*LV, getCFGElementRef(),
                                       currBldrCtx->blockCount(),
                                       Pred->getLocationContext(),
                                       /*CausedByPointerEscape=*/true);
