...
static SVal processArgument(SVal Value, const Expr *ArgumentExpr,
  ...
  if (QualType ArgumentType = ArgumentExpr->getType(); isTransparentUnion(ParamType) &&
...
ObjCMessageKind ObjCMethodCall::getMessageKind() const {
  if (!Data) {
    ...
    if (const auto *const Stmt *S = PM.getParentIgnoreParenCasts(getOriginExpr()); POE = dyn_cast_or_null<PseudoObjectExpr>(S)) {
...
bool ObjCMethodCall::canBeOverridenInSubclass(ObjCInterfaceDecl *IDecl,
  ...
  if (SourceLocation InterfLoc = IDecl->getEndOfDefinitionLoc(); InterfLoc.isValid() && AMgr.isInCodeFile(InterfLoc))
    return false;
...
CallEventManager::getSimpleCall(const CallExpr *CE, ProgramStateRef State,
  ...
  if (const auto *OpCE = dyn_cast<CXXOperatorCallExpr>(CE)) {
    ...
    if (const auto *const FunctionDecl *DirectCallee = OpCE->getDirectCallee(); MD = dyn_cast<CXXMethodDecl>(DirectCallee)) {
...
CallEventManager::getCaller(const StackFrameContext *CalleeCtx,
  ...
  if (const Stmt *CallSite = CalleeCtx->getCallSite(); CallSite) {
    ...
    if (const auto *SVal ThisVal = State->getSVal(ThisPtr); CE = dyn_cast<CXXConstructExpr>(CallSite))
