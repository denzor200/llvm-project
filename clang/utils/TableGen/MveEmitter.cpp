...
Result::Ptr EmitterBase::getCodeForDag(const DagInit *D,
  ...
  if (const Record *Op = cast<DefInit>(D->getOperator())->getDef(); Op->getName() == "seq") {
    ...
    for (unsigned i = 0, e = D->getNumArgs(); i < e; ++i) {
      ...
      if (StringRef ArgName = D->getArgNameStr(i); !ArgName.empty())
...
Result::Ptr EmitterBase::getCodeForDagArg(const DagInit *D, unsigned ArgNum,
  ...
  if (StringRef Name = D->getArgNameStr(ArgNum); !Name.empty()) {
