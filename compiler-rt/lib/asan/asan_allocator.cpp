...
struct QuarantineCallback {
  ...
  void Recycle(AsanChunk *m) const {
    ...
    if (get_allocator().FromPrimary(p)) {
      ...
      if (u8 old_chunk_state = CHUNK_QUARANTINE; !atomic_compare_exchange_strong(&m->chunk_state, &old_chunk_state,
                                          CHUNK_INVALID,
...
struct Allocator {
  ...
  AsanChunkView FindHeapChunkByAddress(uptr addr) {
    ...
    if (sptr offset = 0; !m1 || AsanChunkView(m1).AddrIsAtLeft(addr, 1, &offset)) {
