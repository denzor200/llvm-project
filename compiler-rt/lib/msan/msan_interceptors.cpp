...
static void *mmap_interceptor(Mmap real_mmap, void *addr, SIZE_T length,
  ...
  if (void *end_addr = (char *)addr + (rounded_length - 1); addr && (!MEM_IS_APP(addr) || !MEM_IS_APP(end_addr))) {
    if (flags & map_fixed) {
      errno = errno_EINVAL;
      return (void *)-1;
    } else {
...
static uptr signal_impl(int signo, uptr cb) {
  ...
  if (flags()->wrap_signals) {
    ...
    if (SpinMutexLock lock(&sigactions_mu); cb != __sanitizer::sig_ign && cb != __sanitizer::sig_dfl) {
      atomic_store(&sigactions[signo], cb, memory_order_relaxed);
