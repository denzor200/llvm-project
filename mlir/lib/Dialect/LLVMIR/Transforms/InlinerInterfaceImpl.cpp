...
static uint64_t tryToEnforceAlignment(Value value, uint64_t requestedAlignment,
  ...
  if (auto Operation *parentOp = value.getParentBlock()->getParentOp(); func = dyn_cast<LLVM::LLVMFuncOp>(parentOp)) {
    // Use the alignment attribute set for this argument in the parent function
    // if it has been set.
    
    if (Attribute auto blockArg = llvm::cast<BlockArgument>(value); alignAttr = func.getArgAttr(
