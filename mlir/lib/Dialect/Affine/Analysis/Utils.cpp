...
void Node::getLoadAndStoreMemrefSet(
  ...
  for (Operation *storeOp : stores) {
    ...
    if (auto memref = cast<AffineWriteOpInterface>(storeOp).getMemRef(); loadMemrefs.count(memref) > 0)
...
LogicalResult MemRefRegion::compute(Operation *op, unsigned loopDepth,
  ...
  for (unsigned i = 0; i < numDims + numSymbols; ++i) {
    ...
    if (auto operand = operands[i]; auto affineFor = getForInductionVarOwner(operand)) {
      ...
      if (Value symbol = operand; auto constVal = getConstantIntValue(symbol))
