...
static bool isUniformDefinition(Value value,
  ...
  if (AffineForOp forOp = getForInductionVarOwner(value); forOp && strategy->loopToVectorDim.count(forOp) == 0)
    return true;
...
LogicalResult mlir::affine::vectorizeAffineLoopNest(
  ...
  if (NestedPatternContext mlContext; failed(verifyLoopNesting(loops)))
    return failure();
