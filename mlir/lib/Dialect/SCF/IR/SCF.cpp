...
std::optional<APInt> ForOp::getConstantStep() {
  ...
  if (IntegerAttr step; matchPattern(getStep(), m_Constant(&step)))
...
LogicalResult scf::WhileOp::verify() {
  ...
  if (auto beforeTerminator = verifyAndGetTerminator<scf::ConditionOp>(
      *this, getBefore(),
      "expects the 'before' region to terminate with 'scf.condition'"); !beforeTerminator)
    return failure();
