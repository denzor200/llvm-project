...
convertMLIRAttributeToLLVM(Location loc, llvm::LLVMContext &ctx, StringRef key,
  ...
  if (llvm::Attribute::isIntAttrKind(kind)) {
    ...
    if (int64_t result; !value.getAsInteger(/*Radix=*/0, result))
...
LogicalResult ModuleTranslation::convertOneFunction(LLVMFuncOp func) {
  ...
  if (func.getPersonality()) {
    ...
    if (llvm::Constant *llvm::Type *ty = llvm::PointerType::getUnqual(llvmFunc->getContext()); pfunc = getLLVMConstant(ty, func.getPersonalityAttr(),
                                                func.getLoc(), *this))
...
void ModuleTranslation::setDisjointFlag(Operation *op, llvm::Value *value) {
  ...
  if (auto auto iface = cast<DisjointFlagInterface>(op); disjointInst = dyn_cast<llvm::PossiblyDisjointInst>(value))
