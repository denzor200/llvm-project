...
void PatternEmitter::emitVariadicOperandMatch(DagNode tree,
  ...
  if (StringRef variadicTreeName = variadicArgTree.getSymbol(); !variadicTreeName.empty()) {
    auto res =
...
void PatternEmitter::emitAttributeMatch(DagNode tree, StringRef castedName,
  ...
  if (auto matcher = tree.getArgAsLeaf(argIndex); !matcher.isUnspecified()) {
...
std::string PatternEmitter::handleLocationDirective(DagNode tree) {
  ...
  if (tree.getNumArgs() == 1) {
    ...
    if (DagLeaf leaf = tree.getArgAsLeaf(0); leaf.isStringAttr())
  ...
  for (int i = 0, e = tree.getNumArgs(); i != e; ++i) {
    ...
    if (DagLeaf leaf = tree.getArgAsLeaf(i); leaf.isStringAttr()) {
      if (!strAttr.empty())
...
std::string PatternEmitter::handleReturnTypeArg(DagNode returnType, int i,
  ...
  if (auto dagLeaf = returnType.getArgAsLeaf(i); dagLeaf.isStringAttr())
...
int PatternEmitter::getNodeValueCount(DagNode node) {
  if (node.isOperation()) {
    ...
    if (auto symbol = node.getSymbol(); !symbol.empty()) {
...
std::string PatternEmitter::handleOpCreation(DagNode tree, int resultIndex,
  ...
  if (auto inPattern = numPatArgs - tail.numDirectives; numOpArgs != inPattern) {
    PrintFatalError(loc,
