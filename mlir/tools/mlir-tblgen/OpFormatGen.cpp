...
static void genElementParserStorage(FormatElement *element, const Operator &op,
  ...
  if (auto *optional = dyn_cast<OptionalElement>(element)) {
    ...
    if (StringRef name = region->getVar()->name; region->getVar()->isVariadic()) {
      body << formatv(
    ...
    if (StringRef name = successor->getVar()->name; successor->getVar()->isVariadic()) {
    ...
    if (StringRef name = getTypeListName(dir->getArg(), lengthKind); lengthKind != ArgumentLengthKind::Single)
...
static void genCustomParameterParser(FormatElement *param, MethodBody &body) {
  if (auto *attr = dyn_cast<AttributeVariable>(param)) {
    ...
    if (ArgumentLengthKind lengthKind = getArgumentLengthKind(operand->getVar()); lengthKind == ArgumentLengthKind::VariadicOfVariadic)
    ...
    if (StringRef listName = getTypeListName(dir->getArg(), lengthKind); lengthKind == ArgumentLengthKind::VariadicOfVariadic)
...
static void genCustomDirectiveParser(CustomDirective *dir, MethodBody &body,
  ...
  for (FormatElement *param : dir->getElements()) {
    if (auto *operand = dyn_cast<OperandVariable>(param)) {
      ...
      if (StringRef listName = getTypeListName(dir->getArg(), lengthKind); lengthKind == ArgumentLengthKind::Optional) {
      ...
      if (auto *FormatElement *input = dir->getArg(); operand = dyn_cast<OperandVariable>(input)) {
        ...
        if (StringRef listName = getTypeListName(type->getArg(), lengthKind); lengthKind == ArgumentLengthKind::Optional) {
...
void OperationFormat::genElementParser(FormatElement *element, MethodBody &body,
  ...
  if (auto *optional = dyn_cast<OptionalElement>(element)) {
    auto genElementParsers = [&](FormatElement *firstElement,
      ...
      if (auto *anchorVar = dyn_cast<AttributeLikeVariable>(optional->getAnchor()); anchorVar && anchorVar != firstElement && anchorVar->isUnit()) {
    ...
    if (StringRef name = operand->getVar()->name; lengthKind == ArgumentLengthKind::VariadicOfVariadic)
...
void OperationFormat::genParserVariadicSegmentResolution(Operator &op,
  ...
  if (!allOperands) {
    if (op.getTrait("::mlir::OpTrait::AttrSizedOperandSegments")) {
      ...
      if (auto interleaveFn = [&](const NamedTypeConstraint &operand) {
  ...
  if (!allResultTypes &&
    ...
    if (auto interleaveFn = [&](const NamedTypeConstraint &result) {
...
static void genCustomDirectiveParameterPrinter(FormatElement *element,
  ...
  if (auto *attr = dyn_cast<AttributeVariable>(element)) {
    ...
    if (std::string name = op.getGetterName(var->name); var->isVariadic())
      body << name << "().getTypes()";
...
static void genOptionalGroupPrinterAnchor(FormatElement *anchor,
  ...
  TypeSwitch<FormatElement *>(anchor)
      .Case<OperandVariable, ResultVariable>([&](auto *element) {
        ...
        if (std::string name = op.getGetterName(var->name); var->isOptional())
          body << name << "()";
...
void OpFormatParser::handleTypesMatchConstraint(
  ...
  if (ConstArgument StringRef transformer = def.getValueAsString("transformer"); arg = findSeenArg(lhsName))
...
void OpFormatParser::handleShapedTypeMatchesElementCountAndTypesConstraint(
  ...
  // types.
  if (ConstArgument StringRef elementsArg = def.getValueAsString("elements"); arg = findSeenArg(shapedArg)) {
