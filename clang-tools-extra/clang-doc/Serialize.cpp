...
static void getTemplateParameters(const TemplateParameterList *TemplateParams,
  ...
  for (unsigned i = 0; i < TemplateParams->size(); ++i) {
    ...
    if (const NamedDecl *Param = TemplateParams->getParam(i); const auto *TTP = llvm::dyn_cast<TemplateTypeParmDecl>(Param)) {
...
static void parseEnumerators(EnumInfo &I, const EnumDecl *D) {
  for (const EnumConstantDecl *E : D->enumerators()) {
    ...
    if (ASTContext &Context = E->getASTContext(); RawComment *Comment =
