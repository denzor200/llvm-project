  ...
  auto AddToGroup = [&](const tooling::Replacement &R,
    ...
    if (auto Entry = SM.getFileManager().getOptionalFileRef(Path)) {
      if (SourceTU) {
        ...
        if (auto [It, Inserted] = DiagReplacements[*Entry].try_emplace(R, SourceTU); !Inserted && It->second != SourceTU)
...
bool deleteReplacementFiles(const TUReplacementFiles &Files,
  ...
  for (const auto &Filename : Files) {
    ...
    if (std::error_code Error = llvm::sys::fs::remove(Filename); Error) {
      Success = false;
