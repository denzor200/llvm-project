...
ConfigOptionsProvider::getRawOptions(llvm::StringRef FileName) {
  ...
  if (ConfigOptions.InheritParentConfig.value_or(false)) {
    ...
    if (llvm::ErrorOr<llvm::SmallString<128>> AbsoluteFilePath =
        getNormalizedAbsolutePath(FileName); AbsoluteFilePath) {
...
FileOptionsBaseProvider::getNormalizedAbsolutePath(llvm::StringRef Path) {
  ...
  if (const std::error_code Err = FS->makeAbsolute(NormalizedAbsolutePath); Err)
    return Err;
