...
truncateIfIntegral(const FloatingLiteral &FloatLiteral) {
  ...
  if (const double Value = FloatLiteral.getValueAsApproximateDouble(); std::fmod(Value, 1) == 0) {
...
rewriteInverseDurationCall(const MatchFinder::MatchResult &Result,
  ...
  if (const auto *const std::pair<llvm::StringRef, llvm::StringRef> &InverseFunctions =
      getDurationInverseForScale(Scale); MaybeCallArg = selectFirst<const Expr>(
          "e",
...
rewriteInverseTimeCall(const MatchFinder::MatchResult &Result,
  ...
  if (const auto *const llvm::StringRef InverseFunction = getTimeInverseForScale(Scale); MaybeCallArg = selectFirst<const Expr>(
