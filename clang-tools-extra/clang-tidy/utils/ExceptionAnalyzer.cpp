...
ExceptionAnalyzer::ExceptionInfo::filterByCatch(const Type *HandlerTy,
  ...
  for (const Type *TypeToDelete : TypesToDelete) {
    ...
    if (const auto DeleteIt = ThrownExceptions.find(TypeToDelete); DeleteIt != ThrownExceptions.end()) {
...
ExceptionAnalyzer::ExceptionInfo::filterIgnoredExceptions(
  ...
  for (const auto &ThrownException : ThrownExceptions) {
    ...
    if (const auto *const Type *T = ThrownException.getFirst(); TD = T->getAsTagDecl()) {
...
ExceptionAnalyzer::throwsException(const Stmt *St,
  ...
  if (const auto *Throw = dyn_cast<CXXThrowExpr>(St)) {
    ...
    for (const auto &Exception : Excs.getExceptions()) {
      ...
      if (const CXXRecordDecl *const Type *ExcType = Exception.getFirst(); ThrowableRec = ExcType->getAsCXXRecordDecl()) {
