...
static bool isRealCharType(const clang::QualType &Ty) {
  ...
  if (const auto *const Type *DesugaredType = Ty->getUnqualifiedDesugaredType(); BT = llvm::dyn_cast<BuiltinType>(DesugaredType))
...
void FormatStringConverter::finalizeFormatText() {
  ...
  if (const auto StandardFormatStringRef = StringRef(StandardFormatString); Config.AllowTrailingNewlineRemoval &&
