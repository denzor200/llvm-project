...
std::vector<LocatedSymbol> findImplementors(llvm::DenseSet<SymbolID> IDs,
  ...
  switch (static constexpr trace::Metric FindImplementorsMetric(
      "find_implementors", trace::Metric::Counter, "case"); Predicate) {
...
class FindControlFlow : public RecursiveASTVisitor<FindControlFlow> {
  ...
  bool filterAndTraverse(DynTypedNode D, const Func &Delegate) {
    ...
    if (auto RestoreIgnore = llvm::make_scope_exit(
        [OldIgnore(Ignore), this] { Ignore = OldIgnore; }); getFunctionBody(D))
      Ignore = All;
  ...
  for (auto Base : CXXRD->bases()) {
    ...
    if (!ParentDecl) {
      ...
      if (const TemplateSpecializationType *TS =
        ...
        if (TemplateDecl *TemplateName TN = TS->getTemplateName(); TD = TN.getAsTemplateDecl()) {
