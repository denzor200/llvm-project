...
struct UpdateIndexCallbacks : public ParsingCallbacks {
  ...
  void onPreambleAST(
    ...
    if (auto auto &CI = ASTCtx.getCompilerInvocation(); Loc = Stdlib->add(CI.getLangOpts(), PP.getHeaderSearchInfo()))
  ...
  auto Action = [File = File.str(), Sel, TweakID = TweakID.str(),
    ...
    if (*Effect && (*Effect)->FormatEdits) {
      ...
      for (auto &It : (*Effect)->ApplyEdits) {
        ...
        if (llvm::Error format::FormatStyle Style =
            getFormatStyleForFile(File, E.InitialCode, TFS, false); Err = reformatEdit(E, Style))
