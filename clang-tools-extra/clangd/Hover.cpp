...
std::optional<HoverInfo> getHover(ParsedAST &AST, Position Pos,
  ...
  if (!HI) {
    ...
    if (SelectionTree ST =
        SelectionTree::createRight(AST.getASTContext(), TB, Offset, Offset); const SelectionTree::Node *N = ST.commonAncestor()) {
      // FIXME: Fill in HighlightRange with range coming from N->ASTNode.
      
      if (auto Decls = explicitReferenceTargets(N->ASTNode, DeclRelation::Alias,
                                            AST.getHeuristicResolver()); const auto *DeclToUse = pickDeclToUse(Decls)) {
  ...
  if (!HI->Definition.empty()) {
    ...
    if (auto Replacements = format::reformat(
        Style, HI->Definition, tooling::Range(0, HI->Definition.size())); auto Formatted =
