...
void f2() {
  ...
  std::shared_ptr<D> sinc;
  sinc.reset(dynamic_cast<D *>(poly2.get()));
  // CHECK-MESSAGES: :[[@LINE-1]]:14: warning: passing a raw pointer 'D *' to 'std::shared_ptr<D>' reset method may cause double deletion
}
...
struct S1 {
  ...
  std::shared_ptr<S1> g_reset() { std::shared_ptr<S1> sp; sp.reset(this); return sp; }
  // CHECK-MESSAGES: :[[@LINE-1]]:68: warning: passing a raw pointer 'S1 *' to 'std::shared_ptr<S1>' reset method may cause double deletion
};
