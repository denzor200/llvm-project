...
BinaryFunction::processIndirectBranch(MCInst &Instruction, unsigned Size,
  ...
  if (BC.isAArch64()) {
    ...
    for (const uint32_t Offset :
      ...
      if (auto II = Instructions.find(Offset); II != Instructions.end()) {
        Begin = II;
        break;
      }
    }
...
bool BinaryFunction::isZeroPaddingAt(uint64_t Offset) const {
  ...
  if (Islands) {
    ...
    if (auto Iter = Islands->DataOffsets.upper_bound(Offset); Iter != Islands->DataOffsets.end())
      EndOfCode = *Iter;
  }
...
void BinaryFunction::handleIndirectBranch(MCInst &Instruction, uint64_t Size,
  ...
  switch (IndirectBranchType Result =
      processIndirectBranch(Instruction, Size, Offset, IndirectTarget); Result) {
  default:
...
Error BinaryFunction::disassemble() {
  ...
  for (uint64_t Offset = 0; Offset < getSize(); Offset += Size) {
    ...
    if (MIB->isBranch(Instruction) || MIB->isCall(Instruction)) {
      ...
      if (uint64_t TargetAddress = 0; MIB->evaluateBranch(Instruction, AbsoluteInstrAddr, Size,
                              TargetAddress)) {
...
void BinaryFunction::postProcessJumpTables() {
  ...
  for (auto &JTSite : JTSites) {
    ...
    while (EntryOffset < JT->getSize()) {
      ...
      if (uint64_t TargetOffset = EntryAddress - getAddress(); TargetOffset < getSize()) {
  ...
  for (BinaryBasicBlock &BB : blocks()) {
    for (BinaryBasicBlock::iterator II = BB.begin(); II != BB.end(); ++II) {
      ...
      if (BC.MIB->isTailCall(Instr) || BC.MIB->getJumpTable(Instr)) {
        ...
        if (IndirectBranchType Type = BC.MIB->analyzeIndirectBranch(
            Instr, BB.begin(), II, PtrSize, MemLocInstr, BaseRegNum,
            IndexRegNum, DispValue, DispExpr, PCRelBaseInstr,
            FixedEntryLoadInstr); Type != IndirectBranchType::UNKNOWN || MemLocInstr != nullptr)
          continue;
...
void BinaryFunction::postProcessBranches() {
  ...
  for (BinaryBasicBlock &BB : blocks()) {
    ...
    if (auto LastInstrRI = BB.getLastNonPseudo(); BB.succ_size() == 1) {
...
BinaryFunction::BasicBlockListType BinaryFunction::dfs() const {
  ...
  while (!Stack.empty()) {
    ...
    if (MCInst *UncondBranch = nullptr; BB->analyzeBranch(TBB, FBB, CondBranch, UncondBranch) && CondBranch &&
        BB->succ_size() == 2) {
  ...
  for (BinaryBasicBlock *&BB : BasicBlocks) {
    for (MCInst &Inst : *BB) {
      ...
      if (!IndJmpMatcher->match(
        ...
        if (std::unique_ptr<MCPlusBuilder::MCInstMatcher> IndJmpMatcher2 =
            BC.MIB->matchIndJmp(std::move(LoadMatcherOwner)); !IndJmpMatcher2->match(
                *BC.MRI, *BC.MIB,
                MutableArrayRef<MCInst>(&*BB->begin(), &Inst + 1), -1) ||
            BaseReg1 != BC.MIB->getNoRegister() || Scale != 8) {
