...
bool BinaryContext::analyzeJumpTable(const uint64_t Address,
  ...
  if (const BinaryData *JumpTableBD = getBinaryDataAtAddress(Address); JumpTableBD && JumpTableBD->getSize()) {
...
BinaryFunction *BinaryContext::getBinaryFunctionContainingAddress(
  ...
  if (const uint64_t UsedSize =
      UseMaxSize ? FI->second.getMaxSize() : FI->second.getSize(); Address >= FI->first + UsedSize + (CheckPastEnd ? 1 : 0))
    return nullptr;
