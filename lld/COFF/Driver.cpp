...
void LinkerDriver::linkerMain(ArrayRef<const char *> argsArr) {
  ...
  if (auto *arg = args.getLastArg(OPT_errorlimit)) {
    ...
    if (StringRef s = arg->getValue(); s.getAsInteger(10, n))
  ...
  // Handle /linkrepro and /reproduce.
  {
    ...
    if (std::optional<std::string> llvm::TimeTraceScope timeScope2("Reproducer"); path = getReproduceFile(args)) {
  ...
  // Handle /machine
  {
    ...
    if (auto *llvm::TimeTraceScope timeScope2("Machine arg"); arg = args.getLastArg(OPT_machine)) {
  ...
  ctx.forEachActiveSymtab([&](SymbolTable &symtab) {
    ...
    if (auto *llvm::TimeTraceScope timeScope("Entry point"); arg = args.getLastArg(OPT_entry)) {
      if (!arg->getValue()[0])
        Fatal(ctx) << "missing entry point symbol name";
      symtab.entry = symtab.addGCRoot(symtab.mangle(arg->getValue()), true);
  ...
  if (config->callGraphProfileSort) {
    ...
    if (auto *llvm::TimeTraceScope timeScope("Call graph"); arg = args.getLastArg(OPT_call_graph_ordering_file))
