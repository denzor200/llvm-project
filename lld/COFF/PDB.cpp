...
void PDBLinker::addDebug(TpiSource *source) {
  ...
  if (!ctx.config.debugGHashes) {
    ...
    if (Error ScopedTimer t(ctx.typeMergingTimer); e = source->mergeDebugT(&tMerger)) {
  ...
  if (Error typeError = std::move(source->typeMergingError); typeError) {
...
void PDBLinker::addObjectsToPDB() {
  ...
  // Construct TPI and IPI stream contents.
  {
    ...
    if (ScopedTimer t2(ctx.tpiStreamLayoutTimer); ctx.config.debugGHashes) {
...
void PDBLinker::addPublicsToPDB() {
  ...
  ctx.symtab.forEachSymbol([&publics, this](Symbol *s) {
    ...
    if (auto *def = dyn_cast<Defined>(s); def && def->isLive() && def->getChunk()) {
      ...
      if (StringRef name = def->getName(); name.data()[0] == '_' && name.data()[1] == '_') {
