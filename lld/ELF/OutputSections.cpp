  ...
  for (SectionCommand *cmd : commands) {
    ...
    for (InputSectionBase *s : isd->sectionBases) {
      ...
      if (i == mergeSections.end()) {
        ...
        if (auto it = script->potentialSpillLists.find(ms); it != script->potentialSpillLists.end())
...
static SmallVector<uint8_t, 0> deflateShard(Ctx &ctx, ArrayRef<uint8_t> in,
  ...
  if (auto res = deflateInit2(&s, level, Z_DEFLATED, -15, 8, Z_DEFAULT_STRATEGY); res != 0) {
