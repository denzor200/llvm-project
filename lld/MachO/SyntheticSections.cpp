...
void ObjCSelRefsHelper::initialize() {
  ...
  for (ConcatInputSection *isec : inputSections) {
    ...
    if (const auto *auto Reloc = isec->relocs[0]; sym = Reloc.referent.dyn_cast<Symbol *>()) {
...
void ObjCStubsSection::addEntry(Symbol *sym) {
  ...
  if (StringRef methname = getMethname(sym); !ObjCSelRefsHelper::getSelRef(methname))
...
void SymtabSection::emitObjectFileStab(ObjFile *file) {
  ...
  if (std::error_code ec = sys::fs::make_absolute(path); ec)
...
void InitOffsetsSection::setUp() {
  for (const ConcatInputSection *isec : sections) {
    for (const Reloc &rel : isec->relocs) {
      ...
      if (RelocAttrs attrs = target->getRelocAttrs(rel.type); !attrs.hasAttr(RelocAttrBits::UNSIGNED))
...
void ObjCMethListSection::setUp() {
  for (const ConcatInputSection *isec : inputs) {
    ...
    while (methodNameOff < isec->data.size()) {
      ...
      if (StringRef methname = reloc->getReferentString(); !ObjCSelRefsHelper::getSelRef(methname))
        ObjCSelRefsHelper::makeSelRef(methname);
