...
void ObjFile::parseSections(ArrayRef<SectionHeader> sectionHeaders) {
  ...
  for (const SectionHeader &sec : sectionHeaders) {
    ...
    if (sectionType(sec.flags) == S_CSTRING_LITERALS) {
      ...
      if (auto *isec = make<ConcatInputSection>(section, data, align); isDebugSection(isec->getFlags()) &&
...
macho::Symbol *ObjFile::parseNonSectionSymbol(const NList &sym,
  ...
  switch (bool isPrivateExtern = sym.n_type & N_PEXT || forceHidden; type) {
  case N_UNDF:
...
ArrayRef<uint8_t> ObjFile::getOptimizationHints() const {
  ...
  if (auto *const auto *buf = reinterpret_cast<const uint8_t *>(mb.getBufferStart()); cmd =
...
void DylibFile::parseReexports(const InterfaceFile &interface) {
  ...
  for (const InterfaceFileRef &intfRef : interface.reexportedLibraries()) {
    ...
    if (InterfaceFile::const_target_range targets = intfRef.targets(); is_contained(skipPlatformChecks, intfRef.getInstallName()) ||
...
void DylibFile::handleLDPreviousSymbol(StringRef name, StringRef originalName) {
  ...
  if (unsigned platform; platformStr.getAsInteger(10, platform) ||
...
void DylibFile::handleLDInstallNameSymbol(StringRef name,
  ...
  if (VersionTuple version; !condition.consume_front("os") || version.tryParse(condition))
...
void ArchiveFile::addLazySymbols() {
  ...
  if (!err) {
    ...
    if (Expected<MemoryBufferRef> mbOrErr = child->getMemoryBufferRef(); !mbOrErr) {
...
void ArchiveFile::fetch(const object::Archive::Symbol &sym) {
  ...
  if (Error const object::Archive::Symbol symCopy = sym; e = fetch(c, symCopy.getName()))
