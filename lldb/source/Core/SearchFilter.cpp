...
SearchFilter::DoModuleIteration(const SymbolContext &context,
  ...
  for (ModuleSP module_sp : m_target_sp->GetImages().Modules()) {
    ...
    if (searcher.GetDepth() == lldb::eSearchDepthModule) {
      ...
      if (Searcher::CallbackReturn shouldContinue =
          searcher.SearchCallback(*this, matchingContext, nullptr); shouldContinue == Searcher::eCallbackReturnStop ||
          shouldContinue == Searcher::eCallbackReturnPop)
        return shouldContinue;
    } else {
      
      if (Searcher::CallbackReturn shouldContinue =
          DoCUIteration(module_sp, context, searcher); shouldContinue == Searcher::eCallbackReturnStop)
        return shouldContinue;
