...
bool Debugger::LoadPlugin(const FileSpec &spec, Status &error) {
  if (g_load_plugin_callback) {
    ...
    if (llvm::sys::DynamicLibrary dynlib =
        g_load_plugin_callback(shared_from_this(), spec, error); dynlib.isValid()) {
        ...
        [this](const FileSpec &dir) -> llvm::Error {
          for (auto &entry : m_stream_handlers) {
            ...
            if (std::error_code ec =
                llvm::sys::fs::copy_file(log_path, destination.GetPath()); ec)
...
void Debugger::HandleBreakpointEvent(const EventSP &event_sp) {
  ...
  if (const uint32_t event_type =
    ...
    if (uint32_t num_new_locations =
      ...
      if (BreakpointSP breakpoint =
          Breakpoint::BreakpointEventData::GetBreakpointFromEvent(event_sp); StreamUP output_up = GetAsyncOutputStream()) {
...
bool Debugger::StartIOHandlerThread() {
  if (!m_io_handler_thread.IsJoinable()) {
    // Use larger 8MB stack for this thread
    if (llvm::Expected<HostThread> io_handler_thread = ThreadLauncher::LaunchThread(
        "lldb.debugger.io-handler", [this] { return IOHandlerThread(); },
        8 * 1024 * 1024); io_handler_thread) {
