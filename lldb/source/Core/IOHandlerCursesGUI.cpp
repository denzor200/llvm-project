...
public:
  ...
  void Kill(Window &window) {
    ...
    if (Status destroy_status(m_process->Destroy(false)); destroy_status.Fail()) {
  ...
  void Detach(Window &window) {
    ...
    if (Status detach_status(m_process->Detach(m_keep_stopped_field->GetBoolean())); detach_status.Fail()) {
...
public:
  ...
  TargetSP GetTarget() {
    ...
    if (Status status = m_debugger.GetTargetList().CreateTarget(
        m_debugger, m_executable_field->GetPath(),
        m_arch_field->GetArchString(), GetLoadDependentFiles(),
        &platform_options, target_sp); status.Fail()) {
      SetError(status.AsCString());
      return nullptr;
    }
...
public:
  ...
  void GetExecutableSettings(ProcessLaunchInfo &launch_info) {
    ...
    if (llvm::StringRef target_settings_argv0 = target->GetArg0(); !target_settings_argv0.empty()) {
...
#endif
      if (int ch = m_window_sp->GetChar(); ch == -1) {
...
public:
  ...
  bool WindowDelegateDraw(Window &window, bool force) override {
    ...
    if (ValueObjectList value_list; frame) {
  ...
  bool WindowDelegateDraw(Window &window, bool force) override {
    ...
    if (process) {
      ...
      if (StateIsStoppedState(state, true)) {
        ...
        if (StreamString strm; thread && FormatEntity::Format(m_format, strm, nullptr, &exe_ctx,
