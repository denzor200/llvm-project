...
bool Section::SetFileAddress(lldb::addr_t file_addr) {
  ...
  if (SectionSP parent_sp(GetParent()); parent_sp) {
...
SectionList SectionList::Merge(SectionList &lhs, SectionList &rhs,
  ...
  for (const auto &lhs_section : lhs) {
    ...
    if (auto rhs_section = rhs.FindSectionByName(lhs_section->GetName()); rhs_section)
  ...
  for (const auto &rhs_section : rhs) {
    ...
    if (auto lhs_section = lhs.FindSectionByName(rhs_section->GetName()); !lhs_section)
