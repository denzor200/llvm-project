      ...
      if (const Symbol *trampoline_symbol =
        ...
        if (const Symbol *changed_symbol =
          ...
          if (changed_addr != LLDB_INVALID_ADDRESS) {
            ...
            if (BreakpointSP trampolines_changed_bp_sp =
                target.CreateBreakpoint(changed_addr, true, false); trampolines_changed_bp_sp) {
...
AppleObjCTrampolineHandler::SetupDispatchFunction(Thread &thread,
  ...
  // Scope for mutex locker:
  {
    ...
    if (std::lock_guard<std::mutex> guard(m_impl_function_mutex); !m_impl_code) {
  ...
  if (DiagnosticManager diagnostics; !impl_function_caller->WriteFunctionArguments(
...
AppleObjCTrampolineHandler::GetStepThroughDispatchPlan(Thread &thread,
  ...
  if (!this_dispatch && m_vtables_up) {
    ...
    if (uint32_t flags; m_vtables_up->IsAddressInVTables(curr_pc, flags)) {
