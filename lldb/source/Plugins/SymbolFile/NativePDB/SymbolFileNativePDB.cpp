...
void SymbolFileNativePDB::InitializeObject() {
  ...
  if (auto ts_or_err = m_objfile_sp->GetModule()->GetTypeSystemForLanguage(
      lldb::eLanguageTypeC_plus_plus); auto err = ts_or_err.takeError()) {
...
TypeSP SymbolFileNativePDB::CreateAndCacheType(PdbTypeSymId type_id) {
  ...
  if (IsForwardRefUdt(type_id, m_index->tpi())) {
    ...
    if (auto expected_full_ti =
      ...
      if (auto full_iter = m_types.find(toOpaqueUid(*full_decl_uid)); full_iter != m_types.end()) {
...
size_t SymbolFileNativePDB::ParseTypes(CompileUnit &comp_unit) {
  ...
  for (const uint32_t gid : m_index->globals().getGlobalsTable()) {
    ...
    if (IsTagRecord(PdbTypeSymId{udt.Type, false}, m_index->tpi())) {
      ...
      if (llvm::StringRef name = CVTagRecord::create(cvt).name(); name == udt.Name)
        is_typedef = false;
    }
