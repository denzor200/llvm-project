...
uint32_t SymbolFileSymtab::CalculateAbilities() {
  ...
  if (m_objfile_sp) {
    ...
    if (const Symtab *symtab = m_objfile_sp->GetSymtab(); symtab) {
      ...
      if (lldb_private::Symtab::IndexCollection objc_class_indexes; symtab->AppendSymbolIndexesWithType(eSymbolTypeObjCClass,
                                              objc_class_indexes)) {
    ...
    if (!m_code_indexes.empty()) {
      ...
      for (idx = 0; idx < num_indexes; ++idx) {
        ...
        if (curr_symbol) {
          ...
          if (AddressRange func_range(curr_symbol->GetAddress(), 0); func_range.GetBaseAddress().IsSectionOffset()) {
