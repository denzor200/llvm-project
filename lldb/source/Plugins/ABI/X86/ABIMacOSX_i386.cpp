...
static bool ReadIntegerArgument(Scalar &scalar, unsigned int bit_width,
  ...
  if (Status error; process->ReadScalarIntegerFromMemory(current_stack_argument, byte_size,
  ...
  if (compiler_type.IsIntegerOrEnumerationType(is_signed) ||
    ...
    if (num_bytes <= 8) {
      ...
      if (const RegisterInfo *eax_info = reg_ctx->GetRegisterInfoByName("eax", 0); num_bytes <= 4) {
        ...
        if (uint32_t raw_value = data.GetMaxU32(&offset, 4); reg_ctx->WriteRegisterFromUnsigned(eax_info, raw_value)) {
