...
void ClangExpressionDeclMap::DidParse() {
  if (m_parser_vars && m_parser_vars->m_persistent_vars) {
    ...
    for (size_t pvar_index = 0,
      ...
      if (ExpressionVariableSP pvar_sp(
          m_parser_vars->m_persistent_vars->GetVariableAtIndex(pvar_index)); ClangExpressionVariable *clang_var =
...
bool ClangExpressionDeclMap::LookupLocalVariable(
  ...
  if (!variable_found) {
    ...
    if (auto find_capture = [](ConstString varname,
...
void ClangExpressionDeclMap::AddOneFunction(NameSearchContext &context,
  ...
  if (function) {
    ...
    if (!extern_c) {
      ...
      if (TypeSystem *type_system = function->GetDeclContext().GetTypeSystem(); llvm::isa<TypeSystemClang>(type_system)) {
