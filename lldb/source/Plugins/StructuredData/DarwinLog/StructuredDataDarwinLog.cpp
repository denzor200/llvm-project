...
private:
  static FilterRuleSP CreateOperation(bool accept, size_t attribute_index,
    ...
    if (llvm::Error auto regex = RegularExpression(op_arg); err = regex.GetError()) {
...
bool RunEnableCommand(CommandInterpreter &interpreter) {
  ...
  if (auto enable_options = GetGlobalProperties().GetAutoEnableOptions(); !enable_options.empty()) {
    command_stream << ' ';
...
void StructuredDataDarwinLog::ModulesDidLoad(Process &process,
  ...
  // If we already added the breakpoint, we've got nothing left to do.
  {
    ...
    if (std::lock_guard<std::mutex> locker(m_added_breakpoint_mutex); m_added_breakpoint) {
      LLDB_LOGF(log,
