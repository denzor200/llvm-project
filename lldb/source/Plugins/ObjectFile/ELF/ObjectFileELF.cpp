  ...
  for (i = 0; i < num_symbols; ++i) {
    ...
    if (arch.IsValid() && (arch.GetMachine() == llvm::Triple::aarch64)) {
      ...
      if (symbol_type == eSymbolTypeCode) {
        ...
        if (llvm::StringRef trampoline_name = mangled.GetName().GetStringRef(); trampoline_name.starts_with("__AArch64ADRPThunk_") ||
...
void ObjectFileELF::ParseSymtab(Symtab &lldb_symtab) {
  ...
  if (symbol) {
    ...
    if (Section *reloc_section =
        section_list->FindSectionContainingFileAddress(addr).get(); reloc_section) {
...
DataExtractor ObjectFileELF::GetSegmentData(const ELFProgramHeader &H) {
  ...
  if (IsInMemory()) {
    ...
    if (ProcessSP process_sp = m_process_wp.lock()) {
      ...
      if (const addr_t data_addr = H.p_vaddr + load_bias; DataBufferSP data_sp = ReadMemory(process_sp, data_addr, H.p_memsz))
...
ObjectFileELF::ReadDataFromDynamic(const ELFDynamic *dyn, uint64_t length,
  ...
  if (const lldb::addr_t d_ptr_addr = dyn->d_ptr + offset; ProcessSP process_sp = m_process_wp.lock()) {
...
std::optional<uint32_t> ObjectFileELF::GetNumSymbolsFromDynamicGnuHash() {
  ...
  if (auto data = ReadDataFromDynamic(gnu_hash, sizeof(DtGnuHashHeader))) {
    ...
    if (std::vector<uint32_t> buckets; auto bucket_data = ReadDataFromDynamic(gnu_hash, header.nbuckets * 4,
                                               buckets_offset)) {
      offset = 0;
