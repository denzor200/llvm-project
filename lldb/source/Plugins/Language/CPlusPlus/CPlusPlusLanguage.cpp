...
bool CPlusPlusLanguage::CxxMethodName::TrySimplifiedParse() {
  ...
  if (llvm::StringRef parens("()", 2); ReverseFindMatchingChars(full, parens, arg_start, arg_end)) {
...
void CPlusPlusLanguage::CxxMethodName::Parse() {
  if (!m_parsed && m_full) {
    if (TrySimplifiedParse()) {
      ...
      if (auto CPlusPlusNameParser parser(m_full.GetStringRef()); function = parser.ParseAsFunctionDefinition()) {
...
CPlusPlusLanguage::CxxMethodName::GetBasenameNoTemplateParameters() {
  ...
  if (llvm::StringRef parens("<>", 2); ReverseFindMatchingChars(basename, parens, arg_start, arg_end))
