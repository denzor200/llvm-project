...
lldb::ProcessSP ProcessElfCore::CreateInstance(lldb::TargetSP target_sp,
  ...
  if (crash_file && !can_connect) {
    ...
    if (data_sp && data_sp->GetByteSize() == header_size &&
      ...
      if (lldb::offset_t data_offset = 0; elf_header.Parse(data, &data_offset)) {
...
bool ProcessElfCore::CanDebug(lldb::TargetSP target_sp,
  ...
  if (!m_core_module_sp && FileSystem::Instance().Exists(m_core_file)) {
    ...
    if (Status error(ModuleList::GetSharedModule(core_module_spec, m_core_module_sp,
                                             nullptr, nullptr)); m_core_module_sp) {
...
Status ProcessElfCore::DoLoadCore() {
  ...
  for (const elf::ELFProgramHeader &H : segments) {
    ...
    if (DataExtractor data = core->GetSegmentData(H); H.p_type == llvm::ELF::PT_NOTE) {
...
void ProcessElfCore::UpdateBuildIdForNTFileEntries() {
  ...
  for (NT_FILE_Entry &entry : m_nt_file_entries) {
    ...
    if (UUID uuid = FindBuidIdInCoreMemory(entry.start); uuid.IsValid()) {
...
lldb::addr_t ProcessElfCore::GetImageInfoAddress() {
  ...
  if (Address addr = obj_file->GetImageInfoAddress(&GetTarget()); addr.IsValid())
...
llvm::Error ProcessElfCore::parseNetBSDNotes(llvm::ArrayRef<CoreNote> notes) {
  ...
  for (const auto &note : notes) {
    ...
    if (llvm::StringRef name = note.info.n_name; name == "NetBSD-CORE") {
      if (note.info.n_type == NETBSD::NT_PROCINFO) {
        
        if (llvm::Error error = ParseNetBSDProcInfo(note.data, nlwps, signo,
                                                siglwp, pr_pid); error)
          return error;
        SetID(pr_pid);
...
llvm::Error ProcessElfCore::parseLinuxNotes(llvm::ArrayRef<CoreNote> notes) {
  ...
  for (const auto &note : notes) {
    ...
    case ELF::NT_PRSTATUS: {
      ...
      if (Status status = prstatus.Parse(note.data, arch); status.Fail())
    ...
    case ELF::NT_PRPSINFO: {
      ...
      if (Status status = prpsinfo.Parse(note.data, arch); status.Fail())
        return status.ToError();
