...
llvm::Expected<uint32_t> DynamicRegisterInfo::ByteOffsetFromRegInfoDict(
  ...
  if (uint32_t byte_offset; reg_info_dict.GetValueForKeyAsInteger("offset", byte_offset))
    return byte_offset;
...
DynamicRegisterInfo::SetRegisterInfo(const StructuredData::Dictionary &dict,
  ...
  if (StructuredData::Array *sets = nullptr; dict.GetValueForKeyAsArray("sets", sets)) {
