...
void ThreadPlanTracer::Log() {
  ...
  if (StreamSP bool show_fullpaths = false; stream_sp = GetLogStreamSP()) {
  ...
  if (!m_intptr_type.IsValid()) {
    if (auto target_sp = m_process.CalculateTarget()) {
      ...
      if (auto auto type_system_or_err =
          target_sp->GetScratchTypeSystemForLanguage(eLanguageTypeC); err = type_system_or_err.takeError()) {
        LLDB_LOG_ERROR(
