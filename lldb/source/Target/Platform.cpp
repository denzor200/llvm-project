...
void Platform::GetStatus(Stream &strm) {
  ...
  if (ArchSpec arch(GetSystemArchitecture()); arch.IsValid()) {
...
Status Platform::ResolveExecutable(const ModuleSpec &module_spec,
  ...
  if (resolved_module_spec.GetArchitecture().IsValid() ||
    ...
    if (Status error = ModuleList::GetSharedModule(resolved_module_spec,
                                               exe_module_sp, nullptr, nullptr); exe_module_sp && exe_module_sp->GetObjectFile())
      return error;
    exe_module_sp.reset();
  }
...
const std::vector<ConstString> &Platform::GetTrapHandlerSymbolNames() {
  if (!m_calculated_trap_handlers) {
    ...
    if (std::lock_guard<std::mutex> guard(m_mutex); !m_calculated_trap_handlers) {
