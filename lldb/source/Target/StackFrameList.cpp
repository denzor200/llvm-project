...
uint32_t StackFrameList::GetCurrentInlinedDepth() {
  ...
  if (std::lock_guard<std::mutex> guard(m_inlined_depth_mutex); m_show_inlined_frames && m_current_inlined_pc != LLDB_INVALID_ADDRESS) {
...
bool StackFrameList::FetchFramesUpTo(uint32_t end_idx,
  ...
  if (uint32_t inlined_depth = 0; end_idx > 0 && end_idx != UINT32_MAX) {
...
StackFrameSP StackFrameList::GetFrameAtIndex(uint32_t idx) {
  ...
  { // Now we're accessing m_frames as a reader, so acquire the reader lock.
    ...
    if (std::shared_lock<std::shared_mutex> guard(m_list_mutex); idx < m_frames.size()) {
      frame_sp = m_frames[idx];
