...
bool ThreadPlanStepInRange::ShouldStop(Event *event_ptr) {
  ...
  if (m_virtual_step == eLazyBoolYes) {
    ...
    if (!m_sub_plan_sp && frame_order == eFrameCompareYounger &&
      ...
      if (lldb::StackFrameSP curr_frame = thread.GetStackFrameAtIndex(0); curr_frame) {
...
bool ThreadPlanStepInRange::FrameMatchesAvoidCriteria() {
  ...
  if (size_t num_libraries = libraries_to_avoid.GetSize(); num_libraries > 0) {
    ...
    if (FileSpec frame_library(sc.module_sp->GetFileSpec()); frame_library) {
  ...
  if (avoid_regexp_to_use != nullptr) {
    ...
    if (SymbolContext sc = frame->GetSymbolContext(
        eSymbolContextFunction | eSymbolContextBlock | eSymbolContextSymbol); sc.symbol != nullptr) {
...
bool ThreadPlanStepInRange::DoPlanExplainsStop(Event *event_ptr) {
  ...
  if (m_virtual_step == eLazyBoolYes) {
    ...
    if (StopInfoSP stop_info_sp = GetPrivateStopInfo(); stop_info_sp) {
