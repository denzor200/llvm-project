...
CompilerType ValueObjectRegister::GetCompilerTypeImpl() {
  if (!m_compiler_type.IsValid()) {
    ...
    if (ExecutionContext exe_ctx(GetExecutionContextRef()); auto *target = exe_ctx.GetTargetPtr()) {
      if (auto *exe_module = target->GetExecutableModulePointer()) {
        
        if (auto type_system_or_err =
            exe_module->GetTypeSystemForLanguage(eLanguageTypeC); auto err = type_system_or_err.takeError()) {
