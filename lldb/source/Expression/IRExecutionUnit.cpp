...
void IRExecutionUnit::GetRunnableInfo(Status &error, lldb::addr_t &func_addr,
  ...
  if (log) {
    ...
    if (Status err = DisassembleFunction(disassembly_stream, process_sp); !err.Success()) {
...
ArchSpec IRExecutionUnit::GetArchitecture() {
  ...
  if(Target *ExecutionContext exe_ctx(GetBestExecutionContextScope()); target = exe_ctx.GetTargetPtr())
