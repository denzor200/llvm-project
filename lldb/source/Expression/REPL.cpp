  ...
  if (bool did_quit = false; code.empty()) {
    ...
    if (code[0] == ':') {
      ...
      if (!llvm::StringRef(code).trim().empty()) {
        ...
        if (result.GetStatus() == lldb::eReturnStatusQuit) {
          ...
          if (debugger.CheckTopIOHandlerTypes(
            ...
            if (lldb::IOHandlerSP io_handler_sp(ci.GetIOHandler()); io_handler_sp)
      ...
      if (code[0] == '<') {
        ...
        if (auto path = llvm::StringRef(code.substr(1)).trim().str(); !ReadCode(path, code, error_sp)) {
          return;
        }
      }
      ...
      if (llvm::Error err = OnExpressionEvaluated(exe_ctx, code, expr_options,
        ...
        *error_sp << llvm::toString(std::move(err)) << "\n";
        ...
        if (result_valobj_sp) {
          ...
          if (lldb::Format format = m_format_options.GetFormat(); result_valobj_sp->GetError().Success()) {
