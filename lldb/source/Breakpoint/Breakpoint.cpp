...
bool Breakpoint::SerializedBreakpointMatchesNames(
  ...
  for (size_t i = 0; i < num_names; i++) {
    ...
    if (std::optional<llvm::StringRef> maybe_name =
        names_array->GetItemAtIndexAsString(i); maybe_name && llvm::is_contained(names, *maybe_name))
      return true;
  }
  return false;
}
    ...
    for (ModuleSP module_sp : module_list.Modules()) {
      if (m_filter_sp->ModulePasses(module_sp)) {
        ...
        for (loc_idx = 0; loc_idx < num_locations; loc_idx++) {
          ...
          if (SectionSP section_sp(break_loc_sp->GetAddress().GetSection()); section_sp && section_sp->GetModule() == module_sp) {
...
void Breakpoint::ModuleReplaced(ModuleSP old_module_sp,
  ...
  for (BreakpointLocationSP break_loc_sp : m_locations.BreakpointLocations()) {
    ...
    if (SectionSP section_sp = break_loc_sp->GetAddress().GetSection(); section_sp && section_sp->GetModule() == old_module_sp) {
