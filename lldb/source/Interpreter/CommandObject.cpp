...
bool CommandObject::CheckRequirements(CommandReturnObject &result) {
  ...
  if (GetFlags().AnySet(eCommandProcessMustBeLaunched |
    ...
    if (Process *process = m_interpreter.GetExecutionContext().GetProcessPtr(); process == nullptr) {
...
void CommandObject::GetFormattedCommandArguments(Stream &str,
  ...
  for (int i = 0; i < num_args; ++i) {
    ...
    if (int num_alternatives = arg_entry.size(); (num_alternatives == 2) && IsPairType(arg_entry[0].arg_repetition)) {
