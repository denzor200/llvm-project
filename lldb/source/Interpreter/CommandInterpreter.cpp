...
void CommandInterpreter::ResolveCommand(const char *command_line,
  ...
  if (std::string command = command_line; ResolveCommandImpl(command, result) != nullptr) {
...
static void GetHomeREPLInitFile(FileSpec &init_file, LanguageType language) {
  if (language == eLanguageTypeUnknown) {
    ...
    if (LanguageSet repl_languages = Language::GetLanguagesSupportingREPLs(); auto main_repl_language = repl_languages.GetSingularLanguage())
...
void CommandInterpreter::IOHandlerInputComplete(IOHandler &io_handler,
  ...
  if ((result.Succeeded() &&
    ...
    if (auto DefaultPrintCallback = [&](const CommandReturnObject &result) {
