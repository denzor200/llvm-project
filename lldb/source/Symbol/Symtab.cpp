  ...
  if (!m_name_indexes_computed) {
    ...
    for (uint32_t value = 0; value < num_symbols; ++value) {
      ...
      if (ConstString name = mangled.GetMangledName()) {
        ...
        if (const SymbolType type = symbol->GetType(); type == eSymbolTypeCode || type == eSymbolTypeResolver) {
...
void Symtab::RegisterBacklogEntry(
  ...
  if (auto it = class_contexts.find(decl_context); it != class_contexts.end()) {
    method_to_index.Append(entry);
  } else {
...
Symbol *Symtab::FindFirstSymbolWithNameAndType(ConstString name,
  ...
  if (name) {
    ...
    // The string table did have a string that matched, but we need to check
    // the symbols and match the symbol_type if any was given.
    if (std::vector<uint32_t> matching_indexes; AppendSymbolIndexesWithNameAndType(name, symbol_type, symbol_debug_type,
                                           symbol_visibility,
                                           matching_indexes)) {
...
static void AddSectionsToRangeMap(SectionList *sectlist,
  ...
  for (int i = 0; i < num_sections; i++) {
    ...
    if (SectionSP sect_sp = sectlist->GetSectionAtIndex(i); sect_sp) {
...
bool DecodeCStrMap(const DataExtractor &data, lldb::offset_t *offset_ptr,
  ...
  if (llvm::StringRef identifier((const char *)data.GetData(offset_ptr, 4), 4); identifier != kIdentifierCStrMap)
    return false;
...
bool Symtab::Encode(DataEncoder &encoder) const {
  ...
  if (CacheSignature signature(m_objfile); !signature.Encode(encoder))
    return false;
  ConstStringTable strtab;
