...
lldb::SBProcess SBTarget::AttachToProcessWithID(
  ...
  if (TargetSP target_sp = GetSP()) {
    ...
    if (ProcessInstanceInfo instance_info; target_sp->GetPlatform()->GetProcessInfo(pid, instance_info))
      attach_info.SetUserID(instance_info.GetEffectiveUserID());
...
lldb::SBAddress SBTarget::ResolveLoadAddress(lldb::addr_t vm_addr) {
  ...
  if (TargetSP target_sp = GetSP()) {
    ...
    if (std::lock_guard<std::recursive_mutex> guard(target_sp->GetAPIMutex()); target_sp->ResolveLoadAddress(vm_addr, addr))
      return sb_addr;
  }
...
lldb::SBAddress SBTarget::ResolveFileAddress(lldb::addr_t file_addr) {
  ...
  if (TargetSP target_sp = GetSP()) {
    ...
    if (std::lock_guard<std::recursive_mutex> guard(target_sp->GetAPIMutex()); target_sp->ResolveFileAddress(file_addr, addr))
      return sb_addr;
  }
...
bool SBTarget::GetDescription(SBStream &description,
  ...
  if (TargetSP Stream &strm = description.ref(); target_sp = GetSP()) {
...
lldb::SBSymbolContextList SBTarget::FindGlobalFunctions(const char *name,
  ...
  if (name && name[0]) {
    ...
    if (TargetSP llvm::StringRef name_ref(name); target_sp = GetSP()) {
      ModuleFunctionSearchOptions function_options;
      function_options.include_symbols = true;
      function_options.include_inlines = true;
...
lldb::SBValue SBTarget::FindFirstGlobalVariable(const char *name) {
  ...
  if (SBValueList sb_value_list(FindGlobalVariables(name, 1)); sb_value_list.IsValid() && sb_value_list.GetSize() > 0)
...
SBError SBTarget::ClearModuleLoadAddress(lldb::SBModule module) {
  ...
  if (TargetSP char path[PATH_MAX]; target_sp = GetSP()) {
