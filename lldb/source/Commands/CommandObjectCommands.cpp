...
protected:
  void DoExecute(Args &args, CommandReturnObject &result) override {
    ...
    if (CommandObject::CommandMap::iterator pos; args.empty()) {
...
protected:
  ...
  void IOHandlerInputComplete(IOHandler &io_handler,
    ...
    if (m_regex_cmd_up) {
      ...
      if (StringList lines; lines.SplitIntoLines(data)) {
...
protected:
  ...
  void DoExecute(Args &command, CommandReturnObject &result) override {
    ...
    if (num_args == 1) {
      ...
      if (Status add_error = GetCommandInterpreter().AddUserCommand(
          cmd_name, cmd_sp, m_options.m_overwrite); add_error.Fail()) {
