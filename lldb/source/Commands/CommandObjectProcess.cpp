  ...
  void DoExecute(Args &launch_args, CommandReturnObject &result) override {
    ...
    if (error.Success()) {
      ...
      if (ProcessSP process_sp(target->GetProcessSP()); process_sp) {
        ...
        if (llvm::StringRef data = stream.GetString(); !data.empty())
...
public:
  ...
  public:
    ...
    Status SetOptionValue(uint32_t option_idx, llvm::StringRef option_arg,
      ...
      switch (ArchSpec arch =
          execution_context->GetProcessPtr()->GetSystemArchitecture(); short_option) {
      case 'i':
        do_install = true;
        if (!option_arg.empty())
  ...
  public:
    ...
    llvm::ArrayRef<OptionDefinition> GetDefinitions() override {
      ...
      for (OptionDefinition &value : m_opt_def) {
        ...
        if (llvm::StringRef opt_name = value.long_option; opt_name != "plugin-name")
          continue;
    ...
    if (num_args > 0) {
      for (const auto &arg : signal_args) {
        ...
        if (signals_sp) {
          ...
          if (int32_t signo; llvm::to_integer(arg.c_str(), signo)) {
...
public:
  ...
  void DoExecute(Args &command, CommandReturnObject &result) override {
    ...
    if (llvm::Error TraceSP trace_sp = process_sp->GetTarget().GetTrace(); err = trace_sp->Stop())
