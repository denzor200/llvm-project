  ...
  void DoExecute(Args &command, CommandReturnObject &result) override {
    ...
    for (const Args::ArgEntry &arg : command.entries()) {
      ...
      for (size_t i = 0; i < num_matches; ++i) {
        ...
        if (Module *m = module_list.GetModulePointerAtIndex(i); SymbolFile *sf = m->GetSymbolFile())
          sf->DumpClangAST(result.GetOutputStream(), filter,
                           GetCommandInterpreter().GetDebugger().GetUseColor());
      }
    }
...
protected:
  ...
  void PrintModule(Target &target, Module *module, int indent, Stream &strm) {
    ...
    if (dump_object_name) {
      ...
      if (const char *object_name = module->GetObjectName().GetCString(); object_name)
...
protected:
  ...
  void DoExecute(Args &command, CommandReturnObject &result) override {
    ...
    if (m_options.m_use_one_liner) {
      ...
      if (Status error = hook_ptr->SetScriptCallback(
          m_python_class_options.GetName(),
          m_python_class_options.GetStructuredData()); error.Success())
        result.AppendMessageWithFormat("Stop hook #%" PRIu64 " added.\n",
                                       new_hook_sp->GetID());
      else {
