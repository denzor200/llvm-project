...
CommandObjectDisassemble::GetContainingAddressRanges() {
  ...
  if (Target &target = GetTarget(); target.HasLoadedSections()) {
    ...
    if (Address symbol_containing_address; target.ResolveLoadAddress(m_options.symbol_containing_addr,
    ...
    for (lldb::ModuleSP module_sp : target.GetImages().Modules()) {
      ...
      if (Address file_address; module_sp->ResolveFileAddress(m_options.symbol_containing_addr,
                                        file_address)) {
