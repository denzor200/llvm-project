...
void PseudoLoweringEmitter::addOperandMapping(
  ...
  if (const Init *DagArg = Dag->getArg(DagIdx); const DefInit *DI = dyn_cast<DefInit>(DagArg)) {
...
void PseudoLoweringEmitter::evaluateExpansion(const Record *Rec) {
  ...
  for (const auto &[Idx, DstOp] : enumerate(Insn.Operands)) {
    ...
    if (unsigned MIOpNo = DstOp.MIOperandNo; const auto *SubDag = dyn_cast<DagInit>(Dag->getArg(Idx))) {
