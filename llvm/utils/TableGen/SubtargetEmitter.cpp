  ...
  for (const CodeGenSchedClass &SC : SchedModels.schedClasses()) {
    ...
    if (Writes.empty()) {
      ...
      for (const Record *I : ProcModel.ItinRWDefs) {
        ...
        if (ConstRecVec Matched = I->getValueAsListOfDefs("MatchedItinClasses"); is_contained(Matched, SC.ItinClassDef)) {
...
void SubtargetEmitter::emitHwModeCheck(const std::string &ClassName,
  ...
  if (IsMC) {
    ...
    if (const ArrayRef<const Record *> &Prologs =
        Records.getAllDerivedDefinitions("HwModePredicateProlog"); !Prologs.empty()) {
...
void SubtargetEmitter::parseFeaturesFunction(raw_ostream &OS) {
  ...
  for (const Record *R : Features) {
    ...
    if (StringRef FieldName = R->getValueAsString("FieldName"); Value == "true" || Value == "false")
