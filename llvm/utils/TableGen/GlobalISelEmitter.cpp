...
Expected<InstructionMatcher &> GlobalISelEmitter::createAndImportSelDAGMatcher(
  ...
  for (const TypeSetByHwMode &VTy : Src.getExtTypes()) {
    ...
    if (OperandMatcher &OM = InsnMatcher.addOperand(OpIdx++, "", TempOpIdx); auto Error = OM.addTypeCheckPredicate(VTy, OperandIsAPointer))
...
void GlobalISelEmitter::run(raw_ostream &OS) {
  if (!UseCoverageFile.empty()) {
    ...
    if (auto RuleCoverageBufOrErr = MemoryBuffer::getFile(UseCoverageFile); !RuleCoverageBufOrErr) {
