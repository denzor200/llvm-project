...
Expected<InstructionMatcher &> GlobalISelEmitter::addBuiltinPredicates(
  ...
  if (Predicate.isLoad() || Predicate.isStore() || Predicate.isAtomic()) {
    ...
    if (int64_t MinAlign = Predicate.getMinAlignment(); MinAlign > 0) {
...
Expected<InstructionMatcher &> GlobalISelEmitter::createAndImportSelDAGMatcher(
  ...
  if (Src.isLeaf()) {
    ...
    if (const Init *SrcInit = Src.getLeafValue(); isa<IntInit>(SrcInit)) {
  ...
  if (Src.isLeaf()) {
    ...
    if (const IntInit *const Init *SrcInit = Src.getLeafValue(); SrcIntInit = dyn_cast<IntInit>(SrcInit)) {
      OperandMatcher &OM =
...
Expected<action_iterator> GlobalISelEmitter::importExplicitDefRenderers(
  ...
  for (unsigned I = IsRoot ? 0 : 1, E = DstI->Operands.NumDefs; I != E; ++I) {
    ...
    if (std::string OpName = getMangledRootDefName(OpInfo.Name); IsRoot && M.hasOperand(OpName)) {
...
Expected<action_iterator> GlobalISelEmitter::importExplicitUseRenderers(
  ...
  if (Name == "REG_SEQUENCE") {
    ...
    for (unsigned I = 1; I != ExpectedDstINumUses; I += 2) {
      ...
      if (const DefInit *const TreePatternNode &SubRegChild = Dst.getChild(I + 1); SubRegInit =
...
Error GlobalISelEmitter::constrainOperands(action_iterator InsertPt,
  ...
  if (StringRef DstIName = DstI.getName(); DstIName == "COPY_TO_REGCLASS") {
