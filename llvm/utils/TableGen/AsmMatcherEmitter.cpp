...
ClassInfo *AsmMatcherInfo::getOperandClass(const Record *Rec, int SubOpIdx) {
  if (Rec->isSubClassOf("RegisterOperand")) {
    ...
    if (const DefInit *DI = dyn_cast<DefInit>(R->getValue())) {
      ...
      if (const Record *MatchClass = DI->getDef(); ClassInfo *CI = AsmOperandClasses[MatchClass])
        return CI;
    }
  ...
  if (Rec->isSubClassOf("Operand")) {
    ...
    if (const Record *MatchClass = Rec->getValueAsDef("ParserMatchClass"); ClassInfo *CI = AsmOperandClasses[MatchClass])
      return CI;
