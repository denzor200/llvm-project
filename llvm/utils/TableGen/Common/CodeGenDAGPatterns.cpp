...
std::string TreePredicateFn::getCodeToRunOnSDNode() const {
  ...
  if (std::string ImmCode = getImmCode(); !ImmCode.empty()) {
    if (isLoad())
      PrintFatalError(getOrigPatFragRecord()->getRecord()->getLoc(),
...
void TreePatternNode::SubstituteFormalArguments(
  ...
  for (unsigned i = 0, e = getNumChildren(); i != e; ++i) {
    ...
    if (TreePatternNode &Child = getChild(i); Child.isLeaf()) {
      ...
      // UnsetInit.
      if (const Init *Val = Child.getLeafValue(); isa<UnsetInit>(Val) ||
          (isa<DefInit>(Val) &&
...
bool TreePatternNode::ApplyTypeConstraints(TreePattern &TP, bool NotRegisters) {
  ...
  if (getOperator()->isSubClassOf("Instruction")) {
    ...
    if (!InstInfo.ImplicitDefs.empty()) {
      ...
      if (MVT VT = InstInfo.HasOneImplicitDefWithKnownVT(CDP.getTargetInfo()); VT != MVT::Other)
    ...
    for (unsigned i = NumResults, e = NumFixedOperands; i != e; ++i) {
      ...
      if (OperandNode->isSubClassOf("Operand")) {
        ...
        if (unsigned const DagInit *MIOpInfo = OperandNode->getValueAsDag("MIOperandInfo"); NumArgs = MIOpInfo->getNumArgs()) {
  ...
  if (getOperator()->isSubClassOf("ComplexPattern")) {
    ...
    if (!NotRegisters) {
      ...
      // instead may work).
      if (const ValueTypeByHwMode VVT = getValueTypeByHwMode(T, CGH); !VVT.isSimple() || VVT.getSimple() != MVT::Untyped)
...
TreePatternNodePtr TreePattern::ParseTreePattern(const Init *TheInit,
  ...
  if (Operator->isSubClassOf("ComplexPattern")) {
    for (unsigned i = 0; i < Children.size(); ++i) {
      ...
      if (auto [PrevOp, Inserted] =
          ComplexPatternOperands.try_emplace(Child->getName(), OperandId); !Inserted && PrevOp->getValue() != OperandId) {
...
void CodeGenDAGPatterns::ParseInstructions() {
  ...
  for (const auto &[Instr, TheInst] : Instructions) {
    ...
    if (TreePatternNodePtr ResultPattern = TheInst.getResultPattern(); SrcPattern && ResultPattern) {
...
void CodeGenDAGPatterns::AddPatternToMatch(TreePattern *Pattern,
  ...
  if (std::string Reason; !PTM.getSrcPattern().canPatternMatch(Reason, *this)) {
...
static void CombineChildVariants(
  ...
  do {
    ...
    if (std::string ErrString; R->canPatternMatch(ErrString, CDP) &&
