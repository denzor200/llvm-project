...
class CHR {
 ...
 private:
  ...
  void findScopes(SmallVectorImpl<CHRScope *> &Output) {
    ...
    if (CHRScope *Region *R = RI.getTopLevelRegion(); Scope = findScopes(R, nullptr, nullptr, Output)) {
...
static bool checkBias(K *Key, BranchProbability TrueProb,
  ...
  if (BranchProbability Threshold = getCHRBiasThreshold(); TrueProb >= Threshold) {
    TrueSet.insert(Key);
...
void CHR::checkScopeHoistable(CHRScope *Scope) {
  ...
  if (SmallVector<SelectInst *, 8> &Selects = RI.Selects; RI.HasBranch || !Selects.empty()) {
...
static void hoistValue(Value *V, Instruction *HoistPoint, Region *R,
  ...
  if (auto *DenseSet<Instruction *> &HoistStops = IT->second; I = dyn_cast<Instruction>(V)) {
    if (I == HoistPoint)
      return;
