...
void NumericalStabilitySanitizer::populateShadowStack(
  ...
  if (Function *Fn = CI.getCalledFunction()) {
    ...
    if (LibFunc LFunc; Fn->isIntrinsic() || TLI.getLibFunc(*Fn, LFunc))
      return;
  }
...
bool NumericalStabilitySanitizer::instrumentMemIntrinsic(MemIntrinsic *MI) {
  ...
  if (auto *M = dyn_cast<MemSetInst>(MI)) {
    ...
    if (FunctionCallee SetUnknownFn =
        NsanSetUnknownFns.getFunctionFor(GetMemOpSize(M->getArgOperand(2))); SetUnknownFn.getFunctionType()->getNumParams() == 1)
    ...
    if (FunctionCallee CopyFn =
        NsanCopyFns.getFunctionFor(GetMemOpSize(M->getArgOperand(2))); CopyFn.getFunctionType()->getNumParams() == 2)
