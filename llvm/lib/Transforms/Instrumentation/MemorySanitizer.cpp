...
struct MemorySanitizerVisitor : public InstVisitor<MemorySanitizerVisitor> {
  ...
  Value *originToIntptr(IRBuilder<> &IRB, Value *Origin) {
    ...
    if (unsigned IntptrSize = DL.getTypeStoreSize(MS.IntptrTy); IntptrSize == kOriginSize)
      return Origin;
  ...
  void materializeOneCheck(IRBuilder<> &IRB, Value *ConvertedShadow,
    ...
    if (unsigned SizeIndex = TypeSizeToSizeIndex(TypeSizeInBits); instrumentWithCalls(ConvertedShadow) && !MS.CompileKernel) {
      ...
      if (Value *ConvertedShadow2 =
          IRB.CreateZExt(ConvertedShadow, IRB.getIntNTy(8 * (1 << SizeIndex))); SizeIndex < kNumberOfAccessSizes) {
  ...
  Value *collapseStructShadow(StructType *Struct, Value *Shadow,
    ...
    for (unsigned Idx = 0; Idx < Struct->getNumElements(); Idx++) {
      ...
      if (Value *ShadowBool = convertToBool(ShadowItem, IRB); Aggregator != FalseVal)
  ...
  std::pair<Value *, Value *> getShadowOriginPtrKernelNoVec(Value *Addr,
    ...
    if (Value *AddrCast = IRB.CreatePointerCast(Addr, MS.PtrTy); Getter) {
  ...
  void handleMulByConstant(BinaryOperator &I, Constant *ConstArg,
    ...
    if (Type *Ty = ConstArg->getType(); auto *VTy = dyn_cast<VectorType>(Ty)) {
