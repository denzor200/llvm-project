...
void FuncPGOInstrumentation<Edge, BBInfo>::getInstrumentBBs(
  ...
  for (auto &E : EdgeList) {
    ...
    if (BasicBlock *InstrBB = getInstrBB(E); InstrBB)
...
populateEHOperandBundle(VPCandidateInfo &Cand,
  ...
  if (!isa<IntrinsicInst>(OrigCall)) {
    ...
    if (std::optional<OperandBundleUse> ParentFunclet =
        OrigCall->getOperandBundle(LLVMContext::OB_funclet); ParentFunclet)
    ...
    if (!BlockColors.empty()) {
      ...
      if (BasicBlock::iterator EHPadIt = CV.front()->getFirstNonPHIIt(); EHPadIt->isEHPad())
...
static bool annotateAllFunctions(
  ...
  for (auto &F : M) {
    ...
    if (PGOViewCounts != PGOVCT_None &&
      ...
      if (std::unique_ptr<BlockFrequencyInfo> NewBFI =
          std::make_unique<BlockFrequencyInfo>(F, *NewBPI, LI); PGOViewCounts == PGOVCT_Graph)
        NewBFI->view();
