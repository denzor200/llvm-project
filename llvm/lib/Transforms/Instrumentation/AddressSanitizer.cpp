...
void AddressSanitizer::instrumentMaskedLoadOrStore(
  ...
  SplitBlockAndInsertForEachLane(EVL, LoopInsertBefore->getIterator(),
    ...
    if (Value *MaskElem = IRB.CreateExtractElement(Mask, Index); auto *MaskElemC = dyn_cast<ConstantInt>(MaskElem)) {
      if (MaskElemC->isZero())
        // No check
        return;
      // Unconditional check
...
void AddressSanitizer::instrumentUnusualSizeOrAlignment(
  ...
  if (Value *AddrLong = IRB.CreatePointerCast(Addr, IntptrTy); UseCalls) {
    if (Exp == 0)
...
void ModuleAddressSanitizer::createInitializerPoisonCalls() {
  ...
  for (Use &OP : CA->operands()) {
    ...
    if (ConstantStruct *CS = cast<ConstantStruct>(OP); Function *F = dyn_cast<Function>(CS->getOperand(1))) {
...
void ModuleAddressSanitizer::instrumentGlobals(IRBuilder<> &IRB,
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    if (bool CanUsePrivateAliases =
        TargetTriple.isOSBinFormatELF() || TargetTriple.isOSBinFormatMachO() ||
        TargetTriple.isOSBinFormatWasm(); CanUsePrivateAliases && UsePrivateAlias) {
...
bool ModuleAddressSanitizer::instrumentModule() {
  ...
  // (2) target is ELF.
  if (const uint64_t Priority = GetCtorAndDtorPriority(TargetTriple); UseCtorComdat && TargetTriple.isOSBinFormatELF() && CtorComdat) {
    if (AsanCtorFunction) {
