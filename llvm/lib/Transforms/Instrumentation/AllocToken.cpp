...
public:
  ...
  uint64_t operator()(const CallBase &CB, OptimizationRemarkEmitter &ORE) {
    ...
    if (MDNode *N = getAllocTokenMetadata(CB)) {
      ...
      if (AllocTokenMetadata Metadata{S->getString(), containsPointer(N)}; auto Token = getAllocToken(TokenMode::TypeHash, Metadata, MaxTokens))
        return *Token;
    }
    // Fallback.
...
public:
  ...
  uint64_t operator()(const CallBase &CB, OptimizationRemarkEmitter &ORE) {
    if (MDNode *N = getAllocTokenMetadata(CB)) {
      ...
      if (AllocTokenMetadata Metadata{S->getString(), containsPointer(N)}; auto Token = getAllocToken(TokenMode::TypeHashPointerSplit, Metadata,
                                     MaxTokens))
        return *Token;
    }
