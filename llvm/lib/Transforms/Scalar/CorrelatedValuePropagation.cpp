...
static bool simplifyCommonValuePhi(PHINode *P, LazyValueInfo *LVI,
  ...
  for (unsigned i = 0, e = P->getNumIncomingValues(); i != e; ++i) {
    ...
    if (Value *Incoming = P->getIncomingValue(i); auto *IncomingConstant = dyn_cast<Constant>(Incoming)) {
...
static bool processPHI(PHINode *P, LazyValueInfo *LVI, DominatorTree *DT,
  ...
  for (unsigned i = 0, e = P->getNumIncomingValues(); i < e; ++i) {
    ...
    if (Value *V = getValueOnEdge(LVI, Incoming, P->getIncomingBlock(i), BB, P); V) {
