...
bool LoopPredication::isLoopProfitableToPredicate() {
  ...
  auto ComputeBranchProbability =
    ...
    if (MDNode *unsigned NumSucc = Term->getNumSuccessors(); ProfileData = getValidBranchWeightMDNode(*Term)) {
  ...
  for (const auto &ExitEdge : ExitEdges) {
    ...
    if (BranchProbability ExitingBlockProbability =
        ComputeBranchProbability(ExitEdge.first, ExitEdge.second); ExitingBlockProbability > LatchProbabilityThreshold)
      return false;
  }
