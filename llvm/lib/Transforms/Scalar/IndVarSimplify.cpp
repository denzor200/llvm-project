...
bool IndVarSimplify::simplifyAndExtend(Loop *L,
  ...
  while (!LoopPhis.empty()) {
    ...
    for (; !WideIVs.empty(); WideIVs.pop_back()) {
      ...
      if (PHINode *unsigned Widened; WidePhi = createWideIV(WideIVs.back(), LI, SE, Rewriter,
...
createReplacement(ICmpInst *ICmp, const Loop *L, BasicBlock *ExitingBB,
  ...
  if (SkipLastIter) {
    ...
    if (auto *SmallVector<const SCEV *, 4> Elements; UMin = dyn_cast<SCEVUMinExpr>(MaxIter)) {
