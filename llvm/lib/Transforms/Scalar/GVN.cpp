...
GVNPass::Expression GVNPass::ValueTable::createGEPExpr(GetElementPtrInst *GEP) {
  ...
  if (APInt ConstantOffset(BitWidth, 0); GEP->collectOffset(DL, BitWidth, VariableOffsets, ConstantOffset)) {
...
LoadInst *GVNPass::findLoadToHoistIntoPred(BasicBlock *Pred, BasicBlock *LoadBB,
  ...
  for (Instruction &Inst : *SuccBB) {
    ...
    if (MemDepResult Dep = MD->getDependency(&Inst); Dep.isNonLocal() && !ICF->isDominatedByICFIFromSameBlock(&Inst))
...
void GVNPass::eliminatePartiallyRedundantLoad(
  ...
  for (const auto &AvailableLoad : AvailableLoads) {
    ...
    if (MSSAU) {
      ...
      if (auto *auto *NewAccess = MSSAU->createMemoryAccessInBB(
          NewLoad, nullptr, NewLoad->getParent(), MemorySSA::BeforeTerminator); NewDef = dyn_cast<MemoryDef>(NewAccess))
...
bool GVNPass::ValueTable::areCallValsEqual(uint32_t Num, uint32_t NewNum,
  ...
  if (MemDepResult LocalDep = MD->getDependency(Call); !LocalDep.isNonLocal())
    return false;
    ...
    if (CmpInst *Cmp = dyn_cast<CmpInst>(LHS)) {
      ...
      if (Num < NextNum) {
        for (const auto &Entry : LeaderTable.getLeaders(Num)) {
          ...
          if (Value *NotCmp = Entry.Val; NotCmp && isa<Instruction>(NotCmp)) {
            unsigned NumReplacements;
            if (const BasicBlockEdge *Edge = std::get_if<BasicBlockEdge>(&Root))
              NumReplacements =
...
bool GVNPass::processInstruction(Instruction *I) {
  ...
  if (Value *const DataLayout &DL = I->getDataLayout(); V = simplifyInstruction(I, {DL, TLI, DT, AC})) {
    bool Changed = false;
    if (!I->use_empty()) {
...
bool GVNPass::performScalarPRE(Instruction *CurInst) {
  ...
  for (BasicBlock *P : predecessors(CurrentBlock)) {
    ...
    if (Value *PredV = findLeader(P, TValNo); !PredV) {
  ...
  if (NumWithout != 0) {
    ...
    if (unsigned SuccNum = GetSuccessorNumber(PREPred, CurrentBlock); isCriticalEdge(PREPred->getTerminator(), SuccNum)) {
