...
void LSRInstance::CollectFixupsAndInitialFormulae() {
  ...
  for (const IVStrideUse &U : IU) {
    ...
    LF.Offset = Offset;
    LU.AllFixupsOutsideLoop = LU.AllFixupsOutsideLoop && LF.isUseFullyOutsideLoop(L);
    LU.AllFixupsUnconditional = LU.AllFixupsUnconditional && IsFixupExecutedEachIncrement(LF);
...
void LSRInstance::ImplementSolution(
  ...
  Changed = Changed || RecursivelyDeleteTriviallyDeadInstructionsPermissive(DeadInsts,
                                                                  &TLI, MSSAU);
...
struct SCEVDbgValueBuilder {
  ...
  bool pushSCEV(const llvm::SCEV *S) {
    ...
    if (const SCEVConstant *StartInt = dyn_cast<SCEVConstant>(S)) {
      Success = Success && pushConst(StartInt);
      ...
      Success = Success && pushArithmeticExpr(MulRec, llvm::dwarf::DW_OP_mul);

    } else if (const SCEVUDivExpr *UDiv = dyn_cast<SCEVUDivExpr>(S)) {
      Success = Success && pushSCEV(UDiv->getLHS());
      Success = Success && pushSCEV(UDiv->getRHS());
      ...
      Success = Success && pushCast(Cast, (isa<SCEVSignExtendExpr>(Cast)));

    } else if (const SCEVAddExpr *AddExpr = dyn_cast<SCEVAddExpr>(S)) {
      Success = Success && pushArithmeticExpr(AddExpr, llvm::dwarf::DW_OP_plus);
...
static bool ReduceLoopStrength(Loop *L, IVUsers &IU, ScalarEvolution &SE,
  ...
  Changed = Changed || Reducer.getChanged();
