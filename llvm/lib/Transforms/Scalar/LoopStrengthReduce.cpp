      ...
      if (const SCEVMulExpr *MulRHS = dyn_cast<SCEVMulExpr>(RHS)) {
        if (IgnoreSignificantBits || isMulSExtable(MulRHS, SE)) {
          ...
          if (const SCEVConstant *RC =
            ...
            if (SmallVector<const SCEV *, 4> ROps(drop_begin(MulRHS->operands())); LOps == ROps)
...
void LSRInstance::CollectInterestingTypesAndFactors() {
  ...
  for (const IVStrideUse &U : IU) {
    ...
    do {
      ...
      if (const SCEV *S = Worklist.pop_back_val(); const SCEVAddRecExpr *AR = dyn_cast<SCEVAddRecExpr>(S)) {
        if (AR->getLoop() == L)
...
void LSRInstance::Solve(SmallVectorImpl<const Formula *> &Solution) const {
  ...
  if (const bool EnableDropUnprofitableSolution = [&] {
  ...
  }(); BaselineCost.isLess(SolutionCost)) {
