...
bool LoopIdiomRecognize::processLoopStores(SmallVectorImpl<StoreInst *> &SL,
  ...
  for (unsigned i = 0, e = SL.size(); i < e; ++i) {
    ...
    if (unsigned FirstStoreSize = DL->getTypeStoreSize(SL[i]->getValueOperand()->getType()); FirstStride == FirstStoreSize || -FirstStride == FirstStoreSize) {
      Heads.insert(SL[i]);
      continue;
    }
  ...
  for (StoreInst *I : Heads) {
    ...
    if (const SCEV *StoreSizeSCEV = SE->getConstant(IntIdxTy, StoreSize); processLoopStridedStore(StorePtr, StoreSizeSCEV,
...
bool LoopIdiomRecognize::processLoopMemIntrinsic(
  ...
  for (BasicBlock::iterator I = BB->begin(), E = BB->end(); I != E;) {
    ...
    if (Instruction *Inst = &*I++; MemInst *MI = dyn_cast<MemInst>(Inst)) {
...
bool LoopIdiomRecognize::processLoopMemSet(MemSetInst *MSI,
  ...
  if (const bool IsConstantSize = isa<ConstantInt>(MSI->getLength()); IsConstantSize) {
