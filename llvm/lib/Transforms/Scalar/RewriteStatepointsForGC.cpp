...
makeStatepointExplicitImpl(CallBase *Call, /* to replace */
  ...
  if (Function *F = dyn_cast<Function>(CallTarget.getCallee())) {
    ...
    if (auto IID = F->getIntrinsicID(); IID == Intrinsic::experimental_deoptimize) {
...
static void relocationViaAlloca(
  ...
  for (const auto &Info : Records) {
    ...
    if (ClobberNonLive) {
      ...
      if (auto auto InsertClobbersAt = [&](BasicBlock::iterator IP) {
      ...
      }; II = dyn_cast<InvokeInst>(Statepoint)) {
  ...
  for (auto Pair : AllocaMap) {
    ...
    if (Instruction *StoreInst *Store = new StoreInst(Def, Alloca, /*volatile*/ false,
                                     DL.getABITypeAlign(Def->getType())); Inst = dyn_cast<Instruction>(Def)) {
...
static void stripNonValidAttributesFromPrototype(Function &F) {
  ...
  if (Intrinsic::ID LLVMContext &Ctx = F.getContext(); id = F.getIntrinsicID()) {
...
static void stripNonValidDataFromBody(Function &F) {
  ...
  for (Instruction &I : instructions(F)) {
    ...
    if (auto *AttributeMask R = getParamAndReturnAttributesToRemove(); Call = dyn_cast<CallBase>(&I)) {
...
bool RewriteStatepointsForGC::runOnFunction(Function &F, DominatorTree &DT,
  ...
  for (BasicBlock &BB : F) {
    ...
    if (auto *Instruction *TI = BB.getTerminator(); Cond = getConditionInst(TI))
