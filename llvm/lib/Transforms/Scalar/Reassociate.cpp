...
static bool shouldConvertOrWithNoCommonBitsToAdd(Instruction *Or) {
  ...
  if (Value *VB = Or->user_back(); Or->hasOneUse() && isInteresting(VB))
    return true;
...
ReassociatePass::buildMinimalMultiplyDAG(IRBuilderBase &Builder,
  ...
  for (unsigned LastIdx = 0, Idx = 1, Size = Factors.size();
    ...
    if (Value *M = Factors[LastIdx].Base = buildMultiplyTree(Builder, InnerProduct); Instruction *MI = dyn_cast<Instruction>(M))
      RedoInsts.insert(MI);
  ...
  if (Ops.size() > 2 && Ops.size() <= GlobalReassociateLimit) {
    ...
    for (unsigned i = Ops.size() - 1; i > LimitIdx; --i) {
      ...
      for (int j = i - 1; j >= (int)LimitIdx; --j) {
        ...
        if (unsigned MaxRank = std::max(Ops[i].Rank, Ops[j].Rank); Score > Max || (Score == Max && MaxRank < BestRank)) {
          BestPair = {j, i};
          Max = Score;
          BestRank = MaxRank;
        }
...
PreservedAnalyses ReassociatePass::run(Function &F, FunctionAnalysisManager &) {
  ...
  for (BasicBlock *BI : RPOT) {
    ...
    while (!ToRedo.empty()) {
      ...
      if (Instruction *I = ToRedo.pop_back_val(); isInstructionTriviallyDead(I)) {
