...
static void rewriteMemOpOfSelect(SelectInst &SI, T &I,
  ...
  if (Instruction *ElseTerm = nullptr; Spec.areNoneSpeculatable())
...
public:
  ...
  rewriteTreeStructuredMerge(Partition &P) {
    ...
    for (Slice &S : P) {
      ...
      if (auto *auto *User = cast<Instruction>(S.getUse()->getUser()); LI = dyn_cast<LoadInst>(User)) {
...
private:
  ...
  struct LoadOpSplitter : public OpSplitter<LoadOpSplitter> {
    ...
    void emitFunc(Type *Ty, Value *&Agg, Align Alignment, const Twine &Name) {
      ...
      if (APInt Offset(
          DL.getIndexSizeInBits(Ptr->getType()->getPointerAddressSpace()), 0); AATags &&
...
AllocaInst *SROA::rewritePartition(AllocaInst &AI, AllocaSlices &AS,
  ...
  if (CommonUseTy.first) {
    ...
    if (TypeSize CommonUseSize = DL.getTypeAllocSize(CommonUseTy.first); CommonUseSize.isFixed() && CommonUseSize.getFixedValue() >= P.size()) {
  ...
  auto MigrateOne = [&](DbgVariableRecord *DbgVariable) {
    ...
    for (auto Fragment : Fragments) {
      ...
      auto RemoveOne = [DbgVariable](auto *OldDII) {
        ...
        if (auto SameVariableFragment = [](const auto *LHS, const auto *RHS) {
          return LHS->getVariable() == RHS->getVariable() &&
                 LHS->getDebugLoc()->getInlinedAt() ==
                     RHS->getDebugLoc()->getInlinedAt();
        }; SameVariableFragment(OldDII, DbgVariable))
