...
static bool mayBeVisibleThroughUnwinding(Value *V, Instruction *Start,
  ...
  if (bool RequiresNoCaptureBeforeUnwind; isNotVisibleOnUnwind(getUnderlyingObject(V),
...
bool MemCpyOptPass::moveUp(StoreInst *SI, Instruction *P, const LoadInst *LI) {
  ...
  auto AddArg = [&](Value *Arg) {
    ...
    if (auto *I = dyn_cast<Instruction>(Arg); I && I->getParent() == SI->getParent()) {
...
static bool overreadUndefContents(MemorySSA *MSSA, MemCpyInst *MemCpy,
  ...
  if (auto *MemoryAccess *Clobber = MSSA->getWalker()->getClobberingMemoryAccess(
      MemSrcAccess->getDefiningAccess(), MemCpyLoc, BAA); MD = dyn_cast<MemoryDef>(Clobber))
