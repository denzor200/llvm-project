...
NewGVN::ExprResult NewGVN::createExpression(Instruction *I) const {
  ...
  if (auto *CI = dyn_cast<CmpInst>(I)) {
    ...
    if (Value *V =
        simplifyCmpInst(Predicate, E->getOperand(0), E->getOperand(1), Q); auto Simplified = checkExprResults(E, I, V))
      return Simplified;
...
bool NewGVN::eliminateInstructions(Function &F) {
  ...
  for (auto *CC : reverse(CongruenceClasses)) {
    ...
    if (Value *Leader =
        CC->getStoredValue() ? CC->getStoredValue() : CC->getLeader(); alwaysAvailable(Leader)) {
