...
const Expression *NewGVN::createBinaryExpression(unsigned Opcode, Type *T,
  ...
  if (auto Value *V = simplifyBinOp(Opcode, E->getOperand(0), E->getOperand(1), Q); Simplified = checkExprResults(E, I, V)) {
...
NewGVN::ExprResult NewGVN::createExpression(Instruction *I) const {
  ...
  if (auto *CI = dyn_cast<CmpInst>(I)) {
    ...
    if (isa<Constant>(E->getOperand(0)) ||
      ...
      if (auto Value *V = simplifySelectInst(E->getOperand(0), E->getOperand(1),
                                    E->getOperand(2), Q); Simplified = checkExprResults(E, I, V))
        return Simplified;
    }
  } else if (I->isBinaryOp()) {
    
    if (auto Value *V =
        simplifyBinOp(E->getOpcode(), E->getOperand(0), E->getOperand(1), Q); Simplified = checkExprResults(E, I, V))
      return Simplified;
  } else if (auto *CI = dyn_cast<CastInst>(I)) {
    
    if (auto Value *V =
        simplifyCastInst(CI->getOpcode(), E->getOperand(0), CI->getType(), Q); Simplified = checkExprResults(E, I, V))
      return Simplified;
  } else if (auto *GEPI = dyn_cast<GetElementPtrInst>(I)) {
    
    if (auto Value *V = simplifyGEPInst(GEPI->getSourceElementType(), *E->op_begin(),
                               ArrayRef(std::next(E->op_begin()), E->op_end()),
                               GEPI->getNoWrapFlags(), Q); Simplified = checkExprResults(E, I, V))
      return Simplified;
  } else if (AllConstant) {
...
NewGVN::ExprResult NewGVN::performSymbolicCmpEvaluation(Instruction *I) const {
  ...
  for (const auto &Op : CI->operands()) {
    ...
    if (const auto *auto *PI = PredInfo->getPredicateInfoFor(Op); PBranch = dyn_cast_or_null<PredicateBranch>(PI)) {
      if (PI == LastPredInfo)
        continue;
      LastPredInfo = PI;
...
void NewGVN::moveValueToNewCongruenceClass(Instruction *I, const Expression *E,
  ...
  if (auto *InstMA = dyn_cast_or_null<MemoryDef>(getMemoryAccess(I)); InstMA)
  ...
  if (BasicBlock *TrueSucc, *FalseSucc; match(TI, m_Br(m_Value(Cond), TrueSucc, FalseSucc))) {
    ...
    if (!CondEvaluated) {
      if (auto *I = dyn_cast<Instruction>(Cond)) {
        ...
        if (const auto *auto Res = performSymbolicEvaluation(I, Visited); CE = dyn_cast_or_null<ConstantExpression>(Res.Expr)) {
  ...
  for (auto *CC : reverse(CongruenceClasses)) {
    ...
    if (CC == TOPClass) {
      for (auto *M : *CC) {
        ...
        if (auto *VTE = ValueToExpression.lookup(M); VTE && isa<DeadExpression>(VTE))
