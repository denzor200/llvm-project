...
static bool isComputableLoopNest(ScalarEvolution *SE,
  ...
  for (Loop *L : LoopList) {
    ...
    if (const SCEV *ExitCountOuter = SE->getBackedgeTakenCount(L); isa<SCEVCouldNotCompute>(ExitCountOuter)) {
  ...
  for (Value *User : V->users()) {
    if (PHINode *PHI = dyn_cast<PHINode>(User)) {
      ...
      if (RecurrenceDescriptor::isReductionPHI(PHI, L, RD)) {
        ...
        switch (RecurKind RK = RD.getRecurrenceKind(); RK) {
        case RecurKind::Or:
        case RecurKind::And:
        case RecurKind::Xor:
        case RecurKind::SMin:
...
bool LoopInterchangeLegality::findInductionAndReductions(
  ...
  for (PHINode &PHI : L->getHeader()->phis()) {
    ...
    if (InductionDescriptor ID; InductionDescriptor::isInductionPHI(&PHI, L, SE, ID))
    ...
    auto MoveInstructions = [&i, &WorkList, this, &InductionPHIs, NewLatch]() {
      for (; i < WorkList.size(); i++) {
        ...
        for (Use &U : llvm::make_early_inc_range(WorkList[i]->uses())) {
          ...
          if (Instruction *UserI = cast<Instruction>(U.getUser()); !InnerLoop->contains(UserI->getParent()) ||
