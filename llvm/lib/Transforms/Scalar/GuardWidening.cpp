...
static Value *getCondition(Instruction *I) {
  ...
  if (BasicBlock *IfTrueBB, *IfFalseBB; parseWidenableBranch(I, Cond, WC, IfTrueBB, IfFalseBB))
    return Cond;
...
GuardWideningImpl::mergeChecks(SmallVectorImpl<Value *> &ChecksToHoist,
  ...
  Value *Result = nullptr;
  {
    ...
    if (CmpPredicate Pred0, Pred1; ChecksToWiden.size() == 1 && ChecksToHoist.size() == 1 &&
      ...
      if (std::optional<ConstantRange> ConstantRange CR1 =
          ConstantRange::makeExactICmpRegion(Pred1, RHS1->getValue()); Intersect =
