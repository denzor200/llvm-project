...
Value *ConstantOffsetExtractor::Extract(Value *Idx, GetElementPtrInst *GEP,
  ...
  if (APInt ConstantOffset =
      Extractor.find(Idx, /* SignExtended */ false, /* ZeroExtended */ false,
                     GEP->isInBounds()); ConstantOffset == 0) {
...
bool SeparateConstOffsetFromGEP::reuniteExts(Instruction *I) {
  ...
  if (match(I, m_Add(m_SExt(m_Value(LHS)), m_SExt(m_Value(RHS))))) {
    if (LHS->getType() == RHS->getType()) {
      ...
      if (auto *ExprKey Key = createNormalizedCommutablePair(LHS, RHS); Dom = findClosestMatchingDominator(Key, I, DominatingAdds)) {
        Instruction *NewSExt =
...
void SeparateConstOffsetFromGEP::swapGEPOperand(GetElementPtrInst *First,
  ...
  if (uint64_t ObjectSize; !getObjectSize(NewBase, ObjectSize, DAL, TLI) ||
