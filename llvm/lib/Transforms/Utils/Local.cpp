...
bool llvm::ConstantFoldTerminator(BasicBlock *BB, bool DeleteDeadConditions,
  ...
  if (auto *SI = dyn_cast<SwitchInst>(T)) {
    ...
    if (SI->getNumCases() == 1) {
      ...
      if (MDNode *MakeImplicitMD = SI->getMetadata(LLVMContext::MD_make_implicit); MakeImplicitMD)
...
DIExpression *llvm::getExpressionForConstant(DIBuilder &DIB, const Constant &C,
  ...
  if (auto *FP = dyn_cast<ConstantFP>(&C); FP && Ty.isFloatingPointTy() && Ty.getScalarSizeInBits() <= 64) {
