...
static void replaceUndefValuesInPhi(PHINode *PN,
  ...
  for (unsigned i = 0, e = PN->getNumIncomingValues(); i != e; ++i) {
    ...
    if (Value *V = PN->getIncomingValue(i); !isa<UndefValue>(V)) continue;
  ...
  auto LowerOne = [&](DbgVariableRecord *DDI) {
    ...
    while (!WorkList.empty()) {
      ...
      for (const auto &AIUse : V->uses()) {
        ...
        if (StoreInst *User *U = AIUse.getUser(); SI = dyn_cast<StoreInst>(U)) {
...
CallInst *llvm::createCallMatchingInvoke(InvokeInst *II) {
  ...
  if (uint64_t TotalWeight; NewCall->extractProfTotalWeight(TotalWeight)) {
  ...
  do {
    ...
    for (Instruction &I : *BB) {
      if (auto *CI = dyn_cast<CallInst>(&I)) {
        ...
        if (Function *Value *Callee = CI->getCalledOperand(); F = dyn_cast<Function>(Callee)) {
    ...
    if (auto *Instruction *Terminator = BB->getTerminator(); II = dyn_cast<InvokeInst>(Terminator)) {
...
bool llvm::callsGCLeafFunction(const CallBase *Call,
  ...
  if (LibFunc LF; TLI.getLibFunc(*Call, LF)) {
    return TLI.has(LF);
  }
...
DIExpression *llvm::getExpressionForConstant(DIBuilder &DIB, const Constant &C,
  ...
  if (const ConstantExpr *CE = dyn_cast<ConstantExpr>(&C))
    if (CE->getOpcode() == Instruction::IntToPtr) {
      ...
      if (auto const Value *V = CE->getOperand(0); CI = dyn_cast_or_null<ConstantInt>(V))
...
Value *llvm::invertCondition(Value *Condition) {
  ...
  if (Value *NotCondition; match(Condition, m_Not(m_Value(NotCondition))))
    return NotCondition;
