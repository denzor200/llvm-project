...
Value *LibCallSimplifier::replacePowWithExp(CallInst *Pow, IRBuilderBase &B) {
  ...
  if (hasFloatFn(M, TLI, Ty, LibFunc_exp2, LibFunc_exp2f, LibFunc_exp2l)) {
    ...
    if (APSInt NI(64, false); (IsInteger || IsReciprocal) &&
      ...
      if (Value *FMul = B.CreateFMul(Expo, ConstantFP::get(Ty, N), "mul"); Pow->doesNotAccessMemory())
  ...
  if (Pow->hasApproxFunc() && Pow->hasNoNaNs() && BaseF->isFiniteNonZero() &&
    ...
    if (Log) {
      ...
      if (Value *FMul = B.CreateFMul(Log, Expo, "mul"); Pow->doesNotAccessMemory())
        return copyFlags(*Pow, B.CreateUnaryIntrinsic(Intrinsic::exp2, FMul,
                                                      nullptr, "exp2"));
...
Value *LibCallSimplifier::optimizeTrigInversionPairs(CallInst *CI,
  ...
  if (F && TLI->getLibFunc(F->getName(), Func) &&
    ...
    // Used as error value
    if (LibFunc inverseFunc = llvm::StringSwitch<LibFunc>(Callee->getName())
                              ...
                              .Default(NotLibFunc); Func == inverseFunc)
