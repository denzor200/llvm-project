...
Evaluator::getCalleeWithFormalArgs(CallBase &CB,
  ...
  if (auto *V = CB.getCalledOperand()->stripPointerCasts(); auto *Fn = getFunction(getVal(V)))
    ...
    if (StoreInst *SI = dyn_cast<StoreInst>(CurInst)) {
      ...
      if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(&CB)) {
        ...
        if (II->getIntrinsicID() == Intrinsic::invariant_start) {
          ...
          if (Value *Ptr = PtrArg->stripPointerCasts(); GlobalVariable *GV = dyn_cast<GlobalVariable>(Ptr)) {
      ...
      if (BranchInst *BI = dyn_cast<BranchInst>(CurInst)) {
        ...
        if (Value *Val = getVal(IBI->getAddress())->stripPointerCasts(); BlockAddress *BA = dyn_cast<BlockAddress>(Val))
