...
static ConstantInt *getConstantInt(Value *V, const DataLayout &DL) {
  ...
  if (ConstantInt *CI = dyn_cast<ConstantInt>(V); CI || !isa<Constant>(V) || !V->getType()->isPointerTy())
    return CI;
    ...
    if (ICI->getPredicate() == (isEQ ? ICmpInst::ICMP_EQ : ICmpInst::ICMP_NE)) {
      ...
      */
      if (match(ICI->getOperand(0),
        ...
        if (APInt Mask = ~*RHSC; Mask.isPowerOf2() && (C->getValue() & ~Mask) == C->getValue()) {
          // If we already have a value for the switch, it has to match!
...
static void cloneInstructionsIntoPredecessorBlockAndUpdateSSAUses(
  ...
  if (auto &PredDL = PTI->getDebugLoc()) {
    ...
    if (auto &DL = BB->getTerminator()->getDebugLoc(); !PredDL->getAtomGroup() && DL && DL->getAtomGroup() &&
...
static bool isSafeToHoistInvoke(BasicBlock *BB1, BasicBlock *BB2,
  ...
  for (BasicBlock *Succ : successors(BB1)) {
    for (const PHINode &PN : Succ->phis()) {
      ...
      if (Value *BB2V = PN.getIncomingValueForBlock(BB2); BB1V != BB2V && (BB1V == I1 || BB2V == I2)) {
        return false;
      }
    }
  }
...
static bool shouldHoistCommonInstructions(Instruction *I1, Instruction *I2,
  ...
  if (auto *C2 = dyn_cast<CallInst>(I2); C1 && C2)
...
static void hoistLockstepIdenticalDbgVariableRecords(
  ...
  while (none_of(Itrs, atEnd)) {
    ...
    for (CurrentAndEndIt &Pair : Itrs) {
      ...
      if (DbgRecord &DR = *Pair.first++; HoistDVRs) {
        DR.removeFromParent();
...
bool SimplifyCFGOpt::hoistCommonCodeFromSuccessors(Instruction *TI,
  ...
  if (unsigned int SuccSize = succ_size(BB); SuccSize < 2)
    return false;
...
static bool canSinkInstructions(
  ...
  if (isa<CallBase>(I0)) {
    ...
    if (bool AllCallsAreIndirect = all_of(Insts, IsIndirectCall); HaveIndirectCalls) {
      ...
      for (const Instruction *I : Insts) {
        ...
        if (Value *CurrCallee = cast<CallBase>(I)->getCalledOperand(); !Callee)
          Callee = CurrCallee;
  ...
  for (unsigned OI = 0, OE = I0->getNumOperands(); OI != OE; ++OI) {
    ...
    if (auto SameAsI0 = [&I0, OI](const Instruction *I) {
...
static void sinkLastInstruction(ArrayRef<BasicBlock*> Blocks) {
  ...
  for (unsigned O = 0, E = I0->getNumOperands(); O != E; ++O) {
    ...
    if (bool NeedPHI = any_of(Insts, [&I0, O](const Instruction *I) {
...
static bool sinkCommonCodeFromPredecessors(BasicBlock *BB,
  ...
  for (auto *PredBB : predecessors(BB)) {
    ...
    if (auto *PredBr = dyn_cast<BranchInst>(PredBB->getTerminator()); PredBr && PredBr->isUnconditional())
  ...
  if (!followedByDeoptOrUnreachable) {
    ...
    auto ProfitableToSinkInstruction = [&](LockstepReverseIterator<true> &LRI) {
      ...
      for (Use &U : (*LRI)[0]->operands()) {
        ...
        if (auto It = PHIOperands.find(&U); It != PHIOperands.end() && !all_of(It->second, [&](Value *V) {
...
bool CompatibleSets::shouldBelongToSameSet(ArrayRef<InvokeInst *> Invokes) {
  ...
  if (any_of(Invokes, HasNormalDest)) {
    ...
    if (SmallPtrSet<Value *, 16> EquivalenceSet(llvm::from_range, Invokes); !incomingValuesAreCompatible(
...
static void mergeCompatibleInvokesImpl(ArrayRef<InvokeInst *> Invokes,
  ...
  for (Use &U : MergedInvoke->operands()) {
    ...
    if (bool NeedPHI = any_of(Invokes, [&U](InvokeInst *II) {
  ...
  for (Instruction &CurI : reverse(BrBB->instructionsWithoutDebug(true))) {
    ...
    if (auto *LI = dyn_cast<LoadInst>(&CurI)) {
      if (LI->getPointerOperand() == StorePtr && LI->getType() == StoreTy &&
        ...
        if (bool ExplicitlyDereferenceableOnly; isWritableObject(Obj, ExplicitlyDereferenceableOnly) &&
            capturesNothing(
...
static bool blockIsSimpleEnoughToThreadThrough(BasicBlock *BB,
  ...
  for (Instruction &I : reverse(BB->instructionsWithoutDebug(false))) {
    ...
    for (User *U : I.users()) {
      ...
      if (BasicBlock *UsedInBB = UI->getParent(); UsedInBB == BB) {
        if (isa<PHINode>(UI))
          return false;
      } else
...
static ConstantInt *getKnownValueOnEdge(Value *V, BasicBlock *From,
  ...
  if (auto *I = dyn_cast<Instruction>(V); I && I->getParent() == To)
    return nullptr;
...
foldCondBranchOnValueKnownInPredecessorImpl(BranchInst *BI, DomTreeUpdater *DTU,
  ...
  if (PHINode *PN = dyn_cast<PHINode>(Cond); PN && PN->getParent() == BB) {
...
static bool foldTwoEntryPHINode(PHINode *PN, const TargetTransformInfo &TTI,
  ...
  if (!IsUnpredictable) {
    ...
    if (uint64_t TWeight, FWeight; extractBranchWeights(*DomBI, TWeight, FWeight) &&
        (TWeight + FWeight) != 0) {
      BranchProbability BITrueProb =
          BranchProbability::getBranchProbability(TWeight, TWeight + FWeight);
      BranchProbability Likely = TTI.getPredictableBranchThreshold();
      
      if (BranchProbability BIFalseProb = BITrueProb.getCompl(); IfBlocks.size() == 1) {
        
        if (BranchProbability BIBBProb =
            DomBI->getSuccessor(0) == BB ? BITrueProb : BIFalseProb; BIBBProb >= Likely)
          return false;
      } else {
...
static bool extractPredSuccWeights(BranchInst *PBI, BranchInst *BI,
  ...
  if (bool SuccHasWeights =
      extractBranchWeights(*BI, SuccTrueWeight, SuccFalseWeight); PredHasWeights || SuccHasWeights) {
    if (!PredHasWeights)
...
static Value *ensureValueAvailableInSuccessor(Value *V, BasicBlock *BB,
  ...
  for (auto I = Succ->begin(); isa<PHINode>(I); ++I)
    if (cast<PHINode>(I)->getIncomingValueForBlock(BB) == V) {
      ...
      if (BasicBlock *OtherPredBB = *PredI == BB ? *++PredI : *PredI; PHI->getIncomingValueForBlock(OtherPredBB) == AlternativeV)
        break;
      PHI = nullptr;
    }
  if (PHI)
...
static bool SimplifyCondBranchToCondBranch(BranchInst *PBI, BranchInst *BI,
  ...
  if (!PBI->getMetadata(LLVMContext::MD_unpredictable) &&
    ...
    if (BranchProbability Likely = TTI.getPredictableBranchThreshold(); CommonDestProb >= Likely)
      return false;
  }
  ...
  for (PHINode &PN : CommonDest->phis()) {
    ...
    if (Value *PBIV = PN.getIncomingValue(PBBIdx); BIV != PBIV) {
...
bool SimplifyCFGOpt::simplifyBranchOnICmpChain(BranchInst *BI,
  ...
  if (ExtraCase) {
    ...
    if (AssumptionCache *AC = Options.AC; !isGuaranteedNotToBeUndefOrPoison(ExtraCase, AC, BI, nullptr))
...
static bool isCleanupBlockEmpty(iterator_range<BasicBlock::iterator> R) {
  for (Instruction &I : R) {
    ...
    switch (Intrinsic::ID IntrinsicID = II->getIntrinsicID(); IntrinsicID) {
...
bool SimplifyCFGOpt::simplifyUnreachable(UnreachableInst *UI) {
  ...
  for (BasicBlock *Predecessor : Preds) {
    ...
    if (IRBuilder<> Builder(TI); auto *BI = dyn_cast<BranchInst>(TI)) {
      ...
      if (II->getUnwindDest() == BB) {
        ...
        if (auto *CI = cast<CallInst>(removeUnwindEdge(TI->getParent(), DTU)); !CI->doesNotThrow())
          CI->setDoesNotThrow();
        Changed = true;
      }
...
findContiguousCases(Value *Condition, SmallVectorImpl<ConstantInt *> &Cases,
  ...
  if (size_t ContiguousOffset = Cases.size() - 1; Offset == ContiguousOffset) {
  ...
  if (Max == CR.getUnsignedMax() && Min == CR.getUnsignedMin()) {
    ...
    if (auto [OtherMax, OtherMin] = std::make_pair(*It, *std::next(It)); (Max - OtherMax->getValue()) + (OtherMin->getValue() - Min) ==
        Cases.size() - 2) {
      return ContiguousCasesResult{
  ...
  if (HasDefault && DeadCases.empty()) {
    ...
    if (NumUnknownBits < 64 /* avoid overflow */) {
      ...
      if (SI->getNumCases() == AllNumCases - 1) {
        ...
        if (IntegerType *CondTy = cast<IntegerType>(Cond->getType()); CondTy->getIntegerBitWidth() > 64 ||
...
static PHINode *findPHIForConditionForwarding(ConstantInt *CaseValue,
  ...
  for (PHINode &PHI : Succ->phis()) {
    ...
    if (Value *InValue = PHI.getIncomingValue(Idx); InValue != CaseValue)
      continue;
...
static bool forwardSwitchConditionToPHI(SwitchInst *SI) {
  ...
  for (const auto &Case : SI->cases()) {
    ...
    for (PHINode &Phi : CaseDest->phis()) {
      ...
      if (int SwitchBBIdx = Phi.getBasicBlockIndex(SwitchBlock); Phi.getIncomingValue(SwitchBBIdx) == CaseValue &&
    ...
    if (int PhiIdx; auto *Phi = findPHIForConditionForwarding(CaseValue, CaseDest, &PhiIdx))
...
static bool validLookupTableConstant(Constant *C, const TargetTransformInfo &TTI) {
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(C)) {
    ...
    if (Constant *StrippedC = cast<Constant>(CE->stripInBoundsConstantOffsets()); StrippedC == C || !validLookupTableConstant(StrippedC, TTI))
      return false;
  }
...
static Value *foldSwitchToSelect(const SwitchCaseResultVectorTy &ResultVector,
  ...
  if (ResultVector.size() == 1 && DefaultResult) {
    ...
    if (unsigned CaseCount = CaseValues.size(); isPowerOf2_32(CaseCount)) {
      ...
      if (!AndMask.isZero() && Known.getMaxValue().uge(AndMask)) {
        ...
        if (unsigned FreeBits = Known.countMaxActiveBits() - AndMask.popcount(); FreeBits == Log2_32(CaseCount)) {
...
SwitchReplacement::SwitchReplacement(
  ...
  if (Values.size() < TableSize) {
    ...
    if (bool DefaultValueIsPoison = isa<PoisonValue>(DefaultValue); DefaultValue != SingleValue && !DefaultValueIsPoison)
      SingleValue = nullptr;
  }
...
static void reuseTableCompare(
  ...
  for (auto ValuePair : Values) {
    ...
    if (Constant *CaseConst = ConstantFoldCompareInstOperands(
        CmpInst->getPredicate(), ValuePair.second, CmpOp1, DL); !CaseConst || CaseConst == DefaultConst ||
...
static bool simplifySwitchLookup(SwitchInst *SI, IRBuilder<> &Builder,
  ...
  if (UseSwitchConditionAsTableIndex) {
    ...
    if (HasDefaultResults) {
      ...
      if (const uint64_t UpperBound = CR.getUpper().getLimitedValue(); !CR.isUpperWrapped() &&
...
static bool reduceSwitchRange(SwitchInst *SI, IRBuilder<> &Builder,
  ...
  if (auto *CondTy = cast<IntegerType>(SI->getCondition()->getType()); CondTy->getIntegerBitWidth() > 64 ||
      !DL.fitsInLegalInteger(CondTy->getIntegerBitWidth()))
    return false;
...
static bool simplifySwitchOfPowersOfTwo(SwitchInst *SI, IRBuilder<> &Builder,
  ...
  for (const auto &Case : SI->cases()) {
    ...
    if (uint64_t CaseValue = Case.getCaseValue()->getValue().getZExtValue(); llvm::has_single_bit(CaseValue))
...
static bool simplifySwitchOfCmpIntrinsic(SwitchInst *SI, IRBuilderBase &Builder,
  ...
  if (SI->getNumCases() == 2) {
    ...
    for (auto &Case : SI->cases()) {
      ...
      if (uint32_t Weight = Weights[Case.getSuccessorIndex()]; !OtherSucc || OtherSucc == NewSucc) {
        OtherSucc = NewSucc;
...
template <> struct llvm::DenseMapInfo<const SwitchSuccWrapper *> {
  ...
  static bool isEqual(const SwitchSuccWrapper *LHS,
    ...
    if (auto TKey = DenseMapInfo<SwitchSuccWrapper *>::getTombstoneKey(); LHS == EKey || RHS == EKey || LHS == TKey || RHS == TKey)
      return LHS == RHS;
    ...
    for (PHINode &Phi : Succ->phis()) {
      ...
      if (auto &PredIVs = (*LHS->PhiPredIVs)[&Phi]; PredIVs[A] != PredIVs[B])
        return false;
    }
...
bool SimplifyCFGOpt::simplifyDuplicateSwitchArms(SwitchInst *SI,
  ...
  for (auto &SSW : Cases) {
    ...
    if (const auto [It, Inserted] = ReplaceWith.insert(&SSW); !Inserted) {
...
bool SimplifyCFGOpt::simplifySwitch(SwitchInst *SI, IRBuilder<> &Builder) {
  ...
  if (BasicBlock *BB = SI->getParent(); isValueEqualityComparison(SI)) {
    ...
    if (Value *Cond = SI->getCondition(); SelectInst *Select = dyn_cast<SelectInst>(Cond))
...
bool SimplifyCFGOpt::simplifyIndirectBr(IndirectBrInst *IBI) {
  ...
  for (unsigned I = 0, E = IBI->getNumDestinations(); I != E; ++I) {
    ...
    if (BasicBlock *Dest = IBI->getDestination(I); !Dest->hasAddressTaken() || !Succs.insert(Dest).second) {
...
static bool tryToMergeLandingPad(LandingPadInst *LPad, BranchInst *BI,
  ...
  for (BasicBlock *OtherPred : predecessors(Succ)) {
    ...
    if (LandingPadInst *LPad2 = dyn_cast<LandingPadInst>(I); !LPad2 || !LPad2->isIdenticalTo(LPad))
      continue;
    ++I;
...
bool SimplifyCFGOpt::simplifyCondBranch(BranchInst *BI, IRBuilder<> &Builder) {
  ...
  if (isValueEqualityComparison(BI)) {
    ...
    if (auto I = BB->instructionsWithoutDebug(true).begin(); &*I == BI) {
  ...
  if (BI->getSuccessor(0)->getSinglePredecessor()) {
    if (BI->getSuccessor(1)->getSinglePredecessor()) {
      ...
      if (BI && Options.HoistLoadsStoresWithCondFaulting &&
        ...
        if (auto CanSpeculateConditionalLoadsStores = [&]() {
          for (auto *Succ : successors(BB)) {
        ...
        }; CanSpeculateConditionalLoadsStores()) {
      ...
      if (Instruction *Succ0TI = BI->getSuccessor(0)->getTerminator(); Succ0TI->getNumSuccessors() == 1 &&
    ...
    if (Instruction *Succ1TI = BI->getSuccessor(1)->getTerminator(); Succ1TI->getNumSuccessors() == 1 &&
...
static bool passingValueIsAlwaysUndefined(Value *V, Instruction *I, bool PtrValueMayBeModified) {
  ...
  if (C->isNullValue() || isa<UndefValue>(C)) {
    ...
    auto FindUse = llvm::find_if(I->uses(), [](auto &U) {
      ...
      switch (auto *Use = cast<Instruction>(U.getUser()); Use->getOpcode()) {
      default:
        return false;
  ...
  for (PHINode &PHI : BB->phis())
    for (unsigned i = 0, e = PHI.getNumIncomingValues(); i != e; ++i)
      if (passingValueIsAlwaysUndefined(PHI.getIncomingValue(i), &PHI)) {
        ...
        if (IRBuilder<> Builder(T); BranchInst *BI = dyn_cast<BranchInst>(T)) {
