...
static bool analyzeGlobalAux(const Value *V, GlobalStatus &GS,
  ...
  for (const Use &U : V->uses()) {
    ...
    if (const Constant *const User *UR = U.getUser(); C = dyn_cast<Constant>(UR)) {
      ...
      if (const LoadInst *LI = dyn_cast<LoadInst>(I)) {
        ...
        if (GS.StoredType != GlobalStatus::Stored) {
          ...
          if (const GlobalVariable *const Value *Ptr = SI->getPointerOperand()->stripPointerCasts(); GV = dyn_cast<GlobalVariable>(Ptr)) {
