...
int FunctionComparator::cmpConstants(const Constant *L,
  ...
  if (TypesRes != 0) {
    ...
    if (!TyLWidth) {
      ...
      if (PTyL && PTyR) {
        ...
        if (int unsigned AddrSpaceR = PTyR->getAddressSpace(); Res = cmpNumbers(AddrSpaceL, AddrSpaceR))
          return Res;
      }
      if (PTyL)
        return 1;
  ...
  case Value::ConstantArrayVal: {
    ...
    if (int uint64_t NumElementsR = cast<ArrayType>(TyR)->getNumElements(); Res = cmpNumbers(NumElementsL, NumElementsR))
      return Res;
  ...
  case Value::ConstantStructVal: {
    ...
    if (int unsigned NumElementsR = cast<StructType>(TyR)->getNumElements(); Res = cmpNumbers(NumElementsL, NumElementsR))
      return Res;
  ...
  case Value::ConstantVectorVal: {
    ...
    if (int unsigned NumElementsR = cast<FixedVectorType>(TyR)->getNumElements(); Res = cmpNumbers(NumElementsL, NumElementsR))
      return Res;
    for (uint64_t i = 0; i < NumElementsL; ++i) {
...
int FunctionComparator::cmpGEPs(const GEPOperator *GEPL,
  ...
  if (int unsigned int ASR = GEPR->getPointerAddressSpace(); Res = cmpNumbers(ASL, ASR))
    return Res;
  ...
  do {
    ...
    if (needToCmpOperands) {
      ...
      for (unsigned i = 0, e = InstL->getNumOperands(); i != e; ++i) {
        ...
        if (int Value *OpR = InstR->getOperand(i); Res = cmpValues(OpL, OpR))
          return Res;
