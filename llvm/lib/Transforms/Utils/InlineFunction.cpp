...
static BasicBlock *HandleCallsInBlockInlinedThroughInvoke(
  ...
  for (Instruction &I : llvm::make_early_inc_range(*BB)) {
    ...
    if (auto FuncletBundle = CI->getOperandBundle(LLVMContext::OB_funclet)) {
      ...
      if (Value *UnwindDestToken =
          getUnwindDestToken(FuncletPad, *FuncletUnwindMap); UnwindDestToken && !isa<ConstantTokenNone>(UnwindDestToken))
        continue;
#ifndef NDEBUG
      Instruction *MemoKey;
...
static void AddParamAndFnBasicAttributes(const CallBase &CB,
  ...
  for (unsigned I = 0, E = CB.arg_size(); I < E; ++I) {
    ...
    for (Attribute::AttrKind AK : ExactAttrsToPropagate) {
      ...
      if (Attribute Attr = CB.getParamAttr(I, AK); Attr.isValid())
  ...
  for (auto &BB : *CalledFunction) {
    ...
    if (ValidPG.hasAttributes()) {
      ...
      if (CBRange.isValid()) {
        ...
        if (Attribute NewRange = AL.getRetAttr(Attribute::Range); NewRange.isValid()) {
...
static void updateCallerBFI(BasicBlock *CallSiteBlock,
  ...
  for (auto Entry : VMap) {
    ...
    if (!ClonedBBs.insert(ClonedBB).second) {
      ...
      if (BlockFrequency NewFreq = CallerBFI->getBlockFreq(ClonedBB); NewFreq > Freq)
        Freq = NewFreq;
    }
...
llvm::InlineResult llvm::CanInlineCallSite(const CallBase &CB,
  ...
  if (CallerPersonality) {
    ...
    if (EHPersonality Personality = classifyEHPersonality(CallerPersonality); isScopedEHPersonality(Personality)) {
...
void llvm::InlineFunctionImpl(CallBase &CB, InlineFunctionInfo &IFI,
  ...
  { // Scope to destroy VMap after cloning.
    ...
    if (objcarc::ARCInstKind RVCallKind = objcarc::getAttachedARCFunctionKind(&CB); RVCallKind != objcarc::ARCInstKind::None)
  ...
  if (auto *II = dyn_cast<InvokeInst>(&CB)) {
    ...
    if (BasicBlock::iterator FirstNonPHI = UnwindDest->getFirstNonPHIIt(); isa<LandingPadInst>(FirstNonPHI)) {
  ...
  if (PHI) {
    ...
    if (Value *auto &DL = Caller->getDataLayout(); V = simplifyInstruction(PHI, {DL, nullptr, nullptr, AC})) {
