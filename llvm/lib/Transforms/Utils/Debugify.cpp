...
bool llvm::applyDebugifyMetadata(
  ...
  if (StringRef DIVersionKey = "Debug Info Version"; !M.getModuleFlag(DIVersionKey))
...
static bool checkInstructions(const DebugInstMap &DILocsBefore,
  ...
  for (const auto &L : DILocsAfter) {
    ...
    if (auto WeakInstrPtr = InstToDelete.find(Instr); WeakInstrPtr != InstToDelete.end() && !WeakInstrPtr->second)
      continue;
