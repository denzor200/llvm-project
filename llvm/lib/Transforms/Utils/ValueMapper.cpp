...
void Mapper::flush() {
  ...
  while (!Worklist.empty()) {
    ...
    case WorklistEntry::MapAliasOrIFunc: {
      ...
      if (Constant *Target = mapConstant(E.Data.AliasOrIFunc.Target); auto *GA = dyn_cast<GlobalAlias>(GV))
  ...
  if (auto *CB = dyn_cast<CallBase>(I)) {
    ...
    for (unsigned i = 0; i < Attrs.getNumAttrSets(); ++i) {
      for (int AttrIdx = Attribute::FirstTypeAttr;
        ...
        if (Attribute::AttrKind TypedAttr = (Attribute::AttrKind)AttrIdx; Type *Ty =
