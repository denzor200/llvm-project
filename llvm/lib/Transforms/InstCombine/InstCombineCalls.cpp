...
Instruction *InstCombinerImpl::visitCallInst(CallInst &CI) {
  ...
  if (!CI.use_empty()) {
    ...
    if (SmallVector<Value *, 8> Args(CI.args()); Value *V = simplifyCall(&CI, CI.getCalledOperand(), Args,
  ...
  if (auto *IIFVTy = dyn_cast<FixedVectorType>(II->getType())) {
    ...
    if (APInt AllOnesEltMask(APInt::getAllOnes(VWidth)); Value *V = SimplifyDemandedVectorElts(II, AllOnesEltMask, PoisonElts)) {
      if (V != II)
  ...
  case Intrinsic::smin: {
    ...
    if ((match(I0, m_c_Xor(m_Specific(I1), m_Value(X))) ||
      ...
      if (IID == Intrinsic::smax || IID == Intrinsic::smin) {
        ...
        if (auto KnownSign = getKnownSign(X, SQ.getWithInstruction(II)); KnownSign == std::nullopt) {
          UseOr = false;
          UseAndN = false;
    ...
    if (match(I1, m_APIntAllowPoison(RHSC))) {
      ...
      if (ConstantRange LHS_CR = computeConstantRangeIncludingKnownBits(
          I0, IsSigned, SQ.getWithInstruction(II)); !LHS_CR.isFullSet()) {
