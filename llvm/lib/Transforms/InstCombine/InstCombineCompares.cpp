...
Instruction *InstCombinerImpl::foldICmpAndConstConst(ICmpInst &Cmp,
  ...
  if (!Cmp.getParent()->getParent()->hasFnAttribute(
    ...
    if (Type *FPType = V->getType()->getScalarType(); FPType->isIEEELikeFPTy() && (C1.isZero() || C1 == *C2)) {
      ...
      if (APInt ExponentMask =
          APFloat::getInf(FPType->getFltSemantics()).bitcastToAPInt(); *C2 == ExponentMask) {
...
Instruction *InstCombinerImpl::foldICmpShrConstant(ICmpInst &Cmp,
  ...
  if (IsAShr && Shr->hasOneUse()) {
    ...
    if (Pred == CmpInst::ICMP_SGT) {
      ...
      if (APInt ShiftedC = (C + 1).shl(ShAmtVal) - 1; !C.isMaxSignedValue() && !(C + 1).shl(ShAmtVal).isMinSignedValue() &&
    ...
    if (Pred == CmpInst::ICMP_UGT) {
      ...
      if (APInt ShiftedC = (C + 1).shl(ShAmtVal) - 1; (ShiftedC + 1).ashr(ShAmtVal) == (C + 1) ||
    ...
    if (Pred == CmpInst::ICMP_UGT) {
      ...
      if (APInt ShiftedC = (C + 1).shl(ShAmtVal) - 1; (ShiftedC + 1).lshr(ShAmtVal) == (C + 1))
        return new ICmpInst(Pred, X, ConstantInt::get(ShrTy, ShiftedC));
    }
  }
...
Instruction *InstCombinerImpl::foldICmpBinOpWithConstantViaTruthTable(
  ...
  auto ComputeTable = [&](bool First, bool Second) -> std::optional<bool> {
    ...
    if (Constant *R = Second ? C3 : C4; auto *Res = ConstantFoldBinaryOpOperands(BO->getOpcode(), L, R, DL)) {
      
      if (auto *Val = Res->getType()->isVectorTy() ? Res->getSplatValue() : Res; auto *CI = dyn_cast_or_null<ConstantInt>(Val))
  ...
  for (unsigned I = 0; I < 4; ++I) {
    ...
    if (bool Second = I & 1; auto Res = ComputeTable(First, Second))
      Table[I] = *Res;
    else
      return nullptr;
  }
  ...
  if (SrcType->isVectorTy() == DstType->isVectorTy() &&
    ...
    if (match(Op1, m_APInt(C)) && Bitcast->hasOneUse()) {
      ...
      if (Type *FPType = SrcType->getScalarType(); !Cmp.getParent()->getParent()->hasFnAttribute(
        ...
        if (FPClassTest Mask = APFloat(FPType->getFltSemantics(), *C).classify(); Mask & (fcInf | fcZero)) {
...
InstCombinerImpl::foldICmpInstWithConstantAllowPoison(ICmpInst &Cmp,
  ...
  if (const ICmpInst::Predicate Pred = Cmp.getPredicate(); auto *II = dyn_cast<IntrinsicInst>(Cmp.getOperand(0))) {
...
Instruction *InstCombinerImpl::foldICmpIntrinsicWithConstant(ICmpInst &Cmp,
  ...
  case Intrinsic::ctpop: {
    ...
    if (const SimplifyQuery Q = SQ.getWithInstruction(&Cmp); Instruction *R = foldCtpopPow2Test(Cmp, II, C, Builder, Q))
      return R;
  } break;
  case Intrinsic::scmp:
  case Intrinsic::ucmp:
...
Instruction *InstCombinerImpl::foldICmpBinOp(ICmpInst &I,
  ...
  if (BO0) {
    ...
    if (auto BitwiseAnd = m_c_And(m_Value(), LSubOne); match(BO0, BitwiseAnd) && Pred == ICmpInst::ICMP_ULT) {
...
Instruction *InstCombinerImpl::foldICmpEquality(ICmpInst &I) {
  ...
  if (match(Op0, m_And(m_Value(A), m_Value(B))) &&
    ...
    if (X) {
      ...
      if (int UseCnt =
          ...
          (int(match(X, m_ImmConstant()) && match(Y, m_ImmConstant()))); XorIsNegP2 || UseCnt >= 2) {
        // Build (X^Y) & Z
  ...
  if (Op0->hasOneUse() &&
    ...
    if (unsigned ASize = cast<IntegerType>(A->getType())->getPrimitiveSizeInBits(); ShAmt < ASize) {
      APInt MaskV =
  ...
  if (MulVal->hasNUsesOrMore(2))
    for (User *U : MulVal->users()) {
      ...
      if (TruncInst *TI = dyn_cast<TruncInst>(U)) {
        ...
        if (unsigned TruncWidth = TI->getType()->getPrimitiveSizeInBits(); TruncWidth > MulWidth)
          return nullptr;
  ...
  case ICmpInst::ICMP_NE: {
    ...
    if (Op1Known.isZero()) {
      ...
      if (match(LHS, m_Shl(m_Power2(C1), m_Value(X)))) {
        ...
        if (APInt C2Pow2 = (C2 & ~(*C1 - 1)) + *C1; C2Pow2.isPowerOf2()) {
...
Instruction *InstCombinerImpl::foldFCmpIntToFPConst(FCmpInst &I,
  ...
  if ((int)IntWidth > MantissaWidth) {
    ...
    if (int Exp = ilogb(*RHS); Exp == APFloat::IEK_Inf) {
      ...
      if (int MaxExponent = ilogb(APFloat::getLargest(RHS->getSemantics())); MaxExponent < (int)IntWidth - !LHSUnsigned)
...
static Instruction *foldFabsWithFcmpZero(FCmpInst &I, InstCombinerImpl &IC) {
  ...
  if (!C->isPosZero()) {
    ...
    if (Mode.Input == DenormalMode::PreserveSign ||
      ...
      switch (auto replaceFCmp = [](FCmpInst *I, FCmpInst::Predicate P, Value *X) {
