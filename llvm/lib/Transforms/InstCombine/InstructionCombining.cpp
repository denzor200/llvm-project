  ...
  do {
    ...
    if (I.isAssociative()) {
      ...
      if (Op0 && Op0->getOpcode() == Opcode) {
        ...
        if (Value *Value *C = I.getOperand(1); V = simplifyBinOp(Opcode, B, C, SQ.getWithInstruction(&I))) {
      ...
      if (Op1 && Op1->getOpcode() == Opcode) {
        ...
        if (Value *Value *C = Op1->getOperand(1); V = simplifyBinOp(Opcode, A, B, SQ.getWithInstruction(&I))) {
    ...
    if (I.isAssociative() && I.isCommutative()) {
      ...
      if (Op0 && Op0->getOpcode() == Opcode) {
        ...
        if (Value *Value *C = I.getOperand(1); V = simplifyBinOp(Opcode, C, A, SQ.getWithInstruction(&I))) {
      ...
      if (Op1 && Op1->getOpcode() == Opcode) {
        ...
        // Does "C op A" simplify?
        if (Value *Value *C = Op1->getOperand(1); V = simplifyBinOp(Opcode, C, A, SQ.getWithInstruction(&I))) {
      ...
      if (Constant *C1, *C2, *CRes; Op0 && Op1 &&
...
getBinOpsForFactorization(Instruction::BinaryOps TopOpcode, BinaryOperator *Op,
  ...
  if (TopOpcode == Instruction::Add || TopOpcode == Instruction::Sub) {
    ...
    if (Constant *C; match(Op, m_Shl(m_Value(), m_ImmConstant(C)))) {
      // X << C --> X * (1 << C)
...
static Value *tryFactorization(BinaryOperator &I, const SimplifyQuery &SQ,
  ...
  if (isa<BinaryOperator>(RetVal)) {
    ...
    if (TopLevelOpcode == Instruction::Add && InnerOpcode == Instruction::Mul) {
      ...
      if (const APInt *CInt; match(V, m_APInt(CInt)) && !CInt->isMinSignedValue())
...
InstCombinerImpl::matchSymmetricPair(Value *LHS, Value *RHS) {
  ...
  case Instruction::Select: {
    ...
    if (Value *FalseVal = LHSInst->getOperand(2); Cond == RHSInst->getOperand(0) && TrueVal == RHSInst->getOperand(2) &&
...
Value *InstCombinerImpl::dyn_castNegVal(Value *V) const {
  ...
  if (Value *NegV; match(V, m_Neg(m_Value(NegV))))
    return NegV;
...
static Value *simplifyInstructionWithPHI(Instruction &I, PHINode *PN,
  ...
  if (TerminatorBI && TerminatorBI->isConditional() &&
    ...
    if (std::optional<bool> ImpliedCond = isImpliedCondition(
        TerminatorBI->getCondition(), ICmp->getCmpPredicate(), Ops[0], Ops[1],
        DL, LHSIsTrue); ImpliedCond)
...
Instruction *InstCombinerImpl::foldOpIntoPhi(Instruction &I, PHINode *PN,
  ...
  for (unsigned i = 0; i != NumPHIValues; ++i) {
    ...
    auto WillFold = [&]() {
      ...
      if (const APInt *Ignored; isa<CmpIntrinsic>(InVal) &&
...
Instruction *InstCombinerImpl::foldBinopWithPhiOperands(BinaryOperator &BO) {
  ...
  if (C) {
    ...
    if (auto CanFoldIncomingValuePair = [&](std::tuple<Use &, Use &> T) {
      ...
      if (Phi0UseV == C)
        ...
        return false;
      return true;
    }; all_of(zip(Phi0->operands(), Phi1->operands()),
...
Constant *InstCombinerImpl::unshuffleConstant(ArrayRef<int> ShMask, Constant *C,
  ...
  for (unsigned I = 0; I < NumElts; ++I) {
    ...
    if (Constant *CElt = C->getAggregateElement(I); ShMask[I] >= 0) {
      ...
      if (Constant *NewCElt = NewVecC[ShMask[I]]; !CElt || (!isa<PoisonValue>(NewCElt) && NewCElt != CElt) ||
          I >= NewCNumElts)
        return nullptr;
...
Instruction *InstCombinerImpl::foldVectorBinop(BinaryOperator &Inst) {
  ...
  if (match(&Inst, m_c_BinOp(m_OneUse(m_Shuffle(m_Value(V1), m_Poison(),
    ...
    if (Constant *bool ConstOp1 = isa<Constant>(RHS); NewC =
...
Instruction *InstCombinerImpl::visitGetElementPtrInst(GetElementPtrInst &GEP) {
  ...
  if (auto *GEPFVTy = dyn_cast<FixedVectorType>(GEPType)) {
    ...
    if (Value *APInt AllOnesEltMask(APInt::getAllOnes(VWidth)); V = SimplifyDemandedVectorElts(&GEP, AllOnesEltMask,
                                              PoisonElts)) {
      if (V != &GEP)
        return replaceInstUsesWith(GEP, V);
      return &GEP;
  ...
  if (!GEPEltType->isIntegerTy(8) && GEP.hasAllConstantIndices()) {
    ...
    if (APInt Offset(DL.getIndexTypeSizeInBits(GEPType), 0); GEP.accumulateConstantOffset(DL, Offset))
      return replaceInstUsesWith(
  ...
  if (GEP.getNumIndices() == 1) {
    ...
    if (unsigned AS = GEP.getPointerAddressSpace(); GEP.getOperand(1)->getType()->getScalarSizeInBits() ==
      ...
      if (TyAllocSize == 1) {
        ...
        if (Value *Y; match(GEP.getOperand(1), m_Sub(m_PtrToIntOrAddr(m_Value(Y)),
  ...
  if (!GEP.isInBounds()) {
    ...
    if (uint64_t DerefBytes = UnderlyingPtrOp->getPointerDereferenceableBytes(
      ...
      if (GEP.accumulateConstantOffset(DL, BasePtrOffset) &&
        ...
        if (APInt AllocSize(IdxWidth, DerefBytes); BasePtrOffset.ule(AllocSize)) {
...
isAllocSiteRemovable(Instruction *AI, SmallVectorImpl<WeakTrackingVH> &Users,
  ...
  do {
    ...
    for (User *U : PI->users()) {
      ...
      switch (Instruction *I = cast<Instruction>(U); I->getOpcode()) {
      default:
...
Instruction *InstCombinerImpl::visitAllocSite(Instruction &MI) {
  ...
  if (Removable) {
    for (WeakTrackingVH &User : Users) {
      ...
      if (IntrinsicInst *Instruction *I = cast<Instruction>(&*User); II = dyn_cast<IntrinsicInst>(I)) {
...
Instruction *InstCombinerImpl::visitFree(CallInst &FI, Value *Op) {
  ...
  if (MinimizeSize) {
    ...
    if (LibFunc Func; TLI.getLibFunc(FI, Func) && TLI.has(Func) && Func == LibFunc_free)
...
Instruction *InstCombinerImpl::visitSwitchInst(SwitchInst &SI) {
  ...
  if (ConstantInt *AddRHS; match(Cond, m_Add(m_Value(Op0), m_ConstantInt(AddRHS)))) {
...
Instruction *InstCombinerImpl::visitExtractValueInst(ExtractValueInst &EV) {
  ...
  if (match(&EV, m_ExtractValue<0>(m_Intrinsic<Intrinsic::frexp>(m_Select(
    ...
    if (Value *auto *SelInst =
        cast<SelectInst>(cast<IntrinsicInst>(Agg)->getArgOperand(0)); Result =
...
InstCombinerImpl::pushFreezeToPreventPoisonFromPropagating(FreezeInst &OrigFI) {
  ...
  while (!Worklist.empty()) {
    ...
    for (Use &Op : reverse(I->operands())) {
      ...
      if (Value *OpV = Op.get(); isa<MetadataAsValue>(OpV) || isGuaranteedNotToBeUndefOrPoison(OpV))
        continue;
...
void InstCombinerImpl::tryToSinkInstructionDbgVariableRecords(
  ...
  for (DbgVariableRecord *DVR : DbgVariableRecordsToSink) {
    ...
    if (!FilterOutMap.empty()) {
      ...
      // Filter out.
      if (auto It = FilterOutMap.find(IVP); It != FilterOutMap.end() && It->second != DVR)
        continue;
    }
