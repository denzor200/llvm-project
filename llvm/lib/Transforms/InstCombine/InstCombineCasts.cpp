...
Instruction *InstCombinerImpl::commonCastTransforms(CastInst &CI) {
  ...
  if (auto *Sel = dyn_cast<SelectInst>(Src)) {
    ...
    if (auto *Cmp = dyn_cast<CmpInst>(Sel->getCondition()); !Cmp || Cmp->getOperand(0)->getType() != Sel->getType() ||
  ...
  if (match(Src, m_OneUse(m_Shuffle(m_Value(X), m_Undef(), m_Mask(Mask))))) {
    ...
    if (auto *DestTy = dyn_cast<FixedVectorType>(Ty); SrcTy && DestTy &&
...
bool TypeEvaluationHelper::canAlwaysEvaluateInType(Value *V, Type *Ty) {
  ...
  if (Value *X; (match(V, m_ZExtOrSExt(m_Value(X))) || match(V, m_Trunc(m_Value(X)))) &&
      X->getType() == Ty)
    return true;
...
bool TypeEvaluationHelper::canEvaluateTruncatedPred(Value *V, Type *Ty,
  ...
  switch (Type *OrigTy = V->getType(); I->getOpcode()) {
  case Instruction::Add:
  case Instruction::Sub:
  case Instruction::Mul:
  case Instruction::And:
  ...
  case Instruction::URem: {
    ...
    if (APInt Mask = APInt::getBitsSetFrom(OrigBitWidth, BitWidth); IC.MaskedValueIsZero(I->getOperand(0), Mask, I) &&
  ...
  case Instruction::Shl: {
    ...
    if (KnownBits AmtKnownBits =
        llvm::computeKnownBits(I->getOperand(1), IC.getDataLayout()); AmtKnownBits.getMaxValue().ult(BitWidth))
      return canEvaluateTruncatedImpl(I->getOperand(0), Ty, IC, CxtI) &&
             canEvaluateTruncatedImpl(I->getOperand(1), Ty, IC, CxtI);
    break;
  }
  case Instruction::LShr: {
    ...
    if (APInt ShiftedBits = APInt::getBitsSetFrom(OrigBitWidth, BitWidth); MaxShiftAmt.ult(BitWidth)) {
      ...
      if (auto *Trunc = dyn_cast<TruncInst>(V->user_back())) {
        ...
        if (auto DemandedBits = Trunc->getType()->getScalarSizeInBits(); (MaxShiftAmt + DemandedBits).ule(BitWidth))
          return canEvaluateTruncatedImpl(I->getOperand(0), Ty, IC, CxtI) &&
                 canEvaluateTruncatedImpl(I->getOperand(1), Ty, IC, CxtI);
      }
  ...
  case Instruction::AShr: {
    ...
    if (unsigned ShiftedBits = OrigBitWidth - BitWidth; AmtKnownBits.getMaxValue().ult(BitWidth) &&
...
Instruction *InstCombinerImpl::narrowFunnelShift(TruncInst &Trunc) {
  ...
  auto matchShiftAmount = [&](Value *L, Value *R, unsigned Width) -> Value * {
    ...
    if (APInt HiBitMask = ~APInt::getLowBitsSet(WideWidth, MaxShiftAmountWidth); ShVal0 == ShVal1 || MaskedValueIsZero(L, HiBitMask))
...
Instruction *InstCombinerImpl::narrowBinOp(TruncInst &Trunc) {
  ...
  switch (Value *BinOp1 = BinOp->getOperand(1); BinOp->getOpcode()) {
  case Instruction::And:
  case Instruction::Or:
  case Instruction::Xor:
  case Instruction::Add:
  ...
  case Instruction::AShr: {
    ...
    if (Constant *C; match(BinOp0, m_Trunc(m_Value(A))) && match(BinOp1, m_Constant(C))) {
      
      // If the shift is small enough, all zero/sign bits created by the shift
      // are removed by the trunc.
      if (unsigned MaxShiftAmt = SrcWidth - DestWidth; match(C, m_SpecificInt_ICMP(ICmpInst::ICMP_ULE,
                                      APInt(SrcWidth, MaxShiftAmt)))) {
        auto *OldShift = cast<Instruction>(Trunc.getOperand(0));
        
        if (Constant *bool IsExact = OldShift->isExact(); ShAmt = ConstantFoldIntegerCast(C, A->getType(),
                                                      /*IsSigned*/ true, DL)) {
...
static Instruction *shrinkSplatShuffle(TruncInst &Trunc,
  ...
  if (auto *Shuf = dyn_cast<ShuffleVectorInst>(Trunc.getOperand(0)); Shuf && Shuf->hasOneUse() && match(Shuf->getOperand(1), m_Undef()) &&
...
Instruction *InstCombinerImpl::visitTrunc(TruncInst &Trunc) {
  ...
  if (auto *DestITy = dyn_cast<IntegerType>(DestTy)) {
    if (DestWidth * 2 < SrcWidth) {
      ...
      if (auto *NewDestTy = DestITy->getExtendedType(); shouldChangeType(SrcTy, NewDestTy) &&
  ...
  if (DestWidth == 1) {
    ...
    if (Constant *C2; match(Src, m_OneUse(m_Shr(m_Shl(m_Power2(C1), m_Value(X)),
                                  m_ImmConstant(C2))))) {
...

    {
      ...
      if (const APInt *C; match(Src, m_Shl(m_APInt(C), m_Value(X))) && (*C)[0] == 1) {
    ...
    if (Trunc.hasNoUnsignedWrap() || Trunc.hasNoSignedWrap()) {
      ...
      if (Value *X, *Y; match(Src, m_Xor(m_Value(X), m_Value(Y))))
  ...
  if (match(Src, m_LShr(m_SExt(m_Value(A)), m_Constant(C)))) {
    ...
    if (bool IsExact = OldSh->isExact(); match(C, m_SpecificInt_ICMP(ICmpInst::ICMP_ULE,
                                    APInt(SrcWidth, MaxShiftAmt)))) {
  ...
  if (Src->hasOneUse() &&
    ...
    if (match(Src, m_Shl(m_Value(A), m_Constant(C))) &&
      ...
      if (APInt Threshold = APInt(C->getType()->getScalarSizeInBits(), DestWidth); match(C, m_SpecificInt_ICMP(ICmpInst::ICMP_ULT, Threshold))) {
  ...
  if (match(Src, m_OneUse(m_Intrinsic<Intrinsic::ctlz>(m_ZExt(m_Value(A)),
    ...
    if (unsigned AWidth = A->getType()->getScalarSizeInBits(); AWidth == DestWidth && AWidth > Log2_32(SrcWidth)) {
  ...
  if (match(Src, m_VScale())) {
    if (Trunc.getFunction() &&
      ...
      if (std::optional<unsigned> Attribute Attr =
          Trunc.getFunction()->getFnAttribute(Attribute::VScaleRange); MaxVScale = Attr.getVScaleRangeMax())
...
Instruction *InstCombinerImpl::transformZExtICmp(ICmpInst *Cmp,
  ...
  if (const APInt *Op1CV; match(Cmp->getOperand(1), m_APInt(Op1CV))) {
    ...
    if (Op1CV->isZero() && Cmp->isEquality()) {
      ...
      if (bool IsExpectShAmt = KnownZeroMask.isPowerOf2() &&
                           (Zext.getType()->getScalarSizeInBits() != ShAmt + 1); IsExpectShAmt &&
  ...
  if (Cmp->isEquality()) {
    ...
    if (Value *X, *ShAmt; Cmp->hasOneUse() && match(Cmp->getOperand(1), m_ZeroInt()) &&
        match(Cmp->getOperand(0),
              m_OneUse(m_c_And(m_Shl(m_One(), m_Value(ShAmt)), m_Value(X))))) {
      auto *And = cast<BinaryOperator>(Cmp->getOperand(0));
      
      if (Value *Shift = And->getOperand(X == And->getOperand(0) ? 1 : 0); Zext.getType() == And->getType() ||
...
bool TypeEvaluationHelper::canEvaluateZExtdImpl(Value *V, Type *Ty,
  ...
  switch (unsigned Tmp; I->getOpcode()) {
  ...
  case Instruction::Mul:
    ...
    if (Tmp == 0 && I->isBitwiseLogicOp()) {
      ...
      if (unsigned VSize = V->getType()->getScalarSizeInBits(); IC.MaskedValueIsZero(I->getOperand(1),
  ...
  case Instruction::Shl: {
    ...
    if (uint64_t ShiftAmt; match(I->getOperand(1), m_ConstantInt(ShiftAmt))) {
      if (!canEvaluateZExtdImpl(I->getOperand(0), Ty, BitsToClear, IC, CxtI))
        return false;
...
Instruction *InstCombinerImpl::visitZExt(ZExtInst &Zext) {
  ...
  if (match(Src, m_VScale())) {
    if (Zext.getFunction() &&
      ...
      if (std::optional<unsigned> Attribute Attr =
          Zext.getFunction()->getFnAttribute(Attribute::VScaleRange); MaxVScale = Attr.getVScaleRangeMax()) {
        
        if (unsigned TypeWidth = Src->getType()->getScalarSizeInBits(); Log2_32(*MaxVScale) < TypeWidth)
...
Instruction *InstCombinerImpl::transformSExtICmp(ICmpInst *Cmp,
  ...
  if (ConstantInt *Op1C = dyn_cast<ConstantInt>(Op1)) {
    ...
    if (Cmp->hasOneUse() &&
      ...
      if (APInt KnownZeroMask(~Known.Zero); KnownZeroMask.isPowerOf2()) {
        Value *In = Cmp->getOperand(0);
        ...
        if (!Op1C->isZero() == (Pred == ICmpInst::ICMP_NE)) {
          ...
          if (unsigned ShiftAmt = KnownZeroMask.countr_zero(); ShiftAmt)
          ...
          if (unsigned ShiftAmt = KnownZeroMask.countl_zero(); ShiftAmt)
...
bool TypeEvaluationHelper::canEvaluateSExtdPred(Value *V, Type *Ty) {
  ...
  switch (auto *I = cast<Instruction>(V); I->getOpcode()) {
...
Instruction *InstCombinerImpl::visitSExt(SExtInst &Sext) {
  ...
  if (X) {
    ...
    if (bool HasNSW = cast<TruncInst>(Src)->hasNoSignedWrap(); HasNSW || (ComputeNumSignBits(X, &Sext) > XBitSize - SrcBitSize)) {
  ...
  if (match(Src, m_VScale())) {
    if (Sext.getFunction() &&
      ...
      if (std::optional<unsigned> Attribute Attr =
          Sext.getFunction()->getFnAttribute(Attribute::VScaleRange); MaxVScale = Attr.getVScaleRangeMax())
...
Instruction *InstCombinerImpl::visitFPTrunc(FPTruncInst &FPT) {
  ...
  if (auto *BO = dyn_cast<BinaryOperator>(FPT.getOperand(0)); BO && BO->hasOneUse()) {
    ...
    switch (unsigned DstWidth = Ty->getFPMantissaWidth(); BO->getOpcode()) {
      default: break;
  ...
  if (auto *II = dyn_cast<IntrinsicInst>(FPT.getOperand(0))) {
    ...
    case Intrinsic::trunc: {
      ...
      if (II->getIntrinsicID() != Intrinsic::fabs) {
        ...
        if (FPExtInst *FPExtSrc = dyn_cast<FPExtInst>(Src); !FPExtSrc || FPExtSrc->getSrcTy() != Ty)
          break;
      }
  ...
  if (isa<SIToFPInst>(Src) || isa<UIToFPInst>(Src)) {
    ...
    if (auto *FPCast = cast<CastInst>(Src); isKnownExactCastIntToFP(*FPCast, *this))
...
Instruction *InstCombinerImpl::visitFPExt(CastInst &FPExt) {
  ...
  if (Value *Src = FPExt.getOperand(0); isa<SIToFPInst>(Src) || isa<UIToFPInst>(Src)) {
...
Instruction *InstCombinerImpl::foldItoFPtoI(CastInst &FI) {
  ...
  if (!isKnownExactCastIntToFP(*OpI, *this)) {
    ...
    if (int OutputSize = (int)DestType->getScalarSizeInBits(); OutputSize > OpI->getType()->getFPMantissaWidth())
      return nullptr;
  }

  if (DestType->getScalarSizeInBits() > XType->getScalarSizeInBits()) {
    
    if (bool IsInputSigned = isa<SIToFPInst>(OpI); IsInputSigned && IsOutputSigned)
...
Instruction *InstCombinerImpl::visitIntToPtr(IntToPtrInst &CI) {
  ...
  if (unsigned AS = CI.getAddressSpace(); CI.getOperand(0)->getType()->getScalarSizeInBits() !=
...
Value *InstCombinerImpl::foldPtrToIntOrAddrOfGEP(Type *IntTy, Value *Ptr) {
  ...
  if (Type *IdxTy = DL.getIndexType(PtrTy); match(Ptr, m_OneUse(m_IntToPtr(m_Value(Res)))) &&
...
Instruction *InstCombinerImpl::visitPtrToInt(PtrToIntInst &CI) {
  ...
  if (unsigned PtrSize = DL.getPointerSizeInBits(AS); TySize != PtrSize) {
    Type *IntPtrTy =
...
Instruction *InstCombinerImpl::visitPtrToAddr(PtrToAddrInst &CI) {
  ...
  if (Value *Ptr, *Mask; match(SrcOp, m_OneUse(m_Intrinsic<Intrinsic::ptrmask>(m_Value(Ptr),
                                                            m_Value(Mask)))) &&
      Mask->getType() == Ty)
...
optimizeVectorResizeWithIntegerBitCasts(Value *InVal, VectorType *DestTy,
  ...
  if (SrcElts > DestElts) {
    ...
    if (unsigned DeltaElts = DestElts - SrcElts; IsBigEndian)
...
static Instruction *foldBitCastBitwiseLogic(BitCastInst &BitCast,
  ...
  if (DestTy->isFPOrFPVectorTy()) {
    ...
    if (Value *X, *Y; match(BO->getOperand(0), m_OneUse(m_BitCast(m_Value(X)))) &&
  ...
  while (!PhiWorklist.empty()) {
    ...
    for (Value *IncValue : OldPN->incoming_values()) {
      ...
      if (auto *LI = dyn_cast<LoadInst>(IncValue)) {
        ...
        if (Value *Addr = LI->getOperand(0); Addr == &CI || isa<LoadInst>(Addr))
          return nullptr;
  ...
  for (auto *OldPN : OldPhiNodes) {
    for (User *V : OldPN->users()) {
      if (auto *SI = dyn_cast<StoreInst>(V)) {
        ...
        if (Type *TyA = BCI->getType(); TyA != DestTy || TyB != SrcTy)
          return nullptr;
  ...
  for (auto *OldPN : OldPhiNodes) {
    ...
    for (User *V : make_early_inc_range(OldPN->users())) {
      if (auto *SI = dyn_cast<StoreInst>(V)) {
        ...
        if (Instruction *I = replaceInstUsesWith(*BCI, NewPN); BCI == &CI)
          RetVal = I;
      } else if (auto *PHI = dyn_cast<PHINode>(V)) {
...
Instruction *InstCombinerImpl::visitBitCast(BitCastInst &CI) {
  ...
  if (isa<FixedVectorType>(DestTy)) {
    if (isa<IntegerType>(SrcTy)) {
      ...
      if (isa<TruncInst>(Src) || isa<ZExtInst>(Src)) {
        ...
        if (BitCastInst *CastInst *SrcCast = cast<CastInst>(Src); BCIn = dyn_cast<BitCastInst>(SrcCast->getOperand(0)))
  ...
  if (FixedVectorType *SrcVTy = dyn_cast<FixedVectorType>(SrcTy)) {
    ...
    if (uint64_t IndexC; match(Src, m_OneUse(m_InsertElt(m_OneUse(m_BitCast(m_Value(X))),
  ...
  if (auto *Shuf = dyn_cast<ShuffleVectorInst>(Src)) {
    ...
    if (Shuf->hasOneUse() && DestTy->isVectorTy() &&
      ...
      if (BitCastInst *Tmp; ((Tmp = dyn_cast<BitCastInst>(ShufOp0)) &&
