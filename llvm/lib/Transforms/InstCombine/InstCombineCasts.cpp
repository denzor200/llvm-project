  ...
  case Instruction::AShr: {
    ...
    if (match(BinOp0, m_Trunc(m_Value(A))) && match(BinOp1, m_Constant(C))) {
      ...
      if (unsigned MaxShiftAmt = SrcWidth - DestWidth; match(C, m_SpecificInt_ICMP(ICmpInst::ICMP_ULE,
        ...
        if (bool IsExact = OldShift->isExact(); Constant *ShAmt = ConstantFoldIntegerCast(C, A->getType(),
                                                      /*IsSigned*/ true, DL)) {
...
Instruction *InstCombinerImpl::visitTrunc(TruncInst &Trunc) {
  ...
  if (match(Src, m_VScale())) {
    if (Trunc.getFunction() &&
      ...
      if (Attribute Attr =
          Trunc.getFunction()->getFnAttribute(Attribute::VScaleRange); std::optional<unsigned> MaxVScale = Attr.getVScaleRangeMax())
...
Instruction *InstCombinerImpl::visitZExt(ZExtInst &Zext) {
  ...
  if (match(Src, m_VScale())) {
    if (Zext.getFunction() &&
      ...
      if (Attribute Attr =
          Zext.getFunction()->getFnAttribute(Attribute::VScaleRange); std::optional<unsigned> MaxVScale = Attr.getVScaleRangeMax()) {
...
Instruction *InstCombinerImpl::visitSExt(SExtInst &Sext) {
  ...
  if (match(Src, m_VScale())) {
    if (Sext.getFunction() &&
      ...
      if (Attribute Attr =
          Sext.getFunction()->getFnAttribute(Attribute::VScaleRange); std::optional<unsigned> MaxVScale = Attr.getVScaleRangeMax())
  ...
  if (isa<FixedVectorType>(DestTy)) {
    if (isa<IntegerType>(SrcTy)) {
      ...
      if (isa<TruncInst>(Src) || isa<ZExtInst>(Src)) {
        ...
        if (CastInst *SrcCast = cast<CastInst>(Src); BitCastInst *BCIn = dyn_cast<BitCastInst>(SrcCast->getOperand(0)))
