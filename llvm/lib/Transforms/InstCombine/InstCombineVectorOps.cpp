...
static bool collectSingleShuffleElements(Value *V, Value *LHS, Value *RHS,
  ...
  if (InsertElementInst *IEI = dyn_cast<InsertElementInst>(V)) {
    ...
    if (unsigned InsertedIdx = cast<ConstantInt>(IdxOp)->getZExtValue(); isa<PoisonValue>(ScalarOp)) {  // inserting poison into vector.
...
static Instruction *foldTruncInsEltPair(InsertElementInst &InsElt,
  ...
  if (unsigned VecEltWidth = VTy->getScalarSizeInBits(); ScalarWidth != VecEltWidth * 2 || ShAmt != VecEltWidth)
    return nullptr;
...
Instruction *InstCombinerImpl::visitInsertElementInst(InsertElementInst &IE) {
  ...
  if (isa<FixedVectorType>(IE.getType()) &&
    ...
    auto isShuffleRootCandidate = [](InsertElementInst &Insert) {
      ...
      if (auto *InsertUser = dyn_cast<InsertElementInst>(Insert.user_back()); !InsertUser)
        return true;
      return false;
    };
