...
isOnlyCopiedFromConstantMemory(AAResults *AA,
  ...
  if (MemTransferInst *TheCopy = nullptr; isOnlyCopiedFromConstantMemory(AA, AI, TheCopy, ToDelete))
    return TheCopy;
  return nullptr;
}
...
bool PointerReplacer::collectUsers() {
  ...
  while (!Worklist.empty()) {
    ...
    if (auto *Instruction *Inst = Worklist.pop_back_val(); Load = dyn_cast<LoadInst>(Inst)) {
      if (Load->isVolatile())
        return false;
...
static bool canReplaceGEPIdxWithZero(InstCombinerImpl &IC,
  ...
  auto FirstNZIdx = [](const GetElementPtrInst *GEPI) {
    ...
    for (unsigned IE = GEPI->getNumOperands(); I != IE; ++I) {
      ...
      if (const ConstantInt *Value *V = GEPI->getOperand(I); CI = dyn_cast<ConstantInt>(V))
        if (CI->isZero())
          continue;
  ...
  auto IsAllNonNegative = [&]() {
    for (unsigned i = Idx+1, e = GEPI->getNumOperands(); i != e; ++i) {
      ...
      if (KnownBits Known = IC.computeKnownBits(GEPI->getOperand(i), MemI); Known.isNonNegative())
        continue;
      return false;
    }
