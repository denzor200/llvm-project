...
static void clampCallSiteArgumentStates(Attributor &A, const AAType &QueryingAA,
  ...
  if (bool UsedAssumedInformation = false; !A.checkForAllCallSites(CallSiteCheck, QueryingAA, true,
    ...
    if (BridgeCallBaseContext) {
      ...
      if (bool Success =
          getArgumentStateFromCallBaseContext<AAType, BaseType, StateType,
                                              IRAttributeKind>(
              A, *this, this->getIRPosition(), S); Success)
        return clampStateAndIndicateChange<StateType>(this->getState(), S);
    }
    ...
    auto CalleePred = [&](ArrayRef<const Function *> Callees) {
      for (const Function *Callee : Callees) {
        ...
        if (Attribute::isEnumAttrKind(IRAttributeKind)) {
          ...
          if (bool IsKnown; !AA::hasAssumedIRAttr<IRAttributeKind>(
...
static void followUsesInContext(AAType &AA, Attributor &A,
  ...
  for (unsigned u = 0; u < Uses.size(); ++u) {
    ...
    if (const Instruction *const Use *U = Uses[u]; UserI = dyn_cast<Instruction>(U->getUser())) {
      
      if (bool Found = Explorer.findInContextOf(UserI, EIt, EEnd); Found && AA.followUseInMBEC(A, U, UserI, State))
  ...
  bool forallInterferingAccesses(AA::RangeTy Range, F CB) const {
    ...
    for (const auto &It : OffsetBins) {
      ...
      for (auto Index : It.getSecond()) {
        ...
        if (auto &Access = AccessList[Index]; !CB(Access, IsExact))
          return false;
      }
    }
    return true;
...
ChangeStatus AA::PointerInfo::State::addAccess(
  ...
  for (auto Index : LocalList) {
    ...
    if (auto &A = AccessList[Index]; A.getLocalInst() == &I) {
      AccExists = true;
      AccIndex = Index;
      break;
    }
...
struct AAPointerInfoImpl
  ...
  bool forallInterferingAccesses(
    ...
    if (auto *AI = dyn_cast<AllocaInst>(&getAssociatedValue())) {
      ...
      if (bool IsKnownNoRecurse; AA::hasAssumedIRAttr<Attribute::NoRecurse>(
  ...
  auto UsePred = [&](const Use &U, bool &Follow) -> bool {
    ...
    if (auto *LoadI = dyn_cast<LoadInst>(Usr)) {
      ...
      auto IsValidAssume = [&](IntrinsicInst &IntrI) {
        ...
        if (BasicBlock *IntrBB = IntrI.getParent(); IntrI.getParent() == BB) {
    ...
    if (auto *CB = dyn_cast<CallBase>(Usr)) {
      ...
      if (const auto *TLI =
          A.getInfoCache().getTargetLibraryInfoForFunction(*CB->getFunction()); getFreedOperand(CB, TLI) == U)
        return true;
...
struct AAPointerInfoCallSiteArgument final : AAPointerInfoFloating {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (Argument *Arg = getAssociatedArgument(); Arg) {
      const IRPosition &ArgPos = IRPosition::argument(*Arg);
      
      if (auto *ArgAA =
          A.getAAFor<AAPointerInfo>(*this, ArgPos, DepClassTy::REQUIRED); ArgAA && ArgAA->getState().isValidState())
...
struct AANoUnwindImpl : AANoUnwind {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool UsedAssumedInformation = false; !A.checkForAllInstructions(CheckForNoUnwind, *this, Opcodes,
                                   UsedAssumedInformation))
      return indicatePessimisticFixpoint();
...
ChangeStatus AANoSyncImpl::updateImpl(Attributor &A) {
  ...
  if (bool UsedAssumedInformation = false; !A.checkForAllReadWriteInstructions(CheckRWInstForNoSync, *this,
                                          UsedAssumedInformation) ||
...
struct AANoFreeImpl : public AANoFree {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool UsedAssumedInformation = false; !A.checkForAllCallLikeInstructions(CheckForNoFree, *this,
                                           UsedAssumedInformation))
      return indicatePessimisticFixpoint();
    return ChangeStatus::UNCHANGED;
  }
...
struct AANoFreeFloating : AANoFreeImpl {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool IsKnown; AA::hasAssumedIRAttr<Attribute::NoFree>(A, this,
...
bool AANonNull::isImpliedByIR(Attributor &A, const IRPosition &IRP,
  ...
  if (const Function *InformationCache &InfoCache = A.getInfoCache(); Fn = IRP.getAnchorScope()) {
    if (!Fn->isDeclaration()) {
  ...
  if (IRP.getPositionKind() != IRP_RETURNED) {
    ...
    if (bool UsedAssumedInformation = false; !A.checkForAllInstructions(
            [&](Instruction &I) {
...
struct AANonNullImpl : AANonNull {
  ...
  void initialize(Attributor &A) override {
    ...
    if (Value &V = *getAssociatedValue().stripPointerCasts(); isa<ConstantPointerNull>(V)) {
...
struct AAMustProgressFunction final : AAMustProgressImpl {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool IsKnown; AA::hasAssumedIRAttr<Attribute::WillReturn>(
...
struct AAMustProgressCallSite final : AAMustProgressImpl {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool IsKnownMustProgress; !AA::hasAssumedIRAttr<Attribute::MustProgress>(
...
struct AANoRecurseFunction final : AANoRecurseImpl {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool UsedAssumedInformation = false; A.checkForAllCallSites(CallSitePred, *this, true,
                               UsedAssumedInformation)) {
...
struct AANonConvergentFunction final : AANonConvergentImpl {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool UsedAssumedInformation = false; !A.checkForAllCallLikeInstructions(CalleeIsNotConvergent, *this,
                                           UsedAssumedInformation)) {
      return indicatePessimisticFixpoint();
    }
    return ChangeStatus::UNCHANGED;
...
struct AAUndefinedBehaviorImpl : public AAUndefinedBehavior {
  ...
  bool isAssumedToCauseUB(Instruction *I) const override {
    ...
    case Instruction::Br: {
      ...
      if (auto *BrInst = cast<BranchInst>(I); BrInst->isUnconditional())
        return false;
      return !AssumedNoUBInsts.count(I);
    } break;
    default:
...
struct AAWillReturnImpl : public AAWillReturn {
  ...
  bool isImpliedByMustprogressAndReadonly(Attributor &A, bool KnownOnly) {
    ...
    if (bool IsKnown; AA::isAssumedReadOnly(A, getIRPosition(), *this, IsKnown))
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool UsedAssumedInformation = false; !A.checkForAllCallLikeInstructions(CheckForWillReturn, *this,
                                           UsedAssumedInformation))
      return indicatePessimisticFixpoint();
...
struct CachedReachabilityAA : public BaseTy {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    for (unsigned u = 0, e = QueryVector.size(); u < e; ++u) {
      ...
      if (RQITy *RQI = QueryVector[u]; RQI->Result == RQITy::Reachable::No &&
  ...
  bool rememberResult(Attributor &A, typename RQITy::Reachable Result,
    ...
    if (Result == RQITy::Reachable::Yes || !UsedExclusionSet) {
      ...
      if (RQITy PlainRQI(RQI.From, RQI.To); !QueryCache.count(&PlainRQI)) {
  ...
  bool checkQueryCache(Attributor &A, RQITy &StackRQI,
    ...
    if (StackRQI.ExclusionSet) {
      ...
      if (auto It = QueryCache.find(&PlainRQI); It != QueryCache.end() && (*It)->Result == RQITy::Reachable::No) {
    ...
    if (auto It = QueryCache.find(&StackRQI); It != QueryCache.end()) {
      Result = (*It)->Result;
      return true;
    }
...
struct AAIntraFnReachabilityFunction final
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (auto *LivenessAA =
        A.getAAFor<AAIsDead>(*this, getIRPosition(), DepClassTy::OPTIONAL); LivenessAA &&
...
struct AANoAliasArgument final
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool IsKnownNoSycn; AA::hasAssumedIRAttr<Attribute::NoSync>(
...
struct AANoAliasCallSiteArgument final : AANoAliasImpl {
  ...
  bool isKnownNoAliasDueToNoAliasPreservation(
      ...
      if (ScopeFn) {
        if (auto *CB = dyn_cast<CallBase>(UserI)) {
          if (CB->isArgOperand(&U)) {
            ...
            if (bool IsKnownNoCapture; AA::hasAssumedIRAttr<Attribute::Captures>(
                    A, this, IRPosition::callsite_argument(*CB, ArgNo),
    ...
    if (bool IsAssumedNoCapture = AA::hasAssumedIRAttr<Attribute::Captures>(
        A, this, VIRP, DepClassTy::NONE, IsKnownNoCapture, false, &NoCaptureAA); !IsAssumedNoCapture &&
...
struct AANoAliasReturned final : AANoAliasImpl {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    auto CheckReturnValue = [&](Value &RV) -> bool {
      ...
      if (bool IsKnownNoAlias; !AA::hasAssumedIRAttr<Attribute::NoAlias>(
...
struct AAIsDeadValueImpl : public AAIsDead {
  ...
  bool areAllUsesAssumedDead(Attributor &A, Value &V) {
    ...
    if (!isa<Constant>(V)) {
      ...
      if (std::optional<Constant *> C =
          A.getAssumedConstant(V, *this, UsedAssumedInformation); !C || *C)
        return true;
    }
...
struct AAIsDeadFloating : public AAIsDeadValueImpl {
  ...
  void initialize(Attributor &A) override {
    ...
    if (Instruction *I = dyn_cast<Instruction>(&getAssociatedValue()); !isAssumedSideEffectFree(A, I)) {
    ...
    return llvm::all_of(PotentialCopies, [&](Value *V) {
      ...
      if (auto *LI = dyn_cast<LoadInst>(V)) {
        if (llvm::all_of(LI->uses(), [&](const Use &U) {
              ...
              if (auto &UserI = cast<Instruction>(*U.getUser()); InfoCache.isOnlyUsedByAssume(UserI)) {
                if (AssumeOnlyInst)
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (auto *Instruction *I = dyn_cast<Instruction>(&getAssociatedValue()); SI = dyn_cast_or_null<StoreInst>(I)) {
      if (!isDeadStore(A, *SI))
        return indicatePessimisticFixpoint();
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    if (auto *Value &V = getAssociatedValue(); I = dyn_cast<Instruction>(&V)) {
...
struct AAIsDeadArgument : public AAIsDeadFloating {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    if (Argument &Arg = *getAssociatedArgument(); A.isValidFunctionSignatureRewrite(Arg, /* ReplacementTypes */ {}))
...
struct AAIsDeadCallSiteArgument : public AAIsDeadValueImpl {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    if (UndefValue &UV = *UndefValue::get(U->getType()); A.changeUseAfterManifest(U, UV))
      return ChangeStatus::CHANGED;
    return ChangeStatus::UNCHANGED;
  }
...
struct AAIsDeadReturned : public AAIsDeadValueImpl {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    auto RetInstPred = [&](Instruction &I) {
      ...
      if (ReturnInst &RI = cast<ReturnInst>(I); !isa<UndefValue>(RI.getReturnValue()))
...
identifyAliveSuccessors(Attributor &A, const CallBase &CB,
  ...
  if (bool IsKnownNoReturn; AA::hasAssumedIRAttr<Attribute::NoReturn>(
...
identifyAliveSuccessors(Attributor &A, const InvokeInst &II,
  ...
  if (AAIsDeadFunction::mayCatchAsynchronousExceptions(*II.getFunction())) {
    ...
    if (bool IsKnownNoUnwind; AA::hasAssumedIRAttr<Attribute::NoUnwind>(
...
identifyAliveSuccessors(Attributor &A, const BranchInst &BI,
  ...
  if (BI.getNumSuccessors() == 1) {
    ...
    if (std::optional<Constant *> C =
        A.getAssumedConstant(*BI.getCondition(), AA, UsedAssumedInformation); !C || isa_and_nonnull<UndefValue>(*C)) {
...
struct AADereferenceableImpl : AADereferenceable {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    if (bool IsAssumedNonNull = AA::hasAssumedIRAttr<Attribute::NonNull>(
        A, this, getIRPosition(), DepClassTy::NONE, IsKnownNonNull); IsAssumedNonNull &&
...
static unsigned getKnownAlignForUse(Attributor &A, AAAlign &QueryingAA,
  ...
  if (const IntrinsicInst *II = dyn_cast<IntrinsicInst>(I))
    ...
    case Intrinsic::ptrmask: {
      ...
      if (ConstVals && ConstVals->isValidState() && ConstVals->isAtFixpoint()) {
        ...
        if (Align ConstAlign(UINT64_C(1) << ShiftValue); ConstAlign >= AlignAA->getKnownAlign())
  ...
  if (const auto *CB = dyn_cast<CallBase>(I)) {
    ...
    if (auto *AlignAA = A.getAAFor<AAAlign>(QueryingAA, IRP, DepClassTy::NONE); AlignAA)
...
struct AAAlignCallSiteReturned final
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (const IntrinsicInst *Instruction *I = getIRPosition().getCtxI(); II = dyn_cast<IntrinsicInst>(I)) {
      switch (II->getIntrinsicID()) {
      case Intrinsic::ptrmask: {
        Align Alignment;
        bool Valid = false;
...
struct AANoReturnImpl : public AANoReturn {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool UsedAssumedInformation = false; !A.checkForAllInstructions(CheckForNoReturn, *this,
...
struct AAInstanceInfoImpl : public AAInstanceInfo {
  ...
  void initialize(Attributor &A) override {
    ...
    if (auto *I = dyn_cast<Instruction>(&V)) {
      ...
      if (const auto *CI =
          A.getInfoCache().getAnalysisResultForFunction<CycleAnalysis>(
              *I->getFunction()); mayBeInCycle(CI, I, /* HeaderOnly */ false)) {
        indicatePessimisticFixpoint();
        return;
      }
    }
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    auto UsePred = [&](const Use &U, bool &Follow) {
      ...
      if (auto *CB = dyn_cast<CallBase>(UserI)) {
        ...
        if (auto *Callee = dyn_cast_if_present<Function>(CB->getCalledOperand()); !Callee || !Callee->hasLocalLinkage())
          return true;
        if (!CB->isArgOperand(&U))
          return false;
    ...
    auto EquivalentUseCB = [&](const Use &OldU, const Use &NewU) {
      if (auto *SI = dyn_cast<StoreInst>(OldU.getUser())) {
        ...
        if (auto *Ptr = SI->getPointerOperand()->stripPointerCasts(); (isa<AllocaInst>(Ptr) || isNoAliasCall(Ptr)) &&
...
ChangeStatus AANoCaptureImpl::updateImpl(Attributor &A) {
  ...
  if (AA::hasAssumedIRAttr<Attribute::NoUnwind>(
    ...
    if (bool UsedAssumedInformation = false; IsVoidTy || CheckReturnedArgs(UsedAssumedInformation)) {
...
struct AAValueSimplifyImpl : AAValueSimplify {
  ...
  Value *manifestReplacementValue(Attributor &A, Instruction *CtxI) const {
    ...
    if (Value *NewV = SimplifiedAssociatedValue
                      ? *SimplifiedAssociatedValue
                      : UndefValue::get(getAssociatedType()); NewV && NewV != &getAssociatedValue()) {
      ...
      if (ValueToValueMapTy VMap; reproduceValue(A, *this, *NewV, *getAssociatedType(), CtxI,
                         /* CheckOnly */ true, VMap))
...
struct AAValueSimplifyReturned : AAValueSimplifyImpl {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool UsedAssumedInformation = false; !A.checkForAllInstructions(ReturnInstCB, *this, {Instruction::Ret},
                                   UsedAssumedInformation))
      if (!askSimplifiedValueForOtherAAs(A))
        return indicatePessimisticFixpoint();
...
struct AAValueSimplifyFloating : AAValueSimplifyImpl {
  ...
  void initialize(Attributor &A) override {
    ...
    if (Value &V = getAnchorValue(); isa<Constant>(V))
      indicatePessimisticFixpoint();
  }
  ...
  void initialize(Attributor &A) override {
    ...
    for (Argument &Arg : Fn->args()) {
      if (Arg.hasReturnedAttr()) {
        ...
        if (auto IRP = IRPosition::callsite_argument(*cast<CallBase>(getCtxI()),
                                                 Arg.getArgNo()); IRP.getPositionKind() == IRPosition::IRP_CALL_SITE_ARGUMENT &&
...
struct AAValueSimplifyCallSiteArgument : AAValueSimplifyFloating {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    if (auto *NewV = manifestReplacementValue(A, getCtxI())) {
      ...
      if (Use &U = cast<CallBase>(&getAnchorValue())
                   ->getArgOperandUse(getCallSiteArgNo()); A.changeUseAfterManifest(U, *NewV))
        Changed = ChangeStatus::CHANGED;
    }
...
struct AAHeapToStackFunction final : public AAHeapToStack {
  ...
  void initialize(Attributor &A) override {
    ...
    auto AllocationIdentifierCB = [&](Instruction &I) {
      ...
      if (isRemovableAlloc(CB, TLI)) {
        ...
        if (auto *I8Ty = Type::getInt8Ty(CB->getParent()->getContext()); nullptr != getInitialValueOfAllocation(CB, TLI, I8Ty)) {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    for (auto &It : AllocationInfos) {
      ...
      auto Remark = [&](OptimizationRemark OR) {
        ...
        if (LibFunc IsAllocShared; TLI->getLibFunc(*AI.CB, IsAllocShared))
  ...
  std::optional<APInt> getSize(Attributor &A, const AbstractAttribute &AA,
    ...
    auto Mapper = [&](const Value *V) -> const Value * {
      ...
      if (std::optional<Constant *> bool UsedAssumedInformation = false; SimpleV =
              A.getAssumedConstant(*V, AA, UsedAssumedInformation))
        if (*SimpleV)
          return *SimpleV;
      return V;
...
ChangeStatus AAHeapToStackFunction::updateImpl(Attributor &A) {
  ...
  auto FreeCheck = [&](AllocationInfo &AI) {
    ...
    if (AI.LibraryFunctionId != LibFunc___kmpc_alloc_shared) {
      ...
      if (Instruction *CtxI = isa<InvokeInst>(AI.CB) ? AI.CB : AI.CB->getNextNode(); !Explorer || !Explorer->findInContextOf(UniqueFree, CtxI)) {
  ...
  auto UsesCheck = [&](AllocationInfo &AI) {
    ...
    auto Pred = [&](const Use &U, bool &Follow) -> bool {
      ...
      if (auto *CB = dyn_cast<CallBase>(UserI)) {
        ...
        if (bool IsAssumedNoFree = AA::hasAssumedIRAttr<Attribute::NoFree>(
            A, this, CBIRP, DepClassTy::OPTIONAL, IsKnownNoFree); !IsAssumedNoCapture ||
...
struct AAPrivatizablePtrArgument final : public AAPrivatizablePtrImpl {
  ...
  static void createInitialization(Type *PrivType, Value &Base, Function &F,
    ...
    if (auto *const DataLayout &DL = F.getDataLayout(); PrivStructType = dyn_cast<StructType>(PrivType)) {
  ...
  void createReplacementValues(Align Alignment, Type *PrivType,
    ...
    if (auto *const DataLayout &DL = IP->getDataLayout(); PrivStructType = dyn_cast<StructType>(PrivType)) {
      const StructLayout *PrivStructLayout = DL.getStructLayout(PrivStructType);
      for (unsigned u = 0, e = PrivStructType->getNumElements(); u < e; u++) {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    if (bool UsedAssumedInformation = false; !A.checkForAllInstructions(
            [&](Instruction &I) {
              ...
              if (CallInst &CI = cast<CallInst>(I); CI.isTailCall())
...
struct AAPrivatizablePtrCallSiteArgument final
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool IsAssumedNoCapture = AA::hasAssumedIRAttr<Attribute::Captures>(
        A, this, IRP, DepClassTy::REQUIRED, IsKnownNoCapture); !IsAssumedNoCapture) {
...
ChangeStatus AAMemoryBehaviorFunction::updateImpl(Attributor &A) {
  ...
  if (bool UsedAssumedInformation = false; !A.checkForAllReadWriteInstructions(CheckRWInst, *this,
                                          UsedAssumedInformation))
    return indicatePessimisticFixpoint();
...
ChangeStatus AAMemoryBehaviorFloating::updateImpl(Attributor &A) {
  ...
  if (!Arg || !Arg->hasByValAttr()) {
    ...
    if (const auto *FnMemAA =
        A.getAAFor<AAMemoryBehavior>(*this, FnPos, DepClassTy::OPTIONAL); FnMemAA) {
...
struct AAMemoryLocationImpl : public AAMemoryLocation {
  ...
  static void getKnownStateFromValue(Attributor &A, const IRPosition &IRP,
    ...
    if (Function *AnchorFn = IRP.getAnchorScope(); AnchorFn && A.isRunOn(*AnchorFn))
  ...
  bool checkForAllAccessesToMemoryKind(
    ...
    if (MemoryLocationsKind AssumedMLK = getAssumedNotAccessedLocation(); AssumedMLK == NO_LOCATIONS)
      return true;
...
AAMemoryLocationImpl::categorizeAccessedLocations(Attributor &A, Instruction &I,
  ...
  if (auto *CB = dyn_cast<CallBase>(&I)) {
    ...
    if (bool HasGlobalAccesses = ((~CBAssumedNotAccessedLocs) & NO_GLOBAL_MEM); HasGlobalAccesses) {
      
      if (auto AccessPred = [&](const Instruction *, const Value *Ptr,
...
struct AAMemoryLocationFunction final : public AAMemoryLocationImpl {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (const auto *MemBehaviorAA =
        A.getAAFor<AAMemoryBehavior>(*this, getIRPosition(), DepClassTy::NONE); MemBehaviorAA && MemBehaviorAA->isAssumedReadNone()) {
...
struct AAValueConstantRangeImpl : AAValueConstantRange {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    if (auto &V = getAssociatedValue(); !AssumedConstantRange.isEmptySet() &&
...
struct AAPotentialConstantValuesImpl : AAPotentialConstantValues {
  ...
  bool fillSetWithConstantValues(Attributor &A, const IRPosition &IRP, SetTy &S,
    ...
    if (bool UsedAssumedInformation = false; !A.getAssumedSimplifiedValues(IRP, *this, Values, AA::Interprocedural,
                                      UsedAssumedInformation)) {
...
struct AAPotentialConstantValuesFloating : AAPotentialConstantValuesImpl {
  ...
  static APInt calculateCastInst(const CastInst *CI, const APInt &Src,
    ...
    switch (Instruction::CastOps CastOp = CI->getOpcode(); CastOp) {
    default:
  ...
  static APInt calculateBinaryOperator(const BinaryOperator *BinOp,
    ...
    switch (Instruction::BinaryOps BinOpcode = BinOp->getOpcode(); BinOpcode) {
    default:
      Unsupported = true;
      return LHS;
  ...
  ChangeStatus updateWithSelectInst(Attributor &A, SelectInst *SI) {
    ...
    if (OnlyLeft || OnlyRight) {
      ...
      if (auto Undef = OnlyLeft ? LHSContainsUndef : RHSContainsUndef; Undef)
        unionAssumedWithUndef();
      else {
...
bool AANoUndef::isImpliedByIR(Attributor &A, const IRPosition &IRP,
  ...
  if (Value &Val = IRP.getAssociatedValue(); IRP.getPositionKind() != IRPosition::IRP_RETURNED &&
...
struct AANoUndefImpl : AANoUndef {
  ...
  void initialize(Attributor &A) override {
    ...
    if (Value &V = getAssociatedValue(); isa<UndefValue>(V))
      indicatePessimisticFixpoint();
  ...
  bool followUseInMBEC(Attributor &A, const Use *U, const Instruction *I,
    ...
    if (Function *InformationCache &InfoCache = A.getInfoCache(); F = getAnchorScope()) {
...
struct AANoUndefFloating : public AANoUndefImpl {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (!Stripped) {
      ...
      if (const IRPosition AVIRP = IRPosition::value(*AssociatedValue); AVIRP == getIRPosition() || !VisitValueCB(AVIRP))
        return indicatePessimisticFixpoint();
      return ChangeStatus::UNCHANGED;
    }
...
struct AANoFPClassFloating : public AANoFPClassImpl {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool UsedAssumedInformation = false; !A.getAssumedSimplifiedValues(getIRPosition(), *this, Values,
                                      AA::AnyScope, UsedAssumedInformation)) {
      Values.push_back({getAssociatedValue(), getCtxI()});
    }
...
struct AACallEdgesFunction : public AACallEdgesImpl {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (bool UsedAssumedInformation = false; !A.checkForAllCallLikeInstructions(ProcessCallInst, *this,
                                           UsedAssumedInformation,
                                           /* CheckBBLivenessOnly */ true)) {
  ...
  bool isReachableImpl(Attributor &A, RQITy &RQI,
    ...
    auto CheckReachableCallBase = [&](CallBase *CB) {
      ...
      for (Function *Fn : CBEdges->getOptimisticEdges()) {
        ...
        if (const Instruction &FnFirstInst = Fn->getEntryBlock().front(); !InterFnReachability ||
...
struct AAPotentialValuesImpl : AAPotentialValues {
  ...
  void initialize(Attributor &A) override {
    ...
    if (Value *Stripped = getAssociatedValue().stripPointerCasts(); isa<Constant>(Stripped) && !isa<ConstantExpr>(Stripped)) {
  ...
  virtual void addValue(Attributor &A, StateType &State, Value &V,
    ...
    if (ValIRP.getAssociatedType()->isIntegerTy()) {
      ...
      if (SimpleV.has_value() && !*SimpleV) {
        ...
        if (auto *PotentialConstantsAA = A.getAAFor<AAPotentialConstantValues>(
            *this, ValIRP, DepClassTy::OPTIONAL); PotentialConstantsAA && PotentialConstantsAA->isValidState()) {
...
struct AAPotentialValuesFloating : AAPotentialValuesImpl {
  ...
  bool handleCmp(Attributor &A, Value &Cmp, Value *LHS, Value *RHS,
    ...
    auto CheckPair = [&](Value &LHSV, Value &RHSV) {
      ...
      if (auto *TypedRHS = AA::getWithType(RHSV, *RHS->getType()); TypedLHS && TypedRHS) {
        
        if (Value *NewV = simplifyCmpInst(Pred, TypedLHS, TypedRHS, Q); NewV && NewV != &Cmp) {
          addValue(A, getState(), *NewV, /* CtxI */ nullptr, II.S,
                   getAnchorScope());
          return true;
        }
  ...
  bool handleSelectInst(Attributor &A, SelectInst &SI, ItemInfo II,
    ...
    if (bool NoValueYet = !C.has_value(); NoValueYet || isa_and_nonnull<UndefValue>(*C))
      return true;
...
struct AAPotentialValuesArgument final : AAPotentialValuesImpl {
  ...
  void initialize(Attributor &A) override {
    ...
    if (auto &Arg = cast<Argument>(getAssociatedValue()); Arg.hasPointeeInMemoryValueAttr())
      indicatePessimisticFixpoint();
  }
...
struct AAPotentialValuesReturned : public AAPotentialValuesFloating {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    auto RetInstPred = [&](Instruction &RetI) {
      ...
      if (Value *RetOp = RetI.getOperand(0); isa<UndefValue>(RetOp) || RetOp == NewVal)
        return true;
...
struct AAGlobalValueInfoFloating : public AAGlobalValueInfo {
  ...
  bool checkUse(Attributor &A, const Use &U, bool &Follow,
    ...
    if (auto *Cmp = dyn_cast<ICmpInst>(U.getUser())) {
      ...
      if (int Idx = &Cmp->getOperandUse(0) == &U; isa<Constant>(Cmp->getOperand(Idx)))
        return true;
    ...
    if (isa<ReturnInst>(UInst)) {
      ...
      if (bool UsedAssumedInformation = false; !A.checkForAllCallSites(CallSitePred, *UInst->getFunction(),
...
struct AAIndirectCallInfoCallSite : public AAIndirectCallInfo {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    auto AddPotentialCallees = [&]() {
      for (auto *PotentialCallee : PotentialCallees) {
        ...
        if (bool UsedAssumedInformation = false; CheckPotentialCalleeUse(*PotentialCallee, UsedAssumedInformation))
    ...
    auto CheckPotentialCallee = [&](Function &Fn) {
      ...
      for (int I = NumCBArgs; I < NumFnArgs; ++I) {
        ...
        if (bool IsKnown = false; AA::hasAssumedIRAttr<Attribute::NoUndef>(
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    if (bool UsedAssumedInformation = false; A.isAssumedDead(*CB, this, /*LivenessAA=*/nullptr,
                        UsedAssumedInformation))
      return ChangeStatus::UNCHANGED;
    ...
    if (!CBIsVoid) {
      ...
      for (auto &It : NewCalls) {
        ...
        if (Instruction *CallRet = It.second ? It.second : It.first; CallRet->getType() == CB->getType())
...
struct AAInvariantLoadPointerCallSiteReturned final
  ...
  void initialize(Attributor &A) override {
    ...
    if (const auto &CB = cast<CallBase>(getAnchorValue()); isIntrinsicReturningPointerAliasingArgumentWithoutCapturing(
            &CB, /*MustPreserveNullness=*/false))
      return AAInvariantLoadPointerImpl::initialize(A);
...
static bool makeChange(Attributor &A, InstType *MemInst, const Use &U,
  ...
  if (MemInst->isVolatile()) {
    ...
    if (unsigned NewAS = NewPtrTy->getPointerAddressSpace(); !TTI || !TTI->hasVolatileVariant(MemInst, NewAS))
      return false;
  }
...
struct AAAddressSpaceImpl : public AAAddressSpace {
  ...
  void initialize(Attributor &A) override {
    ...
    if (unsigned AS = getAssociatedType()->getPointerAddressSpace(); AS != FlatAS) {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    auto CheckAddressSpace = [&](Value &Obj) {
      ...
      if (unsigned ObjAS = Obj.getType()->getPointerAddressSpace(); ObjAS != FlatAS)
      ...
      if (F) {
        ...
        if (unsigned AssumedAS = TTI->getAssumedAddrSpace(&Obj); AssumedAS != ~0U)
    ...
    if (auto *AUO = A.getOrCreateAAFor<AAUnderlyingObjects>(getIRPosition(), this,
                                                        DepClassTy::REQUIRED); !AUO->forallUnderlyingObjects(CheckAddressSpace))
      return indicatePessimisticFixpoint();
...
struct AANoAliasAddrSpaceImpl : public AANoAliasAddrSpace {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    auto CheckAddressSpace = [&](Value &Obj) {
      ...
      if (unsigned AS = Obj.getType()->getPointerAddressSpace(); AS == FlatAS)
        return false;
    ...
    if (const AAUnderlyingObjects *AUO = A.getOrCreateAAFor<AAUnderlyingObjects>(
        getIRPosition(), this, DepClassTy::REQUIRED); !AUO->forallUnderlyingObjects(CheckAddressSpace))
      return indicatePessimisticFixpoint();
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    if (unsigned AS = getAssociatedType()->getPointerAddressSpace(); AS != FlatAS || Map.empty())
      return ChangeStatus::UNCHANGED;
...
private:
  void removeAS(unsigned AS) {
    ...
    if (RangeMap::iterator I = Map.find(AS); I != Map.end()) {
      unsigned Upper = I.stop();
      unsigned Lower = I.start();
      I.erase();
      if (Upper == Lower)
...
struct AAAllocationInfoImpl : public AAAllocationInfo {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    switch (unsigned long NumBytesToAllocate = (FixedAllocatedSizeInBits + 7) / 8; I->getOpcode()) {
