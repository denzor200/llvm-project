...
CleanupPointerRootUsers(GlobalVariable *GV,
  ...
  while (!Worklist.empty()) {
    ...
    if (User *U = Worklist.pop_back_val(); StoreInst *SI = dyn_cast<StoreInst>(U)) {
...
static GlobalVariable *SRAGlobal(GlobalVariable *GV, const DataLayout &DL) {
  ...
  if (unsigned NumParts = count_if(Parts, [](const auto &Pair) {
  ...
  while (!GV->use_empty()) {
    ...
    if (StoreInst *SI = dyn_cast<StoreInst>(UI)) {
      ...
      if (StoringOther || SI->getOperand(0) == InitVal) {
        ...
        // global.
        if (Instruction *StoredVal = cast<Instruction>(SI->getOperand(0)); LoadInst *LI = dyn_cast<LoadInst>(StoredVal)) {
