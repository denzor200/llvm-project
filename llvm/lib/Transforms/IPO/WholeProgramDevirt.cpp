...
bool DevirtModule::runForTesting(Module &M, ModuleAnalysisManager &MAM) {
  ...
  if (!ClReadSummary.empty()) {
    ...
    if (auto ReadSummaryFile =
        ExitOnErr(errorOrToExpected(MemoryBuffer::getFile(ClReadSummary))); Expected<std::unique_ptr<ModuleSummaryIndex>> SummaryOrErr =
  ...
  if (!ClWriteSummary.empty()) {
    ...
    if (std::error_code EC; StringRef(ClWriteSummary).ends_with(".bc")) {
