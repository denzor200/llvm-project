...
static void addArgumentReturnedAttrs(const SCCNodeSet &SCCNodes,
  ...
  for (Function *F : SCCNodes) {
    ...
    if (Argument *auto FindRetArg = [&]() -> Argument * {
...
static bool isReturnNonNull(Function *F, const SCCNodeSet &SCCNodes,
  ...
  for (unsigned i = 0; i != FlowsToReturn.size(); ++i) {
    ...
    case Instruction::Invoke: {
      ...
      // proven otherwise
      if (Function *Callee = CB.getCalledFunction(); Callee && SCCNodes.count(Callee)) {
        Speculative = true;
        continue;
      }
      return false;
...
static void addNonNullAttrs(const SCCNodeSet &SCCNodes,
  ...
  for (Function *F : SCCNodes) {
    ...
    if (bool Speculative = false; isReturnNonNull(F, SCCNodes, Speculative)) {
      if (!Speculative) {
