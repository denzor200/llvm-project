...
struct IndexCall : public PointerUnion<CallsiteInfo *, AllocInfo *> {
  ...
  void print(raw_ostream &OS) const {
    ...
    if (PointerUnion<CallsiteInfo *, AllocInfo *> Base = *this; auto *AI = llvm::dyn_cast_if_present<AllocInfo *>(Base)) {
      OS << *AI;
    } else {
...
void CallsiteContextGraph<DerivedCCG, FuncTy, CallTy>::
  ...
  if (Calls.size() == 1) {
    ...
    if (auto &[Call, Ids, Func, SavedContextIds] = Calls[0]; Ids.size() == 1) {
...
bool MemProfContextDisambiguation::applyImport(Module &M) {
  ...
  for (auto &A : M.aliases()) {
    ...
    if (auto *Aliasee = A.getAliaseeObject(); auto *F = dyn_cast<Function>(Aliasee))
