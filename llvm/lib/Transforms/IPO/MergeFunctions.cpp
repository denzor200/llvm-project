...
template <typename FuncContainer> bool MergeFunctions::run(FuncContainer &M) {
  ...
  for (auto &Func : M) {
    ...
    if (Function *FuncPtr = asPtr(Func); isEligibleForMerging(*FuncPtr)) {
...
void MergeFunctions::filterInstsUnrelatedToPDI(
  ...
  auto ExamineDbgDeclare = [&PDIRelated,
    ...
    if (DILocalVariable *DILocVar = DbgDecl->getVariable(); DILocVar->isParameter()) {
      ...
      if (AllocaInst *AI = dyn_cast_or_null<AllocaInst>(DbgDecl->getAddress()); AI) {
...
void MergeFunctions::writeAlias(Function *F, Function *G) {
  ...
  if (const MaybeAlign GAlign = G->getAlign(); FAlign || GAlign)
