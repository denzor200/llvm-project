  ...
  for (VPBlockBase *Block = FirstBB; Block;
    ...
    for (VPRecipeBase &R : *VPBB) {
      if (R.mayWriteToMemory()) {
        ...
        if (auto Loc = vputils::getMemoryLocation(R); !Loc || ScopedNoAliasAAResult::mayAliasInScopes(
...
static bool mergeBlocksIntoPredecessors(VPlan &Plan) {
  ...
  for (VPBasicBlock *VPBB : VPBlockUtils::blocksOnly<VPBasicBlock>(
    ...
    if (auto *PredVPBB =
        dyn_cast_or_null<VPBasicBlock>(VPBB->getSinglePredecessor()); !PredVPBB || PredVPBB->getNumSuccessors() != 1 ||
...
static void simplifyRecipe(VPSingleDefRecipe *Def, VPTypeAnalysis &TypeInfo) {
  ...
  if (VPValue *const DataLayout &DL =
      Plan->getScalarHeader()->getIRBasicBlock()->getDataLayout(); V = tryToFoldLiveIns(*Def, Def->operands(), DL, TypeInfo))
  ...
  if (match(Def, m_Trunc(m_ZExtOrSExt(m_VPValue(A))))) {
    ...
    if (Type *ATy = TypeInfo.inferScalarType(A); TruncTy == ATy) {
  ...
  if (match(Def, m_Not(m_VPValue(A)))) {
    ...
    if (CmpPredicate Pred; match(A, m_Cmp(Pred, m_VPValue(), m_VPValue()))) {
...
void VPlanTransforms::canonicalizeEVLLoops(VPlan &Plan) {
  ...
  if (VPValue *SafeAVL; match(AVL, m_Select(m_VPValue(), m_VPValue(SafeAVL), m_VPValue())))
    AVL = SafeAVL;
  ...
  auto CollectPoisonGeneratingInstrsInBackwardSlice([&](VPRecipeBase *Root) {
    ...
    while (!Worklist.empty()) {
      ...
      if (auto *RecWithFlags = dyn_cast<VPRecipeWithIRFlags>(CurRec)) {
        ...
        if (VPValue *A, *B; match(RecWithFlags, m_BinaryOr(m_VPValue(A), m_VPValue(B))) &&
...
tryToMatchAndCreateExtendedReduction(VPReductionRecipe *Red, VPCostContext &Ctx,
  ...
  // Match reduce(ext)).
  if (VPValue *A; match(VecOp, m_ZExtOrSExt(m_VPValue(A))) &&
...
tryToMatchAndCreateMulAccumulateReduction(VPReductionRecipe *Red,
  ...
  auto ExtendAndReplaceConstantOp = [&Ctx](VPWidenCastRecipe *ExtA,
    ...
    if (const APInt *Const; !match(ValB, m_APInt(Const)) ||
