...
void LoopVectorizationCostModel::collectLoopUniforms(ElementCount VF) {
  ...
  for (auto *V : HasUniformUse) {
    ...
    if (bool UsersAreMemAccesses = all_of(I->users(), [&](User *U) -> bool {
  ...
  do {
    ...
    if (unsigned Opcode =
        TypeSwitch<const VPRecipeBase *, unsigned>(R)
            ...
            }); Subset == Tail || Tail[Subset.size()].first != R) {
...
void VPRecipeBuilder::collectScaledReductions(VFRange &Range) {
  ...
  for (const auto &[Phi, Chains] : ChainsByPhi) {
    for (const auto &[Chain, Scale] : Chains) {
      ...
      // whole chain.
      if (auto AllUsersPartialRdx = [ScaleVal = Scale, RdxPhi = Phi,
                                 this](const User *U) {
        auto *UI = cast<Instruction>(U);
      ...
      }; !all_of(Chain.Reduction->users(), AllUsersPartialRdx)) {
  ...
  for (VPRecipeBase &R : drop_begin(Header->phis())) {
    ...
    if (auto *ReductionPhi = dyn_cast<VPReductionPHIRecipe>(&R)) {
      ...
      if (RecurKind RK = ReductionPhi->getRecurrenceKind(); RecurrenceDescriptor::isAnyOfRecurrenceKind(RK)) {
        ...
        if (auto *PhiR = dyn_cast<VPReductionPHIRecipe>(&R); auto *VPI = dyn_cast<VPInstruction>(PhiR->getStartValue())) {
