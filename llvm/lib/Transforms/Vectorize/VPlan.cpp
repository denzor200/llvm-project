...
Value *VPTransformState::packScalarIntoVectorizedValue(const VPValue *Def,
  ...
  if (Value *LaneExpr = Lane.getAsRuntimeExpr(Builder, VF); auto *StructTy = dyn_cast<StructType>(WideValue->getType())) {
  ...
  if (Plan.getScalarPreheader()->hasPredecessors() && !VectorizingEpilogue) {
    ...
    if (std::optional<MDNode *> RemainderLoopID =
        makeFollowupLoopID(OrigLoopID, {LLVMLoopVectorizeFollowupAll,
                                        LLVMLoopVectorizeFollowupEpilogue}); RemainderLoopID) {
