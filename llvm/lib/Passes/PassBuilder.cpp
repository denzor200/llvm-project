...
parseBoundsCheckingOptions(StringRef Params) {
  ...
  while (!Params.empty()) {
    ...
    if (ParamName == "trap") {
      ...
      if (int8_t Id; ParamEQ == "guard" && !Val.getAsInteger(0, Id)) {
...
static bool isFunctionPassName(StringRef Name, CallbacksT &Callbacks) {
  ...
  if (StringRef NameNoBracket = Name.take_until([](char C) { return C == '<'; }); NameNoBracket == "function")
    return true;
...
Error PassBuilder::parsePassPipeline(ModulePassManager &MPM,
  ...
  if (!isModulePassName(FirstName, ModulePipelineParsingCallbacks)) {
    ...
    if (bool UseMemorySSA; isCGSCCPassName(FirstName, CGSCCPipelineParsingCallbacks)) {
