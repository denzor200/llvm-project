...
bool ELFWriter::isInSymtab(const MCSymbolELF &Symbol) {
  ...
  if (Symbol.isVariable()) {
    ...
    if (const auto *const MCExpr *Expr = Symbol.getVariableValue(); T = dyn_cast<MCTargetExpr>(Expr))
...
void ELFObjectWriter::executePostLayoutBinding() {
  ...
  for (const MCSymbol *Alias : Weakrefs) {
    ...
    if (const auto *auto *Expr = Alias->getVariableValue(); Inner = dyn_cast<MCSymbolRefExpr>(Expr)) {
