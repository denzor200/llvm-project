...
CodeViewContext::getFunctionLineEntries(unsigned FuncId) {
  ...
  for (size_t Idx = LocBegin; Idx != LocEnd; ++Idx) {
    ...
    if (unsigned LocationFuncId = MCCVLines[Idx].getFunctionId(); LocationFuncId == FuncId) {
      ...
      if (auto I = SiteInfo->InlinedAtMap.find(LocationFuncId); I != SiteInfo->InlinedAtMap.end()) {
...
void CodeViewContext::encodeInlineLineTable(const MCAssembler &Asm,
  ...
  for (const MCCVLoc &Loc : Locs) {
    ...
    if (Loc.getFunctionId() == Frag.SiteFuncId) {
      ...
      if (auto I = SiteInfo->InlinedAtMap.find(Loc.getFunctionId()); I != SiteInfo->InlinedAtMap.end()) {
