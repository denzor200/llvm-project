...
void MCAsmStreamer::emitExplicitComments() {
  ...
  if (StringRef Comments = ExplicitCommentToEmit; !Comments.empty())
    OS << Comments;
...
void MCAsmStreamer::emitULEB128Value(const MCExpr *Value) {
  ...
  if (int64_t IntValue; Value->evaluateAsAbsolute(IntValue)) {
...
void MCAsmStreamer::EmitRegisterName(int64_t Register) {
  if (!MAI->useDwarfRegNumForCFI()) {
    ...
    if (std::optional<MCRegister> const MCRegisterInfo *MRI = getContext().getRegisterInfo(); LLVMRegister =
...
void MCAsmStreamer::emitInstruction(const MCInst &Inst,
  ...
  if (StringRef Comments = CommentToEmit; Comments.size() && Comments.back() != '\n')
    getCommentOS() << "\n";
