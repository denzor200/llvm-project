...
void BTFDebug::visitCompositeType(const DICompositeType *CTy,
  ...
  switch (auto Tag = CTy->getTag(); Tag) {
...
bool BTFDebug::IsForwardDeclCandidate(const DIType *Base) {
  if (const auto *CTy = dyn_cast<DICompositeType>(Base)) {
    ...
    if (auto CTag = CTy->getTag(); (CTag == dwarf::DW_TAG_structure_type ||
...
void BTFDebug::visitDerivedType(const DIDerivedType *DTy, uint32_t &TypeId,
  ...
  if (Tag == dwarf::DW_TAG_pointer_type) {
    ...
    if (int TmpTypeId = genBTFTypeTags(DTy, -1); TmpTypeId >= 0) {
      auto TypeDEntry =
...
void BTFDebug::visitMapDefType(const DIType *Ty, uint32_t &TypeId) {
  ...
  switch (uint32_t TmpId; Ty->getTag()) {
  case dwarf::DW_TAG_typedef:
  case dwarf::DW_TAG_const_type:
  case dwarf::DW_TAG_volatile_type:
  case dwarf::DW_TAG_restrict_type:
...
bool BTFDebug::InstLower(const MachineInstr *MI, MCInst &OutMI) {
  if (MI->getOpcode() == BPF::LD_imm64) {
    ...
    if (const MachineOperand &MO = MI->getOperand(1); MO.isGlobal()) {
      ...
      if (auto *GVar = dyn_cast<GlobalVariable>(GVal); GVar) {
        if (!GVar->hasAttribute(BPFCoreSharedInfo::AmaAttr) &&
            !GVar->hasAttribute(BPFCoreSharedInfo::TypeIdAttr))
          return false;
    ...
    if (const MachineOperand &MO = MI->getOperand(3); MO.isGlobal()) {
      ...
      if (auto *GVar = dyn_cast<GlobalVariable>(GVal); GVar && GVar->hasAttribute(BPFCoreSharedInfo::AmaAttr)) {
