...
static std::optional<Instruction *> instCombineSVEUnpack(InstCombiner &IC,
  ...
  if (bool IsSigned = II.getIntrinsicID() == Intrinsic::aarch64_sve_sunpkhi ||
                  II.getIntrinsicID() == Intrinsic::aarch64_sve_sunpklo; auto *ScalarArg = getSplatValue(UnpackArg)) {
    ScalarArg =
    ...
    */
    if (Op2Info.isConstant() && Op2Info.isUniform()) {
      ...
      if (auto VT = TLI->getValueType(DL, Ty); VT.isScalarInteger() && VT.getSizeInBits() <= 64) {
        ...
        if (InstructionCost UsraCost = 2 * AsrCost; Op2Info.isPowerOf2() || Op2Info.isNegatedPowerOf2()) {
...
InstructionCost AArch64TTIImpl::getExtendedReductionCost(
  ...
  if (Opcode == Instruction::Add && VecVT.isSimple() && ResVT.isSimple() &&
    ...
    if (unsigned RevVTSize = ResVT.getSizeInBits(); ((LT.second == MVT::v8i8 || LT.second == MVT::v16i8) &&
         RevVTSize <= 32) ||
...
AArch64TTIImpl::getShuffleCost(TTI::ShuffleKind Kind, VectorType *DstTy,
  ...
  if (Kind == TTI::SK_Broadcast || Kind == TTI::SK_Transpose ||
    ...
    if (static const CostTblEntry ShuffleTbl[] = {
    ...
    }; const auto *Entry = CostTableLookup(ShuffleTbl, Kind, LT.second))
