...
AArch64TTIImpl::getIntrinsicInstrCost(const IntrinsicCostAttributes &ICA,
  ...
  case Intrinsic::vector_insert: {
    ...
    if (const ConstantInt *CIdx = cast<ConstantInt>(Idx); SubVecLK.first == TargetLoweringBase::TypeLegal &&
...
static std::optional<Instruction *> instCombineSVEDupqLane(InstCombiner &IC,
  ...
  if (bool AllowPoison =
      isa<PoisonValue>(CurrentInsertElt) && isa<PoisonValue>(Default); !SimplifyValuePattern(Elts, AllowPoison))
    return std::nullopt;
...
static bool shouldUnrollMultiExitLoop(Loop *L, ScalarEvolution &SE,
  ...
  if (unsigned MaxTC = SE.getSmallConstantMaxTripCount(L); MaxTC > 0 && MaxTC <= 32)
    return false;
