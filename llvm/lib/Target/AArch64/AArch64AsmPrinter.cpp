...
void AArch64AsmPrinter::LowerPATCHABLE_EVENT_CALL(const MachineInstr &MI,
  ...
  if (auto *Sym = MCSymbolRefExpr::create(
...
void AArch64AsmPrinter::emitEndOfAsmFile(Module &M) {
  ...
  if (TT.isOSBinFormatELF()) {
    ...
    if (const auto *PtrAuthELFGOTFlag = mdconst::extract_or_null<ConstantInt>(
        M.getModuleFlag("ptrauth-elf-got")); PtrAuthELFGOTFlag && PtrAuthELFGOTFlag->getZExtValue() == 1)
...
void AArch64AsmPrinter::emitFunctionEntryLabel() {
  ...
  if (TT.isWindowsArm64EC() && !MF->getFunction().hasLocalLinkage()) {
    ...
    for (MDNode *Node : UnmangledNames) {
      ...
      if (MCSymbol *UnmangledSym = MMI->getContext().getOrCreateSymbol(NameStr); std::optional<std::string> MangledName =
...
AArch64AsmPrinter::lowerBlockAddressConstant(const BlockAddress &BA) {
  ...
  if (const Function &Fn = *BA.getFunction(); std::optional<uint16_t> BADisc =
