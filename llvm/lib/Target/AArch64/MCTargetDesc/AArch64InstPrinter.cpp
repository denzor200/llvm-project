...
void AArch64InstPrinter::printInst(const MCInst *MI, uint64_t Address,
  ...
  if (Opcode == AArch64::SBFMXri || Opcode == AArch64::SBFMWri ||
    ...
    if (Op2.isImm() && Op3.isImm()) {
      ...
      if (int64_t imms = Op3.getImm(); Opcode == AArch64::UBFMWri && imms != 0x1F && ((imms + 1) == immr)) {
        AsmMnemonic = "lsl";
        shift = 31 - imms;
  ...
  if ((Opcode == AArch64::MOVZXi || Opcode == AArch64::MOVZWi) &&
    ...
    if (uint64_t Value = (uint64_t)MI->getOperand(1).getImm() << Shift; AArch64_AM::isMOVZMovAlias(Value, Shift,
  ...
  if ((Opcode == AArch64::ORRXri || Opcode == AArch64::ORRWri) &&
    ...
    if (uint64_t Value = AArch64_AM::decodeLogicalImmediate(
        MI->getOperand(2).getImm(), RegWidth); !AArch64_AM::isAnyMOVWMovAlias(Value, RegWidth)) {
      PrintMovImm(Value, RegWidth);
      return;
    }
  }
...
void AArch64AppleInstPrinter::printInst(const MCInst *MI, uint64_t Address,
  ...
  if (bool IsTbx; isTblTbxInstruction(MI->getOpcode(), Layout, IsTbx)) {
  ...
  if (const LdStNInstrDesc *LdStDesc = getLdStNInstrDesc(Opcode)) {
    ...
    if (LdStDesc->NaturalOffset != 0) {
      ...
      if (MCRegister Reg = MI->getOperand(OpNum++).getReg(); Reg != AArch64::XZR) {
        O << ", ";
        printRegName(O, Reg);
      } else {
...
bool AArch64InstPrinter::printRangePrefetchAlias(const MCInst *MI,
  ...
  // 0b11000
  if (unsigned Mask = 0x18; (PRFOp & Mask) != Mask)
...
bool AArch64InstPrinter::printSysAlias(const MCInst *MI,
  ...
  if (CnVal == 7) {
    ...
    case 3: {
      ...
      if (const auto Requires =
          Op2Val == 6 ? AArch64::FeatureSPECRES2 : AArch64::FeaturePredRes; !(STI.hasFeature(AArch64::FeatureAll) || STI.hasFeature(Requires)))
        return false;
    ...
    case 15: {
      ...
      if (const AArch64DC::DC *DC = AArch64DC::lookupDCByEncoding(Encoding); AT && AT->haveFeatures(STI.getFeatureBits())) {
        NeedsReg = true;
        Ins = "at\t";
        Name = std::string(AT->Name);
...
void AArch64InstPrinter::printSImm(const MCInst *MI, unsigned OpNo,
  ...
  if (const MCOperand &Op = MI->getOperand(OpNo); Size == 8)
...
void AArch64InstPrinter::printPostIncOperand(const MCInst *MI, unsigned OpNo,
  ...
  if (const MCOperand &Op = MI->getOperand(OpNo); Op.isReg()) {
    ...
    if (MCRegister Reg = Op.getReg(); Reg == AArch64::XZR)
...
void AArch64InstPrinter::printArithExtend(const MCInst *MI, unsigned OpNum,
  ...
  if (ExtType == AArch64_AM::UXTW || ExtType == AArch64_AM::UXTX) {
    ...
    if ( MCRegister Src1 = MI->getOperand(1).getReg(); ((Dest == AArch64::SP || Src1 == AArch64::SP) &&
...
void AArch64InstPrinter::printRegWithShiftExtend(const MCInst *MI,
  ...
  if (bool DoShift = ExtWidth != 8; SignExtend || DoShift || SrcRegKind == 'w') {
    O << ", ";
...
void AArch64InstPrinter::printPrefetchOp(const MCInst *MI, unsigned OpNum,
  ...
  if (IsSVEPrefetch) {
    ...
    if (auto PRFM = AArch64PRFM::lookupPRFMByEncoding(prfop); PRFM && PRFM->haveFeatures(STI.getFeatureBits())) {
      O << PRFM->Name;
      return;
    }
  }
...
void AArch64InstPrinter::printPSBHintOp(const MCInst *MI, unsigned OpNum,
  ...
  if (auto PSB = AArch64PSBHint::lookupPSBByEncoding(psbhintop); PSB)
    O << PSB->Name;
  else
...
void AArch64InstPrinter::printBTIHintOp(const MCInst *MI, unsigned OpNum,
  ...
  if (auto BTI = AArch64BTIHint::lookupBTIByEncoding(btihintop); BTI)
    O << BTI->Name;
  else
...
void AArch64InstPrinter::printCMHPriorityHintOp(const MCInst *MI,
  ...
  if (auto PHint =
      AArch64CMHPriorityHint::lookupCMHPriorityHintByEncoding(priorityhint_op); PHint)
    O << PHint->Name;
  else
...
void AArch64InstPrinter::printTIndexHintOp(const MCInst *MI, unsigned OpNum,
  ...
  if (auto TIndex = AArch64TIndexHint::lookupTIndexByEncoding(tindexhintop); TIndex)
    O << TIndex->Name;
  else
...
void AArch64InstPrinter::printMatrixTileList(const MCInst *MI, unsigned OpNum,
  ...
  for (unsigned I = 0; I < MaxRegs; ++I) {
    ...
    if (unsigned Reg = RegMask & (1 << I); Reg == 0)
      continue;
...
void AArch64InstPrinter::printAlignedLabel(const MCInst *MI, uint64_t Address,
  ...
  if (const MCOperand &Op = MI->getOperand(OpNum); Op.isImm()) {
    
    if (int64_t Offset = Op.getImm() * 4; PrintBranchImmAsAddress)
...
void AArch64InstPrinter::printSystemPStateField(const MCInst *MI, unsigned OpNo,
  ...
  if (auto PStateImm1 = AArch64PState::lookupPStateImm0_1ByEncoding(Val); PStateImm15 && PStateImm15->haveFeatures(STI.getFeatureBits()))
    O << PStateImm15->Name;
...
void AArch64InstPrinter::printSVEPattern(const MCInst *MI, unsigned OpNum,
  ...
  if (auto unsigned Val = MI->getOperand(OpNum).getImm(); Pat = AArch64SVEPredPattern::lookupSVEPREDPATByEncoding(Val))
    O << Pat->Name;
  else
    markup(O, Markup::Immediate) << '#' << formatImm(Val);
}
...
void AArch64InstPrinter::printSVELogicalImm(const MCInst *MI, unsigned OpNum,
  ...
  if (UnsignedT PrintVal = AArch64_AM::decodeLogicalImmediate(Val, 64); (int16_t)PrintVal == (SignedT)PrintVal)
...
void AArch64InstPrinter::printPHintOp(const MCInst *MI, unsigned OpNum,
  ...
  if (auto PH = AArch64PHint::lookupPHintByEncoding(Op); PH)
    O << PH->Name;
  else
