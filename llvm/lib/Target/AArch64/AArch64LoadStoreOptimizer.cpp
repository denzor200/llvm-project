...
static unsigned isMatchingStore(MachineInstr &LoadInst,
  ...
  switch (unsigned StOpc = StoreInst.getOpcode(); LdOpc) {
  default:
...
static bool isPreLdStPairCandidate(MachineInstr &FirstMI, MachineInstr &MI) {
  ...
  switch (unsigned OpcB = MI.getOpcode(); OpcA) {
  default:
    return false;
  case AArch64::STRSpre:
...
AArch64LoadStoreOpt::mergePairedInsns(MachineBasicBlock::iterator I,
        ...
        [this, RegToRename, GetMatchingSubReg, MergeForward](MachineInstr &MI,
          ...
          if (IsDef) {
            ...
            for (unsigned OpIdx = 0; OpIdx < MI.getNumOperands(); ++OpIdx) {
              ...
              if (MachineOperand &MOP = MI.getOperand(OpIdx); MOP.isReg() && !MOP.isDebug() && MOP.getReg() &&
  ...
  if (bool PairedIsUnscaled = TII->hasUnscaledLdStOffset(Paired->getOpcode()); IsUnscaled != PairedIsUnscaled) {
    ...
    if (int MemSize = TII->getMemScale(*Paired); PairedIsUnscaled) {
  ...
  if (SExtIdx != -1) {
    ...
    if (I->peekDebugInstrNum()) {
      ...
      if (unsigned NewInstrNum; DstRegX == I->getOperand(0).getReg()) {
    ...
    if (Paired->peekDebugInstrNum()) {
      ...
      if (unsigned NewInstrNum; DstRegX == Paired->getOperand(0).getReg()) {
        NewInstrNum = MIBSXTW->getDebugInstrNum();
...
AArch64LoadStoreOpt::promoteLoadFromStore(MachineBasicBlock::iterator LoadI,
  ...
  if (LoadSize == StoreSize && (LoadSize == 4 || LoadSize == 8)) {
    ...
    if (int Imms = Immr + Width - 1; UnscaledLdOffset == UnscaledStOffset) {
...
static bool canRenameMOP(const MachineOperand &MOP,
  ...
  if (MOP.isReg()) {
    ...
    // whole register.
    if (auto *RegClass = TRI->getMinimalPhysRegClass(MOP.getReg()); RegClass->HasDisjunctSubRegs && RegClass->CoveredBySubRegs &&
...
AArch64LoadStoreOpt::findMatchingInsn(MachineBasicBlock::iterator I,
  ...
  for (unsigned Count = 0; MBBI != E && Count < Limit;
    ...
    if (areCandidatesToMergeOrPair(FirstMI, MI, Flags, TII) &&
      ...
      if (bool MIIsUnscaled = TII->hasUnscaledLdStOffset(MI); IsUnscaled != MIIsUnscaled) {
        ...
        if (int MemSize = TII->getMemScale(MI); MIIsUnscaled) {
          // If the unscaled offset isn't a multiple of the MemSize, we can't
      ...
      if (BaseReg == MIBaseReg) {
        ...
        if (IsPreLdSt) {
          ...
          if (bool IsMIRegTheSame =
              TRI->regsOverlap(getLdStRegOp(MI).getReg(),
                               AArch64InstrInfo::getLdStBaseOp(MI).getReg()); IsOutOfBounds || IsBaseRegUsed || IsBaseRegModified ||
              IsMIRegTheSame) {
            LiveRegUnits::accumulateUsedDefed(MI, ModifiedRegUnits,
                                              UsedRegUnits, TRI);
            MemInsns.push_back(&MI);
        ...
        if (int MinOffset = Offset < MIOffset ? Offset : MIOffset; FindNarrowMerge) {
...
maybeMoveCFI(MachineInstr &MI, MachineBasicBlock::iterator MaybeCFI) {
  ...
  switch (const MCCFIInstruction &CFI = MF.getFrameInstructions()[CFIIndex]; CFI.getOperation()) {
...
bool AArch64LoadStoreOpt::isMatchingMovConstInsn(MachineInstr &MemMI,
  ...
  if (MI.getOpcode() == AArch64::MOVKWi &&
    ...
    if (MachineInstr &MovzMI = *MBBI; MovzMI.getOpcode() == AArch64::MOVZWi &&
...
MachineBasicBlock::iterator AArch64LoadStoreOpt::findMatchingUpdateInsnForward(
  ...
  // looking for.
  if (int MIUnscaledOffset = AArch64InstrInfo::getLdStOffsetOp(MemMI).getImm() *
                         TII->getMemScale(MemMI); MIUnscaledOffset != UnscaledOffset)
    return E;
...
bool AArch64LoadStoreOpt::tryToPairLdStInst(MachineBasicBlock::iterator &MBBI) {
  ...
  if (Paired != E) {
    ...
    // of the type.
    if (MachineMemOperand *MemOp =
        MI.memoperands_empty() ? nullptr : MI.memoperands().front(); (MI.mayLoad() && Subtarget->hasLdpAlignedOnly()) ||
      ...
      if (uint64_t TypeAlignment =
          Align(MemOp->getSize().getValue().getKnownMinValue()).value(); MemAlignment < 2 * TypeAlignment) {
        NumFailedAlignmentCheck++;
        return false;
      }
    }
