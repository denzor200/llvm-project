...
bool AArch64DAGToDAGISel::SelectAddrModeFrameIndexSVE(SDValue N, SDValue &Base,
  ...
  if (const TargetLowering *TLI = getTargetLowering(); auto FINode = dyn_cast<FrameIndexSDNode>(N)) {
...
bool AArch64DAGToDAGISel::SelectCmpBranchUImm6Operand(SDNode *P, SDValue N,
  ...
  if (AArch64CC::CondCode CC =
      static_cast<AArch64CC::CondCode>(P->getConstantOperandVal(1)); auto *CN = dyn_cast<ConstantSDNode>(N)) {
