...
public:
  ...
  bool SelectAddrModeIndexedUImm(SDValue N, SDValue &Base, SDValue &OffImm) {
    ...
    if (bool Found = SelectAddrModeIndexed(N, Size, Base, OffImm); Found) {
      if (auto *CI = dyn_cast<ConstantSDNode>(OffImm)) {
        
        if (int64_t C = CI->getSExtValue(); C <= Max)
          return true;
      }
    }
  ...
  bool SelectExtractHigh(SDValue N, SDValue &Res) {
    ...
    if (unsigned Index = N->getConstantOperandVal(1); !VT.is64BitVector() || !LVT.is128BitVector() ||
  ...
  bool SelectEXTImm(SDValue N, SDValue &Imm) {
    ...
    if (int64_t MulImm = cast<ConstantSDNode>(N)->getSExtValue(); MulImm >= 0 && MulImm <= Max) {
      MulImm *= Scale;
      Imm = CurDAG->getTargetConstant(MulImm, SDLoc(N), MVT::i32);
      return true;
    }
...
getExtendTypeForNode(SDValue N, bool IsLoadStore = false) {
  if (N.getOpcode() == ISD::SIGN_EXTEND ||
    ...
    switch (uint64_t AndMask = CSD->getZExtValue(); AndMask) {
    default:
      return AArch64_AM::InvalidShiftExtend;
    case 0xFF:
...
bool AArch64DAGToDAGISel::SelectRDVLImm(SDValue N, SDValue &Imm) {
  ...
  if (int64_t MulImm = cast<ConstantSDNode>(N)->getSExtValue(); (MulImm % std::abs(Scale)) == 0) {
    
    if (int64_t RDVLImm = MulImm / Scale; (RDVLImm >= Low) && (RDVLImm <= High)) {
...
bool AArch64DAGToDAGISel::SelectRDSVLShiftImm(SDValue N, SDValue &Imm) {
  ...
  if (int64_t MulImm = 1LL << cast<ConstantSDNode>(N)->getSExtValue(); MulImm >= Low && MulImm <= High) {
  ...
  if (CurDAG->isBaseWithConstantOffset(N)) {
    if (ConstantSDNode *RHS = dyn_cast<ConstantSDNode>(N.getOperand(1))) {
      if (IsSignedImm) {
        ...
        if (int64_t Range = 0x1LL << (BW - 1); (RHSC & (Size - 1)) == 0 && RHSC >= -(Range << Scale) &&
            RHSC < (Range << Scale)) {
          Base = N.getOperand(0);
        ...
        if (uint64_t Range = 0x1ULL << BW; (RHSC & (Size - 1)) == 0 && RHSC < (Range << Scale)) {
          Base = N.getOperand(0);
          if (Base.getOpcode() == ISD::FrameIndex) {
            int FI = cast<FrameIndexSDNode>(Base)->getIndex();
            Base = CurDAG->getTargetFrameIndex(FI, TLI->getPointerTy(DL));
...
bool AArch64DAGToDAGISel::SelectAddrModeIndexed(SDValue N, unsigned Size,
  ...
  if (CurDAG->isBaseWithConstantOffset(N)) {
    if (ConstantSDNode *RHS = dyn_cast<ConstantSDNode>(N.getOperand(1))) {
      ...
      if (unsigned Scale = Log2_32(Size); isValidAsScaledImmediate(RHSC, 0x1000, Size)) {
        Base = N.getOperand(0);
        if (Base.getOpcode() == ISD::FrameIndex) {
          int FI = cast<FrameIndexSDNode>(Base)->getIndex();
          Base = CurDAG->getTargetFrameIndex(FI, TLI->getPointerTy(DL));
...
bool AArch64DAGToDAGISel::SelectAddrModeUnscaled(SDValue N, unsigned Size,
  ...
  if (ConstantSDNode *RHS = dyn_cast<ConstantSDNode>(N.getOperand(1))) {
    ...
    if (int64_t RHSC = RHS->getSExtValue(); RHSC >= -256 && RHSC < 256) {
      Base = N.getOperand(0);
      if (Base.getOpcode() == ISD::FrameIndex) {
        int FI = cast<FrameIndexSDNode>(Base)->getIndex();
        const TargetLowering *TLI = getTargetLowering();
...
bool AArch64DAGToDAGISel::SelectAddrModeXRO(SDValue N, unsigned Size,
  ...
  if (isa<ConstantSDNode>(RHS)) {
    ...
    if (int64_t ImmOff = (int64_t)RHS->getAsZExtVal(); isValidAsScaledImmediate(ImmOff, 0x1000, Size) ||
...
bool AArch64DAGToDAGISel::SelectAddrModeFrameIndexSVE(SDValue N, SDValue &Base,
  ...
  if (auto const TargetLowering *TLI = getTargetLowering(); FINode = dyn_cast<FrameIndexSDNode>(N)) {
...
static bool isBitfieldExtractOp(SelectionDAG *CurDAG, SDNode *N, unsigned &Opc,
  ...
  switch (unsigned NOpc = N->getMachineOpcode(); NOpc) {
  default:
    return false;
...
static bool isBitfieldPositioningOpFromAnd(SelectionDAG *CurDAG, SDValue Op,
  ...
  if (uint64_t AndImm; !isOpcWithIntImmediate(Op.getNode(), ISD::AND, AndImm))
    return false;
...
static bool isWorthFoldingIntoOrrWithShift(SDValue Dst, SelectionDAG *CurDAG,
  ...
  if (isOpcWithIntImmediate(Dst.getNode(), ISD::AND, AndImm) &&
    ...
    if (isOpcWithIntImmediate(DstOp0.getNode(), ISD::SRL, SrlImm)) {
      ...
      if (uint64_t NumTrailingZeroInShiftedMask = llvm::countr_zero(AndImm); (SrlImm + NumTrailingZeroInShiftedMask) < SizeInBits) {
        unsigned MaskWidth =
...
static bool tryOrrWithShift(SDNode *N, SDValue OrOpd0, SDValue OrOpd1,
  ...
  if (BiggerPattern) {
    ...
    if (uint64_t SrcAndImm; isOpcWithIntImmediate(OrOpd0.getNode(), ISD::AND, SrcAndImm) &&
      ...
      if (uint64_t EncodedShiftImm; isWorthFoldingIntoOrrWithShift(Dst, CurDAG, ShiftedOperand,
                                         EncodedShiftImm)) {
...
bool AArch64DAGToDAGISel::tryShiftAmountMod(SDNode *N) {
  ...
  if (ShiftAmt->getOpcode() == ISD::ADD || ShiftAmt->getOpcode() == ISD::SUB) {
    ...
    if (uint64_t Add1Imm; isIntImmediate(Add1, Add1Imm) && (Add1Imm % Size == 0)) {
...
bool AArch64DAGToDAGISel::tryReadRegister(SDNode *N) {
  ...
  if (Imm == -1) {
    ...
    if (const auto *TheReg =
        AArch64SysReg::lookupSysRegByName(RegString->getString()); TheReg && TheReg->Readable &&
...
bool AArch64DAGToDAGISel::tryWriteRegister(SDNode *N) {
  ...
  if (Imm == -1) {
    ...
    if (auto TheReg = AArch64SysReg::lookupSysRegByName(RegString->getString()); TheReg && TheReg->Writeable &&
        TheReg->haveFeatures(Subtarget->getFeatureBits()))
      Imm = TheReg->Encoding;
    else
      Imm = AArch64SysReg::parseGenericRegister(RegString->getString());
...
bool AArch64DAGToDAGISel::SelectSVESignedArithImm(SDValue N, SDValue &Imm) {
  if (auto CNode = dyn_cast<ConstantSDNode>(N)) {
    ...
    if (SDLoc DL(N); ImmVal >= -128 && ImmVal < 128) {
...
bool AArch64DAGToDAGISel::trySelectXAR(SDNode *N) {
  ...
  if (VT.isScalableVector() &&
    ...
    if (auto *TLI = static_cast<const AArch64TargetLowering *>(getTargetLowering()); !TLI->isAllActivePredicate(*CurDAG, N0.getOperand(0)) ||
...
void AArch64DAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::Constant: {
    ...
    if (ConstantSDNode *ConstNode = cast<ConstantSDNode>(Node); ConstNode->isZero()) {
      if (VT == MVT::i32) {
...
bool AArch64DAGToDAGISel::SelectAddrModeIndexedSVE(SDNode *Root, SDValue N,
  ...
  if (N.getOpcode() == ISD::FrameIndex) {
    ...
    if (int FI = cast<FrameIndexSDNode>(N)->getIndex(); MFI.hasScalableStackID(FI)) {
      Base = CurDAG->getTargetFrameIndex(FI, TLI->getPointerTy(DL));
      OffImm = CurDAG->getTargetConstant(0, SDLoc(N), MVT::i64);
      return true;
    }
  ...
  if (Base.getOpcode() == ISD::FrameIndex) {
    ...
    // We can only encode VL scaled offsets, so only fold in frame indexes
    // referencing SVE objects.
    if (int FI = cast<FrameIndexSDNode>(Base)->getIndex(); MFI.hasScalableStackID(FI))
      Base = CurDAG->getTargetFrameIndex(FI, TLI->getPointerTy(DL));
  }
...
bool AArch64DAGToDAGISel::SelectSVERegRegAddrMode(SDValue N, unsigned Scale,
  ...
  if (auto C = dyn_cast<ConstantSDNode>(RHS)) {
    ...
    if (unsigned Size = 1 << Scale; ImmOff % Size)
      return false;
...
bool AArch64DAGToDAGISel::SelectSMETileSlice(SDValue N, unsigned MaxSize,
  ...
  auto MatchConstantOffset = [&](SDValue CN) -> SDValue {
    if (auto *C = dyn_cast<ConstantSDNode>(CN)) {
      ...
      if (int64_t ImmOff = C->getSExtValue(); (ImmOff > 0 && ImmOff <= MaxSize && (ImmOff % Scale == 0)))
...
bool AArch64DAGToDAGISel::SelectCmpBranchUImm6Operand(SDNode *P, SDValue N,
  ...
  if (auto *AArch64CC::CondCode CC =
      static_cast<AArch64CC::CondCode>(P->getConstantOperandVal(1)); CN = dyn_cast<ConstantSDNode>(N)) {
...
bool AArch64DAGToDAGISel::SelectCmpBranchExtOperand(SDValue N, SDValue &Reg,
  ...
  if (AArch64_AM::ShiftExtendType ET = getExtendTypeForNode(N); (MatchCBB && (ET == AArch64_AM::UXTB || ET == AArch64_AM::SXTB)) ||
