...
public:
  ...
  void applyMemSet(IRBuilder<> &IRB, int64_t Start, int64_t End,
    ...
    for (int64_t Offset = Start - Start % 8; Offset < End; Offset += 8) {
      ...
      if (int LowBits = Offset < Start ? (Start - Offset) * 8 : 0; LowBits)
  ...
  void applyStore(IRBuilder<> &IRB, int64_t Start, int64_t End,
    ...
    for (int64_t Offset = Start - Start % 8; Offset < End; Offset += 8) {
      ...
      if (Value *&CurrentV = Out[Offset]; !CurrentV) {
        CurrentV = V;
      } else {
  ...
  Value *flatten(IRBuilder<> &IRB, Value *V) {
    ...
    if (VectorType *VecTy = dyn_cast<VectorType>(V->getType())) {
      ...
      if (Type *EltTy = VecTy->getElementType(); EltTy->isPointerTy()) {
...
void AArch64StackTagging::tagAlloca(AllocaInst *AI, Instruction *InsertBefore,
  ...
  if (bool LittleEndian = AI->getModule()->getTargetTriple().isLittleEndian(); MergeInit && !F->hasOptNone() && LittleEndian &&
...
Instruction *AArch64StackTagging::insertBaseTaggedPointer(
  ...
  if (const Triple &TargetTriple = M.getTargetTriple(); ClRecordStackHistory == instr && TargetTriple.isAndroid() &&
...
bool AArch64StackTagging::runOnFunction(Function &Fn) {
  ...
  for (auto &I : SInfo.AllocasToInstrument) {
    ...
    if (StandardLifetime) {
      ...
      if (auto TagEnd = [&](Instruction *Node) { untagAlloca(AI, Node, Size); }; !DT || !PDT ||
