...
public:
  ...
  std::optional<std::pair<int64_t, unsigned>> getShiftedVal() const {
    ...
    if (isImm())
      if (auto *CE = dyn_cast<MCConstantExpr>(getImm())) {
        ...
        if (int64_t Val = CE->getValue(); (Val != 0) && (uint64_t(Val >> Width) << Width) == uint64_t(Val))
  ...
  DiagnosticPredicate isSVECpyImm() const {
    ...
    if (auto bool IsByte = std::is_same<int8_t, std::make_signed_t<T>>::value ||
                  std::is_same<int8_t, T>::value; ShiftedImm = getShiftedVal<8>())
  ...
  bool isMovWSymbol(ArrayRef<AArch64::Specifier> AllowedModifiers) const {
    ...
    if (int64_t Addend; !AArch64AsmParser::classifySymbolRef(getImm(), ELFSpec, DarwinSpec,
                                             Addend)) {
      return false;
    }
  ...
  DiagnosticPredicate isSVEDataVectorRegWithShiftExtend() const {
    ...
    if (auto VectorMatch = isSVEDataVectorRegOfWidth<ElementWidth, Class>(); !VectorMatch.isMatch())
      return DiagnosticPredicate::NoMatch;
  ...
  bool isMovImm32Shifter() const {
    ...
    if (AArch64_AM::ShiftExtendType ST = getShiftExtendType(); ST != AArch64_AM::LSL)
      return false;
...
void AArch64Operand::print(raw_ostream &OS, const MCAsmInfo &MAI) const {
  ...
  case k_Barrier: {
    ...
    if (StringRef Name = getBarrierName(); !Name.empty())
      OS << "<barrier " << Name << ">";
    else
  ...
  case k_Prefetch: {
    ...
    if (StringRef Name = getPrefetchName(); !Name.empty())
      OS << "<prfop " << Name << ">";
    else
...
bool AArch64AsmParser::parseSysAlias(StringRef Name, SMLoc NameLoc,
  ...
  if (bool hasTLBID = getSTI().hasFeature(AArch64::FeatureTLBID); Mnemonic == "ic") {
...
AArch64AsmParser::tryParseSVEPredicateVector(OperandVector &Operands) {
  ...
  if (getLexer().is(AsmToken::LBrac)) {
    if (RK == RegKind::SVEPredicateAsCounter) {
      ...
      if (ParseStatus ResIndex = tryParseVectorIndex(Operands); ResIndex.isSuccess())
        return ParseStatus::Success;
    } else {
...
bool AArch64AsmParser::parseOptionalMulOperand(OperandVector &Operands) {
  ...
  if (NextIsHash) {
    ...
    if (const MCExpr *ImmVal; !Parser.parseExpression(ImmVal))
...
bool AArch64AsmParser::parseOperand(OperandVector &Operands, bool isCondCode,
  ...
  switch (auto parseOptionalShiftExtend = [&](AsmToken SavedTok) {
    if (parseOptionalToken(AsmToken::Comma)) {
  ...
  case AsmToken::Identifier: {
    ...
    if (StringRef VecGroup; !parseOptionalVGOperand(Operands, VecGroup)) {
      Operands.push_back(
          AArch64Operand::CreateToken(VecGroup, getLoc(), getContext()));
      return false;
    }
  ...
  case AsmToken::Equal: {
    ...
    if (isa<MCConstantExpr>(SubExprVal)) {
      ...
      if (APInt Simm = APInt(64, Imm << ShiftAmt); !IsXReg && !(Simm.isIntN(32) || Simm.isSignedIntN(32)))
...
bool AArch64AsmParser::validateInstruction(MCInst &Inst, SMLoc &IDLoc,
  ...
  if (Prefix.isActive() &&
    ...
    if (auto PPRRegClass = AArch64MCRegisterClasses[AArch64::PPRRegClassID]; Prefix.isPredicated()) {
      int PgIdx = -1;
  ...
  if (IsWindowsArm64EC) {
    for (unsigned i = 0; i < Inst.getNumOperands(); ++i) {
      if (Inst.getOperand(i).isReg()) {
        ...
        if (MCRegister Reg = Inst.getOperand(i).getReg(); (Reg == AArch64::W13 || Reg == AArch64::X13) ||
  ...
  case AArch64::SUBXri: {
    ...
    if (Inst.getOperand(2).isExpr()) {
      ...
      if (int64_t Addend; classifySymbolRef(Expr, ELFSpec, DarwinSpec, Addend)) {
...
bool AArch64AsmParser::matchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
  ...
  if (NumOperands == 4 && Tok == "lsl") {
    ...
    if (AArch64Operand WidthOp = static_cast<AArch64Operand &>(*Operands[3]); Op1.isScalarReg() && LSBOp.isImm() && WidthOp.isImm()) {
  ...
  if (getSTI().hasFeature(AArch64::FeatureZCZeroingFPWorkaround) &&
    ...
    if (AArch64Operand &Op3 = static_cast<AArch64Operand &>(*Operands[3]); (Op1.isToken() && Op2.isNeonVectorReg() && Op3.isImm()) ||
      ...
      if (StringRef Suffix = Op1.isToken() ? Op1.getToken() : Op2.getToken(); Suffix.lower() == ".2d" &&
...
unsigned AArch64AsmParser::validateTargetOperandClass(MCParsedAsmOperand &AsmOp,
  ...
  switch (auto MatchesOpImmediate = [&](int64_t ExpectedVal) -> MatchResultTy {
    if (!Op.isImm())
