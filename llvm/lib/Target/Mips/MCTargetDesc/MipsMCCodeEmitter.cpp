...
void MipsMCCodeEmitter::LowerCompactBranch(MCInst& Inst) const {
  ...
  if (unsigned Reg1 =  Ctx.getRegisterInfo()->getEncodingValue(RegOp1); Inst.getOpcode() == Mips::BNEC || Inst.getOpcode() == Mips::BEQC ||
...
unsigned MipsMCCodeEmitter::getImmOpValue(const MCInst &MI, const MCOperand &MO,
  ...
  if (unsigned MIFrm = MipsII::getFormat(MCII.get(MI.getOpcode()).TSFlags); !isa<MCSpecifierExpr>(Expr) && MIFrm == MipsII::FrmI) {
...
MipsMCCodeEmitter::getUImm4AndValue(const MCInst &MI, unsigned OpNo,
  ...
  switch (unsigned Value = MO.getImm(); Value) {
    case 128:   return 0x0;
    case 1:     return 0x1;
    case 2:     return 0x2;
    case 3:     return 0x3;
...
MipsMCCodeEmitter::getRegisterListOpValue(const MCInst &MI, unsigned OpNo,
  ...
  for (unsigned I = OpNo, E = MI.getNumOperands() - 2; I < E; ++I) {
    ...
    if (unsigned RegNo = Ctx.getRegisterInfo()->getEncodingValue(Reg); RegNo != 31)
      res++;
    else
      res |= 0x10;
  }
