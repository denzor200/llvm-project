...
MipsInstrInfo::describeLoadedValue(const MachineInstr &MI, Register Reg) const {
  ...
  if (auto DIExpression *Expr =
      DIExpression::get(MI.getMF()->getFunction().getContext(), {}); RegImm = isAddImmediate(MI, Reg)) {
    ...
    // DestReg.
    if (Register DestReg = DestSrc->Destination->getReg(); TRI->isSuperRegister(Reg, DestReg) || TRI->isSubRegister(Reg, DestReg))
      return std::nullopt;
  }
