...
void MipsBranchExpansion::expandToLongBranch(MBBInfo &I) {
  ...
  if (IsPIC) {
    ...
    if (const unsigned BalOp =
      ...
      if (bool hasDelaySlot = buildProperJumpMI(BalTgtMBB, Pos, DL); !hasDelaySlot) {
...
bool MipsBranchExpansion::handleMFLOSlot(Pred Predicate, Safe SafeInSlot) {
  ...
  for (MachineFunction::iterator FI = MFp->begin(); FI != MFp->end(); ++FI) {
    for (Iter I = FI->begin(); I != FI->end(); ++I) {
      ...
      if (bool LastInstInFunction =
          std::next(I) == FI->end() && std::next(FI) == MFp->end(); !LastInstInFunction) {
