...
MipsRegisterBankInfo::getInstrMapping(const MachineInstr &MI) const {
  ...
  switch (InstType InstTy = InstType::Integer; Opc) {
  case G_TRUNC:
  case G_UMULH:
  case G_ZEXTLOAD:
  case G_SEXTLOAD:
...
void MipsRegisterBankInfo::setRegBank(MachineInstr &MI,
  ...
  switch (Register Dest = MI.getOperand(0).getReg(); MI.getOpcode()) {
  case TargetOpcode::G_STORE:
...
void MipsRegisterBankInfo::applyMappingImpl(
  ...
  switch (LegalizationArtifactCombiner ArtCombiner(Builder, MF->getRegInfo(), LegInfo); MI.getOpcode()) {
  case TargetOpcode::G_LOAD:
  case TargetOpcode::G_STORE:
  case TargetOpcode::G_PHI:
  case TargetOpcode::G_SELECT:
  case TargetOpcode::G_IMPLICIT_DEF: {
    ...
    while (!NewInstrs.empty()) {
      ...
      if (auto *MachineInstr *NewMI = NewInstrs.pop_back_val(); Unmerge = dyn_cast<GUnmerge>(NewMI))
