...
static bool canThrow(const Value *V) {
  if (const auto *F = dyn_cast<const Function>(V)) {
    ...
    if (StringRef Name = F->getName(); Name == "setjmp" || Name == "longjmp" || Name == "emscripten_longjmp")
      return false;
...
bool WebAssemblyLowerEmscriptenEHSjLj::runEHOnFunction(Function &F) {
  ...
  for (BasicBlock &BB : F) {
    ...
    if (auto *BasicBlock::iterator I = BB.getFirstNonPHIIt(); LPI = dyn_cast<LandingPadInst>(I))
  ...
  for (auto &[UnwindDest, NewPreds] : UnwindDestToNewPreds) {
    for (PHINode &PN : UnwindDest->phis()) {
      ...
      for (unsigned Idx = 0, E = PN.getNumIncomingValues(); Idx != E; ++Idx) {
        ...
        if (auto *Value *V = PN.getIncomingValue(Idx); II = dyn_cast<InvokeInst>(V))
