...
static MachineInstr *getVRegDef(unsigned Reg, const MachineInstr *Insert,
  ...
  if (LIS != nullptr) {
    ...
    if (SlotIndex InstIndex = LIS->getInstructionIndex(*Insert); const VNInfo *ValNo = LIS->getInterval(Reg).getVNInfoBefore(InstIndex))
...
static bool hasSingleUse(unsigned Reg, MachineRegisterInfo &MRI,
  ...
  for (auto &I : MRI.use_nodbg_operands(Reg)) {
    ...
    if (const auto &Result = LI.Query(LIS->getInstructionIndex(*I.getParent())); Result.valueIn() == DefVNI) {
      if (!Result.isKill())
        return false;
      if (HasOne)
        return false;
...
public:
  explicit TreeWalkerState(MachineInstr *Insert) {
    ...
    if (const iterator_range<mop_iterator> &Range = Insert->explicit_uses(); !Range.empty())
  ...
  void pushOperands(MachineInstr *Instr) {
    ...
    if (const iterator_range<mop_iterator> &Range(Instr->explicit_uses()); !Range.empty())
      Worklist.push_back(reverse(Range));
  }
