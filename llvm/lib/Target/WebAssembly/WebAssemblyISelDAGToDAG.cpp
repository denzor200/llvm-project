...
void WebAssemblyDAGToDAGISel::Select(SDNode *Node) {
  ...
  switch (MachineFunction &MF = CurDAG->getMachineFunction(); Node->getOpcode()) {
  ...
  case WebAssemblyISD::RET_CALL: {
    ...
    for (size_t i = 1; i < Node->getNumOperands(); ++i) {
      ...
      if (i == 1 && Op->getOpcode() == WebAssemblyISD::Wrapper) {
        ...
        if (auto *SDValue NewOp = Op->getOperand(0); GlobalOp = dyn_cast<GlobalAddressSDNode>(NewOp.getNode())) {
          if (isa<Function>(
