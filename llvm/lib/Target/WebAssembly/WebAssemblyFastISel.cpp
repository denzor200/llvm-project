...
bool WebAssemblyFastISel::computeAddress(const Value *Obj, Address &Addr) {
  ...
  case Instruction::Sub: {
    ...
    if (const auto *const Value *RHS = U->getOperand(1); CI = dyn_cast<ConstantInt>(RHS)) {
...
bool WebAssemblyFastISel::selectCall(const Instruction *I) {
  ...
  if (!IsVoid) {
    ...
    switch (MVT::SimpleValueType RetTy = getSimpleType(Call->getType()); RetTy) {
    case MVT::i1:
    case MVT::i8:
    case MVT::i16:
    case MVT::i32:
  ...
  for (unsigned I = 0, E = Call->arg_size(); I < E; ++I) {
    ...
    if (MVT::SimpleValueType ArgTy = getSimpleType(V->getType()); ArgTy == MVT::INVALID_SIMPLE_VALUE_TYPE)
      return false;
