...
SPIRVType *SPIRVGlobalRegistry::getOpTypeStruct(
  ...
  if (uint64_t TotalSize = 0; matchPeeledArrayPattern(Ty, OriginalElementType, TotalSize)) {
...
SPIRVGlobalRegistry::getOrCreateOpTypeSampler(MachineIRBuilder &MIRBuilder) {
  ...
  if (const MachineInstr *const MachineFunction *MF = &MIRBuilder.getMF(); MI = findMI(Key, MF))
    return MI;
  const MachineInstr *NewMI =
      createOpType(MIRBuilder, [&](MachineIRBuilder &MIRBuilder) {
