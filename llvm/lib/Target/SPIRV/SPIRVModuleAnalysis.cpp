...
getSymbolicOperandRequirements(SPIRV::OperandCategory::OperandCategory Category,
  ...
  if (ReqCaps.empty()) {
    ...
    if (ReqCaps.size() == 1) {
      ...
      if (auto Cap = ReqCaps[0]; Reqs.isCapabilityAvailable(Cap)) {
      ...
      for (size_t i = 0, Sz = UseCaps.size(); i < Sz; ++i) {
        ...
        if (auto Cap = UseCaps[i]; i == Sz - 1 || !AvoidCaps.S.contains(Cap)) {
          ReqExts.append(getSymbolicOperandExtensions(
              SPIRV::OperandCategory::CapabilityOperand, Cap));
          return {true, {Cap}, std::move(ReqExts), ReqMinVer, ReqMaxVer};
        }
...
static void addOpDecorateReqs(const MachineInstr &MI, unsigned DecIndex,
  ...
  if (Dec == SPIRV::Decoration::BuiltIn) {
    ...
    if (SPIRV::LinkageType::LinkageType LnkType =
        static_cast<SPIRV::LinkageType::LinkageType>(LinkageOp); LnkType == SPIRV::LinkageType::LinkOnceODR)
...
void addPrintfRequirements(const MachineInstr &MI,
  ...
  if (const SPIRVType *PtrType = GR->getSPIRVTypeForVReg(MI.getOperand(4).getReg()); PtrType) {
    ...
    if (MachineOperand ASOp = PtrType->getOperand(1); ASOp.isImm()) {
...
static void collectReqs(const Module &M, SPIRV::ModuleAnalysisInfo &MAI,
  ...
  if (auto Node = M.getNamedMetadata("spirv.ExecutionMode"); Node) {
    ...
    for (unsigned i = 0; i < Node->getNumOperands(); i++) {
      ...
      if (auto *const MDOperand &MDOp = MDN->getOperand(1); CMeta = dyn_cast<ConstantAsMetadata>(MDOp)) {
