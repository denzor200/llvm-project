...
bool LatticeCell::convertToProperty() {
  ...
  if (uint32_t Ps = !isTop() ? properties()
                         : Everything; Ps != ConstantProperties::Unknown) {
    Properties = Ps;
    setProperty();
  } else {
    setBottom();
...
bool LatticeCell::add(const Constant *LC) {
  ...
  if (!isProperty()) {
    ...
    while (Index < Size) {
      ...
      if (const Constant *C = Values[Index]; C == LC)
        return false;
      Index++;
    }
...
Bottomize:
  ...
  for (unsigned i = 1, n = PN.getNumOperands(); i < n; i += 2) {
    ...
    if (unsigned PBN = PB->getNumber(); !EdgeExec.count(CFGEdge(PBN, MBN))) {
...
void MachineConstPropagator::visitBranchesFrom(const MachineInstr &BrI) {
  ...
  if (EvalOk) {
    ...
    if (FallsThru) {
      ...
      if (MachineFunction::const_iterator Next = std::next(BI); Next != MF.end())
        Targets.insert(&*Next);
    }
  } else {
...
bool MachineConstPropagator::computeBlockSuccessors(const MachineBasicBlock *MB,
  ...
  if (DoNext) {
    ...
    if (MachineFunction::const_iterator NextI = std::next(BI); NextI != MB->getParent()->end())
...
bool MachineConstEvaluator::evaluateCMPrr(uint32_t Cmp, const RegSubRegPair &R1,
  ...
  for (unsigned i = 0; i < LS2.size(); ++i) {
    ...
    if (bool Computed = constToInt(LS2.Values[i], A) &&
                    evaluateCMPri(Cmp, R1, A, Inputs, Res); !Computed)
      return false;
    IsTrue &= Res;
    IsFalse &= !Res;
  }
...
bool MachineConstEvaluator::evaluateCMPri(uint32_t Cmp, const RegSubRegPair &R1,
  ...
  for (unsigned i = 0; i < LS.size(); ++i) {
    ...
    if (bool Computed = constToInt(LS.Values[i], A) &&
                    evaluateCMPii(Cmp, A, A2, Res); !Computed)
      return false;
    IsTrue &= Res;
    IsFalse &= !Res;
  }
...
bool MachineConstEvaluator::evaluateCMPrp(uint32_t Cmp, const RegSubRegPair &R1,
  ...
  for (unsigned i = 0; i < LS.size(); ++i) {
    ...
    if (bool Computed = constToInt(LS.Values[i], A) &&
                    evaluateCMPpi(NegCmp, Props2, A, Res); !Computed)
      return false;
    IsTrue &= Res;
    IsFalse &= !Res;
  }
...
bool MachineConstEvaluator::evaluateCMPii(uint32_t Cmp, const APInt &A1,
  ...
  if (Cmp & Comparison::U) {
    ...
    if (APInt Zx2 = A2.zext(MaxW); Cmp & Comparison::L)
      Result = Zx1.ult(Zx2);
...
bool MachineConstEvaluator::evaluateANDrr(const RegSubRegPair &R1,
  ...
  for (unsigned i = 0; i < LS2.size(); ++i) {
    ...
    if (bool Eval = constToInt(LS2.Values[i], A) &&
                evaluateANDri(R1, A, Inputs, RC); !Eval)
      return false;
    Result.meet(RC);
  }
...
bool MachineConstEvaluator::evaluateANDri(const RegSubRegPair &R1,
  ...
  for (unsigned i = 0; i < LS1.size(); ++i) {
    ...
    if (bool Eval = constToInt(LS1.Values[i], A) &&
                evaluateANDii(A, A2, ResA); !Eval)
      return false;
...
bool MachineConstEvaluator::evaluateORrr(const RegSubRegPair &R1,
  ...
  for (unsigned i = 0; i < LS2.size(); ++i) {
    ...
    if (bool Eval = constToInt(LS2.Values[i], A) &&
                evaluateORri(R1, A, Inputs, RC); !Eval)
      return false;
    Result.meet(RC);
  }
  return !Result.isBottom();
...
bool MachineConstEvaluator::evaluateXORrr(const RegSubRegPair &R1,
  ...
  for (unsigned i = 0; i < LS2.size(); ++i) {
    ...
    if (bool Eval = constToInt(LS2.Values[i], A) &&
                evaluateXORri(R1, A, Inputs, RC); !Eval)
      return false;
    Result.meet(RC);
  }
  return !Result.isBottom();
...
bool MachineConstEvaluator::evaluateZEXTr(const RegSubRegPair &R1,
  ...
  for (unsigned i = 0; i < LS1.size(); ++i) {
    ...
    if (bool Eval = constToInt(LS1.Values[i], A) &&
                evaluateZEXTi(A, Width, Bits, XA); !Eval)
      return false;
    const Constant *C = intToConst(XA);
    Result.add(C);
  }
...
bool MachineConstEvaluator::evaluateSEXTr(const RegSubRegPair &R1,
  ...
  for (unsigned i = 0; i < LS1.size(); ++i) {
    ...
    if (bool Eval = constToInt(LS1.Values[i], A) &&
                evaluateSEXTi(A, Width, Bits, XA); !Eval)
      return false;
    const Constant *C = intToConst(XA);
    Result.add(C);
  }
...
bool MachineConstEvaluator::evaluateCLBr(const RegSubRegPair &R1, bool Zeros,
  ...
  for (unsigned i = 0; i < LS1.size(); ++i) {
    ...
    if (bool Eval = constToInt(LS1.Values[i], A) &&
                evaluateCLBi(A, Zeros, Ones, CA); !Eval)
      return false;
...
bool MachineConstEvaluator::evaluateCTBr(const RegSubRegPair &R1, bool Zeros,
  ...
  for (unsigned i = 0; i < LS1.size(); ++i) {
    ...
    if (bool Eval = constToInt(LS1.Values[i], A) &&
                evaluateCTBi(A, Zeros, Ones, CA); !Eval)
      return false;
    const Constant *C = intToConst(CA);
    Result.add(C);
  }
...
bool MachineConstEvaluator::evaluateEXTRACTr(const RegSubRegPair &R1,
  ...
  if (LS1.isProperty()) {
    ...
    if (uint32_t Ps = LS1.properties(); Ps & ConstantProperties::Zero) {
  ...
  for (unsigned i = 0; i < LS1.size(); ++i) {
    ...
    if (bool Eval = constToInt(LS1.Values[i], A) &&
                evaluateEXTRACTi(A, Bits, Offset, Signed, CA); !Eval)
      return false;
    const Constant *C = intToConst(CA);
    Result.add(C);
  }
...
bool MachineConstEvaluator::evaluateSplatr(const RegSubRegPair &R1,
  ...
  for (unsigned i = 0; i < LS1.size(); ++i) {
    ...
    if (bool Eval = constToInt(LS1.Values[i], A) &&
                evaluateSplati(A, Bits, Count, SA); !Eval)
      return false;
...
namespace {
  ...
  public:
    ...
    bool runOnMachineFunction(MachineFunction &MF) override {
      ...
      if (const Function &F = MF.getFunction(); skipFunction(F))
        return false;
...
bool HexagonConstEvaluator::evaluate(const MachineInstr &MI,
  ...
  if (MI.isCopy()) {
    ...
    if (bool Eval = evaluateCOPY(SrcR, Inputs, RC); !Eval)
      return false;
  ...
  switch (Opc) {
    ...
    case Hexagon::CONST64:
    {
      ...
      if (const MachineOperand &VO = MI.getOperand(1); !VO.isImm())
        return false;
    ...
    case Hexagon::A2_xorp:
    {
      ...
      if (bool Eval = evaluateHexLogical(MI, Inputs, Outputs); !Eval)
        return false;
      break;
    }
    ...
    case Hexagon::S2_setbit_i:
    {
      ...
      if (bool Eval = evaluateORri(R, A, Inputs, RC); !Eval)
        return false;
      Outputs.update(DefR.Reg, RC);
      break;
    }
    ...
    case Hexagon::C2_muxii:
    {
      ...
      if (bool Eval = evaluateHexCondMove(MI, Inputs, Outputs); !Eval)
        return false;
      break;
    }
    ...
    case Hexagon::A2_zxth:
    {
      ...
      if (bool Eval = evaluateHexExt(MI, Inputs, Outputs); !Eval)
        return false;
      break;
    }
    ...
    case Hexagon::S2_ct1p:
    {
      ...
      if (bool Eval = evaluateCTBr(R1, !Ones, Ones, Inputs, T); !Eval)
        return false;
    ...
    case Hexagon::S2_clbp:
    {
      ...
      if (bool Eval = evaluateCLBr(R1, !OnlyOnes, !OnlyZeros, Inputs, T); !Eval)
        return false;
      // All of these instructions return a 32-bit value. The evaluate
      // will generate the same type as the operand, so truncate the
      // result if necessary.
    ...
    // vzxtbh, vzxthw
    {
      ...
      if (bool Eval = evaluateHexVector1(MI, Inputs, Outputs); !Eval)
        return false;
      break;
    }
...
bool HexagonConstEvaluator::evaluate(const RegSubRegPair &R,
  ...
  if (const TargetRegisterClass *RC = MRI->getRegClass(R.Reg); RC != &Hexagon::DoubleRegsRegClass)
    return false;
  ...
  for (unsigned i = 0; i < Input.size(); ++i) {
    ...
    if (const Constant *C = Input.Values[i]; !constToInt(C, A))
      return false;
    if (!A.isIntN(64))
      return false;
...
bool HexagonConstEvaluator::rewrite(MachineInstr &MI, const CellMap &Inputs) {
  ...
  if (unsigned NumOp = MI.getNumOperands(); NumOp == 0)
    return false;
...
bool HexagonConstEvaluator::evaluateHexRSEQ32(RegSubRegPair RL,
  ...
  for (unsigned i = 0; i < LN; ++i) {
    ...
    if (bool Eval = constToInt(LSL.Values[i], LoVs[i]); !Eval)
      return false;
    assert(LoVs[i].getBitWidth() == 32);
  }
  for (unsigned i = 0; i < HN; ++i) {
    
    if (bool Eval = constToInt(LSH.Values[i], HiVs[i]); !Eval)
      return false;
...
bool HexagonConstEvaluator::evaluateHexCompare(const MachineInstr &MI,
  ...
  if (Classic) {
    ...
    if (bool Computed = evaluateHexCompare2(Opc, Src1, Src2, Inputs, Result); Computed) {
...
bool HexagonConstEvaluator::evaluateHexCompare2(unsigned Opc,
  ...
  if (bool Imm1 = Src1.isImm(), Imm2 = Src2.isImm(); Reg1) {
    
    if (RegSubRegPair R1(getRegSubRegPair(Src1)); Reg2) {
    ...
    if (APInt A1 = getCmpImm(Opc, 1, Src1); Reg2) {
      RegSubRegPair R2(getRegSubRegPair(Src2));
      uint32_t NegCmp = Comparison::negate(Cmp);
...
bool HexagonConstEvaluator::evaluateHexExt(const MachineInstr &MI,
  ...
  if (bool Eval = Signed ? evaluateSEXTr(R1, BW, Bits, Inputs, RC)
                     : evaluateZEXTr(R1, BW, Bits, Inputs, RC); !Eval)
    return false;
  Outputs.update(DefR.Reg, RC);
  return true;
}
    ...
    if (!L.isSingle()) {
      ...
      if (W == 32) {
        ...
        if (A.isSignedIntN(8)) {
          ...
          if (int32_t Lo = V & 0xFFFFFFFFLL; isInt<8>(Hi) && isInt<8>(Lo)) {
  ...
  switch (Opc) {
    ...
    case Hexagon::A2_and:
    {
      ...
      if (getCell(R1, Inputs, LS1) && LS1.isSingle()) {
        ...
        if (APInt M1; constToInt(LS1.Value, M1) && !~M1)
          CopyOf = 2;
      }
      else if (getCell(R2, Inputs, LS2) && LS2.isSingle()) {
        
        if (APInt M1; constToInt(LS2.Value, M1) && !~M1)
          CopyOf = 1;
      }
      if (!CopyOf)
        return false;
...
bool HexagonConstEvaluator::rewriteHexBranch(MachineInstr &BrI,
  ...
  if (unsigned NumOp = BrI.getNumOperands(); !NumOp)
    return false;
  ...
  if (NumTargets > 0) {
    ...
    if (bool Moot = B.isLayoutSuccessor(TargetB); !Moot) {
