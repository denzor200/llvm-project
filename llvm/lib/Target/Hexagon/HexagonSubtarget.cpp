...
bool HexagonSubtarget::isHVXVectorType(EVT VecTy, bool IncludeBool) const {
  ...
  if (unsigned VecWidth = VecTy.getSizeInBits(); VecWidth != 8 * HwLen && VecWidth != 16 * HwLen)
    return false;
...
#undef GET_HVX_INTRINSICS
  ...
  if (auto FoundScalar = std::lower_bound(BS, ES, Scalar{Opc, 0}, CmpOpcode); FoundScalar != ES && FoundScalar->Opcode == Opc)
