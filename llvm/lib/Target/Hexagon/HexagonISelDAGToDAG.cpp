...
void HexagonDAGToDAGISel::SelectSHL(SDNode *N) {
  ...
  if (Shl_0.getOpcode() == ISD::SUB) {
    ...
    // Val
    if (SDValue Sub_1 = Shl_0.getOperand(1); ConstantSDNode *C1 = dyn_cast<ConstantSDNode>(Sub_0)) {
      if (C1->getSExtValue() != 0 || Sub_1.getOpcode() != ISD::SHL)
        return Default();
      SDValue Shl2_0 = Sub_1.getOperand(0); // Val
      // Const
      if (SDValue Shl2_1 = Sub_1.getOperand(1); ConstantSDNode *C2 = dyn_cast<ConstantSDNode>(Shl2_1)) {
...
bool HexagonDAGToDAGISel::SelectGlobalAddress(SDValue &N, SDValue &R,
  ...
  case ISD::ADD: {
    ...
    if (ConstantSDNode *Const = dyn_cast<ConstantSDNode>(N1)) {
      ...
      if (SDValue Addr = N0.getOperand(0); GlobalAddressSDNode *GA = dyn_cast<GlobalAddressSDNode>(Addr)) {
