...
void HexagonExpandCondsets::updateDeadsInRange(Register Reg, LaneBitmask LM,
  ...
  for (auto &SI : PredDefs) {
    ...
    if (auto P = Range.extendInBlock(Undefs, LIS->getMBBStartIdx(BB), SI); P.first != nullptr || P.second)
      SI = SlotIndex();
  }
  ...
  for (auto &Seg : Range) {
    ...
    for (auto &Op : DefI->operands()) {
      ...
      if (auto P = IsRegDef(Op); P.second && Seg.end.isDead()) {
        Op.setIsDead(true);
      } else if (P.first) {
        DefRegs.insert(Op);
        Op.setIsDead(false);
...
bool HexagonExpandCondsets::split(MachineInstr &MI,
  ...
  if (ST.isReg() && SF.isReg()) {
    ...
    if (RegisterRef RT(ST); RT == RegisterRef(SF)) {
...
bool HexagonExpandCondsets::coalesceRegisters(RegisterRef R1, RegisterRef R2) {
  ...
  if (unsigned BW1, BW2; !isIntReg(R1, BW1) || !isIntReg(R2, BW2) || BW1 != BW2)
    return false;
