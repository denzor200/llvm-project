    ...
    for (MachineInstr &MI : MBB) {
      ...
      case PPC::XXPERMDI: {
        ...
        if (int immed = MI.getOperand(3).getImm(); immed == 2) {
          ...
          if (unsigned trueReg2 = lookThruCopyLike(MI.getOperand(2).getReg(),
                                               VecIdx); trueReg1 == trueReg2)
  ...
  for (unsigned EntryIdx = 0; EntryIdx < SwapVector.size(); ++EntryIdx) {
      ...
      for (MachineInstr &UseMI : MRI->use_nodbg_instructions(DefReg)) {
        ...
        if (SwapVector[UseIdx].IsSwap && !SwapVector[UseIdx].IsLoad &&
          ...
          for (MachineInstr &UseOfUseMI :
            ...
            if (int UseOfUseIdx = SwapMap[&UseOfUseMI]; SwapVector[UseOfUseIdx].IsStore) {
              SwapVector[Repr].WebRejected = 1;
              LLVM_DEBUG(
                  dbgs() << format(
    ...
    // other than a swap instruction.
      ...
      for (MachineInstr &UseMI : MRI->use_nodbg_instructions(DefReg)) {
        ...
        if (int UseIdx = SwapMap[&UseMI]; SwapVector[UseIdx].VSEMI->getOpcode() != MI->getOpcode()) {
          SwapVector[Repr].WebRejected = 1;
