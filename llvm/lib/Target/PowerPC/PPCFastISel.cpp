  ...
  switch (Opcode) {
    ...
    case Instruction::GetElementPtr: {
      ...
      for (User::const_op_iterator II = U->op_begin() + 1, IE = U->op_end();
        ...
        if (StructType *const Value *Op = *II; STy = GTI.getStructTypeOrNull()) {
...
bool PPCFastISel::PPCEmitLoad(MVT VT, Register &ResultReg, Address &Addr,
  ...
  // Indexed form.
    ...
    if (auto MIB = BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, MIMD, TII.get(Opc),
                       ResultReg); IndexReg)
...
bool PPCFastISel::SelectBranch(const Instruction *I) {
  ...
  if (const CmpInst *MachineBasicBlock *FBB = FuncInfo.getMBB(BI->getSuccessor(1)); CI = dyn_cast<CmpInst>(BI->getCondition())) {
  ...
  if (CModel == CodeModel::Small) {
    ...
    if (auto MIB = BuildMI(
        *FuncInfo.MBB, FuncInfo.InsertPt, MIMD,
        IsAIXTocData ? TII.get(PPC::ADDItoc8) : TII.get(PPC::LDtoc), DestReg); IsAIXTocData)
