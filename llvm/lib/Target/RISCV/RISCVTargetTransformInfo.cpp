...
RISCVTTIImpl::getShuffleCost(TTI::ShuffleKind Kind, VectorType *DstTy,
  ...
  if (auto *FVTp = dyn_cast<FixedVectorType>(SrcTy);
    ...
    if (InstructionCost VRegSplittingCost = costShuffleViaVRegSplitting(
        *this, LT.second, ST->getRealVLen(),
        Kind == TTI::SK_InsertSubvector ? DstTy : SrcTy, Mask, CostKind); VRegSplittingCost.isValid())
...
RISCVTTIImpl::getIntrinsicInstrCost(const IntrinsicCostAttributes &ICA,
  ...
  if (ST->hasVInstructions() && RetTy->isVectorTy()) {
    if (auto LT = getTypeLegalizationCost(RetTy);
      ...
      if (const auto *MVT EltTy = LT.second.getVectorElementType(); Entry = CostTableLookup(VectorIntrinsicCostTable,
                                              ICA.getID(), EltTy))
