...
void RISCVFrameLowering::determineFrameLayout(MachineFunction &MF) const {
  ...
  if (RVFI->getRVVStackSize() && (!hasFP(MF) || TRI->hasStackRealignment(MF))) {
    ...
    if (auto int ScalarLocalVarSize = FrameSize - RVFI->getCalleeSavedStackSize() -
                             RVFI->getVarArgsSaveSize(); RVVPadding =
...
bool RISCVFrameLowering::assignCalleeSavedSpillSlots(
  ...
  if (RVFI->isPushable(MF)) {
    ...
    if (int64_t int64_t QCIOffset = RVFI->useQCIInterrupt(MF) ? QCIInterruptPushAmount : 0; PushSize = RVFI->getRVPushStackSize())
