...
bool RISCVInstructionSelector::hasAllNBitUsers(const MachineInstr &MI,
  ...
  for (auto &UserOp : MRI->use_nodbg_operands(DestReg)) {
    ...
    case RISCV::SRLI: {
      ...
      // N-Bit user.
      if (unsigned ShAmt = UserMI.getOperand(2).getImm(); Bits > ShAmt && hasAllNBitUsers(UserMI, Bits - ShAmt, Depth + 1))
        break;
      return false;
    }
    }
...
RISCVInstructionSelector::selectShiftMask(MachineOperand &Root,
  ...
  if (mi_match(ShAmtReg, *MRI, m_GAnd(m_Reg(AndSrcReg), m_ICst(AndMask)))) {
    ...
    if (APInt ShMask(AndMask.getBitWidth(), ShiftWidth - 1); ShMask.isSubsetOf(AndMask)) {
      ...
      if (KnownBits Known = VT->getKnownBits(AndSrcReg); ShMask.isSubsetOf(AndMask | Known.Zero))
        ShAmtReg = AndSrcReg;
    }
  }
...
RISCVInstructionSelector::selectSExtBits(MachineOperand &Root,
  ...
  if (MachineInstr *RootDef = MRI->getVRegDef(RootReg); RootDef->getOpcode() == TargetOpcode::G_SEXT_INREG &&
...
RISCVInstructionSelector::selectZExtBits(MachineOperand &Root,
  ...
  if (uint64_t Mask = maskTrailingOnes<uint64_t>(Bits); mi_match(RootReg, *MRI, m_GAnd(m_Reg(RegX), m_SpecificICst(Mask)))) {
...
RISCVInstructionSelector::selectSHXADD_UWOp(MachineOperand &Root,
  ...
  if (mi_match(
    ...
    if (Mask.isShiftedMask()) {
      ...
      if (unsigned Trailing = Mask.countr_zero(); Leading == 32 - ShAmt && C2 == Trailing && Trailing > ShAmt) {
        Register DstReg = MRI->createVirtualRegister(&RISCV::GPRRegClass);
        return {{[=](MachineInstrBuilder &MIB) {
          MachineIRBuilder(*MIB.getInstr())
...
RISCVInstructionSelector::renderVLOp(MachineOperand &Root) const {
  ...
  if (MachineInstr *RootDef = MRI->getVRegDef(Root.getReg()); RootDef->getOpcode() == TargetOpcode::G_CONSTANT) {
...
RISCVInstructionSelector::selectAddrRegImm(MachineOperand &Root) const {
  ...
  if (isBaseWithConstantOffset(Root, *MRI)) {
    ...
    if (int64_t RHSC = RHSDef->getOperand(1).getCImm()->getSExtValue(); isInt<12>(RHSC)) {
...
static unsigned selectZalasrLoadStoreOp(unsigned GenericOpc, unsigned OpSize) {
  ...
  switch (const bool IsStore = GenericOpc == TargetOpcode::G_STORE; OpSize) {
  default:
...
bool RISCVInstructionSelector::select(MachineInstr &MI) {
  ...
  case TargetOpcode::G_CONSTANT: {
    ...
    if (int64_t Imm = MI.getOperand(1).getCImm()->getSExtValue(); !materializeImm(DstReg, Imm, MIB))
      return false;
  ...
  case TargetOpcode::G_FCONSTANT: {
    ...
    if (Size == 16 || Size == 32 || (Size == 64 && Subtarget->is64Bit())) {
      ...
      if (FPimm.isPosZero()) {
        ...
        if (APInt Imm = FPimm.bitcastToAPInt(); !materializeImm(GPRReg, Imm.getSExtValue(), MIB))
          return false;
      }
...
bool RISCVInstructionSelector::selectFPCompare(MachineInstr &MI,
  ...
  if (legalizeFCmpPredicate(LHS, RHS, Pred, NeedInvert)) {
    ...
    if (auto Cmp = MIB.buildInstr(getFCmpOpcode(Pred, Size), {TmpReg}, {LHS, RHS}); !Cmp.constrainAllUses(TII, TRI, RBI))
      return false;
  ...
  if (NeedInvert) {
    ...
    if (auto Xor = MIB.buildInstr(RISCV::XORI, {DstReg}, {TmpReg}).addImm(1); !Xor.constrainAllUses(TII, TRI, RBI))
      return false;
  }
