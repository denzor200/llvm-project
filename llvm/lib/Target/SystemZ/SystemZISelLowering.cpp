...
SystemZVectorConstantInfo::SystemZVectorConstantInfo(APInt IntImm) {
  ...
  while (Width > 8) {
    ...
    if (APInt LowValue = SplatBits.trunc(HalfSize); HighValue != LowValue || 8 > HalfSize)
      break;
...
SystemZTargetLowering::lowerINTRINSIC_W_CHAIN(SDValue Op,
  ...
  if (unsigned Opcode, CCValid; isIntrinsicWithCCAndChain(Op, Opcode, CCValid)) {
    assert(Op->getNumValues() == 2 && "Expected only CC result and chain");
...
SystemZTargetLowering::lowerINTRINSIC_WO_CHAIN(SDValue Op,
  ...
  if (unsigned Opcode, CCValid; isIntrinsicWithCC(Op, Opcode, CCValid)) {
...
static SDValue getGeneralPermuteNode(SelectionDAG &DAG, const SDLoc &DL,
  ...
  if (unsigned StartIndex, OpNo0, OpNo1; isShlDoublePermute(Bytes, StartIndex, OpNo0, OpNo1))
...
SDValue GeneralShuffle::getNode(SelectionDAG &DAG, const SDLoc &DL) {
  ...
  for (; Stride * 2 < Ops.size(); Stride *= 2) {
    for (unsigned I = 0; I < Ops.size() - Stride; I += Stride * 2) {
      ...
      if (const Permute *SmallVector<int, SystemZ::VectorBytes> NewBytesMap(SystemZ::VectorBytes); P = matchDoublePermute(NewBytes, NewBytesMap)) {
...
SDValue SystemZTargetLowering::lowerShift(SDValue Op, SelectionDAG &DAG,
  ...
  if (auto *unsigned ElemBitSize = VT.getScalarSizeInBits(); BVN = dyn_cast<BuildVectorSDNode>(Op1)) {
    ...
    if (bool HasAnyUndefs; BVN->isConstantSplat(SplatBits, SplatUndef, SplatBitSize, HasAnyUndefs,
                             ElemBitSize, true) &&
...
SDValue SystemZTargetLowering::combineSTORE(
  ...
  if (Subtarget.hasVector() && DCI.Level == BeforeLegalizeTypes &&
    ...
    if (isa<BuildVectorSDNode>(Op1) &&
      ...
      if (auto *SDValue SplatVal = Op1->getOperand(0); C = dyn_cast<ConstantSDNode>(SplatVal))
...
SystemZTargetLowering::getJumpConditionMergingParams(Instruction::BinaryOps Opc,
  ...
  const auto isFlagOutOpCC = [](const Value *V) {
    ...
    if (const auto *const APInt *RHSC; I = dyn_cast<Instruction>(V)) {
...
SystemZTargetLowering::computeKnownBitsForTargetNode(const SDValue Op,
  ...
  // not used
  if (unsigned Tmp0, Tmp1; Op.getResNo() == 1 && isIntrinsicWithCC(Op, Tmp0, Tmp1)) {
