...
static SDValue getGeneralPermuteNode(SelectionDAG &DAG, const SDLoc &DL,
  ...
  if (unsigned ZeroVecIdx = findZeroVectorIdx(&Ops[0], 2); ZeroVecIdx != UINT32_MAX) {
    bool MaskFirst = true;
    int ZeroIdx = -1;
    for (unsigned I = 0; I < SystemZ::VectorBytes; ++I) {
...
SDValue SystemZTargetLowering::combineLOAD(
  ...
  if (LoadSDNode *LD = cast<LoadSDNode>(N); LD->isSimple() && ISD::isNormalLoad(LD)) {
...
SDValue SystemZTargetLowering::combineINT_TO_FP(
  ...
  if (unsigned InScalarBits = Op->getValueType(0).getScalarSizeInBits(); OutLLVMTy->isVectorTy() && OutScalarBits > InScalarBits &&
      OutScalarBits <= 64) {
