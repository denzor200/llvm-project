...
bool AVRDAGToDAGISel::selectIndexedLoad(SDNode *N) {
  ...
  switch (int Offs = cast<ConstantSDNode>(LD->getOffset())->getSExtValue(); VT.SimpleTy) {
  case MVT::i8: {
...
unsigned AVRDAGToDAGISel::selectIndexedProgMemLoad(const LoadSDNode *LD, MVT VT,
  ...
  if (int Offs = cast<ConstantSDNode>(LD->getOffset())->getSExtValue(); VT.SimpleTy == MVT::i8 && Offs == 1 && Bank == 0)
    Opcode = AVR::LPMRdZPi;
...
bool AVRDAGToDAGISel::SelectInlineAsmMemoryOperand(
  ...
  if (const RegisterSDNode *RegNode = dyn_cast<RegisterSDNode>(Op); RegNode &&
...
template <> bool AVRDAGToDAGISel::select<ISD::STORE>(SDNode *N) {
  ...
  if (const RegisterSDNode *RN = dyn_cast<RegisterSDNode>(BasePtr.getOperand(0)); !RN || (RN->getReg() != AVR::SP)) {
    return false;
  }
...
template <> bool AVRDAGToDAGISel::select<AVRISD::CALL>(SDNode *N) {
  ...
  if (unsigned Op = Callee.getOpcode(); Op == ISD::TargetGlobalAddress || Op == ISD::TargetExternalSymbol) {
    return false;
  }
