...
bool X86InstructionSelector::selectCopy(MachineInstr &I,
  ...
  if (DstReg.isPhysical()) {
    ...
    if (DstSize > SrcSize && SrcRegBank.getID() == X86::GPRRegBankID &&
      ...
      if (const TargetRegisterClass *DstRC = getRegClassFromGRPhysReg(DstReg); SrcRC != DstRC) {
  ...
  if (SrcRegBank.getID() == X86::GPRRegBankID &&
    ...
    if (const TargetRegisterClass *SrcRC = getRegClassFromGRPhysReg(SrcReg); DstRC != SrcRC) {
...
bool X86InstructionSelector::select(MachineInstr &I) {
  ...
  if (unsigned Opcode = I.getOpcode(); !isPreISelGenericOpcode(Opcode) && !I.isPreISelOpcode()) {
...
unsigned X86InstructionSelector::getPtrLoadStoreOp(const LLT &Ty,
  ...
  if (Ty.isPointer() && X86::GPRRegBankID == RB.getID()) {
    ...
    switch (bool IsLoad = (Opc == TargetOpcode::G_LOAD); Ty.getSizeInBits()) {
    default:
      break;
    case 32:
...
unsigned X86InstructionSelector::getLoadStoreOp(const LLT &Ty,
  ...
  if (bool HasVLX = STI.hasVLX(); Ty == LLT::scalar(8)) {
...
static bool X86SelectAddress(MachineInstr &I, const X86TargetMachine &TM,
  ...
  case TargetOpcode::G_PTR_ADD: {
    if (auto COff = getIConstantVRegSExtVal(I.getOperand(2).getReg(), MRI)) {
      ...
      if (int64_t Imm = *COff; isInt<32>(Imm)) { // Check for displacement overflow.
...
bool X86InstructionSelector::selectLoadStoreOp(MachineInstr &I,
  ...
  if (auto &MemOp = **I.memoperands_begin(); MemOp.isAtomic()) {
...
bool X86InstructionSelector::selectExtract(MachineInstr &I,
  ...
  if (bool HasVLX = STI.hasVLX(); SrcTy.getSizeInBits() == 256 && DstTy.getSizeInBits() == 128) {
    if (HasVLX)
...
bool X86InstructionSelector::selectInsert(MachineInstr &I,
  ...
  if (bool HasVLX = STI.hasVLX(); DstTy.getSizeInBits() == 256 && InsertRegTy.getSizeInBits() == 128) {
    if (HasVLX)
...
bool X86InstructionSelector::materializeFP(MachineInstr &I,
  ...
  if (unsigned char OpFlag = STI.classifyLocalReference(nullptr); CM == CodeModel::Large && STI.is64Bit()) {
...
bool X86InstructionSelector::selectImplicitDefOrPHI(
  ...
  if (!MRI.getRegClassOrNull(DstReg)) {
    ...
    if (const TargetRegisterClass *RC = getRegClass(DstTy, DstReg, MRI); !RBI.constrainGenericRegister(DstReg, *RC, MRI)) {
      LLVM_DEBUG(dbgs() << "Failed to constrain " << TII.getName(I.getOpcode())
                        << " operand\n");
      return false;
    }
...
bool X86InstructionSelector::selectSelect(MachineInstr &I,
  ...
  if (const TargetRegisterClass *DstRC = getRegClass(Ty, DstReg, MRI); !RBI.constrainGenericRegister(DstReg, *DstRC, MRI)) {
