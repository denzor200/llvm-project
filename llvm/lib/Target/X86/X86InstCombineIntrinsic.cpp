...
static Value *getBoolVecFromMask(Value *Mask, const DataLayout &DL) {
  ...
  if (Value *ExtMask; match(Mask, m_SExt(m_Value(ExtMask))) &&
...
static Value *simplifyX86varShift(const IntrinsicInst &II,
  ...
  if (KnownBits KnownAmt =
      llvm::computeKnownBits(Amt, II.getDataLayout()); KnownAmt.getMaxValue().ult(BitWidth)) {
    return (LogicalShift ? (ShiftLeft ? Builder.CreateShl(Vec, Amt)
                                      : Builder.CreateLShr(Vec, Amt))
                         : Builder.CreateAShr(Vec, Amt));
  }
...
X86TTIImpl::instCombineIntrinsic(InstCombiner &IC, IntrinsicInst &II) const {
  ...
  case Intrinsic::x86_bmi_pext_64:
    if (auto *MaskC = dyn_cast<ConstantInt>(II.getArgOperand(1))) {
      ...
      if (unsigned MaskIdx, MaskLen; MaskC->getValue().isShiftedMask(MaskIdx, MaskLen)) {
  ...
  case Intrinsic::x86_avx512_cvttsd2usi64: {
    ...
    if (Value *unsigned VWidth = cast<FixedVectorType>(Arg->getType())->getNumElements(); V = SimplifyDemandedVectorEltsLow(Arg, VWidth, 1)) {
  ...
  case Intrinsic::x86_avx512_psll_w_512: {
    ...
    if (Value *unsigned VWidth = cast<FixedVectorType>(Arg1->getType())->getNumElements(); V = SimplifyDemandedVectorEltsLow(Arg1, VWidth, VWidth / 2)) {
  ...
  case Intrinsic::x86_sse4a_extrq: {
    ...
    if (Value *auto *CIIndex =
        C1 ? dyn_cast_or_null<ConstantInt>(C1->getAggregateElement((unsigned)1))
           : nullptr; V = simplifyX86extrq(II, Op0, CILength, CIIndex, IC.Builder)) {
      return IC.replaceInstUsesWith(II, V);
    }
  ...
  case Intrinsic::x86_sse4a_extrqi: {
    ...
    if (Value *auto *CIIndex = dyn_cast<ConstantInt>(II.getArgOperand(2)); V = simplifyX86extrq(II, Op0, CILength, CIIndex, IC.Builder)) {
      return IC.replaceInstUsesWith(II, V);
    }
  ...
  case Intrinsic::x86_sse4a_insertqi: {
    ...
    if (auto *CIIndex = dyn_cast<ConstantInt>(II.getArgOperand(3)); CILength && CIIndex) {
      ...
      if (Value *APInt Idx = CIIndex->getValue().zextOrTrunc(6); V = simplifyX86insertq(II, Op0, Op1, Len, Idx, IC.Builder)) {
        return IC.replaceInstUsesWith(II, V);
      }
    }
  ...
  case Intrinsic::x86_avx512_pshuf_b_512: {
    ...
    if (KnownBits KnownMask(8); IC.SimplifyDemandedBits(&II, 1, APInt(8, 0b10001111), KnownMask))
      return &II;
    break;
  }
  ...
  case Intrinsic::x86_avx512_vpermilvar_ps_512: {
    ...
    if (KnownBits KnownMask(32); IC.SimplifyDemandedBits(&II, 1, APInt(32, 0b00011), KnownMask))
      return &II;
    break;
  }
  ...
  case Intrinsic::x86_avx512_vpermilvar_pd_512: {
    ...
    if (KnownBits KnownMask(64); IC.SimplifyDemandedBits(&II, 1, APInt(64, 0b00010), KnownMask))
      return &II;
    break;
  }
