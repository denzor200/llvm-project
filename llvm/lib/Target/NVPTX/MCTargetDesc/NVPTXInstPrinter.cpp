...
void NVPTXInstPrinter::printRegName(raw_ostream &OS, MCRegister Reg) {
  ...
  switch (unsigned RCId = (Reg.id() >> 28); RCId) {
...
void NVPTXInstPrinter::printCvtMode(const MCInst *MI, int OpNum, raw_ostream &O,
  ...
  if (int64_t Imm = MO.getImm(); Modifier == "ftz") {
    // FTZ flag
...
void NVPTXInstPrinter::printFTZFlag(const MCInst *MI, int OpNum,
  ...
  if (const int Imm = MO.getImm(); Imm)
    O << ".ftz";
}
...
void NVPTXInstPrinter::printAtomicCode(const MCInst *MI, int OpNum,
  ...
  if (int Imm = (int)MO.getImm(); Modifier == "sem") {
    
    switch (auto Ordering = NVPTX::Ordering(Imm); Ordering) {
...
void NVPTXInstPrinter::printMmaCode(const MCInst *MI, int OpNum, raw_ostream &O,
  ...
  if (int Imm = (int)MO.getImm(); Modifier.empty() || Modifier == "version") {
...
void NVPTXInstPrinter::printUsedBytesMaskPragma(const MCInst *MI, int OpNum,
  ...
  if (uint32_t Imm = (uint32_t)Op.getImm(); Imm != UINT32_MAX) {
...
void NVPTXInstPrinter::printRegisterOrSinkSymbol(const MCInst *MI, int OpNum,
  ...
  if (const MCOperand &Op = MI->getOperand(OpNum); Op.isReg() && Op.getReg() == MCRegister::NoRegister)
    O << "_";
  else
    printOperand(MI, OpNum, O);
}
