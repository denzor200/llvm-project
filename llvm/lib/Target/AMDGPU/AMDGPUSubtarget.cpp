...
std::pair<unsigned, unsigned> AMDGPUSubtarget::getOccupancyWithWorkGroupSizes(
  ...
  if (MinWavesPerCU >= MaxWavesPerCU) {
    ...
    if (unsigned MinWavesPerCUForWGSize =
      ...
      if (unsigned unsigned ExcessSlots = MinWavesPerCU - MinWavesPerCUForWGSize; ExcessSlotsPerWG = ExcessSlots / MinWGsPerCU) {
  ...
  if (auto *CI = dyn_cast<CallInst>(I)) {
    ...
    if (const Function *F = CI->getCalledFunction(); F) {
      ...
      if (Dim <= 3) {
        ...
        if (std::optional<unsigned> ReqdSize = getReqdWorkGroupSize(*Kernel, Dim); ReqdSize)
