...
void SIInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  if (unsigned SrcSize = RI.getRegSizeInBits(*SrcRC); Fix16BitCopies) {
...
MachineInstr *SIInstrInfo::buildShrunkInst(MachineInstr &MI,
  ...
  for (const MachineOperand &Use : MI.explicit_uses()) {
    ...
    if (int OpTy = MI.getDesc().operands()[Idx++].OperandType; OpTy == AMDGPU::OPERAND_INPUT_MODS || OpTy == MCOI::OPERAND_IMMEDIATE)
      continue;
