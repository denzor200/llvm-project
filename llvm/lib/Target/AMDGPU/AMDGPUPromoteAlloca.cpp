...
PreservedAnalyses AMDGPUPromoteAllocaPass::run(Function &F,
  ...
  if (bool Changed = AMDGPUPromoteAllocaImpl(TM, LI).run(F, /*PromoteToLDS=*/true); Changed) {
...
AMDGPUPromoteAllocaToVectorPass::run(Function &F, FunctionAnalysisManager &AM) {
  ...
  if (bool Changed = AMDGPUPromoteAllocaImpl(TM, LI).run(F, /*PromoteToLDS=*/false); Changed) {
    PreservedAnalyses PA;
    PA.preserveSet<CFGAnalyses>();
    return PA;
  }
...
bool AMDGPUPromoteAllocaImpl::run(Function &F, bool PromoteToLDS) {
  ...
  for (AllocaInst *AI : Allocas) {
    ...
    if (const unsigned AllocaCost = DL->getTypeSizeInBits(AI->getAllocatedType()); AllocaCost <= VectorizationBudget) {
...
static Value *promoteAllocaUserToVector(
  ...
  case Instruction::Call: {
    ...
    if (auto *MSI = dyn_cast<MemSetInst>(Inst)) {
      ...
      if (const unsigned BytesPerElt = DL.getTypeStoreSize(VecEltTy); BytesPerElt > 1) {
        

        // If the element type of the vector is a pointer, we need to first cast
        // to an integer, then use a PtrCast.
        if (Value *EltBytes = Builder.CreateVectorSplat(BytesPerElt, Elt); VecEltTy->isPointerTy()) {
...
bool AMDGPUPromoteAllocaImpl::tryPromoteAllocaToVector(AllocaInst &Alloca) {
  ...
  if (auto *ArrayTy = dyn_cast<ArrayType>(AllocaTy)) {
    ...
    if (VectorType::isValidElementType(ElemTy) && NumElems > 0) {
      ...
      if (unsigned ElementSize = DL->getTypeSizeInBits(ElemTy) / 8; ElementSize > 0) {
