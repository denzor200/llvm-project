  ...
  for (const BasicBlock *BB : L->getBlocks()) {
    ...
    for (const Instruction &I : *BB) {
      ...
      if (AS == AMDGPUAS::PRIVATE_ADDRESS) {
        ...
        if (const Value *V = getUnderlyingObject(GEP->getPointerOperand()); !isa<GlobalVariable>(V) && !isa<Argument>(V))
          continue;
...
InstructionCost GCNTTIImpl::getCFInstrCost(unsigned Opcode,
  ...
  switch (const int CBrCost = SCost ? 5 : 7; Opcode) {
  case Instruction::Br: {
...
Value *GCNTTIImpl::rewriteIntrinsicWithAddressSpace(IntrinsicInst *II,
  ...
  case Intrinsic::ptrmask: {
    ...
    if (const GCNTargetMachine &TM =
      ...
      if (KnownBits Known = computeKnownBits(MaskOp, DL, nullptr, II); Known.countMinLeadingOnes() < 32)
        return nullptr;
