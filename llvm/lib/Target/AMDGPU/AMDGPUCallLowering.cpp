...
struct AMDGPUOutgoingValueHandler : public CallLowering::OutgoingValueHandler {
  ...
  void assignValueToReg(Register ValVReg, Register PhysReg,
    ...
    if (const SIRegisterInfo *TRI
      ...
      if (LLT S32 = LLT::scalar(32); Ty != S32) {
  ...
  for (auto InputID : InputRegs) {
    ...
    if (IncomingArg) {
      ...
      if (std::optional<uint32_t> Id =
          AMDGPUMachineFunction::getLDSKernelIdMetadata(MF.getFunction()); Id) {
...
bool AMDGPUCallLowering::lowerChainCall(MachineIRBuilder &MIRBuilder,
  ...
  if (const Function *const Value *CalleeV = Callee.OrigValue->stripPointerCasts(); F = dyn_cast<Function>(CalleeV)) {
...
bool AMDGPUCallLowering::lowerCall(MachineIRBuilder &MIRBuilder,
  ...
  if (Info.CanLowerReturn && !Info.OrigRet.Ty->isVoidTy()) {
    ...
    if (CallReturnHandler Handler(MIRBuilder, MRI, MIB); !determineAndHandleAssignments(Handler, Assigner, InArgs, MIRBuilder,
