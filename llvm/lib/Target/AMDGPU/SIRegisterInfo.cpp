...
void SIRegisterInfo::resolveFrameIndex(MachineInstr &MI, Register BaseReg,
  ...
  case AMDGPU::V_ADD_CO_U32_e64: {
    ...
    if (int64_t TotalOffset = ImmOp->getImm() + Offset; TotalOffset == 0) {
      MI.setDesc(TII->get(AMDGPU::COPY));
  ...
  switch (MI->getOpcode()) {
    ...
    case AMDGPU::V_ADD_CO_U32_e64: {
      ...
      if ((!OtherOp->isImm() || OtherOp->getImm() != 0) && MaterializedReg) {
        ...
        if (unsigned MaterializedRegFlags =
            MaterializedReg != FrameReg ? RegState::Kill : 0; isVGPRClass(getPhysRegBaseClass(MaterializedReg))) {
    ...
    case AMDGPU::S_ADD_U32: {
      ...
      // handling, but we do an in-place update of the result register here to
      // avoid scavenging another register.
      if (int64_t Offset = FrameInfo.getObjectOffset(Index); OtherOp.isImm()) {
