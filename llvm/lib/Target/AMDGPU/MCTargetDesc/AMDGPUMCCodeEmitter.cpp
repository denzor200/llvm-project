...
void AMDGPUMCCodeEmitter::getSDWASrcEncoding(const MCInst &MI, unsigned OpNo,
  ...
  if (const MCOperand &MO = MI.getOperand(OpNo); MO.isReg()) {
    ...
    if (auto Enc = getLitEncoding(Desc, MO, OpNo, STI); Enc && *Enc != 255) {
...
void AMDGPUMCCodeEmitter::getMachineOpValueT16(
  ...
  if ((int)OpNo == AMDGPU::getNamedOperandIdx(MI.getOpcode(),
    ...
    if (int VDstMOIdx =
      ...
      if (auto DstReg = MI.getOperand(VDstMOIdx).getReg(); AMDGPU::isHi16Reg(DstReg, MRI))
...
void AMDGPUMCCodeEmitter::getMachineOpValueCommon(
  ...
  if (AMDGPU::isSISrcOperand(Desc, OpNo)) {
    ...
    if (auto bool HasMandatoryLiteral =
        AMDGPU::hasNamedOperand(MI.getOpcode(), AMDGPU::OpName::imm); Enc = getLitEncoding(Desc, MO, OpNo, STI, HasMandatoryLiteral)) {
      Op = *Enc;
      return;
    }
