...
bool AMDGPUOperand::isLiteralImm(MVT type) const {
  ...
  if (bool Allow64Bit =
      (type == MVT::i64 || type == MVT::f64) && AsmParser->has64BitLiterals(); !Imm.IsFPImm) {
    // We got int literal token.
...
bool AMDGPUAsmParser::validateOpSel(const MCInst &Inst) {
  ...
  if (isPackedFP32Inst(Opc) && isGFX12Plus()) {
    ...
    if (int Src2Idx = AMDGPU::getNamedOperandIdx(Opc, AMDGPU::OpName::src2); Src2Idx != -1) {
...
bool AMDGPUAsmParser::validateTHAndScopeBits(const MCInst &Inst,
  ...
  if (unsigned THType = AMDGPU::getTemporalHintType(TID); THType == AMDGPU::CPol::TH_TYPE_ATOMIC) {
...
bool AMDGPUAsmParser::ParseDirectivePALMetadataBegin() {
  ...
  if (auto *PALMetadata = getTargetStreamer().getPALMetadata(); !PALMetadata->setFromString(String))
...
bool AMDGPUAsmParser::parseCnt(int64_t &IntVal) {
  ...
  if (bool Sat = CntName.ends_with("_sat"); CntName == "vmcnt" || CntName == "vmcnt_sat") {
