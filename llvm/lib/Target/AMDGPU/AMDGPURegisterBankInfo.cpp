...
public:
  ...
  void applyBank(MachineInstr &MI) {
    ...
    if (const unsigned Opc = MI.getOpcode(); Opc == AMDGPU::G_ANYEXT || Opc == AMDGPU::G_ZEXT ||
        Opc == AMDGPU::G_SEXT) {
...
bool AMDGPURegisterBankInfo::applyMappingLoad(
  ...
  if (const RegisterBank *DstBank =
    ...
    if (LoadSize == 32) {
      ...
      if (const LLT S32 = LLT::scalar(32); MI.getOpcode() == AMDGPU::G_SEXTLOAD) {
  ...
  if (AMDGPU::isExtendedGlobalAddrSpace(MMO->getAddrSpace()) ||
    ...
    if (LegalizerHelper Helper(B.getMF(), O, B); LoadTy.isVector()) {
...
void AMDGPURegisterBankInfo::applyMappingImpl(
  ...
  case AMDGPU::G_XOR: {
    ...
    if (DstTy.getSizeInBits() == 1) {
      ...
      if (LegalizerHelper Helper(*MF, ApplyBank, B); Helper.widenScalar(MI, 0, LLT::scalar(32)) !=
          LegalizerHelper::Legalized)
        llvm_unreachable("widen scalar should have succeeded");
      return;
    }
    ...
    if (DstTy.getSizeInBits() == 16 && DstBank == &AMDGPU::SGPRRegBank) {
      ...
      // as "not".
      if (LegalizerHelper Helper(*MF, ApplySALU, B); MI.getOpcode() == AMDGPU::G_XOR &&
  ...
  case AMDGPU::G_ABS: {
    ...
    if (const RegisterBank *SrcBank = MRI.getRegBankOrNull(SrcReg); SrcBank && SrcBank == &AMDGPU::VGPRRegBank) {
      ...
      if (LegalizerHelper Helper(*MF, Apply, B); Helper.lowerAbsToMaxNeg(MI) != LegalizerHelper::Legalized)
    ...
    while (Start->getOpcode() != FrameSetupOpcode) {
      ...
      if (Start->getOpcode() == AMDGPU::COPY) {
        ...
        if (auto &Dst = Start->getOperand(0); Dst.isReg()) {
          ...
          if (Register Reg = Dst.getReg(); Reg.isPhysical() && MI.readsRegister(Reg, TRI)) {
            IsCopy = true;
          } else {
...
unsigned AMDGPURegisterBankInfo::getMappingType(const MachineRegisterInfo &MRI,
  ...
  for (const MachineOperand &MO : MI.operands()) {
    ...
    if (const RegisterBank *Register Reg = MO.getReg(); Bank = getRegBank(Reg, MRI, *TRI)) {
...
AMDGPURegisterBankInfo::getInstrMapping(const MachineInstr &MI) const {
  ...
  case AMDGPU::G_AMDGPU_MAD_I64_I32: {
    ...
    for (unsigned i = 0; i < 5; ++i) {
      ...
      if (const RegisterBank *Register Reg = MI.getOperand(i).getReg(); Bank = getRegBank(Reg, MRI, *TRI)) {
        if (Bank->getID() != AMDGPU::SGPRRegBankID) {
          AllSalu = false;
          if (i == 2 || i == 3) {
            MulSalu = false;
