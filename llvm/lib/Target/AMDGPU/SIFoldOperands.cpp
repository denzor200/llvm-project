...
bool SIFoldOperandsImpl::updateOperand(FoldCandidate &Fold) const {
  ...
  if ((Fold.isImm() || Fold.isFI() || Fold.isGlobal()) && Fold.needsShrink()) {
    ...
    if (auto Liveness = MBB->computeRegisterLiveness(TRI, AMDGPU::VCC, MI, 16); Liveness != MachineBasicBlock::LQR_Dead) {
  ...
  auto tryToFoldAsFMAAKorMK = [&]() {
    ...
    if (bool FoldAsFMAAKorMK =
      ...
      if (OpNo == 1) {
        ...
        if (Register OldReg = Op1.getReg(); Op2.isImm()) {
  ...
  for (unsigned K = 1; K < PHI.getNumExplicitOperands(); K += 2) {
    ...
    if (MachineInstr *Def = MRI->getVRegDef(Reg)) {
      ...
      if (Def->isCopy()) {
        ...
        if (unsigned AGPRSubReg = AMDGPU::NoSubRegister; isAGPRCopy(*TRI, *MRI, *Def, AGPRSrc, AGPRSubReg)) {
          MO.setReg(AGPRSrc);
