...
bool ARMPreAllocLoadStoreOpt::CanFormLdStDWord(
  ...
  if (isT2) {
    ...
    if (int Limit = (1 << 8) * Scale; OffImm >= Limit || (OffImm & (Scale-1)))
      return false;
...
bool ARMPreAllocLoadStoreOpt::DistributeIncrements(Register Base) {
  ...
  if (BaseAccess && Increment) {
    ...
    if (unsigned NewPostIncOpcode = getPostIndexedLoadStoreOpcode(
        BaseAccess->getOpcode(), IncrementOffset > 0 ? ARM_AM::add : ARM_AM::sub); !isLegalAddressImm(NewPostIncOpcode, IncrementOffset, TII)) {
