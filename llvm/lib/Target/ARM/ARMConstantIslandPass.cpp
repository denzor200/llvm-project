...
bool ARMConstantIslands::findAvailableWater(CPUser &U, unsigned UserOffset,
  ...
  for (water_iterator IP = std::prev(WaterList.end()), B = WaterList.begin();;
    ...
    if (unsigned Growth; isWaterInRange(UserOffset, WaterBB, U, Growth) &&
...
void ARMConstantIslands::createNewWater(unsigned CPUserIndex,
  ...
  if (LastIT) {
    ...
    if (ARMCC::CondCodes CC = getITInstrPredicate(*MI, PredReg); CC != ARMCC::AL)
      MI = LastIT;
  }
...
bool ARMConstantIslands::optimizeThumb2Branches() {
  ...
  auto FindCmpForCBZ = [this](ImmBranch &Br, ImmCompare &ImmCmp,
    ...
    if (ARMCC::CondCodes Pred = getInstrPredicate(*Br.MI, PredReg); Pred == ARMCC::EQ)
      NewOpc = ARM::tCBZ;
