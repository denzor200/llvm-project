...
std::pair<SDValue, MachinePointerInfo> ARMTargetLowering::computeAddrForCallArg(
  ...
  if (MachineFunction &MF = DAG.getMachineFunction(); IsTailCall) {
        Offset += SPDiff;
...
ARMTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  if (isa<GlobalAddressSDNode>(Callee)) {
    ...
    if (auto *GV = cast<GlobalAddressSDNode>(Callee)->getGlobal(); CLI.CB) {
  ...
  for (unsigned i = 0, realArgIdx = 0, e = ArgLocs.size();
    ...
    if (VA.needsCustom() &&
      ...
      if (auto ArgVT = Outs[realArgIdx].ArgVT; isCmseNSCall && (ArgVT == MVT::f16)) {
  ...
  if (Subtarget->genLongCalls()) {
    ...
    if (isa<GlobalAddressSDNode>(Callee)) {
      ...
      if (const char *Sym = S->getSymbol(); Subtarget->genExecuteOnly()) {
        if (Subtarget->useMovt())
          ++NumMovwMovt;
        Callee = DAG.getNode(ARMISD::Wrapper, dl, PtrVt,
                             DAG.getTargetGlobalAddress(GVal, dl, PtrVt));
    ...
    if (const char *Sym = S->getSymbol(); isARMFunc && Subtarget->isThumb1Only() && !Subtarget->hasV5TOps()) {
      unsigned ARMPCLabelIndex = AFI->createPICLabelUId();
...
bool ARMTargetLowering::IsEligibleForTailCallOptimization(
  ...
  if (bool isCallerStructRet = MF.getFunction().hasStructRetAttr(); isCalleeStructRet != isCallerStructRet) {
  ...
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee)) {
    ...
    if (const Triple &TT = getTargetMachine().getTargetTriple(); GV->hasExternalWeakLinkage() &&
...
ARMTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
 ...
 // Report error if cmse entry function returns structure through first ptr arg.
  ...
  for (unsigned i = 0, realRVLocIdx = 0;
    ...
    if (Subtarget->hasFullFP16() && getTM().isTargetHardFloat()) {
      ...
      if (Arg.getValueType() == MVT::f32 && Arg.getOpcode() == ISD::BITCAST) {
        ...
        if (SDValue ZE = Arg.getOperand(0); ZE.getOpcode() == ISD::ZERO_EXTEND && ZE.getValueType() == MVT::i32) {
          
          if (SDValue BC = ZE.getOperand(0); BC.getOpcode() == ISD::BITCAST && BC.getValueType() == MVT::i16) {
            Arg = BC.getOperand(0);
            ReturnF16 = true;
          }
        }
  ...
  if (const MCPhysReg *I =
      TRI->getCalleeSavedRegsViaCopy(&DAG.getMachineFunction()); I) {
    for (; *I; ++I) {
...
bool ARMTargetLowering::isUsedByReturnOnly(SDNode *N, SDValue &Chain) const {
  ...
  if (Copy->getOpcode() == ISD::CopyToReg) {
    ...
    for (SDNode *U : VMov->users()) {
      ...
      if (SDValue UseChain = U->getOperand(0); Copies.count(UseChain.getNode()))
        // Second CopyToReg
        Copy = U;
      else {
...
ARMTargetLowering::LowerGlobalTLSAddress(SDValue Op, SelectionDAG &DAG) const {
  ...
  switch (TLSModel::Model model = getTargetMachine().getTLSModel(GA->getGlobal()); model) {
...
static SDValue LowerATOMIC_FENCE(SDValue Op, SelectionDAG &DAG,
  ...
  if (auto SSID = static_cast<SyncScope::ID>(Op.getConstantOperandVal(2)); SSID == SyncScope::SingleThread)
    return Op;
...
SDValue ARMTargetLowering::LowerFormalArguments(
  ...
  if (isVarArg && MFI.hasVAStart()) {
    ...
    if (unsigned RegIdx = CCInfo.getFirstUnallocated(GPRArgRegs); RegIdx != std::size(GPRArgRegs))
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (int index = VA.getValNo(); index != lastInsIndex)
        {
          ISD::ArgFlagsTy Flags = Ins[index].Flags;
...
static bool isFloatingPointZero(SDValue Op) {
  if (ConstantFPSDNode *CFP = dyn_cast<ConstantFPSDNode>(Op))
    ...
    if (Op.getOperand(1).getOpcode() == ARMISD::Wrapper) {
      ...
      if (ConstantPoolSDNode *SDValue WrapperOp = Op.getOperand(1).getOperand(0); CP = dyn_cast<ConstantPoolSDNode>(WrapperOp))
    ...
    if (SDValue BitcastOp = Op->getOperand(0); BitcastOp->getOpcode() == ARMISD::VMOVIMM &&
...
SDValue ARMTargetLowering::getARMCmp(SDValue LHS, SDValue RHS, ISD::CondCode CC,
  ...
  if (ConstantSDNode *RHSC = dyn_cast<ConstantSDNode>(RHS.getNode())) {
    ...
    if (unsigned C = RHSC->getZExtValue(); !isLegalICmpImmediate((int32_t)C)) {
  ...
  if (Subtarget->isThumb1Only() && LHS->getOpcode() == ISD::AND &&
    ...
    if (uint64_t RHSV = RHSC->getZExtValue(); isMask_32(Mask) && (RHSV & ~Mask) == 0 && Mask != 255 && Mask != 65535) {
      
      if (unsigned ShiftBits = llvm::countl_zero(Mask); RHSV && (RHSV > 255 || (RHSV << ShiftBits) <= 255)) {
...
SDValue ARMTargetLowering::LowerSELECT(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (unsigned Opc = Cond.getOpcode(); Cond.getResNo() == 1 &&
  ...
  if (Cond.getOpcode() == ARMISD::CMOV && Cond.hasOneUse()) {
    ...
    if (const ConstantSDNode *CMOVFalse =
      dyn_cast<ConstantSDNode>(Cond.getOperand(1)); CMOVTrue && CMOVFalse) {
...
SDValue ARMTargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (VT == MVT::i32 &&
    ...
    if (SDValue ShiftV = DAG.getNode(ISD::SRA, dl, VT, SatValue,
                                 DAG.getConstant(31, dl, VT)); isNullConstant(LowerSatConstant)) {
  ...
  if (LHS.getValueType() == MVT::i32) {
    ...
    if (Subtarget->hasFPARMv8Base() && (TrueVal.getValueType() == MVT::f16 ||
      ...
      if (ARMCC::CondCodes CondCode = IntCCToARMCC(CC); CondCode == ARMCC::LT || CondCode == ARMCC::LE ||
...
ARMTargetLowering::OptimizeVFPBrcond(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (bool RHSOk = canChangeToInt(RHS, RHSSeenZero, Subtarget); LHSOk && RHSOk && (LHSSeenZero || RHSSeenZero)) {
...
SDValue ARMTargetLowering::LowerBRCOND(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (bool OptimizeMul = (Opc == ISD::SMULO || Opc == ISD::UMULO) &&
                      !Subtarget->isThumb1Only(); Cond.getResNo() == 1 &&
      (Opc == ISD::SADDO || Opc == ISD::UADDO || Opc == ISD::SSUBO ||
...
SDValue ARMTargetLowering::LowerFCOPYSIGN(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (bool UseNEON = !InGPR && Subtarget->hasNEON(); UseNEON) {
...
SDValue ARMTargetLowering::LowerConstantFP(SDValue Op, SelectionDAG &DAG,
  ...
  if (ST->genExecuteOnly()) {
    ...
    switch (SDLoc DL(CFP); VT.getSimpleVT().SimpleTy) {
      default:
...
static bool isVTRNMask(ArrayRef<int> M, EVT VT, unsigned &WhichResult) {
  ...
  if (unsigned EltSz = VT.getScalarSizeInBits(); EltSz == 64)
    return false;
  ...
  for (unsigned i = 0; i < M.size(); i += NumElts) {
    ...
    for (unsigned j = 0; j < NumElts; j += Half) {
      ...
      for (unsigned k = 0; k < Half; ++k) {
        ...
        if (int MIdx = M[i + j + k]; MIdx >= 0 && (unsigned) MIdx != Idx)
          return false;
        Idx += 2;
      }
    }
...
static SDValue LowerBuildVectorOfFPExt(SDValue BV, SelectionDAG &DAG,
  ...
  for (unsigned i = 1; i < 4; i++) {
    ...
    if (auto Check = [](SDValue Trunc, SDValue Op, unsigned Idx) {
      return Trunc.getOpcode() == ISD::FP_EXTEND &&
             Trunc.getOperand(0).getOpcode() == ISD::EXTRACT_VECTOR_ELT &&
             Trunc.getOperand(0).getOperand(0) == Op &&
             Trunc.getOperand(0).getConstantOperandVal(1) == Idx;
    }; !Check(BV.getOperand(i), Op0, 2 * i + Offset))
      return SDValue();
  }
...
static SDValue LowerBUILD_VECTORToVIDUP(SDValue Op, SelectionDAG &DAG,
  ...
  for (unsigned I = 2; I < NumElts; I++) {
    ...
    if (SDValue OpI = Op.getOperand(I); OpI.getOpcode() != ISD::ADD || OpI.getOperand(0) != Op0 ||
...
SDValue ARMTargetLowering::LowerBUILD_VECTOR(SDValue Op, SelectionDAG &DAG,
  ...
  if (BVN->isConstantSplat(SplatBits, SplatUndef, SplatBitSize, HasAnyUndefs)) {
    ...
    if ((ST->hasNEON() && SplatBitSize <= 64) ||
      ...
      if ((VT == MVT::v2f32 || VT == MVT::v4f32) && SplatBitSize == 32) {
        ...
        if (int ImmVal = ARM_AM::getFP32Imm(SplatBits); ImmVal != -1) {
  ...
  if (hasDominantValue && EltSize <= 32) {
    if (!isConstant) {
      ...
      if (ConstantSDNode *constIndex; Value->getOpcode() == ISD::EXTRACT_VECTOR_ELT &&
    ...
    if (usesOnlyOneValue) {
      ...
      if (SDValue Val = IsSingleInstrConstant(Value, DAG, ST, dl); isConstant && Val.getNode())
...
static bool isLegalMVEShuffleOp(unsigned PFEntry) {
  ...
  switch (unsigned OpNum = (PFEntry >> 26) & 0x0F; OpNum) {
  case OP_COPY:
  case OP_VREV:
  case OP_VDUP0:
  case OP_VDUP1:
...
bool ARMTargetLowering::isShuffleMaskLegal(ArrayRef<int> M, EVT VT) const {
  if (VT.getVectorNumElements() == 4 &&
    ...
    if (unsigned Cost = (PFEntry >> 30); Cost <= 4 && (Subtarget->hasNEON() || isLegalMVEShuffleOp(PFEntry)))
      return true;
  }
  ...
  if (unsigned EltSize = VT.getScalarSizeInBits(); EltSize >= 32 ||
...
static SDValue GeneratePerfectShuffle(unsigned PFEntry, SDValue LHS,
  ...
  switch (EVT VT = OpLHS.getValueType(); OpNum) {
...
static SDValue LowerVECTOR_SHUFFLEUsingMovs(SDValue Op,
  ...
  for (int Part = 0; Part < 4; ++Part) {
    ...
    if (int Elt = getMovIdx(ShuffleMask, Part * QuarterSize, QuarterSize); Elt != -1) {
...
static SDValue LowerVECTOR_SHUFFLE(SDValue Op, SelectionDAG &DAG,
  ...
  if (NumElts == 4) {
    ...
    if (unsigned Cost = (PFEntry >> 30); Cost <= 4) {
      if (ST->hasNEON())
        ...
        if (unsigned PFEntryRHS = PerfectShuffleTable[RHSID]; isLegalMVEShuffleOp(PFEntryLHS) && isLegalMVEShuffleOp(PFEntryRHS))
          return GeneratePerfectShuffle(PFEntry, V1, V2, DAG, dl);
      }
    }
  }
...
static bool isExtendedBUILD_VECTOR(SDNode *N, SelectionDAG &DAG,
  ...
  for (unsigned i = 0, e = N->getNumOperands(); i != e; ++i) {
    ...
    if (ConstantSDNode *SDNode *Elt = N->getOperand(i).getNode(); C = dyn_cast<ConstantSDNode>(Elt)) {
      unsigned EltSize = VT.getScalarSizeInBits();
      
      if (unsigned HalfSize = EltSize / 2; isSigned) {
...
static bool isAddSubSExt(SDNode *N, SelectionDAG &DAG) {
  ...
  if (unsigned Opcode = N->getOpcode(); Opcode == ISD::ADD || Opcode == ISD::SUB) {
...
static SDValue LowerMUL(SDValue Op, SelectionDAG &DAG) {
  ...
  if (bool isN1SExt = isSignExtended(N1, DAG); isN0SExt && isN1SExt)
    NewOpc = ARMISD::VMULLs;
  else {
    bool isN0ZExt = isZeroExtended(N0, DAG);
    
    if (bool isN1ZExt = isZeroExtended(N1, DAG); isN0ZExt && isN1ZExt)
...
static SDValue createGPRPairNodei64(SelectionDAG &DAG, SDValue V) {
  ...
  if (bool isBigEndian = DAG.getDataLayout().isBigEndian(); isBigEndian)
    std::swap(VLo, VHi);
...
SDValue ARMTargetLowering::LowerCMP(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (RHS.getOpcode() == ISD::SUB) {
    ...
    // Check if it's 0 - X
    if (SDValue SubRHS = RHS.getOperand(1); isNullConstant(SubLHS)) {
...
void ARMTargetLowering::EmitSjLjDispatchBlock(MachineInstr &MI,
  ...
  if (unsigned NumLPads = LPadList.size(); Subtarget->isThumb2()) {
    Register NewVReg1 = MRI->createVirtualRegister(TRC);
  ...
  for (MachineBasicBlock *BB : InvokeBBs) {
    ...
    while (!Successors.empty()) {
      ...
      if (MachineBasicBlock *SMBB = Successors.pop_back_val(); SMBB->isEHPad()) {
...
ARMTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  switch (bool isThumb2 = Subtarget->isThumb2(); MI.getOpcode()) {
  default: {
    MI.print(errs());
  ...
  case ARM::tMOVCCr_pseudo: {
    ...
    if (const TargetRegisterInfo *TRI = Subtarget->getRegisterInfo(); !MI.killsRegister(ARM::CPSR, /*TRI=*/nullptr) &&
...
void ARMTargetLowering::AdjustInstrPostInstrSelection(MachineInstr &MI,
  ...
  if (NewOpc) {
    ...
    if (Subtarget->isThumb1Only()) {
      ...
      for (unsigned i = MI.getNumOperands(); i--;) {
        ...
        if (const MachineOperand& op = MI.getOperand(i); op.isReg() && op.isUse()) {
          
          if (int DefIdx = MCID->getOperandConstraint(i, MCOI::TIED_TO); DefIdx != -1)
  ...
  for (unsigned i = MCID->getNumOperands(), e = MI.getNumOperands(); i != e;
    ...
    if (const MachineOperand &MO = MI.getOperand(i); MO.isReg() && MO.isDef() && MO.getReg() == ARM::CPSR) {
      definesCPSR = true;
      if (MO.isDead())
        deadCPSR = true;
      MI.removeOperand(i);
...
static SDValue PerformUMLALCombine(SDNode *N, SelectionDAG &DAG,
  ...
  if (SDNode* AddeNode = N->getOperand(3).getNode(); (AddcNode->getOpcode() == ARMISD::ADDC) &&
...
static SDValue PerformAddcSubcCombine(SDNode *N,
  ...
  if (N->getOpcode() == ARMISD::SUBC && N->hasAnyUseOfValue(1)) {
    ...
    if (SDValue RHS = N->getOperand(1); LHS->getOpcode() == ARMISD::ADDE &&
  ...
  if (Subtarget->isThumb1Only()) {
    ...
    if (ConstantSDNode *SDValue RHS = N->getOperand(1); C = dyn_cast<ConstantSDNode>(RHS)) {
      
      if (int32_t imm = C->getSExtValue(); imm < 0 && imm > std::numeric_limits<int>::min()) {
        SDLoc DL(N);
...
static SDValue PerformAddeSubeCombine(SDNode *N,
  ...
  if (Subtarget->isThumb1Only()) {
    ...
    if (ConstantSDNode *SDValue RHS = N->getOperand(1); C = dyn_cast<ConstantSDNode>(RHS)) {
      ...
      if (int64_t imm = C->getSExtValue(); imm < 0) {
        SDLoc DL(N);
...
static SDValue PerformVSELECTCombine(SDNode *N,
  ...
  if (ConstantSDNode *Const =
      isConstOrConstSplat(XOR->getOperand(1), /*AllowUndefs*/ false,
                          /*AllowTruncation*/ true); !Const || !Const->isOne())
    return SDValue();
  ...
  auto DistrubuteVecReduceLoad = [&](SDValue N0, SDValue N1, bool IsForward) {
    ...
    if (N0.getOpcode() == ISD::ADD && N0->hasOneUse()) {
      if (IsVecReduce(N0.getOperand(0)) && IsVecReduce(N0.getOperand(1))) {
        ...
        if (int IsBefore = IsKnownOrderedLoad(N0.getOperand(0).getOperand(0),
                                         N0.getOperand(1).getOperand(0)); IsBefore < 0) {
          X = N0.getOperand(0);
          N0 = N0.getOperand(1);
        } else if (IsBefore > 0) {
          X = N0.getOperand(1);
...
bool ARMTargetLowering::isDesirableToCommuteXorWithShift(
  ...
  if (auto *ShiftC = dyn_cast<ConstantSDNode>(N->getOperand(0).getOperand(1)); XorC && ShiftC) {
    
    if (unsigned MaskIdx, MaskLen; XorC->getAPIntValue().isShiftedMask(MaskIdx, MaskLen)) {
...
static SDValue PerformMULCombine(SDNode *N,
  ...
  if (MulAmt >= 0) {
    ...
    if (uint64_t MulAmtAbs = -MulAmt; llvm::has_single_bit<uint32_t>(MulAmtAbs + 1)) {
...
static SDValue CombineANDShift(SDNode *N,
  ...
  if (!LeftShift && isMask_32(C1)) {
    ...
    if (uint32_t C3 = llvm::countl_zero(C1); C2 < C3) {
  ...
  if (LeftShift && isMask_32(~C1)) {
    ...
    if (uint32_t C3 = llvm::countr_zero(C1); C2 < C3) {
  ...
  if (LeftShift && isShiftedMask_32(C1)) {
    ...
    if (uint32_t C3 = llvm::countl_zero(C1); Trailing == C2 && C2 + C3 < 32) {
      SDValue SHL = DAG.getNode(ISD::SHL, DL, MVT::i32, N0->getOperand(0),
  ...
  if (!LeftShift && isShiftedMask_32(C1)) {
    ...
    if (uint32_t C3 = llvm::countr_zero(C1); Leading == C2 && C2 + C3 < 32) {
      SDValue SHL = DAG.getNode(ISD::SRL, DL, MVT::i32, N0->getOperand(0),
                                DAG.getConstant(C2 + C3, DL, MVT::i32));
      return DAG.getNode(ISD::SHL, DL, MVT::i32, SHL,
                         DAG.getConstant(C3, DL, MVT::i32));
...
static SDValue PerformORCombineToBFI(SDNode *N,
  ...
  if (DAG.MaskedValueIsZero(N1, MaskC->getAPIntValue()) &&
    ...
    if (unsigned LSB = llvm::countr_zero(Mask); ShAmtC != LSB)
      return SDValue();
...
static SDValue PerformORCombine(SDNode *N,
  ...
  if (BVN && (Subtarget->hasNEON() || Subtarget->hasMVEIntegerOps()) &&
    ...
    if (SplatBitSize == 8 || SplatBitSize == 16 || SplatBitSize == 32 ||
      ...
      if (SDValue Val =
          isVMOVModifiedImm(SplatBits.getZExtValue(), SplatUndef.getZExtValue(),
                            SplatBitSize, DAG, dl, VorrVT, VT, OtherModImm); Val.getNode()) {
        SDValue Input =
  ...
  if (Subtarget->hasNEON() && N1.getOpcode() == ISD::AND && VT.isVector() &&
    ...
    if (BuildVectorSDNode *BVN1 = dyn_cast<BuildVectorSDNode>(N1->getOperand(1)); BVN0 && BVN0->isConstantSplat(SplatBits0, SplatUndef, SplatBitSize,
...
static SDValue PerformXORCombine(SDNode *N,
  ...
  if (Subtarget->hasMVEIntegerOps()) {
    ...
    if (const TargetLowering *TLI = Subtarget->getTargetLowering(); TLI->isConstTrueVal(N1) &&
...
static SDValue PerformBFICombine(SDNode *N, SelectionDAG &DAG) {
  ...
  if (N->getOperand(0).getOpcode() == ARMISD::BFI) {
    ...
    if (APInt ToMask2 = ~N0.getConstantOperandAPInt(2); !N0.hasOneUse() || (ToMask1 & ToMask2) != 0 ||
...
static SDValue PerformCMPZCombine(SDNode *N, SelectionDAG &DAG) {
  ...
  if (SDValue ARMCC::CondCodes Cond; C = IsCMPZCSINC(N, Cond))
    if (Cond == ARMCC::EQ)
      return C;
  return SDValue();
}
...
static SDValue PerformCSETCombine(SDNode *N, SelectionDAG &DAG) {
  ...
  if (SDValue ARMCC::CondCodes Cond; C = IsCMPZCSINC(N->getOperand(3).getNode(), Cond)) {
...
static SDValue PerformVMOVhrCombine(SDNode *N,
  ...
  if (Op0->getOpcode() == ISD::BITCAST) {
    ...
    if (SDValue Copy = Op0->getOperand(0); Copy.getValueType() == MVT::f32 &&
...
static bool hasNormalLoadOperand(SDNode *N) {
  ...
  for (unsigned i = 0; i < NumElts; ++i) {
    ...
    if (SDNode *Elt = N->getOperand(i).getNode(); ISD::isNormalLoad(Elt) && !cast<LoadSDNode>(Elt)->isVolatile())
      return true;
  }
  return false;
}
...
PerformARMBUILD_VECTORCombine(SDNode *N, TargetLowering::DAGCombinerInfo &DCI) {
  ...
  for (unsigned Idx = 0; Idx < NumElts; ++Idx) {
    ...
    if (SDValue Elt = N->getOperand(Idx); Elt->getOpcode() == ISD::BITCAST) {
...
PerformPREDICATE_CASTCombine(SDNode *N, TargetLowering::DAGCombinerInfo &DCI) {
  ...
  if (Op.getValueType() == MVT::i32) {
    ...
    if (const TargetLowering &TLI = DCI.DAG.getTargetLoweringInfo(); TLI.SimplifyDemandedBits(Op, DemandedMask, DCI))
      return SDValue(N, 0);
  }
  return SDValue();
}
...
static SDValue PerformInsertEltCombine(SDNode *N,
  ...
  if (SDNode *Elt = N->getOperand(1).getNode(); VT.getVectorElementType() != MVT::i64 ||
...
static SDValue PerformExtractEltCombine(SDNode *N,
  ...
  if (Op0.getValueType() == MVT::v4i32 &&
    ...
    if (SDValue MOV = BV.getOperand(Offset < 2 ? 0 : 1); MOV.getOpcode() == ARMISD::VMOVDRR)
...
static bool isValidBaseUpdate(SDNode *N, SDNode *User) {
  ...
  if (const unsigned MaxSteps = 1024; SDNode::hasPredecessorHelper(N, Visited, Worklist, MaxSteps) ||
...
static SDValue CombineBaseUpdate(SDNode *N,
  ...
  for (unsigned I = 0; I < NumValidUpd; I++) {
    ...
    if (BaseUpdateUser &User = BaseUpdates[I]; TryCombineBaseUpdate(Target, User, /*SimpleConstIncOnly=*/true, DCI))
      return SDValue();
  }
...
static bool CombineVLDDUP(SDNode *N, TargetLowering::DAGCombinerInfo &DCI) {
  ...
  for (SDUse &Use : VLD->uses()) {
    ...
    if (SDNode *User = Use.getUser(); User->getOpcode() != ARMISD::VDUPLANE ||
...
static SDValue PerformFADDVCMLACombine(SDNode *N, SelectionDAG &DAG) {
  ...
  auto ReassocComplex = [&](SDValue A, SDValue B) {
    ...
    if (unsigned Opc = A.getConstantOperandVal(0); Opc != Intrinsic::arm_mve_vcmlaq)
      return SDValue();
...
static SDValue PerformVECREDUCE_ADDCombine(SDNode *N, SelectionDAG &DAG,
  ...
  if (Op->getOpcode() == ISD::ZERO_EXTEND &&
    ...
    if (SDValue Mul = Op->getOperand(0); Mul->getOperand(0) == Mul->getOperand(1) &&
...
static SDValue PerformReduceShuffleCombine(SDNode *N, SelectionDAG &DAG) {
  ...
  if (N->getNumOperands() != VecOp + 1) {
    ...
    if (auto *Shuf2 = dyn_cast<ShuffleVectorSDNode>(N->getOperand(VecOp + 1)); !Shuf2 || !Shuf2->getOperand(1).isUndef() || Shuf2->getMask() != Mask)
      return SDValue();
  }
...
static SDValue PerformVQMOVNCombine(SDNode *N,
  ...
  if (const TargetLowering &TLI = DCI.DAG.getTargetLoweringInfo(); TLI.SimplifyDemandedVectorElts(Op0, Op0DemandedElts, DCI))
    return SDValue(N, 0);
  return SDValue();
}
...
static SDValue PerformVQDMULHCombine(SDNode *N,
  ...
  if (auto *Shuf1 = dyn_cast<ShuffleVectorSDNode>(RHS); Shuf0 && Shuf1 && Shuf0->getMask().equals(Shuf1->getMask()) &&
...
SDValue ARMTargetLowering::PerformIntrinsicCombine(SDNode *N,
  ...
  switch (unsigned IntNo = N->getConstantOperandVal(0); IntNo) {
  default:
  ...
  case Intrinsic::arm_mve_vqrdmlash_predicated: {
    ...
    if (APInt DemandedMask = APInt::getLowBitsSet(32, BitWidth); SimplifyDemandedBits(N->getOperand(3), DemandedMask, DCI))
      return SDValue();
    break;
  }
  ...
  case Intrinsic::arm_mve_maxav_predicated: {
    ...
    if (APInt DemandedMask = APInt::getLowBitsSet(32, BitWidth); SimplifyDemandedBits(N->getOperand(1), DemandedMask, DCI))
      return SDValue();
    break;
  }
...
static SDValue PerformShiftCombine(SDNode *N,
  ...
  if (ST->isThumb1Only() && N->getOpcode() == ISD::SHL && VT == MVT::i32 &&
    ...
    if (isMask_32(AndMask)) {
      ...
      if (uint32_t MaskedBits = llvm::countl_zero(AndMask); MaskedBits > ShiftAmt) {
        SDLoc DL(N);
        SDValue SHL = DAG.getNode(ISD::SHL, DL, MVT::i32, N0->getOperand(0),
  ...
  switch (int64_t Cnt; N->getOpcode()) {
...
static SDValue PerformExtendCombine(SDNode *N, SelectionDAG &DAG,
  ...
  if ((ST->hasNEON() || ST->hasMVEIntegerOps()) &&
    ...
    if (const TargetLowering &TLI = DAG.getTargetLoweringInfo(); VT == MVT::i32 &&
...
static SDValue SearchLoopIntrinsic(SDValue N, ISD::CondCode &CC, int &Imm,
  ...
  case ISD::INTRINSIC_W_CHAIN: {
    ...
    if (unsigned IntOp = N.getConstantOperandVal(1); IntOp != Intrinsic::test_start_loop_iterations &&
...
ARMTargetLowering::PerformCMOVCombine(SDNode *N, SelectionDAG &DAG) const {
  ...
  if (!Subtarget->isThumb1Only() && Subtarget->hasV6T2Ops()) {
    ...
    if (SDValue R = PerformCMOVToBFICombine(N, DAG); R)
      return R;
  }
  ...
  if (Res.getNode()) {
    ...
    if (KnownBits Known = DAG.computeKnownBits(SDValue(N,0)); Known.Zero == 0xfffffffe)
...
static SDValue PerformBITCASTCombine(SDNode *N,
  ...
  if (ST->hasMVEIntegerOps() && Src.getOpcode() == ARMISD::VDUP) {
    ...
    if (EVT SrcVT = Src.getValueType(); SrcVT.getScalarSizeInBits() == DstVT.getScalarSizeInBits())
...
SDValue ARMTargetLowering::PerformMVETruncCombine(
  ...
  if (N->getNumOperands() == 2 &&
    ...
    if (auto *S1 = cast<ShuffleVectorSDNode>(N->getOperand(1).getNode()); S0->getOperand(0) == S1->getOperand(0) &&
...
SDValue ARMTargetLowering::PerformDAGCombine(SDNode *N,
  ...
  case ARMISD::SMULWB: {
    ...
    if (APInt DemandedMask = APInt::getLowBitsSet(BitWidth, 16); SimplifyDemandedBits(N->getOperand(1), DemandedMask, DCI))
      return SDValue();
    break;
  }
  case ARMISD::SMULWT: {
    ...
    if (APInt DemandedMask = APInt::getHighBitsSet(BitWidth, 16); SimplifyDemandedBits(N->getOperand(1), DemandedMask, DCI))
      return SDValue();
    break;
  }
  case ARMISD::SMLALBB:
  ...
  case ARMISD::UQSUB16b: {
    ...
    if (APInt DemandedMask = APInt::getLowBitsSet(BitWidth, 16); (SimplifyDemandedBits(N->getOperand(0), DemandedMask, DCI)) ||
  ...
  case ARMISD::SMLALBT: {
    ...
    if (APInt HighMask = APInt::getHighBitsSet(HighWidth, 16); (SimplifyDemandedBits(N->getOperand(0), LowMask, DCI)) ||
  ...
  case ARMISD::SMLALTB: {
    ...
    if (APInt LowMask = APInt::getLowBitsSet(LowWidth, 16); (SimplifyDemandedBits(N->getOperand(0), HighMask, DCI)) ||
  ...
  case ARMISD::SMLALTT: {
    ...
    if (APInt DemandedMask = APInt::getHighBitsSet(BitWidth, 16); (SimplifyDemandedBits(N->getOperand(0), DemandedMask, DCI)) ||
        (SimplifyDemandedBits(N->getOperand(1), DemandedMask, DCI)))
      return SDValue();
    break;
  }
  ...
  case ARMISD::UQSUB8b: {
    ...
    if (APInt DemandedMask = APInt::getLowBitsSet(BitWidth, 8); (SimplifyDemandedBits(N->getOperand(0), DemandedMask, DCI)) ||
        (SimplifyDemandedBits(N->getOperand(1), DemandedMask, DCI)))
      return SDValue();
    break;
  }
...
bool ARMTargetLowering::isLegalAddressingMode(const DataLayout &DL,
  ...
  default:
    ...
    switch (int Scale = AM.Scale; VT.getSimpleVT().SimpleTy) {
    default: return false;
    case MVT::i1:
    case MVT::i8:
    case MVT::i32:
...
static bool getARMIndexedAddressParts(SDNode *Ptr, EVT VT,
  ...
  if (VT == MVT::i16 || ((VT == MVT::i8 || VT == MVT::i1) && isSEXTLoad)) {
    ...
    if (ConstantSDNode *RHS = dyn_cast<ConstantSDNode>(Ptr->getOperand(1))) {
      ...
      if (int RHSC = (int)RHS->getZExtValue(); RHSC < 0 && RHSC > -256) {
    ...
    if (ConstantSDNode *RHS = dyn_cast<ConstantSDNode>(Ptr->getOperand(1))) {
      ...
      if (int RHSC = (int)RHS->getZExtValue(); RHSC < 0 && RHSC > -0x1000) {
        assert(Ptr->getOpcode() == ISD::ADD);
        isInc = false;
        Offset = DAG.getConstant(-RHSC, SDLoc(Ptr), RHS->getValueType(0));
        Base = Ptr->getOperand(0);
    ...
    if (Ptr->getOpcode() == ISD::ADD) {
      ...
      if (ARM_AM::ShiftOpc ShOpcVal=
        ARM_AM::getShiftOpcForNode(Ptr->getOperand(0).getOpcode()); ShOpcVal != ARM_AM::no_shift) {
...
static bool getT2IndexedAddressParts(SDNode *Ptr, EVT VT,
  ...
  if (ConstantSDNode *RHS = dyn_cast<ConstantSDNode>(Ptr->getOperand(1))) {
    ...
    if (int RHSC = (int)RHS->getZExtValue(); RHSC < 0 && RHSC > -0x100) { // 8 bits.
      assert(Ptr->getOpcode() == ISD::ADD);
      isInc = false;
      Offset = DAG.getConstant(-RHSC, SDLoc(Ptr), RHS->getValueType(0));
      return true;
...
bool ARMTargetLowering::getPostIndexedAddressParts(SDNode *N, SDNode *Op,
  ...
  if (Subtarget->isThumb1Only()) {
    ...
    if (auto *RHS = dyn_cast<ConstantSDNode>(Op->getOperand(1)); !RHS || RHS->getZExtValue() != 4)
      return false;
...
void ARMTargetLowering::computeKnownBitsForTargetNode(const SDValue Op,
  ...
  case ARMISD::SUBE:
    ...
    if (Op.getResNo() == 0) {
      ...
      if (SDValue RHS = Op.getOperand(1); Op->getOpcode() == ARMISD::ADDE && isNullConstant(LHS) &&
          isNullConstant(RHS)) {
  ...
  case ISD::INTRINSIC_W_CHAIN: {
    ...
    switch (Intrinsic::ID IntID =
        static_cast<Intrinsic::ID>(Op->getConstantOperandVal(1)); IntID) {
    default: return;
  ...
  case ARMISD::VBICIMM: {
    ...
    if (unsigned EltBits = Op.getScalarValueSizeInBits(); EltBits != DecEltBits) {
...
bool ARMTargetLowering::SimplifyDemandedBitsForTargetNode(
  ...
  case ARMISD::LSRL: {
    ...
    if (Op.getResNo() == 0 && !Op->hasAnyUseOfValue(1) &&
      ...
      if (unsigned ShAmt = Op->getConstantOperandVal(2); ShAmt < 32 && OriginalDemandedBits.isSubsetOf(APInt::getAllOnes(32)
                                                        << (32 - ShAmt)))
  ...
  case ARMISD::VBICIMM: {
    ...
    if (uint64_t Mask = ARM_AM::decodeVMOVModImm(ModImm, EltBits); (OriginalDemandedBits & Mask) == 0)
...
ARMTargetLowering::getConstraintType(StringRef Constraint) const {
  ...
  if (unsigned S = Constraint.size(); S == 1) {
    switch (Constraint[0]) {
    default:  break;
...
SDValue ARMTargetLowering::LowerDivRem(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (VT == MVT::i64 && isa<ConstantSDNode>(Op.getOperand(1))) {
    ...
    if (SmallVector<SDValue> Result; expandDIVREMByConstant(Op.getNode(), Result, MVT::i32, DAG)) {
        SDValue Res0 =
...
SDValue ARMTargetLowering::LowerREM(SDNode *N, SelectionDAG &DAG) const {
  ...
  if (VT == MVT::i64 && isa<ConstantSDNode>(N->getOperand(1))) {
    ...
    if (SmallVector<SDValue> Result; expandDIVREMByConstant(N, Result, MVT::i32, DAG))
...
SDValue ARMTargetLowering::LowerFP_EXTEND(SDValue Op, SelectionDAG &DAG) const {
  ...
  for (unsigned Sz = SrcSz; Sz <= 32 && Sz < DstSz; Sz *= 2) {
    ...
    if (MVT DstVT = (Sz == 16 ? MVT::f32 : MVT::f64); Supported) {
      if (IsStrict) {
...
void ARMTargetLowering::insertSSPDeclarations(Module &M) const {
  ...
  if (RTLIB::LibcallImpl SecurityCookieVar =
    ...
    if (Function *FunctionCallee SecurityCheckCookie =
        M.getOrInsertFunction(getLibcallImplName(SecurityCheckCookieLibcall),
                              Type::getVoidTy(M.getContext()),
                              PointerType::getUnqual(M.getContext())); F = dyn_cast<Function>(SecurityCheckCookie.getCallee()))
...
bool ARMTargetLowering::canCreateUndefOrPoisonForTargetNode(
  ...
  switch (unsigned Opcode = Op.getOpcode(); Opcode) {
  case ARMISD::VORRIMM:
  case ARMISD::VBICIMM:
    return false;
  }
...
bool ARMTargetLowering::lowerInterleavedStore(Instruction *Store,
  ...
  for (unsigned StoreCount = 0; StoreCount < NumStores; ++StoreCount) {
    ...
    for (unsigned i = 0; i < Factor; i++) {
      ...
      if (unsigned IdxI = StoreCount * LaneLen * Factor + i; Mask[IdxI] >= 0) {
        ...
        for (unsigned j = 1; j < LaneLen; j++) {
          ...
          if (unsigned IdxJ = StoreCount * LaneLen * Factor + j; Mask[IdxJ * Factor + IdxI] >= 0) {
