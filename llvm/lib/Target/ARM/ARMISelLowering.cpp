...
SDValue ARMTargetLowering::LowerINTRINSIC_VOID(
  ...
  switch (unsigned IntNo =
      Op.getConstantOperandVal(Op.getOperand(0).getValueType() == MVT::Other); IntNo) {
    default:
...
static bool isFloatingPointZero(SDValue Op) {
  if (ConstantFPSDNode *CFP = dyn_cast<ConstantFPSDNode>(Op))
    ...
    if (Op.getOperand(1).getOpcode() == ARMISD::Wrapper) {
      ...
      if (SDValue WrapperOp = Op.getOperand(1).getOperand(0); ConstantPoolSDNode *CP = dyn_cast<ConstantPoolSDNode>(WrapperOp))
...
SDValue ARMTargetLowering::ReconstructShuffle(SDValue Op,
  ...
  for (auto &Source : Sources) {
    ...
    if (EVT SrcEltTy = Source.Vec.getValueType().getVectorElementType(); SrcEltTy.bitsLT(SmallestEltTy))
...
static bool isExtendedBUILD_VECTOR(SDNode *N, SelectionDAG &DAG,
  ...
  for (unsigned i = 0, e = N->getNumOperands(); i != e; ++i) {
    ...
    if (SDNode *Elt = N->getOperand(i).getNode(); ConstantSDNode *C = dyn_cast<ConstantSDNode>(Elt)) {
      unsigned EltSize = VT.getScalarSizeInBits();
...
static EVT getExtensionTo64Bits(const EVT &OrigVT) {
  ...
  switch (MVT::SimpleValueType OrigSimpleTy = OrigVT.getSimpleVT().SimpleTy; OrigSimpleTy) {
...
void ARMTargetLowering::SetupEntryBlockForSjLj(MachineInstr &MI,
  ...
  if (MachineMemOperand *FIMMOSt =
      MF->getMachineMemOperand(MachinePointerInfo::getFixedStack(*MF, FI),
                               MachineMemOperand::MOStore, 4, Align(4)); isThumb2) {
    // Incoming value: jbuf
    //   ldr.n  r5, LCPI1_1
    //   orr    r5, r5, #1
    //   add    r5, pc
...
ARMTargetLowering::EmitStructByval(MachineInstr &MI,
  ...
  if (Subtarget->useMovt()) {
    ...
    if (MachineMemOperand *CPMMO =
        MF->getMachineMemOperand(MachinePointerInfo::getConstantPool(*MF),
                                 MachineMemOperand::MOLoad, 4, Align(4)); IsThumb)
...
static SDValue PerformAddcSubcCombine(SDNode *N,
  ...
  if (Subtarget->isThumb1Only()) {
    ...
    if (SDValue RHS = N->getOperand(1); ConstantSDNode *C = dyn_cast<ConstantSDNode>(RHS)) {
...
static SDValue PerformANDCombine(SDNode *N,
  ...
  if (BVN && (Subtarget->hasNEON() || Subtarget->hasMVEIntegerOps()) &&
    ...
    if (SplatBitSize == 8 || SplatBitSize == 16 || SplatBitSize == 32 ||
      ...
      if (SDValue Val = isVMOVModifiedImm((~SplatBits).getZExtValue(),
                                      SplatUndef.getZExtValue(), SplatBitSize,
                                      DAG, dl, VbicVT, VT, OtherModImm); Val.getNode()) {
        SDValue Input =
...
ARMTargetLowering::PerformCMOVCombine(SDNode *N, SelectionDAG &DAG) const {
  ...
  if (N->getConstantOperandVal(2) == ARMCC::EQ ||
    ...
    if (ARMCC::CondCodes Cond; SDValue C = IsCMPZCSINC(N->getOperand(3).getNode(), Cond)) {
      if (N->getConstantOperandVal(2) == ARMCC::NE)
...
void ARMTargetLowering::insertSSPDeclarations(Module &M) const {
  ...
  if (RTLIB::LibcallImpl SecurityCookieVar =
    ...
    if (FunctionCallee SecurityCheckCookie =
        M.getOrInsertFunction(getLibcallImplName(SecurityCheckCookieLibcall),
                              Type::getVoidTy(M.getContext()),
                              PointerType::getUnqual(M.getContext())); Function *F = dyn_cast<Function>(SecurityCheckCookie.getCallee()))
