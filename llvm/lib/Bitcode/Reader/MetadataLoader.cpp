  ...
  void upgradeCUVariables() {
    ...
    if (NamedMDNode *CUNodes = TheModule.getNamedMetadata("llvm.dbg.cu"))
      for (unsigned I = 0, E = CUNodes->getNumOperands(); I != E; ++I) {
        ...
        if (auto *CU = cast<DICompileUnit>(CUNodes->getOperand(I)); auto *GVs = dyn_cast_or_null<MDTuple>(CU->getRawGlobalVariables()))
        ...
        if (CU->getRawImportedEntities()) {
          ...
          if (!EntitiesToRemove.empty()) {
            ...
            for (auto *I : EntitiesToRemove) {
              ...
              if (auto *Entity = cast<DIImportedEntity>(I); auto *SP = findEnclosingSubprogram(
