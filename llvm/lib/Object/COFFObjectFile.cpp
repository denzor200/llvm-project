...
void COFFObjectFile::moveSymbolNext(DataRefImpl &Ref) const {
  ...
  if (auto End = reinterpret_cast<uintptr_t>(StringTable); SymbolTable16) {
...
Error COFFObjectFile::initLoadConfigPtr() {
  ...
  if (is64()) {
    ...
    if (Config->Size >=
      ...
      if (Error uint64_t ChpeOff = Config->CHPEMetadataPointer; E =
...
ExportDirectoryEntryRef::getSymbolName(StringRef &Result) const {
  ...
  for (const ulittle16_t *I = Start, *E = Start + NumEntries;
    ...
    if (Error const ulittle32_t *NamePtr = reinterpret_cast<const ulittle32_t *>(IntPtr); EC = OwningObject->getRvaPtr(NamePtr[Offset], IntPtr,
                                           "export symbol name"))
      return EC;
    Result = StringRef(reinterpret_cast<const char *>(IntPtr));
    return Error::success();
