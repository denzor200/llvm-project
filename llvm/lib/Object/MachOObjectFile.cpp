...
bool MachOObjectFile::isSectionBitcode(DataRefImpl Sec) const {
  ...
  if (Expected<StringRef> StringRef SegmentName = getSectionFinalSegmentName(Sec); NameOrErr = getSectionName(Sec))
...
void MachOObjectFile::getRelocationTypeName(
  ...
  switch (unsigned Arch = this->getArch(); Arch) {
    case Triple::x86: {
      ...
      if (static const char *const Table[] =  {
        "GENERIC_RELOC_VANILLA",
        "GENERIC_RELOC_PAIR",
        "GENERIC_RELOC_SECTDIFF",
        "GENERIC_RELOC_PB_LA_PTR",
        "GENERIC_RELOC_LOCAL_SECTDIFF",
        "GENERIC_RELOC_TLV" }; RType > 5)
        res = "Unknown";
      else
        res = Table[RType];
      break;
    ...
    case Triple::arm: {
      ...
      if (static const char *const Table[] =  {
        ...
        "ARM_RELOC_HALF",
        "ARM_RELOC_HALF_SECTDIFF" }; RType > 9)
        res = "Unknown";
      else
        res = Table[RType];
      break;
    ...
    case Triple::ppc: {
      ...
      if (static const char *const Table[] =  {
        ...
        "PPC_RELOC_LOCAL_SECTDIFF" }; RType > 15)
        res = "Unknown";
      else
        res = Table[RType];
      break;
