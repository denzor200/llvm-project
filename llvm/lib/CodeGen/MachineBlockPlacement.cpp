...
void MachineBlockPlacement::buildCFGChains() {
  ...
  for (MachineFunction::iterator FI = F->begin(), FE = F->end(); FI != FE;
    ...
    while (true) {
      Cond.clear();
      // For analyzeBranch.
      if (MachineBasicBlock *TBB = nullptr, *FBB = nullptr; !TII->analyzeBranch(*BB, TBB, FBB, Cond) || !FI->canFallThrough())
        break;
  ...
  for (MachineBasicBlock *ChainBB : FunctionChain) {
    ...
    //   }
    // }
    if (MachineBasicBlock *TBB = nullptr, *FBB = nullptr; !TII->analyzeBranch(*PrevBB, TBB, FBB, Cond)) {
  ...
  Cond.clear();
  // For analyzeBranch.
  if (MachineBasicBlock *TBB = nullptr, *FBB = nullptr; !TII->analyzeBranch(F->back(), TBB, FBB, Cond)) {
