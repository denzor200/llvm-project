...
void RAGreedy::enqueue(PQueue &CurQueue, const LiveInterval *LI) {
  ...
  if (auto Stage = ExtraInfo->getOrInitStage(Reg); Stage == RS_New) {
    Stage = RS_Assign;
...
unsigned DefaultPriorityAdvisor::getPriority(const LiveInterval &LI) const {
  ...
  if (LiveRangeStage Stage = RA.getExtraInfo().getStage(LI); Stage == RS_Split) {
...
MCRegister RAGreedy::doRegionSplit(const LiveInterval &VirtReg,
  ...
  if (BestCand != NoCand) {
    ...
    if (unsigned GlobalSplitCandidate &Cand = GlobalCand[BestCand]; B = Cand.getBundles(BundleCand, BestCand)) {
...
MCRegister RAGreedy::trySplit(const LiveInterval &VirtReg,
  ...
  if (ExtraInfo->getStage(VirtReg) < RS_Split2) {
    ...
    if (MCRegister PhysReg = tryRegionSplit(VirtReg, Order, NewVRegs); PhysReg || !NewVRegs.empty())
      return PhysReg;
  }
...
MCRegister RAGreedy::selectOrSplitImpl(const LiveInterval &VirtReg,
  ...
  if (MCRegister PhysReg =
    ...
    if (CSRCost.getFrequency() &&
      ...
      if (MCRegister CSRReg = tryAssignCSRFirstTime(VirtReg, Order, PhysReg,
                                                CostPerUseLimit, NewVRegs); CSRReg || !NewVRegs.empty())
  ...
  if (Stage < RS_Spill && !VirtReg.empty()) {
    ...
    if (MCRegister PhysReg = trySplit(VirtReg, Order, NewVRegs, FixedRegisters); PhysReg || (NewVRegs.size() - NewVRegSizeBefore))
      return PhysReg;
  }
