...
SelectionDAGBuilder::EmitBranchForMergedCondition(const Value *Cond,
  ...
  // the caseblock.
  if (const CmpInst *const BasicBlock *BB = CurBB->getBasicBlock(); BOp = dyn_cast<CmpInst>(Cond)) {
...
bool SelectionDAGBuilder::EmitFuncArgumentDbgValue(
  ...
  if (!Op && N.getNode()) {
    ...
    if (LoadSDNode *SDValue LCandidate = peekThroughBitcasts(N); LNode = dyn_cast<LoadSDNode>(LCandidate.getNode()))
...
void SelectionDAGBuilder::visitVectorExtractLastActive(const CallInst &I,
  ...
  if (Value *Default = I.getOperand(2); !isa<PoisonValue>(Default) && !isa<UndefValue>(Default)) {
...
static void addStackMapLiveVars(const CallBase &Call, unsigned StartIdx,
  ...
  for (unsigned I = StartIdx; I < Call.arg_size(); I++) {
    ...
    if (FrameIndexSDNode *SDValue Op = Builder.getValue(Call.getArgOperand(I)); FI = dyn_cast<FrameIndexSDNode>(Op)) {
...
void SelectionDAGISel::LowerArguments(const Function &F) {
  ...
  for (const Argument &Arg : F.args()) {
    ...
    if (!TM.Options.EnableFastISel && Res.getOpcode() == ISD::BUILD_PAIR) {
      ...
      if (LoadSDNode *unsigned LowAddressOp = DAG.getDataLayout().isBigEndian() ? 1 : 0; LNode =
...
SelectionDAGBuilder::HandlePHINodesInSuccessorBlocks(const BasicBlock *LLVMBB) {
  ...
  for (const BasicBlock *SuccBB : successors(LLVMBB->getTerminator())) {
    ...
    for (const PHINode &PN : SuccBB->phis()) {
      ...
      if (const auto *const Value *PHIOp = PN.getIncomingValueForBlock(LLVMBB); C = dyn_cast<Constant>(PHIOp)) {
