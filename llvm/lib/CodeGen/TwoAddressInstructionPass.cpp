...
void TwoAddressInstructionImpl::processTiedPairs(MachineInstr *MI,
  ...
  for (auto &TP : TiedPairs) {
    ...
    if (LIS) {
      ...
      if (SlotIndex endIdx =
          LIS->getInstructionIndex(*MI).getRegSlot(IsEarlyClobber); RegA.isVirtual()) {
  ...
  for (MachineBasicBlock &MBBI : *MF) {
    ...
    for (MachineBasicBlock::iterator mi = MBB->begin(), me = MBB->end();
      ...
      if (TiedOperands.size() == 1) {
        ...
        if (SmallVectorImpl<std::pair<unsigned, unsigned>> &TiedPairs
          = TiedOperands.begin()->second; TiedPairs.size() == 1) {
