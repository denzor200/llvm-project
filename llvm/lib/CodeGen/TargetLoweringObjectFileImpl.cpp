...
static void GetObjCImageInfo(Module &M, unsigned &Version, unsigned &Flags,
  ...
  for (const auto &MFE: ModuleFlags) {
    ...
    if (StringRef Key = MFE.Key->getString(); Key == "Objective-C Image Info Version") {
...
void TargetLoweringObjectFileELF::emitLinkerDirectives(MCStreamer &Streamer,
  ...
  if (NamedMDNode *auto &C = getContext(); LinkerOptions = M.getNamedMetadata("llvm.linker.options")) {
...
static StringRef handlePragmaClangSection(const GlobalObject *GO,
  ...
  if (const GlobalVariable *GV = dyn_cast<GlobalVariable>(GO); GV && GV->hasImplicitSection()) {
    ...
    if (auto Attrs = GV->getAttributes(); Attrs.hasAttribute("bss-section") && Kind.isBSS())
...
MCSection *TargetLoweringObjectFileELF::getSectionForMachineBasicBlock(
  ...
  if (StringRef FunctionSectionName = MBB.getParent()->getSection()->getName(); FunctionSectionName == ".text" ||
      FunctionSectionName.starts_with(".text.")) {
    // Function is in a regular .text section.
    
    if (StringRef FunctionName = MBB.getParent()->getName(); MBB.getSectionID() == MBBSectionID::ColdSectionID) {
...
void TargetLoweringObjectFileMachO::emitModuleMetadata(MCStreamer &Streamer,
  ...
  if (Error bool TAAParsed; E = MCSectionMachO::ParseSectionSpecifier(
