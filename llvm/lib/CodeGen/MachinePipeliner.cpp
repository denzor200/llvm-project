...
static bool findLoopIncrementValue(const MachineOperand &Op, int &Value) {
  ...
  while (true) {
    ...
    if (Def->isCopy()) {
      ...
      if (bool OffsetIsScalable; TII->getMemOperandWithOffset(*Def, BaseOp, Offset, OffsetIsScalable,
                                       TRI)) {
...
void SMSchedule::orderDependence(const SwingSchedulerDAG *SSD, SUnit *SU,
  ...
  for (std::deque<SUnit *>::iterator I = Insts.begin(), E = Insts.end(); I != E;
    ...
    for (MachineOperand &MO : MI->operands()) {
      ...
      if (unsigned BasePos, OffsetPos; ST.getInstrInfo()->getBaseAndOffsetPosition(*MI, BasePos, OffsetPos))
...
SwingSchedulerDDG::SwingSchedulerDDG(std::vector<SUnit> &SUnits, SUnit *EntrySU,
  ...
  for (SUnit &SU : SUnits) {
    ...
    if (const LoopCarriedEdges::OrderDep *SUnit *Src = &SU; OD = LCE.getOrderDepOrNull(Src)) {
...
void LoopCarriedEdges::modifySUnits(std::vector<SUnit> &SUnits,
  ...
  for (SUnit &SU : SUnits) {
    ...
    if (auto *SUnit *Src = &SU; OrderDep = getOrderDepOrNull(Src)) {
