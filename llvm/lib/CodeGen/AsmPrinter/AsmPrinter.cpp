...
void AsmPrinter::emitCallGraphSection(const MachineFunction &MF,
  ...
  if (const auto *TypeId = extractNumericCGTypeId(F); IsIndirectTarget && TypeId)
...
static void emitGlobalConstantFP(APFloat APF, Type *ET, AsmPrinter &AP) {
  ...
  if (const uint64_t *p = API.getRawData(); AP.getDataLayout().isBigEndian() && !ET->isPPC_FP128Ty()) {
...
void AsmPrinter::emitGlobalConstant(const DataLayout &DL, const Constant *CV,
  ...
  if (uint64_t Size = DL.getTypeAllocSize(CV->getType()); Size)
