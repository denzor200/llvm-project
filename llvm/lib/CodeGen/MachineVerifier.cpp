...
void MachineVerifier::verifyPreISelGenericInstruction(const MachineInstr *MI) {
  ...
  case TargetOpcode::G_EXTRACT_VECTOR_ELT: {
    ...
    if (auto TLI = MF->getSubtarget().getTargetLowering(); IdxTy.getSizeInBits() != TLI->getVectorIdxWidth(MF->getDataLayout())) {
...
MachineVerifier::visitMachineOperand(const MachineOperand *MO, unsigned MONum) {
  ...
  case MachineOperand::MO_Register: {
    ...
    if (unsigned SubIdx = MO->getSubReg(); Reg.isPhysical()) {
      if (SubIdx) {
  ...
  if (MRI->tracksLiveness())
    for (const auto &MBB : *MF)
      for (MachineBasicBlock::RegisterMaskPair P : MBB.liveins()) {
        ...
        if (bool hasAliases = MCRegAliasIterator(LiveInReg, TRI, false).isValid(); hasAliases || isAllocatable(LiveInReg) || isReserved(LiveInReg))
          continue;
        for (const MachineBasicBlock *Pred : MBB.predecessors()) {
