...
void GISelValueTracking::computeKnownBitsImpl(Register R, KnownBits &Known,
  ...
  case TargetOpcode::G_CONCAT_VECTORS: {
    ...
    for (const auto &[I, MO] : enumerate(drop_begin(MI.operands()))) {
      ...
      if (APInt DemandedSub =
          DemandedElts.extractBits(NumSubVectorElts, I * NumSubVectorElts); !!DemandedSub) {
...
unsigned GISelValueTracking::computeNumSignBits(Register R,
  ...
  case TargetOpcode::G_XOR: {
    ...
    if (unsigned Src1NumSignBits =
        computeNumSignBits(Src1, DemandedElts, Depth + 1); Src1NumSignBits != 1) {
  ...
  default: {
    ...
    if (unsigned NumBits =
        TL.computeNumSignBitsForTargetInstr(*this, R, DemandedElts, MRI, Depth); NumBits > 1)
