...
void GISelValueTracking::computeKnownBitsImpl(Register R, KnownBits &Known,
  ...
  case TargetOpcode::G_SHUFFLE_VECTOR: {
    ...
    if (unsigned NumElts = MRI.getType(MI.getOperand(1).getReg()).getNumElements(); !getShuffleDemandedElts(NumElts, MI.getOperand(3).getShuffleMask(),
...
unsigned GISelValueTracking::computeNumSignBits(Register R,
  ...
  case TargetOpcode::G_SHUFFLE_VECTOR: {
    ...
    if (unsigned NumElts = MRI.getType(Src1).getNumElements(); !getShuffleDemandedElts(NumElts, MI.getOperand(3).getShuffleMask(),
                                DemandedElts, DemandedLHS, DemandedRHS))
      return 1;
  ...
  case TargetOpcode::G_SPLAT_VECTOR: {
    ...
    if (unsigned NumSrcBits = MRI.getType(Src).getSizeInBits(); NumSrcSignBits > (NumSrcBits - TyBits))
