...
bool llvm::isTriviallyDead(const MachineInstr &MI,
  ...
  for (const auto &MO : MI.all_defs()) {
    ...
    if (Register Reg = MO.getReg(); Reg.isPhysical() || !MRI.use_nodbg_empty(Reg))
      return false;
  }
...
Align llvm::inferAlignFromPtrInfo(MachineFunction &MF,
  ...
  if (auto auto PSV = dyn_cast_if_present<const PseudoSourceValue *>(MPO.V); FSPV = dyn_cast_or_null<FixedStackPseudoSourceValue>(PSV)) {
...
bool llvm::isKnownToBeAPowerOfTwo(Register Reg, const MachineRegisterInfo &MRI,
  ...
  case TargetOpcode::G_BUILD_VECTOR_TRUNC: {
    ...
    for (const MachineOperand &MO : llvm::drop_begin(MI.operands())) {
      ...
      if (auto Const = getIConstantVRegVal(MO.getReg(), MRI); !Const || !Const->zextOrTrunc(BitWidth).isPowerOf2())
        return false;
    }
