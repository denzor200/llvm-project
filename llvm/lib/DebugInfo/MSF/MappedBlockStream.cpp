...
Error MappedBlockStream::readLongestContiguousChunk(uint64_t Offset,
  ...
  if (uint64_t MsfOffset = blockToOffset(StreamLayout.Blocks[First], BlockSize); auto EC = MsfData.readBytes(MsfOffset, BlockSize, BlockData))
    return EC;
...
bool MappedBlockStream::tryReadContiguously(uint64_t Offset, uint64_t Size,
  ...
  if (uint64_t MsfOffset = blockToOffset(FirstBlockAddr, BlockSize); auto EC = MsfData.readBytes(MsfOffset, BlockSize, BlockData)) {
...
Error MappedBlockStream::readBytes(uint64_t Offset,
  ...
  while (BytesLeft > 0) {
    ...
    if (uint64_t Offset = blockToOffset(StreamBlockAddr, BlockSize); auto EC = MsfData.readBytes(Offset, BlockSize, BlockData))
      return EC;
