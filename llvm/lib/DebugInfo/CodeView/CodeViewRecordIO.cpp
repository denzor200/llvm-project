...
Error CodeViewRecordIO::mapInteger(TypeIndex &TypeInd, const Twine &Comment) {
  if (isStreaming()) {
    ...
    if (std::string TypeNameStr = Streamer->getTypeName(TypeInd); !TypeNameStr.empty())
...
Error CodeViewRecordIO::mapStringZ(StringRef &Value, const Twine &Comment) {
  if (isStreaming()) {
    ...
    if (auto StringRef S = Value.take_front(maxFieldLength() - 1); EC = Writer->writeCString(S))
      return EC;
  } else {
...
Error CodeViewRecordIO::mapStringZVectorZ(std::vector<StringRef> &Value,
  ...
  if (!isReading()) {
    ...
    if (auto uint8_t FinalZero = 0; EC = mapInteger(FinalZero))
      return EC;
  } else {
    StringRef S;
