...
class LVForwardReferences {
  ...
  void add(StringRef Name, TypeIndex TIForward) {
    ...
    if (auto [It, Inserted] =
        ForwardTypesNames.try_emplace(Name, TIForward, TypeIndex::None()); !Inserted) {
  ...
  void update(StringRef Name, TypeIndex TIReference) {
    ...
    if (auto [It, Inserted] =
        ForwardTypesNames.try_emplace(Name, TypeIndex::None(), TIReference); !Inserted) {
...
Error LVLogicalVisitor::visitKnownRecord(CVType &Record, ClassRecord &Class,
  ...
  if (!TIFieldList.isNoneType()) {
    ...
    if (CVType CVFieldList = Types.getType(TIFieldList); Error Err = finishVisitation(CVFieldList, TI, Scope))
      return Err;
  }
...
Error LVLogicalVisitor::visitKnownRecord(CVType &Record, EnumRecord &Enum,
  ...
  if (!TIFieldList.isNoneType()) {
    ...
    if (CVType CVFieldList = Types.getType(TIFieldList); Error Err = finishVisitation(CVFieldList, TIFieldList, Scope))
      return Err;
  }
...
Error LVLogicalVisitor::visitKnownRecord(CVType &Record, FuncIdRecord &Func,
  ...
  if (LVScope *FunctionDcl = static_cast<LVScope *>(Element)) {
    ...
    if (!TIParent.isNoneType()) {
      ...
      if (CVType CVParentScope = ids().getType(TIParent); Error Err = finishVisitation(CVParentScope, TIParent, FunctionDcl))
        return Err;
    }
...
Error LVLogicalVisitor::visitKnownRecord(CVType &Record,
  ...
  if (LVScope *MemberFunction = static_cast<LVScope *>(Element)) {
    ...
    if (ProcessArgumentList) {
      ...
      if (CVType CVArguments = Types.getType(MF.getArgumentList()); Error Err = finishVisitation(CVArguments, MF.getArgumentList(),
                                       MemberFunction))
        return Err;
    }
  }
...
Error LVLogicalVisitor::visitKnownRecord(CVType &Record, ProcedureRecord &Proc,
  ...
  if (LVScope *FunctionDcl = static_cast<LVScope *>(Element)) {
    ...
    if (ProcessArgumentList) {
      ...
      if (CVType CVArguments = Types.getType(Proc.getArgumentList()); Error Err = finishVisitation(CVArguments, Proc.getArgumentList(),
                                       FunctionDcl))
        return Err;
    }
  }
...
Error LVLogicalVisitor::visitKnownRecord(CVType &Record, UnionRecord &Union,
  ...
  if (!Union.getFieldList().isNoneType()) {
    ...
    if (CVType CVFieldList = Types.getType(Union.getFieldList()); Error Err = finishVisitation(CVFieldList, TI, Scope))
      return Err;
  }
...
void LVLogicalVisitor::processLines() {
  ...
  for (const TypeIndex &Entry : Shared->LineRecords) {
    ...
    if (UdtSourceLineRecord Line; Error Err = TypeDeserializer::deserializeAs(
