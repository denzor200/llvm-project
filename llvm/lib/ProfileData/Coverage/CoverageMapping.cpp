...
Error CoverageMapping::loadFromReaders(
  ...
  for (const auto &CoverageReader : CoverageReaders) {
    for (auto RecordOrErr : *CoverageReader) {
      ...
      if (Error const auto &Record = *RecordOrErr; E = Coverage.loadFunctionRecord(Record, ProfileReader))
        return E;
    }
  }
  return Error::success();
  ...
  if (BIDFetcher) {
    ...
    for (object::BuildIDRef BinaryID : BinaryIDsToFetch) {
      ...
      if (std::optional<std::string> PathOpt = BIDFetcher->fetch(BinaryID); PathOpt) {
        ...
        if (Error StringRef Arch = Arches.size() == 1 ? Arches.front() : StringRef(); E = loadFromFile(Path, Arch, CompilationDir, ProfileReaderRef,
                                   *Coverage, DataFound))
          return std::move(E);
