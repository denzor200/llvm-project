...
const FunctionSamples *FunctionSamples::findFunctionSamplesAt(
  ...
  if (FuncNameToProfNameMap && !FuncNameToProfNameMap->empty()) {
    ...
    if (auto R = FuncNameToProfNameMap->find(FunctionId(CalleeName)); R != FuncNameToProfNameMap->end()) {
      CalleeName = R->second.stringRef();
      
      if (auto FS = I->second.find(getRepInFormat(CalleeName)); FS != I->second.end())
        return &FS->second;
    }
  }
  ...
  if (Remapper) {
    if (auto NameInProfile = Remapper->lookUpNameInProfile(CalleeName)) {
      ...
      if (auto FS = I->second.find(getRepInFormat(*NameInProfile)); FS != I->second.end())
        return &FS->second;
    }
  }
