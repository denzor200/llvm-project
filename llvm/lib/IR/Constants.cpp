    ...
    if (CE->getOpcode() == Instruction::Sub) {
      ...
      if (ConstantExpr *RHS = dyn_cast<ConstantExpr>(CE->getOperand(1)); LHS && RHS &&
        ...
        if (auto *RHSGV =
          ...
          if (auto *LHS = LHSOp0->stripInBoundsConstantOffsets(); auto *LHSGV = dyn_cast<GlobalValue>(LHS)) {
...
bool ConstantFP::isValueValidForType(Type *Ty, const APFloat& Val) {
  ...
  switch (bool losesInfo; Ty->getTypeID()) {
  default:
...
Instruction *ConstantExpr::getAsInstruction() const {
  ...
  switch (ArrayRef<Value*> Ops(ValueOperands); getOpcode()) {
  case Instruction::Trunc:
  case Instruction::PtrToAddr:
  case Instruction::PtrToInt:
  case Instruction::IntToPtr:
