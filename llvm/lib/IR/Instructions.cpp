...
bool CallBase::isIndirectCall() const {
  ...
  if (const Value *V = getCalledOperand(); isa<Function>(V) || isa<Constant>(V))
    return false;
  return !isInlineAsm();
}
...
Attribute CallBase::getParamAttrOnCalledFunction(unsigned ArgNo,
  ...
  if (auto *Value *V = getCalledOperand(); F = dyn_cast<Function>(V))
...
static bool hasNonZeroFPOperands(const CmpInst *Cmp) {
  ...
  if (auto *auto *RHS = dyn_cast<Constant>(Cmp->getOperand(1)); Const = LHS ? LHS : RHS) {
