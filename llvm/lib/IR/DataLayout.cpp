...
Error DataLayout::parseAggregateSpec(StringRef Spec) {
  ...
  if (!Components[0].empty()) {
    ...
    if (unsigned BitWidth; !to_integer(Components[0], BitWidth, 10) || BitWidth != 0)
...
Error DataLayout::parsePointerSpec(StringRef Spec) {
  ...
  while (!AddrSpaceStr.empty()) {
    ...
    if (char C = AddrSpaceStr.front(); C == 'e') {
      ExternalState = true;
    } else if (C == 'u') {
      UnstableRepr = true;
    } else if (isAlpha(C)) {
...
Error DataLayout::parseSpecification(
  ...
  switch (StringRef Rest = Spec.drop_front(); Specifier) {
  case 's':
...
int64_t DataLayout::getIndexedOffsetInType(Type *ElemTy,
  ...
  for (; GTI != GTE; ++GTI) {
    ...
    if (StructType *Value *Idx = GTI.getOperand(); STy = GTI.getStructTypeOrNull()) {
