...
DecodeIITType(unsigned &NextElt, ArrayRef<unsigned char> Infos,
  ...
  switch (IIT_Info Info = IIT_Info(Infos[NextElt++]); Info) {
  case IIT_Done:
...
static Type *DecodeFixedType(ArrayRef<Intrinsic::IITDescriptor> &Infos,
  ...
  case IITDescriptor::SameVecWidthArgument: {
    ...
    if (auto *Type *Ty = Tys[D.getArgumentNumber()]; VTy = dyn_cast<VectorType>(Ty))
      return VectorType::get(EltTy, VTy->getElementCount());
    return EltTy;
  }
  case IITDescriptor::VecElementArgument: {
    
    if (VectorType *Type *Ty = Tys[D.getArgumentNumber()]; VTy = dyn_cast<VectorType>(Ty))
...
matchIntrinsicType(Type *Ty, ArrayRef<Intrinsic::IITDescriptor> &Infos,
  ...
  case IITDescriptor::Subdivide4Argument: {
    ...
    if (auto *Type *NewTy = ArgTys[D.getArgumentNumber()]; VTy = dyn_cast<VectorType>(NewTy)) {
      int SubDivs = D.Kind == IITDescriptor::Subdivide2Argument ? 1 : 2;
