  ...
  if (CodeGenOnly) {
    ...
    for (auto &Mod : Modules) {
      Pool.async([&](int count) {
        ...
        if (auto OutputBuffer = codegenModule(*TheModule, *TMBuilder.create()); SavedObjectsDirectoryPath.empty())
      ...
      Pool.async([&](int count) {
        ...
        if (SavedObjectsDirectoryPath.empty()) {
          ...
          if (!CacheEntryPath.empty()) {
            ...
            if (auto ReloadedBufferOrErr = CacheEntry.tryLoadingBuffer(); auto EC = ReloadedBufferOrErr.getError()) {
