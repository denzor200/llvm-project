...
std::string llvm::computeLTOCacheKey(
  ...
  if (!Conf.SampleProfile.empty()) {
    ...
    if (auto FileOrErr = MemoryBuffer::getFile(Conf.SampleProfile); FileOrErr) {
...
Error ThinBackendProc::emitFiles(
  ...
  if (ShouldEmitImportsFiles) {
    ...
    if (Error ImportsFilesError = EmitImportsFiles(
        ModulePath, NewModulePath + ".imports", ModuleToSummariesForIndex); ImportsFilesError)
...
public:
  ...
  void removeFile(StringRef FileName) {
    ...
    if (std::error_code EC = sys::fs::remove(FileName, true); EC && EC != std::make_error_code(std::errc::no_such_file_or_directory))
