...
LTOModule::objcClassNameFromExpression(const Constant *c, std::string &name) {
  if (const ConstantExpr *ce = dyn_cast<ConstantExpr>(c)) {
    ...
    if (GlobalVariable *Constant *op = ce->getOperand(0); gvn = dyn_cast<GlobalVariable>(op)) {
      
      if (ConstantDataArray *Constant *cn = gvn->getInitializer(); ca = dyn_cast<ConstantDataArray>(cn)) {
        if (ca->isCString()) {
...
void LTOModule::parseSymbols() {
  for (auto Sym : SymTab.symbols()) {
    ...
    if (!GV) {
      ...
      if (StringRef Name = Buffer; IsUndefined)
