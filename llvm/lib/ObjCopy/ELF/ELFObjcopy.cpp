...
static Error handleArgs(const CommonConfig &Config, const ELFConfig &ELFConfig,
  ...
  for (const NewSectionInfo &AddedSection : Config.AddSection) {
    ...
    if (auto AddSection = [&](StringRef Name, ArrayRef<uint8_t> Data) -> Error {
      ...
      return Error::success();
    }; Error E = handleUserSection(AddedSection, AddSection))
      return createFileError(Config.InputFilename, std::move(E));
  }

  for (const NewSectionInfo &NewSection : Config.UpdateSection) {
    
    if (auto UpdateSection = [&](StringRef Name, ArrayRef<uint8_t> Data) {
      return Obj.updateSection(Name, Data);
    }; Error E = handleUserSection(NewSection, UpdateSection))
      return createFileError(Config.InputFilename, std::move(E));
  }
