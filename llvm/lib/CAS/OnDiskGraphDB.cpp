...
Expected<DataRecordHandle> DataRecordHandle::createWithError(
  ...
  if (Expected<char *> Layout L(I); Mem = Alloc(L.getTotalSize()))
...
Error TempFile::discard() {
  ...
  if (FD != -1) {
    ...
    if (std::error_code sys::fs::file_t File = sys::fs::convertFDToNativeFile(FD); EC = sys::fs::closeFile(File))
  ...
  if (std::error_code RemoveEC; !TmpName.empty()) {
