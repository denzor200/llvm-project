...
bool CallAnalyzer::visitAlloca(AllocaInst &I) {
  ...
  if (I.isArrayAllocation()) {
    ...
    if (auto *Constant *Size = getSimplifiedValue<Constant>(I.getArraySize()); AllocSize = dyn_cast_or_null<ConstantInt>(Size)) {
...
bool CallAnalyzer::visitSub(BinaryOperator &I) {
  ...
  if (LHSBase) {
    ...
    if (RHSBase && LHSBase == RHSBase) {
      ...
      if (Constant *Constant *CRHS = ConstantInt::get(RHS->getContext(), RHSOffset); C = ConstantExpr::getSub(CLHS, CRHS)) {
        SimplifiedValues[&I] = C;
        ++NumConstantPtrDiffs;
        return true;
      }
...
bool CallAnalyzer::visitExtractValue(ExtractValueInst &I) {
  ...
  if (Value *Value *Op = I.getAggregateOperand(); SimpleOp = getSimplifiedValueUnchecked(Op)) {
    SimplifyQuery SQ(DL);
...
InlineResult CallAnalyzer::analyze() {
  ...
  for (Argument &FAI : F.args()) {
    ...
    if (ConstantInt *Value *PtrArg = *CAI; C = stripAndComputeInBoundsConstantOffsets(PtrArg)) {
  ...
  for (unsigned Idx = 0; Idx != BBWorklist.size(); ++Idx) {
    ...
    if (BranchInst *BI = dyn_cast<BranchInst>(TI)) {
      if (BI->isConditional()) {
        ...
        if (ConstantInt *Value *Cond = BI->getCondition(); SimpleCond = getSimplifiedValue<ConstantInt>(Cond)) {
      ...
      if (ConstantInt *Value *Cond = SI->getCondition(); SimpleCond = getSimplifiedValue<ConstantInt>(Cond)) {
