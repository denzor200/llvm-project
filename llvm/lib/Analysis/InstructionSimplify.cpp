...
static Value *simplifyAssociativeBinOp(Instruction::BinaryOps Opcode,
  ...
  if (Op0 && Op0->getOpcode() == Opcode) {
    ...
    if (Value *Value *C = RHS; V = simplifyBinOp(Opcode, B, C, Q, MaxRecurse)) {
  ...
  if (Op1 && Op1->getOpcode() == Opcode) {
    ...
    if (Value *Value *C = Op1->getOperand(1); V = simplifyBinOp(Opcode, A, B, Q, MaxRecurse)) {
  ...
  if (Op0 && Op0->getOpcode() == Opcode) {
    ...
    if (Value *Value *C = RHS; V = simplifyBinOp(Opcode, C, A, Q, MaxRecurse)) {
  ...
  if (Op1 && Op1->getOpcode() == Opcode) {
    ...
    // Does "C op A" simplify?
    if (Value *Value *C = Op1->getOperand(1); V = simplifyBinOp(Opcode, C, A, Q, MaxRecurse)) {
...
static Value *threadBinOpOverSelect(Instruction::BinaryOps Opcode, Value *LHS,
  ...
  if ((FV && !TV) || (TV && !FV)) {
    ...
    if (Instruction *Simplified = dyn_cast<Instruction>(FV ? FV : TV); Simplified && Simplified->getOpcode() == unsigned(Opcode) &&
...
static Value *simplifyByDomEq(unsigned Opcode, Value *Op0, Value *Op1,
  ...
  if (Imp && *Imp) {
    ...
    switch (Type *Ty = Op0->getType(); Opcode) {
...
static Value *simplifySubInst(Value *Op0, Value *Op1, bool IsNSW, bool IsNUW,
  ...
  if (match(Op0, m_Zero())) {
    ...
    if (KnownBits Known = computeKnownBits(Op1, Q); Known.Zero.isMaxSignedValue()) {
  ...
  if (IsNUW) {
    ...
    if (Value *X; match(Op1, m_Xor(m_Value(X), m_Specific(Op0))) &&
...
static Value *simplifyMulInst(Value *Op0, Value *Op1, bool IsNSW, bool IsNUW,
  ...
  if (Value *X = nullptr; Q.IIQ.UseInstrInfo &&
      (match(Op0,
...
static bool isDivZero(Value *X, Value *Y, const SimplifyQuery &Q,
  ...
  if (const APInt *C; match(Y, m_APInt(C)) && computeKnownBits(X, Q).getMaxValue().ult(*C))
    return true;
...
static Value *simplifyDivRem(Instruction::BinaryOps Opcode, Value *Op0,
  ...
  if (match(Op0, m_c_Mul(m_Value(X), m_Specific(Op1)))) {
    ...
    // X == A / Y for some A.
    if (auto *Mul = cast<OverflowingBinaryOperator>(Op0); (IsSigned && Q.IIQ.hasNoSignedWrap(Mul)) ||
...
static Value *simplifyDiv(Instruction::BinaryOps Opcode, Value *Op0, Value *Op1,
  ...
  if (const APInt *DivC; IsExact && match(Op1, m_APInt(DivC))) {
    ...
    if (DivC->countr_zero()) {
      ...
      if (KnownBits KnownOp0 = computeKnownBits(Op0, Q); KnownOp0.countMaxTrailingZeros() < DivC->countr_zero())
        return PoisonValue::get(Op0->getType());
    }
...
static Value *simplifyRem(Instruction::BinaryOps Opcode, Value *Op0, Value *Op1,
  ...
  if (Q.IIQ.UseInstrInfo) {
    ...
    if (const APInt *C0; match(Op1, m_APInt(C0))) {
...
static Value *simplifySRemInst(Value *Op0, Value *Op1, const SimplifyQuery &Q,
  ...
  if (Value *X; match(Op1, m_SExt(m_Value(X))) && X->getType()->isIntOrIntVectorTy(1))
    return ConstantInt::getNullValue(Op0->getType());
...
static Value *simplifyRightShift(Instruction::BinaryOps Opcode, Value *Op0,
  ...
  if (IsExact) {
    ...
    if (KnownBits Op0Known = computeKnownBits(Op0, Q); Op0Known.One[0])
      return Op0;
  }
...
static Value *simplifyLShrInst(Value *Op0, Value *Op1, bool IsExact,
  ...
  if (Q.IIQ.UseInstrInfo && match(Op1, m_APInt(ShRAmt)) &&
    ...
    if (const unsigned EffWidthY = YKnown.countMaxActiveBits(); ShRAmt->uge(EffWidthY))
      return X;
  }
...
static Value *simplifyAShrInst(Value *Op0, Value *Op1, bool IsExact,
  ...
  if (Value *X; Q.IIQ.UseInstrInfo && match(Op0, m_NSWShl(m_Value(X), m_Specific(Op1))))
    return X;
...
static Value *simplifyAndOrOfICmpsWithCtpop(ICmpInst *Cmp0, ICmpInst *Cmp1,
  ...
  if (const APInt *C; !match(Cmp0, m_ICmp(Pred0, m_Intrinsic<Intrinsic::ctpop>(m_Value(X)),
                          m_APInt(C))) ||
...
static Value *simplifyAndOrOfCmps(const SimplifyQuery &Q, Value *Op0,
  ...
  if (auto *Cast1 = dyn_cast<CastInst>(Op1); Cast0 && Cast1 && Cast0->getOpcode() == Cast1->getOpcode() &&
...
static Value *simplifyLogicOfAddSub(Value *Op0, Value *Op1,
  ...
  if (Constant *C1, *C2; (match(Op0, m_Add(m_Value(X), m_Constant(C1))) &&
...
static Value *simplifyAndInst(Value *Op0, Value *Op1, const SimplifyQuery &Q,
  ...
  if (match(Op1, m_Power2(PowerC)) &&
    ...
    if (KnownBits Known = computeKnownBits(Shift, Q); PowerC->getActiveBits() >= Known.getMaxValue().getActiveBits())
  ...
  if (Q.IIQ.UseInstrInfo && match(Op1, m_APInt(Mask)) &&
    ...
    if (const unsigned EffWidthY = YKnown.countMaxActiveBits(); EffWidthY <= ShftCnt) {
...
static Value *simplifyOrInst(Value *Op0, Value *Op1, const SimplifyQuery &Q,
  ...
  if ((match(Op0, m_Shl(m_AllOnes(), m_Value(X))) &&
    ...
    if (const APInt *C; (match(X, m_Sub(m_APInt(C), m_Specific(Y))) ||
...
static Value *simplifyXorInst(Value *Op0, Value *Op1, const SimplifyQuery &Q,
  ...
  // (xor (sub nuw C_Mask, X), C_Mask) -> X
  {
    ...
    if (Value *X; match(Op0, m_NUWSub(m_Specific(Op1), m_Value(X))) &&
  ...
  if (ICmpInst::isEquality(Pred)) {
    ...
    if (haveNonOverlappingStorage(LHS, RHS)) {
      ...
      if (getObjectSize(LHS, LHSSize, DL, TLI, Opts) && LHSSize != 0 &&
        ...
        if (APInt Dist = LHSOffset - RHSOffset; Dist.isNonNegative() ? Dist.ult(LHSSize) : (-Dist).ult(RHSSize))
          return ConstantInt::get(getCompareTy(LHS),
      ...
      struct CustomCaptureTracker : public CaptureTracker {
        ...
        Action captured(const Use *U, UseCaptureInfo CI) override {
          ...
          if (auto *ICmp = dyn_cast<ICmpInst>(U->getUser())) {
            ...
            if (auto *LI = dyn_cast<LoadInst>(ICmp->getOperand(OtherIdx)); LI && isa<GlobalVariable>(LI->getPointerOperand()))
              return Continue;
          }
...
static Value *simplifyICmpOfBools(CmpPredicate Pred, Value *LHS, Value *RHS,
  ...
  Type *ITy = getCompareTy(LHS); // The return type.
    // The operand type.
  if (Type *OpTy = LHS->getType(); !OpTy->isIntOrIntVectorTy(1))
    return nullptr;
  ...
  auto ExtractNotLHS = [](Value *V) -> Value * {
    ...
    if (Value *X; match(V, m_Not(m_Value(X))))
      return X;
    return nullptr;
  };
...
static Value *simplifyICmpWithZero(CmpPredicate Pred, Value *LHS, Value *RHS,
  ...
  // The return type.
  switch (Type *ITy = getCompareTy(LHS); Pred) {
  default:
...
static Value *simplifyICmpWithConstant(CmpPredicate Pred, Value *LHS,
  ...
  if (match(LHS, m_ElementWiseBitCast(m_UIToFP(m_Value(X))))) {
    ...
    if (bool TrueIfSigned; isSignBitCheck(Pred, *C, TrueIfSigned))
...
static Value *simplifyICmpWithBinOpOnLHS(CmpPredicate Pred, BinaryOperator *LBO,
  ...
  // icmp pred (or X, Y), X
  if (Value *Y = nullptr; match(LBO, m_c_Or(m_Value(Y), m_Specific(RHS)))) {
  ...
  if (match(LBO, m_URem(m_Value(), m_Specific(RHS)))) {
    ...
    case ICmpInst::ICMP_SGE: {
      ...
      if (KnownBits Known = computeKnownBits(RHS, Q); !Known.isNonNegative())
        break;
      [[fallthrough]];
    }
    case ICmpInst::ICMP_EQ:
...
static Value *simplifyICmpWithBinOp(CmpPredicate Pred, Value *LHS, Value *RHS,
  ...
  if (LBO->getOperand(0) == RBO->getOperand(0)) {
    ...
    case Instruction::Shl: {
      ...
      if (bool NSW = Q.IIQ.hasNoSignedWrap(LBO) && Q.IIQ.hasNoSignedWrap(RBO); !NUW || (ICmpInst::isSigned(Pred) && !NSW) ||
    ...
    case Instruction::Or: {
      ...
      if (const APInt *C1, *C2; ICmpInst::isRelational(Pred) &&
...
static Value *simplifyICmpWithDominatingAssume(CmpPredicate Predicate,
  ...
  for (Value *AssumeBaseOp : {LHS, RHS}) {
    for (auto &AssumeVH : Q.AC->assumptionsFor(AssumeBaseOp)) {
      ...
      if (std::optional<bool> CallInst *Assume = cast<CallInst>(AssumeVH); Imp = isImpliedCondition(
...
static Value *simplifyFCmpInst(CmpPredicate Pred, Value *LHS, Value *RHS,
  ...
  if (C && Q.CxtI) {
    ...
    if (auto [ClassVal, ClassTest] = fcmpToClassTest(Pred, *ParentF, LHS, C); ClassVal) {
  ...
  if (C) {
    ...
    if (C->isNegative() && !C->isNegZero()) {
      ...
      switch (FPClassTest Interested = KnownFPClass::OrderedLessThanZeroMask; Pred) {
      ...
      case FCmpInst::FCMP_UNE: {
        ...
        if (KnownFPClass KnownClass = computeLHSClass(Interested); KnownClass.cannotBeOrderedLessThanZero())
          return getTrue(RetTy);
        break;
      }
    ...
    if (const APFloat *C2; (match(LHS, m_Intrinsic<Intrinsic::minnum>(m_Value(), m_APFloat(C2))) &&
         *C2 < *C) ||
      ...
      switch (bool IsMaxNum =
          cast<IntrinsicInst>(LHS)->getIntrinsicID() == Intrinsic::maxnum; Pred) {
      case FCmpInst::FCMP_OEQ:
  ...
  if (match(RHS, m_AnyZeroFP())) {
    ...
    case FCmpInst::FCMP_OLT: {
      ...
      if (KnownFPClass Known = computeLHSClass(Interested); Known.cannotBeOrderedLessThanZero())
...
static Value *simplifyCmpSelOfMaxMin(Value *CmpLHS, Value *CmpRHS,
  ...
  if (auto *Shuf = dyn_cast<ShuffleVectorInst>(FVal); Shuf && Shuf->isSelect()) {
...
static Value *simplifySelectWithICmpCond(Value *CondVal, Value *TrueVal,
  ...
  if (TrueVal->getType()->isIntOrIntVectorTy()) {
    ...
    if (SelectPatternResult::isMinOrMax(SPF) && Pred == getMinMaxPred(SPF)) {
      ...
      if (APInt LimitC = getMinMaxLimit(getInverseMinMaxFlavor(SPF),
                                    X->getType()->getScalarSizeInBits()); match(Y, m_SpecificInt(LimitC)))
        return X;
    }
  }
  ...
  if (Pred == ICmpInst::ICMP_EQ && match(CmpRHS, m_Zero())) {
    ...
    if (const APInt *Y; match(CmpLHS, m_And(m_Value(X), m_APInt(Y))))
...
bool isSelectWithIdenticalPHI(PHINode &PN, PHINode &IdenticalPN) {
  ...
  for (unsigned i = 0; i < 2; i++) {
    ...
    if (BasicBlock *PredBB = PN.getIncomingBlock(i); PN.getIncomingValue(i) !=
...
static Value *simplifySelectInst(Value *Cond, Value *TrueVal, Value *FalseVal,
  ...
  if (Constant *TrueC, *FalseC; isa<FixedVectorType>(TrueVal->getType()) &&
...
static Value *simplifyGEPInst(Type *SrcTy, Value *Ptr,
  ...
  if (Indices.size() == 1) {
    ...
    if (Type *Ty = SrcTy; !IsScalableVec && Ty->isSized()) {
      Value *P;
      uint64_t C;
  ...
  if (!IsScalableVec && Q.DL.getTypeAllocSize(LastType) == 1 &&
    ...
    if (unsigned IdxWidth =
        Q.DL.getIndexSizeInBits(Ptr->getType()->getPointerAddressSpace()); Q.DL.getTypeSizeInBits(Indices.back()->getType()) == IdxWidth) {
...
Value *llvm::simplifyInsertElementInst(Value *Vec, Value *Val, Value *Idx,
  ...
  if (auto *IdxC = dyn_cast<Constant>(Idx); VecC && ValC && IdxC)
...
static Value *simplifyExtractValueInst(Value *Agg, ArrayRef<unsigned> Idxs,
  ...
  for (auto *IVI = dyn_cast<InsertValueInst>(Agg); IVI != nullptr;
    ...
    if (unsigned NumCommonIdxs = std::min(NumInsertValueIdxs, NumIdxs); InsertValueIdxs.slice(0, NumCommonIdxs) ==
...
static Value *simplifyExtractElementInst(Value *Vec, Value *Idx,
  ...
  if (auto *IdxC = dyn_cast<ConstantInt>(Idx)) {
    ...
    if (auto *IE = dyn_cast<InsertElementInst>(Vec); IE && IE->getOperand(2) == Idx)
...
static Value *simplifyCastInst(unsigned CastOpc, Value *Op, Type *Ty,
  ...
  if (auto *CI = dyn_cast<CastInst>(Op)) {
    ...
    if (Type *DstTy = Ty; Src->getType() == Ty) {
      auto FirstOp = CI->getOpcode();
      
      if (auto SecondOp = static_cast<Instruction::CastOps>(CastOpc); CastInst::isEliminableCastPair(FirstOp, SecondOp, SrcTy, MidTy, DstTy,
  ...
  if (Value *Ptr, *X; (CastOpc == Instruction::PtrToInt || CastOpc == Instruction::PtrToAddr) &&
      match(Op,
...
static Value *simplifyShuffleVectorInst(Value *Op0, Value *Op1,
  ...
  if (!Scalable && match(Op0, m_InsertElt(m_Value(), m_Constant(C),
    ...
    if (int InsertIndex = IndexC->getZExtValue(); all_of(Indices, [InsertIndex](int MaskElt) {
...
static Value *simplifyFNegInst(Value *Op, FastMathFlags FMF,
  ...
  // fneg (fneg X) ==> X
  if (Value *X; match(Op, m_FNeg(m_Value(X))))
    return X;
...
static Constant *propagateNaN(Constant *In) {
  ...
  if (auto *VecTy = dyn_cast<FixedVectorType>(Ty)) {
    ...
    for (unsigned i = 0; i != NumElts; ++i) {
      ...
      if (Constant *EltC = In->getAggregateElement(i); EltC && isa<PoisonValue>(EltC))
        NewC[i] = EltC;
...
simplifyFAddInst(Value *Op0, Value *Op1, FastMathFlags FMF,
  ...
  if (Value *X; FMF.noSignedZeros() && FMF.allowReassoc() &&
...
static Value *simplifyFMAFMul(Value *Op0, Value *Op1, FastMathFlags FMF,
  ...
  if (match(Op1, m_AnyZeroFP())) {
    ...
    if (KnownFPClass Known = computeKnownFPClass(Op0, FMF, fcInf | fcNan, Q); Known.isKnownNever(fcInf | fcNan)) {
  ...
  if (Value *X; Op0 == Op1 && match(Op0, m_Sqrt(m_Value(X))) && FMF.allowReassoc() &&
...
simplifyFDivInst(Value *Op0, Value *Op1, FastMathFlags FMF,
  ...
  if (FMF.noNaNs()) {
    ...
    if (Value *X; FMF.allowReassoc() && match(Op0, m_c_FMul(m_Value(X), m_Specific(Op1))))
      return X;
...
static Value *simplifyUnaryIntrinsic(Function *F, Value *Op0,
  ...
  if (removesFPFraction(IID)) {
    ...
    if (auto *II = dyn_cast<IntrinsicInst>(Op0); (II && removesFPFraction(II->getIntrinsicID())) ||
  ...
  case Intrinsic::ctpop: {
    ...
    if (unsigned BitWidth = Op0->getType()->getScalarSizeInBits(); MaskedValueIsZero(Op0, APInt::getHighBitsSet(BitWidth, BitWidth - 1),
                          Q))
      return Op0;
    break;
  }
...
Value *llvm::simplifyBinaryIntrinsic(Intrinsic::ID IID, Type *ReturnType,
  ...
  switch (unsigned BitWidth = ReturnType->getScalarSizeInBits(); IID) {
  ...
  case Intrinsic::cttz: {
    ...
    if (Value *X; match(Op0, m_Shl(m_One(), m_Value(X))))
      return X;
    break;
  }
  case Intrinsic::ctlz: {
    
    if (Value *X; match(Op0, m_LShr(m_Negative(), m_Value(X))))
      return X;
  ...
  case Intrinsic::ptrmask: {
    ...
    if (Constant *C; match(Op1, m_ImmConstant(C))) {
  ...
  case Intrinsic::umin: {
    ...
    if (const APInt *C; match(Op1, m_APIntAllowPoison(C))) {
      ...
      if (auto *MinMax0 = dyn_cast<IntrinsicInst>(Op0); MinMax0 && MinMax0->getIntrinsicID() == IID) {
        // TODO: loosen undef/splat restrictions for vector constants.
        Value *M00 = MinMax0->getOperand(0), *M01 = MinMax0->getOperand(1);
        
        if (const APInt *InnerC; (match(M00, m_APInt(InnerC)) || match(M01, m_APInt(InnerC))) &&
  ...
  case Intrinsic::ucmp: {
    ...
    if (ICmpInst::Predicate PredGT =
        IID == Intrinsic::scmp ? ICmpInst::ICMP_SGT : ICmpInst::ICMP_UGT; isICmpTrue(PredGT, Op0, Op1, Q, RecursionLimit))
  ...
  case Intrinsic::minimumnum: {
    ...
    if (Constant *C = dyn_cast<Constant>(Op1)) {
      ...
      if (VectorType *VTy = dyn_cast<VectorType>(C->getType())) {
        ...
        if (Constant *ElementCount ElemCount = VTy->getElementCount(); SplatVal = C->getSplatValue()) {
  ...
  case Intrinsic::vector_extract: {
    ...
    if (Value *X = nullptr; match(Op0, m_Intrinsic<Intrinsic::vector_insert>(m_Value(), m_Value(X),
                                                         m_Zero())) &&
...
static Value *simplifyIntrinsic(CallBase *Call, Value *Callee,
  ...
  if (!NumOperands) {
    ...
    case Intrinsic::vscale: {
      ...
      if (const APInt *ConstantRange CR = getVScaleRange(Call->getFunction(), 64); C = CR.getSingleElement())
  ...
  case Intrinsic::masked_gather: {
    ...
    if (Value *PassthruArg = Args[2]; maskIsAllZeroOrUndef(MaskArg))
      return PassthruArg;
    return nullptr;
  }
  case Intrinsic::fshl:
  case Intrinsic::fshr: {
    ...
    if (match(ShAmtArg, m_APInt(ShAmtC))) {
      ...
      if (APInt BitWidth = APInt(ShAmtC->getBitWidth(), ShAmtC->getBitWidth()); ShAmtC->urem(BitWidth).isZero())
        return Args[IID == Intrinsic::fshl ? 0 : 1];
    }
  ...
  case Intrinsic::experimental_constrained_fma: {
    ...
    if (Value *auto *FPI = cast<ConstrainedFPIntrinsic>(Call); V = simplifyFPOp(Args, {}, Q, *FPI->getExceptionBehavior(),
                                *FPI->getRoundingMode()))
      return V;
    return nullptr;
  }
  ...
  case Intrinsic::vector_insert: {
    ...
    if (Value *X = nullptr; match(SubVec,
  ...
  case Intrinsic::experimental_gc_relocate: {
    ...
    if (Value *BasePtr = GCR.getBasePtr(); isa<UndefValue>(DerivedPtr) || isa<UndefValue>(BasePtr)) {
  ...
  case Intrinsic::experimental_vp_reverse: {
    ...
    if (Value *X; match(Mask, m_AllOnes()) &&
...
Value *llvm::simplifyCall(CallBase *Call, Value *Callee, ArrayRef<Value *> Args,
  ...
  if (auto *F = dyn_cast<Function>(Callee); F && F->isIntrinsic())
...
static Value *simplifyInstructionWithOperands(Instruction *I,
  ...
  switch (const SimplifyQuery Q = SQ.CxtI ? SQ : SQ.getWithInstruction(I); I->getOpcode()) {
  default:
