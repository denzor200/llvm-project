    ...
    for (const Instruction &I : BB) {
      ...
      if (IsThinLTO) {
        if (isNonVolatileLoad(&I)) {
          ...
          if (auto *Value *Stored = I.getOperand(0); GV = dyn_cast<GlobalValue>(Stored))
...
static void findFuncPointers(const Constant *I, uint64_t StartingOffset,
  ...
  if (auto *const DataLayout &DL = M.getDataLayout(); C = dyn_cast<ConstantStruct>(I)) {
...
recordTypeIdCompatibleVtableReferences(ModuleSummaryIndex &Index,
  ...
  for (MDNode *Type : Types) {
    ...
    if (auto *uint64_t Offset =
        cast<ConstantInt>(
            cast<ConstantAsMetadata>(Type->getOperand(0))->getValue())
            ->getZExtValue(); TypeId = dyn_cast<MDString>(TypeID))
