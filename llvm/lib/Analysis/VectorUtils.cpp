    ...
    for (auto AI = std::next(BI); AI != E; ++AI) {
      ...
      auto DependentMember = [&](InterleaveGroup<Instruction> *Group,
        ...
        for (uint32_t Index = 0; Index < Group->getFactor(); ++Index) {
          ...
          if (Instruction *MemberOfGroupB = Group->getMember(Index); MemberOfGroupB && !canReorderMemAccessesForInterleavedGroups(
      ...
      if (auto GroupA = getInterleaveGroup(A); A->mayWriteToMemory() && GroupA != GroupB) {
