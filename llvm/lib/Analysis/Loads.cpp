...
static bool isDereferenceableAndAlignedPointer(
  ...
  if (const auto *Call = dyn_cast<CallBase>(V)) {
    ...
    if (uint64_t ObjSize; getObjectSize(V, ObjSize, DL, TLI, Opts)) {
      
      if (APInt KnownDerefBytes(Size.getBitWidth(), ObjSize); KnownDerefBytes.getBoolValue() && KnownDerefBytes.uge(Size) &&
          isKnownNonZero(V, SimplifyQuery(DL, DT, AC, CtxI)) &&
          !V->canBeFreed()) {
...
Value *llvm::findAvailablePtrLoadStore(
  ...
  while (ScanFrom != ScanBB->begin()) {
    ...
    if (StoreInst *SI = dyn_cast<StoreInst>(Inst)) {
      ...
      if (Value *StorePtr = SI->getPointerOperand()->stripPointerCasts(); (isa<AllocaInst>(StrippedPtr) || isa<GlobalVariable>(StrippedPtr)) &&
