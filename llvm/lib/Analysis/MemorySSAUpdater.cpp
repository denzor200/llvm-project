...
void MemorySSAUpdater::insertUse(MemoryUse *MU, bool RenameUses) {
  ...
  if (RenameUses && InsertedPHIs.size()) {
    ...
    if (auto *BasicBlock *StartBlock = MU->getBlock(); Defs = MSSA->getWritableBlockDefs(StartBlock)) {
...
static MemoryAccess *getNewDefiningAccessForClone(
  ...
  if (MemoryDef *DefMUD = dyn_cast<MemoryDef>(InsnDefining)) {
    ...
    if (MemoryAccess *MemoryPhi *DefPhi = cast<MemoryPhi>(InsnDefining); NewDefPhi = MPhiMap.lookup(DefPhi))
...
void MemorySSAUpdater::cloneUsesAndDefs(
  ...
  for (const MemoryAccess &MA : *Acc) {
    if (const MemoryUseOrDef *MUD = dyn_cast<MemoryUseOrDef>(&MA)) {
      ...
      if (Instruction *Instruction *Insn = MUD->getMemoryInst(); NewInsn =
    ...
    if (auto DefsList = MSSA->getWritableBlockDefs(BlockWithDefsToReplace)) {
      for (auto &DefToReplaceUses : *DefsList) {
        ...
        for (Use &U : llvm::make_early_inc_range(DefToReplaceUses.uses())) {
          ...
          if (MemoryPhi *MemoryAccess *Usr = cast<MemoryAccess>(U.getUser()); UsrPhi = dyn_cast<MemoryPhi>(Usr)) {
