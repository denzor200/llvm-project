...
std::pair<const SCEV *, const SCEV *> llvm::getStartAndEndForAccess(
  ...
  if (SE->isLoopInvariant(PtrExpr, Lp)) {
    ...
    if (const auto *const SCEV *Step = AR->getStepRecurrence(*SE); CStep = dyn_cast<SCEVConstant>(Step)) {
...
bool LoopAccessInfo::canAnalyzeLoop() {
  ...
  if (const SCEV *ExitCount = PSE->getSymbolicMaxBackedgeTakenCount(); isa<SCEVCouldNotCompute>(ExitCount)) {
