...
TEST_F(OpenMPIRBuilderTest, CriticalDirective) {
  ...
  if (const llvm::MaybeAlign const llvm::Align PtrAlign = DL.getPointerABIAlignment(GV->getAddressSpace()); Alignment = GV->getAlign())
...
TEST_F(OpenMPIRBuilderTest, SingleDirective) {
  ...
  for (auto &FI : *ExitBB) {
    ...
    if (auto Instruction *cur = &FI; CI = dyn_cast<CallInst>(cur)) {
