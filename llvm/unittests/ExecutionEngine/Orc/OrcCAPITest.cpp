...
TEST_F(OrcCAPITestBase, MaterializationUnitCreation) {
  ...
  if (LLVMErrorRef LLVMOrcMaterializationUnitRef MU = LLVMOrcAbsoluteSymbols(Pairs, 1); E = LLVMOrcJITDylibDefine(MainDylib, MU))
...
TEST_F(OrcCAPITestBase, ResourceTrackerDefinitionLifetime) {
  ...
  if (LLVMErrorRef LLVMOrcThreadSafeModuleRef TSM = createTestModule(SumExample, "sum.ll"); E = LLVMOrcLLJITAddLLVMIRModuleWithRT(Jit, RT, TSM))
...
TEST_F(OrcCAPITestBase, ResourceTrackerTransfer) {
  ...
  if (LLVMErrorRef LLVMOrcThreadSafeModuleRef TSM = createTestModule(SumExample, "sum.ll"); E = LLVMOrcLLJITAddLLVMIRModuleWithRT(Jit, DefaultRT, TSM))
    FAIL() << "Failed to add LLVM IR module to LLJIT (triple = " << TargetTriple
           << "): " << toString(E);
  LLVMOrcJITTargetAddress Addr;
...
TEST_F(OrcCAPITestBase, AddObjectBuffer) {
  ...
  if (LLVMErrorRef LLVMMemoryBufferRef ObjBuffer = createTestObject(SumExample, "sum.ll"); E = LLVMOrcObjectLayerAddObjectFile(ObjLinkingLayer,
                                                       MainDylib, ObjBuffer))
...
TEST_F(OrcCAPITestBase, ExecutionTest) {
  ...
  if (LLVMErrorRef LLVMOrcThreadSafeModuleRef TSM = createTestModule(SumExample, "sum.ll"); E = LLVMOrcLLJITAddLLVMIRModule(Jit, MainDylib, TSM))
    FAIL() << "Failed to add LLVM IR module to LLJIT (triple = " << TargetTriple
           << ")" << toString(E);
  LLVMOrcJITTargetAddress TestFnAddr;
  if (LLVMErrorRef E = LLVMOrcLLJITLookup(Jit, &TestFnAddr, "sum"))
