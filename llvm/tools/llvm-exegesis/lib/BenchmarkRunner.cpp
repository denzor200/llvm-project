...
// in glibc 2.29 and later.
    ...
    if (int SetAffinityReturn = sched_setaffinity(0, sizeof(CPUMask), &CPUMask); SetAffinityReturn == -1) {
...
// unmap it later
  ...
  Expected<SmallVector<int64_t, 4>> runWithCounter(
    ...
    if (Error PossibleBenchmarkError = createSubProcessAndRunBenchmark(
        CounterName, Value, ValidationCounters, ValidationCounterValues); PossibleBenchmarkError)
