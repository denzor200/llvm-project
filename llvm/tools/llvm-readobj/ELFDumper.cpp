...
ELFDumper<ELFT>::getShndxTable(const Elf_Shdr *Symtab) const {
  if (Symtab) {
    ...
    if (auto It = ShndxTables.find(Symtab); It != ShndxTables.end())
      return It->second;
  }
  return {};
}
...
void ELFDumper<ELFT>::loadDynamicTable() {
  ...
  if (DynamicSec) {
    ...
    if (Expected<DynRegionInfo> RegOrErr =
        createDRI(DynamicSec->sh_offset, DynamicSec->sh_size, sizeof(Elf_Dyn)); RegOrErr) {
      FromSec = *RegOrErr;
  ...
  for (const Elf_Shdr &Sec : Sections) {
    ...
    case ELF::SHT_DYNSYM:
      ...
      if (!DynSymRegion) {
        ...
        if (Expected<DynRegionInfo> RegOrErr =
            createDRI(Sec.sh_offset, Sec.sh_size, Sec.sh_entsize); RegOrErr) {
...
template <typename ELFT> void ELFDumper<ELFT>::parseDynamicTable() {
  ...
  for (const Elf_Dyn &Dyn : dynamic_table()) {
    ...
    case ELF::DT_SYMENT: {
      ...
      if (uint64_t Val = Dyn.getVal(); Val != sizeof(Elf_Sym))
  ...
  if (StringTableBegin) {
    ...
    if (const uint64_t Offset = (const uint8_t *)StringTableBegin - Obj.base(); StringTableSize > FileSize - Offset)
      reportUniqueWarning(
  ...
  if (HashTable && IsHashTableSupported && DynSymRegion) {
    ...
    if (const uint64_t Offset = DynSymRegion->Addr - Obj.base(); DerivedSize > FileSize - Offset)
      reportUniqueWarning(
...
MipsGOTParser<ELFT>::getPltSym(const Entry *E) const {
  ...
  if (int64_t Offset = std::distance(getPltEntries().data(), E); PltRelSec->sh_type == ELF::SHT_REL) {
...
void ELFDumper<ELFT>::printReloc(const Relocation<ELFT> &R, unsigned RelIndex,
  ...
  if (Expected<RelSymbol<ELFT>> Target = getRelocationTarget(R, SymTab); !Target)
...
static std::string getSectionHeadersNumString(const ELFFile<ELFT> &Obj,
  ...
  if (const typename ELFT::Ehdr &ElfHeader = Obj.getHeader(); ElfHeader.e_shnum != 0)
...
static const EnumEntry<unsigned> *getObjectFileEnumEntry(unsigned Type) {
  ...
  if (auto It = llvm::find_if(ElfObjectFileType, [&](const EnumEntry<unsigned> &E) {
    return E.Value == Type;
  }); It != ArrayRef(ElfObjectFileType).end())
    return It;
  return nullptr;
}
...
template <class ELFT> void GNUELFDumper<ELFT>::printGroupSections() {
  ...
  for (const GroupSection &G : V) {
    ...
    for (const GroupMember &GM : G.Members) {
      ...
      if (const GroupSection *MainGroup = Map[GM.Index]; MainGroup != &G)
        this->reportUniqueWarning(
...
std::string GNUELFDumper<ELFT>::getSymbolSectionNdx(
  ...
  if (ExtraSymInfo) {
    ...
    if (auto Sec = this->Obj.getSection(SectionIndex); !Sec) {
      this->reportUniqueWarning(Sec.takeError());
    } else {
      
      if (auto SecName = this->Obj.getSectionName(**Sec); !SecName)
...
void GNUELFDumper<ELFT>::printSymbol(const Elf_Sym &Symbol, unsigned SymIndex,
  ...
  if (unsigned char SymbolType = Symbol.getType(); this->Obj.getHeader().e_machine == ELF::EM_AMDGPU &&
  ...
  if (Symbol.st_other & ~0x3) {
    if (this->Obj.getHeader().e_machine == ELF::EM_AARCH64) {
      ...
      if (uint8_t Other = Symbol.st_other & ~0x3; Other & STO_AARCH64_VARIANT_PCS) {
      ...
      if (uint8_t Other = Symbol.st_other & ~0x3; Other & STO_RISCV_VARIANT_CC) {
...
void GNUELFDumper<ELFT>::printProgramHeaders(
  ...
  if (PrintProgramHeaders) {
    ...
    if (const Elf_Ehdr &Header = this->Obj.getHeader(); Header.e_phnum == 0) {
...
template <class ELFT> void ELFDumper<ELFT>::printDynamicRelocationsHelper() {
  ...
  auto DumpCrelRegion = [&](DynRegionInfo &Region) {
    ...
    if (!Region.template getAsArrayRef<uint8_t>().empty()) {
      ...
      if (auto RelsOrRelas =
          Obj.decodeCrel(ArrayRef<uint8_t>(Region.Addr, ObjSize - Offset)); !RelsOrRelas) {
...
static std::string getGNUProperty(uint32_t Type, uint32_t DataSize,
  ...
  switch (auto DumpBit = [&](uint32_t Flag, StringRef Name) {
    if (PrData & Flag) {
      PrData &= ~Flag;
      OS << Name;
      if (PrData)
...
static bool printGNUNote(raw_ostream &OS, uint32_t NoteType,
  ...
  case ELF::NT_GNU_ABI_TAG: {
    ...
    if (const GNUAbiTag &AbiTag = getGNUAbiTag<ELFT>(Desc); !AbiTag.IsValid)
...
static AMDGPUNote getAMDGPUNote(uint32_t NoteType, ArrayRef<uint8_t> Desc) {
  ...
  case ELF::NT_AMDGPU_METADATA: {
    ...
    if (MsgPackString.contains("amdhsa.")) {
      ...
      if (AMDGPU::HSAMD::V3::MetadataVerifier Verifier(true); !Verifier.verify(MsgPackDoc.getRoot()))
...
StringRef getNoteTypeName(const typename ELFT::Note &Note, unsigned ELFType) {
  ...
  if (Name == "FreeBSD") {
    if (ELFType == ELF::ET_CORE) {
      ...
      if (StringRef Result = FindNote(FreeBSDCoreNoteTypes); !Result.empty())
        return Result;
  ...
  if (ELFType == ELF::ET_CORE && Name.starts_with("NetBSD-CORE")) {
    ...
    if (StringRef Result = FindNote(NetBSDCoreNoteTypes); !Result.empty())
      return Result;
    return FindNote(CoreNoteTypes);
  }
  if (ELFType == ELF::ET_CORE && Name.starts_with("OpenBSD")) {
    ...
    if (StringRef Result = FindNote(OpenBSDCoreNoteTypes); !Result.empty())
      return Result;
    return FindNote(CoreNoteTypes);
  }
  if (Name == "AMD")
...
static void processNotesHelper(
  ...
  if (ArrayRef<typename ELFT::Shdr> Sections = cantFail(Obj.sections()); !IsCoreFile && !Sections.empty()) {
...
template <class ELFT> void GNUELFDumper<ELFT>::printNotes() {
  ...
  auto ProcessNote = [&](const Elf_Note &Note, bool IsCore) -> Error {
    ...
    if (StringRef NoteType =
        getNoteTypeName<ELFT>(Note, this->Obj.getHeader().e_type); !NoteType.empty())
      OS << NoteType << '\n';
    else
    ...
    if (Name == "GNU") {
      ...
      if (const AMDNote N = getAMDNote<ELFT>(Type, Descriptor); !N.Type.empty()) {
        OS << "    " << N.Type << ":\n        " << N.Value << '\n';
        return Error::success();
      }
    } else if (Name == "AMDGPU") {
      
      if (const AMDGPUNote N = getAMDGPUNote<ELFT>(Type, Descriptor); !N.Type.empty()) {
        OS << "    " << N.Type << ":\n        " << N.Value << '\n';
        return Error::success();
      }
      ...
      if (Type == ELF::NT_FILE) {
        ...
        if (Expected<CoreNote> DataExtractor DescExtractor(
            Descriptor, ELFT::Endianness == llvm::endianness::little,
            sizeof(Elf_Addr)); NoteOrErr = readCoreNote(DescExtractor)) {
...
template <typename ELFT> void ELFDumper<ELFT>::printMemtag() {
  ...
  for (const typename ELFT::Dyn &Entry : dynamic_table()) {
    ...
    switch (uintX_t Tag = Entry.getTag(); Tag) {
    case DT_AARCH64_MEMTAG_GLOBALSSZ:
...
uint64_t ELFDumper<ELFT>::getAndPrintSFrameFDEStartAddress(
  ...
  if (auto Reloc = llvm::lower_bound(
      Relocations, Offset, [](auto R, uint64_t O) { return R.Offset < O; }); Reloc == Relocations.end() || Reloc->Offset != Offset) {
...
void ELFDumper<ELFT>::forEachRelocationDo(
  ...
  switch (const bool IsMips64EL = this->Obj.isMips64EL(); Sec.sh_type) {
  case ELF::SHT_REL:
...
SmallVector<uint32_t> ELFDumper<ELFT>::getSymbolIndexesForFunctionAddress(
  ...
  for (uint32_t Index : Symbols->second) {
    ...
    if (FunctionSec) {
      ...
      if (Expected<const Elf_Shdr *> const Elf_Sym &Sym = *cantFail(Obj.getSymbol(this->DotSymtabSec, Index)); SecOrErr =
...
void ELFDumper<ELFT>::printStackSize(const Relocation<ELFT> &R,
  ...
  if (Sym) {
    ...
    if (Expected<const Elf_Shdr *> SectionOrErr =
        this->Obj.getSection(*Sym, SymTab, this->getShndxTable(SymTab)); !SectionOrErr) {
      reportUniqueWarning(
...
void ELFDumper<ELFT>::printNonRelocatableStackSizes(
  ...
  for (const Elf_Shdr &Sec : cantFail(Obj.sections())) {
    ...
    while (Offset < Contents.size()) {
      ...
      if (uint64_t SymValue = Data.getAddress(&Offset); !printFunctionStackSize(SymValue, /*FunctionSec=*/std::nullopt, Sec,
                                  Data, &Offset))
        break;
    }
  }
...
void GNUELFDumper<ELFT>::printStackSizes() {
  ...
  }; this->Obj.getHeader().e_type == ELF::ET_REL)
  ...
  const Elf_Ehdr &E = this->Obj.getHeader();
  {
    ...
    if (E.e_machine == EM_MIPS)
      ...
      case ELF::ELFABIVERSION_AMDGPU_HSA_V6: {
        ...
        if (auto std::string FlagStr; VersionFlag = E.e_flags & ELF::EF_AMDGPU_GENERIC_VERSION) {
          unsigned Version =
...
template <class ELFT> void LLVMELFDumper<ELFT>::printSectionHeaders() {
  ...
  for (const Elf_Shdr &Sec : cantFail(this->Obj.sections())) {
    ...
    if (opts::SectionSymbols) {
      ...
      if (ListScope D(W, "Symbols"); this->DotSymtabSec) {
        ...
        for (const Elf_Sym &Sym : Symbols) {
          ...
          if (const Elf_Shdr *SymSec = unwrapOrError(
              this->FileName,
              this->Obj.getSection(Sym, this->DotSymtabSec, ShndxTable)); SymSec == &Sec)
...
void JSONELFDumper<ELFT>::printAuxillaryDynamicTableEntryInfo(
  ...
  switch (auto FormatFlags = [this, Value = Entry.getVal()](auto Flags) {
  ...
  auto ProcessNote = [&](const Elf_Note &Note, bool IsCore) -> Error {
    ...
    if (Name == "GNU") {
      ...
      if (Type == ELF::NT_FILE) {
        ...
        if (Expected<CoreNote> DataExtractor DescExtractor(
            Descriptor, ELFT::Endianness == llvm::endianness::little,
            sizeof(Elf_Addr)); N = readCoreNote(DescExtractor)) {
...
template <class ELFT> void LLVMELFDumper<ELFT>::printStackSizes() {
  ...
  if (ListScope L(W, "StackSizes"); this->Obj.getHeader().e_type == ELF::ET_REL)
