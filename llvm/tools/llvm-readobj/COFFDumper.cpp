  ...
  for (const SectionRef &S : Obj->sections()) {
    ...
    if (StringRef SectionName = unwrapOrError(Obj->getFileName(), S.getName()); SectionName == ".debug$T") {
      ...
      if (GHash) {
        ...
        if (Error std::vector<GloballyHashedType> Hashes =
            GloballyHashedType::hashTypes(Types); E =
...
void COFFDumper::printUnwindInfo() {
  ...
  switch (ListScope D(W, "UnwindInformation"); Obj->getMachine()) {
...
void COFFDumper::printCOFFDirectives() {
  for (const SectionRef &Section : Obj->sections()) {
    ...
    if (StringRef Name = unwrapOrError(Obj->getFileName(), Section.getName()); Name != ".drectve")
      continue;
