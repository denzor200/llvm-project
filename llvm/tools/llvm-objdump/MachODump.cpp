        ...
        if (r_extern) {
          ...
          if (cputype == MachO::CPU_TYPE_ARM && r_type == MachO::ARM_RELOC_PAIR)
            ...
            if (r_symbolnum == MachO::R_ABS)
              ...
              if (uint32_t nsects = O->section_end()->getRawDataRefImpl().d.a; r_symbolnum > 0 && r_symbolnum <= nsects) {
                object::DataRefImpl DRI;
                DRI.d.a = r_symbolnum-1;
                
                if (StringRef SegName = O->getSectionFinalSegmentName(DRI); Expected<StringRef> NameOrErr = O->getSectionName(DRI))
...
void objdump::parseInputMachO(MachOUniversalBinary *UB) {
      ...
      for (MachOUniversalBinary::object_iterator I = UB->begin_objects(),
        ...
        if (ArchFlags[i] == I->getArchFlagName()) {
          ...
          if (ObjOrErr) {
            ...
            if (ObjectFile &O = *ObjOrErr.get(); MachOObjectFile *MachOOF = dyn_cast<MachOObjectFile>(&O))
  ...
  for (MachOUniversalBinary::object_iterator I = UB->begin_objects(),
    ...
    if (ObjOrErr) {
      ...
      if (ObjectFile &Obj = *ObjOrErr.get(); MachOObjectFile *MachOOF = dyn_cast<MachOObjectFile>(&Obj))
        ProcessMachO(Filename, MachOOF, "", ArchitectureName);
    } else if (Error E =
                   isNotObjectErrorInvalidFileType(ObjOrErr.takeError())) {
      reportError(std::move(E), Filename, "", ArchitectureName);
...
static const char *get_symbol_64(uint32_t sect_offset, SectionRef S,
  ...
  if (reloc_found && isExtern) {
    ...
    if (StringRef Name = unwrapOrError(Symbol.getName(), info->O->getFileName()); !Name.empty()) {
      SymbolName = Name.data();
      return SymbolName;
    }
  }
...
#undef CHECK_THUMB_TARGET_INFO_CREATION
  ...
  for (unsigned SectIdx = 0; SectIdx != Sections.size(); SectIdx++) {
    ...
    for (const SymbolRef &Symbol : MachOOF->symbols()) {
      ...
      if (SymbolRef::Type ST =
          unwrapOrError(Symbol.getType(), MachOOF->getFileName()); ST == SymbolRef::ST_Function || ST == SymbolRef::ST_Data ||
          ST == SymbolRef::ST_Other) {
        uint64_t Address = cantFail(Symbol.getValue());
        StringRef SymName =
