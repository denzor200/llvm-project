...
static void addPltEntries(const MCSubtargetInfo &STI, const ObjectFile &Obj,
  ...
  for (auto Plt : ElfObj->getPltEntries(STI)) {
    if (Plt.Symbol) {
      ...
      if (uint8_t SymbolType = getElfSymbolType(Obj, Symbol); Expected<StringRef> NameOrErr = Symbol.getName()) {
        if (!NameOrErr->empty())
...
disassembleObject(ObjectFile &Obj, const ObjectFile &DbgObj,
  ...
  if (SecondaryTarget && isArmElf(Obj)) {
    ...
    if (auto PltSectionRef = SectionNames.find(".plt"); PltSectionRef != SectionNames.end()) {
      ...
      while (Index < End) {
        ...
        if (DumpARMELFData) {
          ...
          if (Iter1 != BBAddrMapLabels.end()) {
            ...
            if (auto Iter2 = AllLabels.find(SectionAddr + Index); Iter2 != AllLabels.end())
              FOS << "<" << Iter2->second << ">:\n";
          }
...
void Dumper::printSymbol(const SymbolRef &Symbol,
  ...
  if (O.isELF()) {
    ...
    switch (uint8_t Other = ELFSymbolRef(Symbol).getOther(); Other) {
...
static void dumpInput(StringRef file) {
  ...
  if (Binary &Binary = *OBinary.getBinary(); Archive *A = dyn_cast<Archive>(&Binary))
    dumpArchive(A);
