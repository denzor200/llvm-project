...
ErrorOr<std::unique_ptr<DWARFFile>> DwarfLinkerForBinary::loadObject(
  ...
  if (ErrorOrObj) {
    ...
    if (Error Error E = RL.link(*ErrorOrObj); NewE = handleErrors(
...
bool DwarfLinkerForBinary::linkImpl(
  ...
  for (const auto &Obj : Map.objects()) {
    ...
    if (ErrorOr<std::unique_ptr<DWARFFile>> auto DLBRelocMap = std::make_shared<DwarfLinkerForBinaryRelocationMap>(); ErrorOrObj =
  ...
  if (Options.ResourceDir && !ParseableSwiftInterfaces.empty()) {
    ...
    if (auto StringRef ArchName = Triple::getArchTypeName(Map.getTriple().getArch()); E = copySwiftInterfaces(ArchName))
      return error(toString(std::move(E)));
  }
